...
describe('default layout drawer lifecycle', () => {
  ...
  it('uses the shared narrow strip on non-workspace routes and returns focus to its opener', async () => {
    ...
    const backdrop = wrapper.get('[data-test="app-left-drawer-backdrop"]')
    expect(backdrop.classes()).toEqual(expect.arrayContaining(['fixed', 'inset-0', 'z-40', 'bg-black/30']))
    expect(backdrop.classes()).not.toContain('bg-opacity-75')
    expect((backdrop.element as HTMLElement).style.zIndex).toBe('40')
