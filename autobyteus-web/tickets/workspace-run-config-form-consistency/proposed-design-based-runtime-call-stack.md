...
└── components/workspace/history/WorkspaceAgentRunsTreePanel.vue:emit('run-created', { type:'agent', definitionId }) # route host to /workspace
```
...
```text
[ENTRY] components/workspace/running/RunningAgentsPanel.vue:createAgentRun(definitionId)
...
```text
[ENTRY] components/workspace/running/RunningAgentsPanel.vue:createTeamRun(definitionId)
