...
| --- | --- | --- | --- | --- | --- |
| Select existing agent run from left running panel | Pass | None | Pass | None | Pass |
| Create new agent run from left group action | Pass | None | Pass | None | Pass |
| Remove selected agent run and auto-select fallback | Pass | None | Pass | None | Pass |
| Select existing team run from left running panel | Pass | None | Pass | None | Pass |
...
- `[F-002] Use case: remove selected agent run | Type: Cleanliness/Docs | Evidence: /Users/normy/autobyteus_org/autobyteus-web/stores/agentRunStore.ts action comment said "close tab" though center tabs were removed | Required update: update wording to "close agent run".`
