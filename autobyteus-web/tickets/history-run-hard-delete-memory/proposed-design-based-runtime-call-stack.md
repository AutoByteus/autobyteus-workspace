...
└── [ASYNC] stores/runHistoryStore.ts:deleteRun(runId)
    ...
    │       ├── decision: AgentRunManager.getAgentRun(runId) === null
    ...
    ├── [STATE] stores/agentContextsStore.ts:removeRun(runId) if present
...
├── [STATE] stores/agentContextsStore.ts:removeRun(runId) if exists
