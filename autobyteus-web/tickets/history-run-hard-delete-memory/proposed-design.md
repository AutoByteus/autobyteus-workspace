...
- Backend termination (`terminateAgentRun`) marks history as IDLE but does not delete memory folder.
...
| `autobyteus-server-ts/src/run-history/services/run-history-service.ts` | Modify | Authoritative hard-delete rules and IO execution | `deleteRunHistory(runId)` | runId -> `{ success, message }` | `AgentRunManager`, `RunHistoryIndexStore`, `MemoryFileStore`, `fs/promises` |
...
| --- | --- | --- | --- | --- |
| `RunHistoryService` | `AgentRunManager`, index store, memory store, fs | `RunHistoryResolver` | Medium | Keep all delete guards in service so resolver/UI cannot bypass safety checks. |
