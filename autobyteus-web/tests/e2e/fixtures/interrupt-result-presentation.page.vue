  ...
  <main class="min-h-screen bg-slate-100 p-8 text-slate-900" data-test="interrupt-result-probe">
    <section class="mx-auto max-w-2xl rounded-xl bg-white p-6 shadow">
      ...
      <div class="mt-3 grid grid-cols-2 gap-2 text-sm">
        ...
        <p data-test="team-active">teamActive={{ teamContext.isActive }}</p>
...
const TEAM_MEMBER_ROUTE_KEY = 'review_group/critic';
...
const teamMemberNode: AgentTeamMemberNode = {
  memberKind: 'agent',
  memberName: 'critic',
  displayName: 'Critic',
  memberPath: ['review_group', 'critic'],
  memberRouteKey: TEAM_MEMBER_ROUTE_KEY,
  memberRunId: TEAM_MEMBER_RUN_ID,
  agentDefinitionId: 'browser-critic-definition',
  currentStatus: AgentStatus.Running,
  isTaskTeamChildProjection: true,
  parentTaskTeamRunId: 'browser-task-team-run',
  taskTeamRelativeMemberRouteKey: 'critic',
  taskTeamRelativeMemberPath: ['critic'],
  structuralSourceRouteKey: TEAM_MEMBER_ROUTE_KEY,
  structuralSourcePath: ['review_group', 'critic'],
};
const teamContext: AgentTeamContext = {
  ...
  memberNodesByRouteKey: new Map([[TEAM_MEMBER_ROUTE_KEY, teamMemberNode]]),
  leafAgentContextsByRouteKey: new Map([[TEAM_MEMBER_ROUTE_KEY, teamMemberContext]]),
  coordinatorMemberRouteKey: TEAM_MEMBER_ROUTE_KEY,
  historicalHydration: null,
  focusedMemberRouteKey: TEAM_MEMBER_ROUTE_KEY,
  isActive: true,
  isSubscribed: false,
};
...
onMounted(() => {
  ...
  globalWindow.__interruptProbe = {
    ...
    snapshot: () => ({
      ...
      teamIsActive: teamContext.isActive,
      teamSubscribed: teamContext.isSubscribed,
