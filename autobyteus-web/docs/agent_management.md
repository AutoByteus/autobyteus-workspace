...
│   │   └── RunningAgentRunRow.vue      # Individual instance row
│   └── config/
...
│   ├── agentContextsStore.ts           # Running agent runs state
...
| Action                           | Description                                |
| -------------------------------- | ------------------------------------------ |
| `createRunFromTemplate()`   | Create instance from `agentRunConfigStore` |
| `removeInstance(id)`             | Remove agent run                      |
