...
Mobile **Auto approve tools** is available when the selected agent or team has an active launch config. It writes the existing `autoExecuteTools` launch-config field; it must not create a mobile-only approval flag or change backend approval semantics. The ordinary new-launch default remains off for non-AGY runtimes. Selecting `antigravity_cli` in a new editable launch draft sets this option on; an explicit subsequent off choice is retained and can cause AGY headless tool denial without an in-chat approval response. For team launches, the global team setting is preserved in the team config and inherited by generated member configs unless an existing member override explicitly supersedes it.
