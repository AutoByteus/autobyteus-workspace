...
4.  **Token Usage Meter (`TokenUsageMeterStore`)**:
    ...
    - `TeamTokenUsageSummary.vue` renders the subordinate compact team comparison table/list for team workspaces. It lists visible leaf members with focused state, input/output/total tokens, total cost/status, input/output cost details when available, and an explicitly labeled `Team total` aggregate as the final row when available.
    ...
    - Browser-facing proof should validate clean agent/team headers with no token chip and validate the Token tab against server/GraphQL-backed summaries, including focused member primary selection, compact team comparison rows without horizontal overflow, subordinate final-row team total, estimated, price-missing, partial-price, local/no-bill, mixed-currency, cache-positive, reasoning-token, model/runtime, usage-report, and current-prompt display where present.
    ...
    - Current durable regression coverage includes GraphQL E2E for cached gross input, provider-specific semantics, local/no-bill, custom missing price, mixed currency, and runtime field names, plus frontend store/component tests for live aggregation, GraphQL hydration replacement, focused team member primary selection, team comparison rows, clean header rendering, Token Meter hierarchy, cache-aware rows, price-status labels, and current prompt fields. Latest visual evidence for the cache-aware Token Meter is under `tickets/token-input-prompt-discrepancy-analysis/implementation-evidence/`.
