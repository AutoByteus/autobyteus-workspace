...
show `api_e2e_engineer` in Focus/Grid/Spotlight while interrupt safety remains
...
4.  **Token Usage Meter (`TokenUsageMeterStore`)**:
    ...
    - Preserves nullable estimated API costs, currency, reasoning-output token/cost sub-breakdowns, and `apiCostStatus` (`estimated`, `price_missing`, `partial_price_missing`, or `mixed`). Missing price data is rendered as unpriced, never as `$0`, and mixed-currency aggregate costs stay nullable instead of being summed under one currency label.
    - Powers `TokenUsageHeaderChip.vue` and the right-side `Token` tab (`TokenUsageMeterPanel.vue`) for run/team totals, focused-member totals, latest model/runtime, event count, and context pressure. The internal tab id may remain `usage`; the user-visible label is token-oriented.
    - `TokenUsageMeterPanel.vue` is presentation-only: it renders compact paired Input, Output, and Total cards that place each token count with its related cost estimate. Cost rows are quiet secondary text but keep accessible labels/titles, and the Total card is subtly highlighted.
    - Reasoning output appears only inside the Output card and only when the server summary reports positive reasoning output tokens. It uses a native disclosure chip with a chevron and explanatory copy stating that thinking tokens are included in output tokens and estimated output cost.
    - Unknown context pressure is intentionally hidden; the context-pressure block renders only when both a numeric pressure percentage and effective context budget are present.
    - Browser-facing proof should validate the real header chip and Token tab against server/GraphQL-backed summaries, including estimated, unpriced, partial-price, mixed-currency, reasoning-token, model/runtime, event count, and context-pressure display where present; the frontend must still not calculate prices.
    - Live store coverage must preserve runtime-native summary fields from server events, including Codex-style reasoning tokens/cost, latest runtime/ingestion/model metadata, and context budget/pressure fields used by the token meter.
    - Historical delivery evidence confirms the ledger-backed display contract for AutoByteus+LM Studio unpriced usage, Codex App Server estimated usage, and Claude Agent SDK unpriced usage through real local browser screenshots. Current durable regression coverage also includes component/composable/store tests for the Token tab label, compact paired meter cards, accessible quiet cost rows, native thinking-token disclosure, hidden zero-thinking state, reasoning/currency fields, and the DS-007 runtime-like Codex summary baseline. Latest visual evidence for the compact disclosure UI is `/Users/normy/.autobyteus/browser-artifacts/6b2c05-1782396115079.png`.
