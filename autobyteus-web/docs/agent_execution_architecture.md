...
5.  **Settings Token Statistics (`tokenUsageStatisticsStore`)**:
    ...
    - The page defaults to By Task. Top-level rows are standalone agent runs or root team runs; repeated executions remain separate by concrete run/team identity; team member agent usage is rendered only as nested member rows so team totals are not double-counted.
    ...
    - The date-range copy is static `Usage during period`. The frontend must not send a `rangeMode` variable or render a `Tasks created in period` selector until a backend-created-time filtering contract exists.
    - By Model remains a secondary diagnostics tab grouped by runtime/model pair, with runtime fallbacks and the same server-owned cost/status semantics as the task table.
    - Focused coverage for this surface includes backend GraphQL E2E plus store/page/table component specs for By Task default behavior, team expansion, first-usage fallback, cost breakdowns, absence of `rangeMode`, and runtime/model diagnostics.
