...
5.  **Settings Token Statistics (`tokenUsageStatisticsStore`)**:
    ...
    - The Task table is a reduced nine-column scan view: `Task / Run`, `Runtime`, `Model(s)`, `Input`, `Output`, `Input Cost`, `Output Cost`, `Total Cost`, and `Created Time`. It does not render standalone `Type` or `Status` columns; row kind is conveyed by hierarchy/metadata, complete-estimate status is suppressed in main rows, and non-complete price status remains visible inline in the `Total Cost` cell plus the expanded breakdown.
    - Sortable Task headers (`Task / Run`, `Runtime`, `Input`, `Output`, `Total Cost`, `Created Time`) show persistent neutral or active direction glyphs and accessible sort state. `Model(s)`, `Input Cost`, and `Output Cost` remain non-sortable plain headers. Row cost details open through one always-visible `Details` control in the `Total Cost` cell; `Input Cost` and `Output Cost` are plain values, not duplicate hover-only toggles.
