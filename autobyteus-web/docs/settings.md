...
- **Phone Setup tab:** contains `components/settings/PhoneSetupGuideCard.vue` plus `components/settings/PhoneAccessCard.vue` when the current window is the embedded desktop/server node. Remote-node windows show a note that Phone Access controls must be opened on the server node, while the guide remains useful for preparing the HTTPS URL.
- **Phone Access controls:** the Phone Access card is the desktop-owned pairing and revocation surface for the phone/PWA Remote Access flow. Operators can enable or disable Phone Access, choose or manually enter a Tailscale/VPN/private-network HTTPS URL, create a short-lived pairing QR/link, inspect active phones, view revoked/history records separately, revoke one active phone, or revoke all active phones. New desktop QR creation blocks HTTP URLs. See [Phone Access / Remote Access](./remote_access.md) for the user and packaging guide.
...
- **Docker Guide tab:** `components/settings/DockerNodeStartGuideCard.vue` gives packaged-app users copyable no-clone commands for running a published server Docker node. This content is tutorial/help guidance rather than saved node settings, so it is intentionally separated from the management form flow.
  ...
  - Docker guide copy intentionally distinguishes private named-volume server state (`/home/autobyteus/data`, `/root`, and the legacy build workspace volume) from host-visible bind-mounted user folders (`/home/autobyteus/workspace` for the node workspace and `/home/autobyteus/shared` for cross-node sharing).
