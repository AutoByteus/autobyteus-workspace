...
5.  **Settings Token Statistics (`tokenUsageStatisticsStore`)**:
    ...
    - The page defaults to the `Task` grouping. Top-level rows are standalone agent runs or root team runs; backend-provided `children` contain concrete member runs, including delegated/task-created agent runs with exact canonical run identity. Member usage is rendered only under its root team so totals are not double-counted. The current backend emits the supported Team-to-member level; deeper execution topology is not reconstructed by Token Usage.
    - The store/table consume server-owned `children`, `rootTeamRunId`, `runId`, and task metadata. They must not reconstruct hierarchy from task records, migration-only execution paths, memory paths, or display names.
    - Row labels use token-usage-owned display fields captured or migrated into current run records. Created time uses `runCreatedAt` when available; otherwise the store/table preserve `createdTimeSource` and label the timestamp as first usage observed rather than true run creation. Settings statistics does not add workspace display fields or inactive no-usage roster rows.
    - Model rows render server-owned `modelDisplayName` beside the unchanged raw `llmModel` identity. Task rows render `modelDisplayNames` positionally beside raw `models`; these arrays are derived from one ordered backend projection and the frontend does not parse opaque provider identifiers. AutoByteus custom-provider labels prefer the persisted provider-name identity summary and fall back to current provider metadata only when a migrated/current record has no recoverable snapshot; built-in and non-AutoByteus labels retain their current behavior.
    ...
    - The frontend sends no `rangeMode`. It renders the approved creation-time/lifetime-total helper but not `Usage during period`, `Select Date Range:`, `Group by:`, a separate `By Task` / `By Model` tab row, or a mode selector. Exact observation-period reporting would require a separately designed time-bucket/history contract.
    ...
    - Focused coverage includes current-record GraphQL E2E plus store/page/table component specs for Task default grouping, exact root/run grouping, member children, creation-time selection, lifetime copy, first-usage fallback, reduced Task columns, compact sort affordances, Total Cost disclosure controls, cost-inclusive accessible labels, absence of `rangeMode`, degraded-history guidance, and runtime/model diagnostics. Browser evidence also exercises normal lifetime results and degraded/fatal server states without claiming Electron-shell coverage.
