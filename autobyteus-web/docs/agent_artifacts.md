...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------- |
| **Store**            | [agentArtifactsStore.ts](file:///home/ryan-ai/SSD/autobyteus_org_workspace/autobyteus-web/stores/agentArtifactsStore.ts)                   | Pinia store managing artifact state per run.                    |
...
| `fetchArtifactsForRun()` | Session restore              | Loads persisted artifacts from backend (future feature).                                                                           |
...
1. **Artifact List View**: Shows all artifacts for the current run with status indicators.
...
2.  **Ingestion (Service Layer)**:
    ...
    - `segmentHandler` identifies the segment ID and dispatches `store.appendArtifactContent(runId, delta)`.
