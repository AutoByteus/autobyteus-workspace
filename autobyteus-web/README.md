...
Custom backend endpoint for dev proxy:

```bash
BACKEND_PROXY_URL=http://localhost:8002 pnpm dev
```

Notes:
- `BACKEND_PROXY_URL` controls Vite dev proxy target for `/graphql` and `/rest`.
- WebSocket defaults are derived from the same base URL unless overridden with `BACKEND_*_WS_ENDPOINT`.
