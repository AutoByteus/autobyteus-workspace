...
describe('workspaceStore File Explorer reconnect snapshot refresh', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockStreamingInstances.length = 0;
  });

  it('does not duplicate the initial snapshot refresh on first connect', async () => {
    ...
    mockStreamingInstances[0].options.onConnect?.('initial-session');
  ...
  it('refreshes the File Explorer snapshot when the live stream reconnects after an abnormal close', async () => {
    ...
    await store.fileExplorerSnapshotRefreshes.get('ws-1');
