    ...
    <div v-else class="flex min-h-0 flex-1 overflow-hidden" data-test="team-communication-split">
      ...
      >
        <template v-for="section in sections" :key="section.key">
          <section v-if="section.messages.length > 0" class="mb-4" :data-test="`team-communication-${section.key}`">
          ...
          <div class="mb-3 flex items-start justify-between gap-3">
            <div class="min-w-0">
              <div class="flex items-center gap-2">
                ...
                />
                <span class="truncate text-sm font-semibold text-gray-900">{{ compactMessageLabel(selectedMessage) }}</span>
              </div>
              <div class="mt-1 text-[0.6875rem] text-gray-500">{{ counterpartMetadata(selectedMessage) }}</div>
            </div>
            <span class="shrink-0 text-[0.6875rem] text-gray-400">{{ formatTimestamp(selectedMessage.createdAt) }}</span>
          </div>
          <MarkdownRenderer
            :content="selectedMessage.content"
            class="team-communication-message-markdown rounded-md bg-gray-50 p-3 text-xs leading-relaxed text-gray-700"
...
const counterpartMetadata = (message: TeamCommunicationPerspectiveMessage): string => {
  ...
  return message.direction === 'sent'
    ? `${t('workspace.components.workspace.team.TeamCommunicationPanel.to_counterpart')} ${counterpart}`
    : `${t('workspace.components.workspace.team.TeamCommunicationPanel.from_counterpart')} ${counterpart}`;
};
