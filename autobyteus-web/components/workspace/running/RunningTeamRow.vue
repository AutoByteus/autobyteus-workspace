    ...
    >
      <div class="flex items-center space-x-1.5 min-w-0">
        ...
        <span class="text-sm text-gray-700 truncate">
          {{ formatId(teamRun.teamId) }}
        </span>
      </div>

      <!-- Delete Button -->
      <button
        @click.stop="$emit('delete', teamRun.teamId)"
...
watch(() => props.isSelected, (selected) => {
  if (selected) {
    ...
    if (!props.teamRun.focusedMemberName && props.coordinatorName) {
      emit('select-member', props.teamRun.teamId, props.coordinatorName);
    }
  }
}, { immediate: true });
...
const handleTeamClick = () => {
  ...
  emit('select', props.teamRun.teamId);
};

const handleMemberSelect = (memberName: string) => {
  emit('select-member', props.teamRun.teamId, memberName);
};
