<template>
  <div 
    ...
    @click="$emit('select', runContext.state.agentId)"
  >
    ...
    <!-- Actions -->
    <button
      @click.stop="$emit('delete', runContext.state.agentId)"
...
const runLabel = computed(() => {
  const name = props.runContext.config.agentDefinitionName || 'Agent';
  const agentId = props.runContext.state.agentId;
...
const statusColor = computed(() => {
  switch (props.runContext.state.currentStatus) {
