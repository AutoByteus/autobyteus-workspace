<template>
  <section class="w-full max-w-5xl" data-testid="application-packages-manager">
    <div class="mb-4">
      <h2 class="text-xl font-semibold text-gray-900">{{ $t('settings.components.settings.ApplicationPackagesManager.application_packages') }}</h2>
      <p class="mt-1 text-sm text-gray-600">
        Link a local application package root or paste a public GitHub repository URL.
        GitHub application packages are installed into app-managed storage, while local paths remain in place.
      </p>
    </div>
    ...
    <div class="mb-4 rounded border border-gray-200 bg-white">
      <ul class="divide-y divide-gray-100">
        ...
        >
          <div class="min-w-0">
            ...
            <div class="mt-1 text-xs text-gray-500">
              {{ sourceKindLabel(pkg.sourceKind) }} | Applications: {{ pkg.applicationCount }}
            </div>
          ...
          <div class="ml-4 flex items-center gap-2">
            ...
            >
              {{ pkg.isDefault ? 'Built-in' : sourceKindLabel(pkg.sourceKind) }}
            </span>
            <button
        ...
        <li v-if="sortedPackages.length === 0" class="px-4 py-6 text-sm text-gray-500">{{ $t('settings.components.settings.ApplicationPackagesManager.no_application_packages_configured') }}</li>
      </ul>
    </div>

    <div class="rounded border border-gray-200 bg-gray-50 p-4">
      <label class="mb-2 block text-sm font-medium text-gray-700" for="application-package-source">{{ $t('settings.components.settings.ApplicationPackagesManager.package_path_or_github_url') }}</label>
      <div class="flex gap-2">
        <input
          ...
          :placeholder="$t('settings.components.settings.ApplicationPackagesManager.absolute_path_to_application_package_or')"
