...
import ProjectsFeatureToggleCard from '../ProjectsFeatureToggleCard.vue'
...
vi.mock('~/stores/projectsCapabilityStore', () => ({
  useProjectsCapabilityStore: () => capabilityStoreMock,
}))
...
describe('ProjectsFeatureToggleCard', () => {
  beforeEach(() => {
    capabilityState.status = 'resolved'
  ...
  it('loads the typed capability and shows the default-disabled state', async () => {
    const wrapper = mount(ProjectsFeatureToggleCard)
    await flushPromises()

    expect(capabilityStoreMock.ensureResolved).toHaveBeenCalledTimes(1)
    expect(wrapper.get('[data-testid="projects-feature-status"]').text()).toBe('Disabled')
    expect(wrapper.get('[data-testid="projects-feature-toggle"]').attributes('aria-checked')).toBe('false')
  ...
  it('toggles the typed backend capability and refreshes generic settings as a secondary sync', async () => {
    ...
    await wrapper.get('[data-testid="projects-feature-toggle"]').trigger('click')
    await flushPromises()
    ...
    wrapper.unmount()
    const nextWrapper = mount(ProjectsFeatureToggleCard)
    await flushPromises()
    expect(nextWrapper.get('[data-testid="projects-feature-status"]').text()).toBe('Enabled')
    expect(nextWrapper.get('[data-testid="projects-feature-toggle"]').attributes('aria-checked')).toBe('true')
  ...
  it('keeps the switch disabled while capability resolution is still pending', async () => {
    ...
    const toggle = wrapper.get('[data-testid="projects-feature-toggle"]')
