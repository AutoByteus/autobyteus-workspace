<template>
  ...
  >
    ...
    >
      ...
      <aside
        ref="panelRef"
        class="flex h-full min-h-0 flex-col overflow-hidden border-l border-slate-200 bg-white/95 text-slate-900 shadow-2xl shadow-slate-900/12 backdrop-blur-xl"
        :style="panelStyle"
      ...
      >
        ...
        <div class="flex-1 space-y-3 overflow-y-auto px-4 py-4">
          <section class="space-y-3">
            <button
              type="button"
              class="flex w-full items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3 text-left text-sm font-semibold text-slate-800 shadow-sm transition-colors hover:border-slate-300 hover:bg-slate-50"
...
const MIN_REMAINING_CANVAS_WIDTH = 320
...
const readMaxPanelWidth = (): number => {
  ...
  return Math.max(
    MIN_PANEL_WIDTH,
    Math.min(MAX_PANEL_WIDTH, window.innerWidth - MIN_REMAINING_CANVAS_WIDTH),
  )
}
...
const panelRootClasses = computed(() => (
  isPanelOpen.value
    ? 'order-last relative z-20 flex h-full min-h-0 flex-shrink-0'
