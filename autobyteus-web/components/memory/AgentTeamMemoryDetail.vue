<template>
  <div class="mx-auto max-w-5xl p-6">
    ...
    <header class="mb-6 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
      <p class="text-xs font-semibold uppercase tracking-wide text-gray-400">{{ $t('memory.components.memory.AgentTeamMemoryDetail.agent_team_memory_detail') }}</p>
      <h1 class="mt-1 text-2xl font-bold text-gray-900">{{ teamName }} Memory</h1>
      <p class="mt-2 text-sm text-gray-500">{{ store.teamRuns.total }} team runs · Stable ID: <span class="font-mono">{{ teamDefinitionId }}</span></p>
    </header>

    <section class="rounded-xl border border-gray-200 bg-white shadow-sm">
      <div class="flex flex-col gap-3 border-b border-gray-100 p-4 md:flex-row md:items-center md:justify-between">
        <h2 class="font-semibold text-gray-900">{{ $t('memory.components.memory.AgentTeamMemoryDetail.team_runs') }}</h2>
        <div class="flex gap-2">
          <input v-model="searchInput" type="text" :placeholder="$t('memory.components.memory.AgentTeamMemoryDetail.search_team_runs')" class="min-w-[260px] rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" @keyup.enter="applySearch" />
      ...
      <div class="p-4">
        <div v-if="store.teamRuns.loading && store.teamRuns.entries.length === 0" class="py-12 text-center text-sm text-gray-500">{{ $t('memory.components.memory.AgentTeamMemoryDetail.loading_team_runs') }}</div>
        ...
        <div v-else-if="store.teamRuns.entries.length === 0" class="py-12 text-center text-sm text-gray-500">{{ $t('memory.components.memory.AgentTeamMemoryDetail.no_team_runs_match_this_filter') }}</div>
        <div v-else class="space-y-3">
          <article v-for="run in store.teamRuns.entries" :key="run.teamRunId" class="rounded-xl border border-gray-200 p-4">
            ...
            <div class="mt-4 border-t border-gray-100 pt-3">
              <h4 class="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">{{ $t('memory.components.memory.AgentTeamMemoryDetail.team_member_memories') }}</h4>
