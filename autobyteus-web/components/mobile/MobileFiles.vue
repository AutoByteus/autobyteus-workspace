...
const currentFolder = computed<MobileWorkspaceFileNode | null>(() => folderStack.value.at(-1) ?? activeWorkspace.value?.fileExplorer ?? null);
...
const loadedWorkspaceFiles = computed(() => activeWorkspace.value?.fileExplorer ? flattenFiles(activeWorkspace.value.fileExplorer) : []);
