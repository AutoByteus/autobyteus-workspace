...
const DEFAULT_BACKEND_PROXY_URL = 'http://localhost:8000'
...
const toWsBaseUrl = (value: string): string => {
...
const backendProxyUrl =
  normalizeBaseUrl(process.env.BACKEND_PROXY_URL) ||
  normalizeBaseUrl(process.env.BACKEND_NODE_BASE_URL) ||
  normalizeBaseUrl(process.env.BACKEND_REST_BASE_URL?.replace(/\/rest\/?$/, '')) ||
  DEFAULT_BACKEND_PROXY_URL
const backendProxyWsUrl =
  normalizeBaseUrl(process.env.BACKEND_PROXY_WS_URL) || toWsBaseUrl(backendProxyUrl)
...
if (isElectronBuild) {
  ...
  serverUrls = {
    ...
    restBaseUrl: '/rest',
    agentWsEndpoint: process.env.BACKEND_AGENT_WS_ENDPOINT || `${backendProxyWsUrl}/ws/agent`,
    teamWsEndpoint: process.env.BACKEND_TEAM_WS_ENDPOINT || `${backendProxyWsUrl}/ws/agent-team`,
    graphqlWsEndpoint: process.env.BACKEND_GRAPHQL_WS_ENDPOINT || `${backendProxyWsUrl}/graphql`,
    transcriptionWsEndpoint: process.env.BACKEND_TRANSCRIPTION_WS_ENDPOINT || `${backendProxyWsUrl}/ws/transcribe`,
    terminalWsEndpoint: process.env.BACKEND_TERMINAL_WS_ENDPOINT || `${backendProxyWsUrl}/ws/terminal`,
    fileExplorerWsEndpoint: process.env.BACKEND_FILE_EXPLORER_WS_ENDPOINT || `${backendProxyWsUrl}/ws/file-explorer`
  };
} else {
