...
export const evaluateTeamRunLaunchReadiness = (
  ...
  const subjects = [
    ...Object.values(view.teamsByAddress).map((scope) => ({ kind: 'TEAM' as const, address: scope.address, name: scope.displayName, config: scope.effectiveConfig })),
    ...Object.values(view.agentsByAddress).map((scope) => ({ kind: 'AGENT' as const, address: scope.address, name: scope.displayName, config: scope.effectiveConfig })),
  ]
  ...
  for (const subject of subjects) {
    ...
    if (!workspaceId || !rootPath) issues.push({
