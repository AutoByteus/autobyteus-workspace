...
function createToolCallSegment(
  ...
  if (
    ...
    typeof metadataArguments.query !== 'string'
  ) {
    metadataArguments.query = metadata.query;
  }
  if (Array.isArray(metadata?.queries) && !Array.isArray(metadataArguments.queries)) {
    metadataArguments.queries = metadata.queries;
  }
