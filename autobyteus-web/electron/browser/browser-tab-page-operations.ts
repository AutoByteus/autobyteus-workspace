...
import {
  ...
  type BrowserDomSnapshotScriptResult,
} from './browser-dom-snapshot-script'
import {
  cleanBrowserPageContent,
  type BrowserReadPageCleaningMode,
} from './browser-page-cleaner'
import type { BrowserScreenshotArtifactWriter } from './browser-screenshot-artifact-writer'
import {
  type CaptureBrowserScreenshotResult,
  type ExecuteBrowserJavascriptResult,
  type BrowserDomSnapshotRequest,
  type BrowserDomSnapshotResult,
  BrowserTabError,
  type BrowserTabRecord,
  type ReadBrowserPageResult,
...
export class BrowserTabPageOperations {
  constructor(private readonly screenshotWriter: BrowserScreenshotArtifactWriter) {}
  ...
  async readPage(
    session: BrowserTabRecord,
    cleaningMode: BrowserReadPageCleaningMode,
  ): Promise<ReadBrowserPageResult> {
    try {
      ...
      return {
        ...
        content: cleanBrowserPageContent(
  ...
  async domSnapshot(
    ...
    try {
      const result = await session.view.webContents.executeJavaScript(
        `(${BROWSER_DOM_SNAPSHOT_SCRIPT})(${JSON.stringify({
      ...
      const snapshot =
        result && typeof result === 'object'
          ? (result as Partial<BrowserDomSnapshotScriptResult>)
          : {}
      ...
      return {
        ...
        elements: Array.isArray(snapshot.elements)
          ? (snapshot.elements as BrowserDomSnapshotElement[])
          : [],
      }
    } catch (error) {
      throw new BrowserTabError(
