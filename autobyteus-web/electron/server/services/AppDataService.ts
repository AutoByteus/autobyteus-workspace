...
export class AppDataService {
  ...
  initializeFirstRun(serverDir: string): void {
    ...
    try {
      ...
      const requiredServerFiles = ['package.json']
      ...
      // Always generate a deterministic runtime .env on first run.
      const envFileDest = path.join(this.appDataDir, '.env')
      fs.writeFileSync(envFileDest, this.buildDefaultEnvFileContents(), 'utf8')
      logger.info(`Generated runtime .env at: ${envFileDest}`)
