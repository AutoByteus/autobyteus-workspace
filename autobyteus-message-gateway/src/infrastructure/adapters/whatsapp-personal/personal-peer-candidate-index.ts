...
export class PersonalPeerCandidateIndex {
  private readonly maxCandidatesPerSession: number;
  private readonly sessionIndex = new Map<string, Map<string, PersonalSessionPeerCandidate>>();
  ...
  private ensureSessionBucket(
    sessionId: string,
  ): Map<string, PersonalSessionPeerCandidate> {
    let bucket = this.sessionIndex.get(sessionId);
    if (!bucket) {
      bucket = new Map<string, PersonalSessionPeerCandidate>();
      this.sessionIndex.set(sessionId, bucket);
    }
    return bucket;
  }

  private trimToMax(bucket: Map<string, PersonalSessionPeerCandidate>): void {
    if (bucket.size <= this.maxCandidatesPerSession) {
      return;
    }

    const sortedByOldestFirst = [...bucket.entries()].sort(
      (left, right) => Date.parse(left[1].lastMessageAt) - Date.parse(right[1].lastMessageAt),
    );
...
const toLimit = (value: number | undefined, fallback: number): number => {
  if (value === undefined) {
    return fallback;
  }
