...
export type AccountPeerCandidate = PersonalSessionPeerCandidate;

export type AccountPeerCandidateObservation = AccountPeerCandidate & {
  accountId: string;
};

export type ListAccountPeerCandidatesOptions = {
  accountId: string;
  includeGroups?: boolean;
  limit?: number;
};

export type ListAccountPeerCandidatesResult = {
  accountId: string;
  updatedAt: string;
  items: AccountPeerCandidate[];
};

export type AccountPeerCandidateIndexConfig = {
  maxCandidatesPerAccount: number;
  candidateTtlSeconds: number;
  nowMs?: () => number;
};

type StoredObservation = AccountPeerCandidateObservation & {
  observedAtMs: number;
};

export class AccountPeerCandidateIndex {
  ...
  constructor(config: AccountPeerCandidateIndexConfig) {
  ...
  recordObservation(observation: AccountPeerCandidateObservation): void {
  ...
  listCandidates(
    options: ListAccountPeerCandidatesOptions,
  ): ListAccountPeerCandidatesResult {
  ...
  pruneExpired(nowMs = this.nowMs()): void {
