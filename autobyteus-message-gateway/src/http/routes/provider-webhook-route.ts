...
export function registerProviderWebhookRoutes(app: FastifyInstance, deps: ProviderWebhookDeps): void {
  app.post("/webhooks/:provider", async (request, reply) => {
    try {
      const provider = parseExternalChannelProvider((request.params as Record<string, unknown>).provider);
