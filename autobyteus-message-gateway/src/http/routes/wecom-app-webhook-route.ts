...
export function registerWeComAppWebhookRoutes(
  app: FastifyInstance,
  deps: WeComAppWebhookRouteDeps,
): void {
  app.get("/webhooks/wecom-app/:accountId", async (request, reply) => {
    const accountId = normalizePathParam((request.params as Record<string, unknown>).accountId);
  ...
  app.post("/webhooks/wecom-app/:accountId", async (request, reply) => {
    try {
      ...
      const signature = deps.wecomAdapter.verifyInboundSignature(
        mappedRequest,
        mappedRequest.rawBody,
      );
      if (!signature.valid) {
        return reply.code(401).send({
      ...
      const result = await deps.inboundMessageService.handleInbound(
...
const normalizePathParam = (value: unknown): string =>
  normalizeRequiredString(value, "accountId");
