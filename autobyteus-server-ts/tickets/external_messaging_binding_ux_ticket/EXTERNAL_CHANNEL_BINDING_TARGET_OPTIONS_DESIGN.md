...
## Summary
`targetId` is currently a raw input in web. This is error-prone because external-channel dispatch requires a running agent/team run id. This ticket adds a setup-focused query in `external-channel` GraphQL resolver to provide active target options for dropdown selection.
