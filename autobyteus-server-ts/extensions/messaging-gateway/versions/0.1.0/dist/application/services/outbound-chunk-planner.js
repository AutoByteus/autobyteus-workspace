...
export class OutboundChunkPlanner {
    ...
    resolveChunkLimit(payload) {
        if (payload.provider === ExternalChannelProvider.WHATSAPP &&
        ...
        if (payload.provider === ExternalChannelProvider.WECHAT &&
            payload.transport === ExternalChannelTransport.PERSONAL_SESSION) {
            return 1200;
        }
        if (payload.provider === ExternalChannelProvider.DISCORD &&
            payload.transport === ExternalChannelTransport.BUSINESS_API) {
            return 2000;
        }
        return 1000;
...
const splitText = (input, maxLength) => {
...
//# sourceMappingURL=outbound-chunk-planner.js.map