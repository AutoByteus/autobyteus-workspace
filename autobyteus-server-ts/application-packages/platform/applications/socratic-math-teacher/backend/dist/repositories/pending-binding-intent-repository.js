...
export const createPendingBindingIntentRepository = (db) => ({
    getByBindingIntentId(bindingIntentId) {
        const row = db
            .prepare(`SELECT binding_intent_id, lesson_id, status, binding_id, created_at, updated_at, committed_at
    ...
    insertPendingIntent(input) {
        db.prepare(`INSERT INTO pending_binding_intents (
         ...
         updated_at,
         committed_at
       ) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(input.bindingIntentId, input.lessonId, input.status, input.bindingId, input.createdAt, input.updatedAt, input.committedAt);
    },
    markCommitted(input) {
