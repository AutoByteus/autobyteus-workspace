...
import { buildInterruptGenerationCommandAck, normalizeInterruptCommandId } from "./interrupt-generation-command-ack.js";
import {
...
export const handleTeamInterruptGenerationCommand = async (input: {
  ...
  const target = { target_kind: "team_member" as const, team_run_id: input.teamRunId, execution_address: address };
  ...
  if (!commandId) { sendAck(buildInterruptGenerationCommandAck({ commandId, target })); return; }
  if (!address) {
    ...
    sendAck(buildInterruptGenerationCommandAck({
      commandId, target,
      validationFailure: { code: TEAM_COMMAND_INVALID_TARGET_CODE, message: TEAM_COMMAND_INVALID_TARGET_MESSAGE },
    }));
    return;
  }
  if (!input.activeRun) {
    sendAck(buildInterruptGenerationCommandAck({
      commandId, target,
      validationFailure: { code: "RUN_NOT_FOUND", message: `Team run '${input.teamRunId}' is not active.` },
    }));
    return;
  }
  try {
    const result = await input.activeRun.interruptMember(address.memberAddress, address.taskAgentRunId);
