...
import type { RuntimeToolState, SegmentState } from "./runtime-memory-event-accumulator-state.js";
...
export class RuntimeMemoryEventAccumulator {
  ...
  private recordToolCall(event: AgentRunEvent): void {
    ...
    const createsOrderedCall = !tool.callObserved && !tool.callRawTraceId;
    tool.callObserved = true;
    this.mergeToolObservation(tool, event.payload);
    if (!tool.callRawTraceId && this.isCallReady(tool)) {
      this.persistToolCall(tool, event, createsOrderedCall);
    } else if (createsOrderedCall) {
      this.flushOpenReasoningSegments(identity.turnId, event.eventType);
    }
  ...
  private recordToolResult(event: AgentRunEvent): void {
    ...
    const hadObservedCall = Boolean(tool.callObserved || tool.callRawTraceId);
    if (!tool.callRawTraceId) {
      ...
      this.persistToolCall(tool, event, !hadObservedCall);
    }
    if (!tool.toolName) {
      console.warn(
  ...
  private persistToolCall(
    ...
    event: AgentRunEvent,
    flushReasoningBoundary: boolean,
  ): void {
    if (tool.callRawTraceId || !this.isCallReady(tool)) return;
    if (flushReasoningBoundary) {
