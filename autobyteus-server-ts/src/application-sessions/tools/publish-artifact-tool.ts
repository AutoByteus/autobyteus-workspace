...
const TOOL_DESCRIPTION =
  "Publish one application artifact into the owning application session. The platform derives application and producer provenance from the runtime context.";
...
const buildArgumentSchema = (): ParameterSchema => {
  ...
  schema.addParameter(new ParameterDefinition({ name: "artifactKey", type: ParameterType.STRING, description: "Stable artifact upsert key within the current application session.", required: true }));
