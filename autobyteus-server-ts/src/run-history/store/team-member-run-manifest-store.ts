...
const normalizeManifest = (manifest: TeamMemberRunManifest): TeamMemberRunManifest => ({
  ...
  runVersion: Number.isFinite(manifest.runVersion)
    ? Math.max(1, Math.floor(manifest.runVersion))
    : 1,
  ...
  memberAgentId: normalizeRequiredString(manifest.memberAgentId, "memberAgentId"),
  hostNodeId: normalizeOptionalString(manifest.hostNodeId),
...
const validateManifest = (value: unknown): TeamMemberRunManifest | null => {
  ...
  return {
    ...
    memberAgentId: payload.memberAgentId,
    hostNodeId: normalizeOptionalString(payload.hostNodeId),
...
export class TeamMemberRunManifestStore {
  ...
  getManifestPath(teamRunId: string, memberAgentId: string): string {
    return path.join(this.layoutStore.getMemberDirPath(teamRunId, memberAgentId), "run_manifest.json");
  }

  async writeManifest(teamRunId: string, manifest: TeamMemberRunManifest): Promise<void> {
    const normalized = normalizeManifest(manifest);
    const manifestPath = this.getManifestPath(teamRunId, normalized.memberAgentId);
  ...
  async readManifest(teamRunId: string, memberAgentId: string): Promise<TeamMemberRunManifest | null> {
    try {
      const raw = await fs.readFile(this.getManifestPath(teamRunId, memberAgentId), "utf-8");
      const parsed = JSON.parse(raw);
      const manifest = validateManifest(parsed);
      if (!manifest) {
        logger.warn(
          `Invalid team member run manifest format for team '${teamRunId}', member '${memberAgentId}'.`,
        );
        return null;
      }
      ...
      if (!message.includes("ENOENT")) {
        logger.warn(
          `Failed reading team member run manifest for team '${teamRunId}' member '${memberAgentId}': ${message}`,
        );
      }
      return null;
    }
