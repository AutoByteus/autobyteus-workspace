...
} from "../run-projection-provider-port.js";
...
export class ClaudeSessionRunProjectionProvider implements RunProjectionProvider {
  readonly providerId = "claude_session_projection";
  ...
  async buildProjection(input: RunProjectionProviderInput): Promise<RunProjection | null> {
    ...
    const sessionId =
      asString(input.runtimeReference?.sessionId) ??
      asString(input.runtimeReference?.threadId) ??
      input.runId;

    const messages = await this.runtimeService.getSessionMessages(sessionId);
...
let cachedClaudeSessionRunProjectionProvider: ClaudeSessionRunProjectionProvider | null = null;

export const getClaudeSessionRunProjectionProvider = (): ClaudeSessionRunProjectionProvider => {
  if (!cachedClaudeSessionRunProjectionProvider) {
    cachedClaudeSessionRunProjectionProvider = new ClaudeSessionRunProjectionProvider();
  }
  return cachedClaudeSessionRunProjectionProvider;
};
