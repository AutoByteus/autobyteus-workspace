...
import { MARK_TASK_FAILED_TOOL_NAME } from "./task-delegation-tool-contract.js";
import { getTaskDelegationToolManifestEntry } from "./task-delegation-tool-manifest.js";
import { buildMarkTaskFailedParameterSchema } from "./task-delegation-tool-parameter-schemas.js";
import {
...
export class ServerOwnedMarkTaskFailedTool extends BaseTool<NativeTaskDelegationToolExecutionContext, Record<string, unknown>, string> {
  static CATEGORY = ToolCategory.TASK_MANAGEMENT;

  static getName(): string {
    return MARK_TASK_FAILED_TOOL_NAME;
  }

  static getDescription(): string {
    return getTaskDelegationToolManifestEntry(MARK_TASK_FAILED_TOOL_NAME).description;
  }
  ...
  protected async _execute(
    ...
    try {
      const entry = getTaskDelegationToolManifestEntry(MARK_TASK_FAILED_TOOL_NAME);
...
export function registerMarkTaskFailedTool(): BaseTool {
  registerToolClass(ServerOwnedMarkTaskFailedTool);
  return defaultToolRegistry.createTool(MARK_TASK_FAILED_TOOL_NAME) as BaseTool;
}
