...
export const TASK_DELEGATION_TOOL_MANIFEST: TaskDelegationToolManifestEntry[] = [
  {
    ...
    description:
      'Delegate one ready-to-run task to an explicit accountable target. Provide target={kind:"member"|"team", name}, complete task details in description (objective, context, constraints, done conditions, expected output, and reference guidance), and optional reference_files. Member targets start one task-agent; team targets start one task-scoped team run whose ingress coordinator receives the packet. The execution target later submits its result with submit_task_result.',
  ...
  },
  {
    ...
    description:
      "Submit a reviewable result for the delegated task bound to the current task-agent or task-team ingress context. Provide a non-empty message and optional reference_files. The system records the submission, moves the task to awaiting_review, and notifies the task review owner.",
