...
export class XmlUpdatePromptToolParsingState extends XmlToolParsingState {
  ...
  private static PROMPT_ID_PATTERN = /<arg\s+name=["']prompt_id["']\s*>([^<]+)<\/arg>/i;
  private static NEW_CONTENT_ARG_OPEN_PATTERN = /<arg\s+name=["']new_content["']\s*>/i;
  ...
  private getEndMetadata(): Record<string, any> {
    ...
    if (this.accumulatedContent) args.new_content = this.accumulatedContent;
  ...
  run(): void {
    if (!this.context.hasMoreChars()) {
      if (this.contentBuffering.includes(XmlUpdatePromptToolParsingState.CLOSING_TAG)) {
        if (!this.segmentStarted) {
          this.context.emitSegmentStart(
            (this.constructor as typeof XmlUpdatePromptToolParsingState).SEGMENT_TYPE,
    ...
    if (!this.capturedPromptId) {
      const match = XmlUpdatePromptToolParsingState.PROMPT_ID_PATTERN.exec(this.contentBuffering);
      if (match) {
        this.capturedPromptId = match[1].trim();
      }
    }

    const contentMatch = XmlUpdatePromptToolParsingState.NEW_CONTENT_ARG_OPEN_PATTERN.exec(
  ...
  private processSeekMarkerContent(chunk: string): void {
    ...
    const startMarker = (this.constructor as typeof XmlUpdatePromptToolParsingState).START_CONTENT_MARKER;
    ...
    const closingIdx = this.contentSeekBuffer.indexOf(
      (this.constructor as typeof XmlUpdatePromptToolParsingState).CONTENT_ARG_CLOSE_TAG,
    );
    if (closingIdx !== -1) {
  ...
  private processMarkerContent(chunk: string): void {
    ...
    const endMarker = (this.constructor as typeof XmlUpdatePromptToolParsingState).END_CONTENT_MARKER;
  ...
  private processPassthrough(chunk: string): void {
    ...
    const closingTag = (this.constructor as typeof XmlUpdatePromptToolParsingState).CLOSING_TAG;
