...
export class MemoryFileStore {
  ...
  constructor(baseDir: string, options: { agentRootSubdir?: string } = {}) {
    this.baseDir = baseDir;
    this.agentRootSubdir = options.agentRootSubdir ?? "agents";
  }

  private getAgentRootDir(): string {
    if (!this.agentRootSubdir) {
      return this.baseDir;
    }
    return path.join(this.baseDir, this.agentRootSubdir);
  }

  getRunDir(runId: string): string {
    return path.join(this.getAgentRootDir(), runId);
  }

  listRunDirs(): string[] {
    const agentsDir = this.getAgentRootDir();
