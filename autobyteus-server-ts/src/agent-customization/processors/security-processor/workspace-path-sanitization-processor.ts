...
export class WorkspacePathSanitizationProcessor extends BaseAgentUserInputMessageProcessor {
  ...
  async process(
    ...
    if (!(workspace instanceof FileSystemWorkspace) || !workspace.rootPath) {
      logger.debug(
        `Agent '${agentId}': Workspace is not a FileSystemWorkspace or has no rootPath. Skipping path sanitization.`,
      );
      return message;
    }
    ...
    if (originalContent !== sanitizedContent) {
      logger.info(
        `Agent '${agentId}': Sanitized user message by removing workspace path '${rootPath}'.`,
      );
      message.content = sanitizedContent;
    } else {
      logger.debug(
        `Agent '${agentId}': No occurrences of workspace path '${rootPath}' found in message. No sanitization needed.`,
      );
    }
