...
export const projectAgentPresentationMessage = (event: AgentPresentationEvent): AgentPresentationMessage => {
  ...
  switch (type) {
    ...
    case "TOOL_EXECUTION_COMPLETED": return parse({ type, payload: { invocation_id: event.details.invocationId, tool_name: event.details.toolName, turn_id: event.details.turnId, arguments: event.details.arguments, result: event.details.result } });
