...
export class AgentRunManager {
  ...
  async createAgentRun(options: AgentRunConfigInput): Promise<string> {
    ...
    const runId = agent.agentId;
    logger.info(
      `Successfully created and started agent run '${runId}' from definition '${built.agentName}'.`,
    );
    return runId;
  }
  ...
  async restoreAgentRun(
    ...
    const built = await this.buildAgentConfig(options);
    const singleAgentMemoryDir = path.join(
    ...
    const agent = this.agentFactory.restoreAgent(
      options.runId,
      built.agentConfig,
      singleAgentMemoryDir,
    ) as AgentLike & { start?: () => void };
    agent.start?.();
    await this.waitForIdle(agent as Agent);
    const runId = agent.agentId;
    logger.info(
      `Successfully restored and started agent run '${runId}' from definition '${built.agentName}'.`,
    );
    return runId;
  }
  ...
  getAgentEventStream(runId: string): AgentEventStream | null {
    ...
    if (!agent) {
      logger.warn(
        `AgentRunManager: Attempted to get event stream for non-existent run '${runId}'.`,
      );
      return null;
    }
