...
export class SkillImprovementSkillTargetResolver {
  constructor(private readonly skillService: Pick<SkillService, "resolveConfiguredSkillsForAgent"> = SkillService.getInstance()) {}

  async resolveForAgentDefinition(agentDefinition: AgentDefinition): Promise<SkillImprovementSkillTarget[]> {
    const skills = this.skillService.resolveConfiguredSkillsForAgent(agentDefinition);
    return Promise.all(skills.map(async (skill): Promise<SkillImprovementSkillTarget> => {
