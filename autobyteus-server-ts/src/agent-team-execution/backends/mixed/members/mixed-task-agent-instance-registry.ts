...
import { cloneTaskAgentInstanceIdentity, type StartTaskAgentInstanceRequest } from "../../../domain/task-agent-instance.js";
...
export type TaskAgentInstanceDeliveryAccess = {
  deliverInterAgentMessageToTaskAgent(address: AgentTeamAddress, taskAgentRunId: string, request: ResolvedInterAgentMessageDeliveryRequest, beforePublishMemberInput?: (() => void) | null): Promise<AgentOperationResult>;
...
export class MixedTaskAgentInstanceRegistry implements TaskAgentInstanceDeliveryAccess {
  ...
  async start(request: StartTaskAgentInstanceRequest): Promise<AgentOperationResult> {
    ...
    if (!source || source.address !== request.sourceNode.address) return { accepted: false, code: "TARGET_MEMBER_NOT_FOUND", message: `Task Agent target '${address}' was not found.` };
    if (request.receiver.taskAgentRunId !== request.identity.taskAgentRunId) return { accepted: false, code: "TASK_AGENT_IDENTITY_MISMATCH", message: "Task Agent receiver and identity do not match." };
    const runId = request.identity.taskAgentRunId.trim();
    const existing = this.handles.get(runId);
    if (existing?.isActive()) return { accepted: false, code: "TASK_AGENT_ALREADY_ACTIVE", message: `Task AgentRun '${runId}' is already active.` };
    existing?.dispose();
    const handle = this.createHandle(request, source.runtimeKind);
    this.handles.set(runId, handle);
    try {
      const result = await handle.postMessage(request.message);
      if (result.accepted) this.cache.remember(request.identity, handle);
      else await this.cleanup(runId, handle);
      return result;
    } catch (error) { await this.cleanup(runId, handle); throw error; }
  }
  ...
  deliverInterAgentMessageToTaskAgent(address: AgentTeamAddress, taskAgentRunId: string, request: ResolvedInterAgentMessageDeliveryRequest, beforePublishMemberInput: (() => void) | null = null) {
    const resolved = this.resolve(address, taskAgentRunId);
    return "accepted" in resolved ? Promise.resolve(resolved) : resolved.deliverInterMemberMessage(request, beforePublishMemberInput);
  }
  ...
  resolveTaskAgentLogicalContext(runId: string): MixedTeamMemberContext | null {
    ...
    return this.options.teamContext.runtimeContext.memberContexts.find((member) => member.address === handle.context.address) ?? null;
  }

  dispose(): void {
    for (const handle of this.handles.values()) handle.dispose();
    this.handles.clear(); this.cache.forgetTeam(this.rootId());
  }

  private createHandle(request: StartTaskAgentInstanceRequest, runtimeKind: import("../../../../runtime-management/runtime-kind-enum.js").RuntimeKind) {
    return new MixedAgentMemberHandle({
      teamContext: this.options.teamContext,
      context: new MixedAgentMemberContext({ address: request.receiver.memberAddress, agentRunId: request.identity.taskAgentRunId, runtimeKind, platformAgentRunId: null }),
      config: Object.freeze({ ...request.sourceNode, agentRunId: request.identity.taskAgentRunId, platformAgentRunId: null }),
  ...
  private async cleanup(runId: string, handle: MixedAgentMemberHandle) { try { await handle.terminate(); } catch { handle.dispose(); } finally { this.forget(runId); } }
  private forget(runId: string) { this.handles.delete(runId.trim()); this.cache.forget(this.rootId(), runId); }
  private rootId() { return this.options.teamContext.config.rootTeam.teamRunId; }
}
