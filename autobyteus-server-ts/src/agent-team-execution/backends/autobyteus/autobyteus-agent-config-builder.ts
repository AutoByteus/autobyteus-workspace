...
export class AutoByteusAgentConfigBuilder {
  ...
  async build(
    ...
    return new AgentConfig(
      ...
      ),
      {
        ...
        ...(memberConfig.applicationExecutionContext
          ? { [APPLICATION_EXECUTION_CONTEXT_KEY]: memberConfig.applicationExecutionContext }
          : {}),
      },
      skillPaths,
      memoryDir,
  ...
  private resolveSkillPaths(memberName: string, agentDef: AgentDefinition): string[] {
    ...
    for (const skill of this.skillService.resolveConfiguredSkillsForAgent(agentDef)) {
      ...
      logger.info(
        `Resolved skill '${skill.name}' to path: ${skill.rootPath} for team member '${memberName}'`,
      );
    }
