...
const TEAM_MEMBER_KEYS = new Set([
  "memberAddress", "displayName", "agentDefinitionId",
...
const requiredMemberAddress = (value: unknown): string => {
  const normalized = requiredString(
    value,
    "launchOverride.memberProfiles[].memberAddress",
  );
  try {
    return assertAgentTeamAddress(normalized);
  } catch {
    throw new ApplicationLaunchOverrideValidationError(
      "SAVED_OVERRIDE_MALFORMED",
      "launchOverride.memberProfiles[].memberAddress must be a canonical rooted team address.",
    );
  }
};
...
const normalizeMemberOverride = (
  ...
  return {
    memberAddress: requiredMemberAddress(record.memberAddress),
    displayName: requiredString(record.displayName, "launchOverride.memberProfiles[].displayName"),
...
const assertCurrentTopology = (
  ...
  const savedByAddress = new Map<string, ApplicationTeamMemberLaunchOverride>();
  for (const member of saved) {
    if (savedByAddress.has(member.memberAddress)) {
      throw new ApplicationLaunchOverrideValidationError(
        "SAVED_OVERRIDE_MALFORMED",
        `Application slot '${slotKey}' has duplicate memberAddress '${member.memberAddress}'.`,
      );
    }
    savedByAddress.set(member.memberAddress, member);
  }
  const currentByAddress = new Map(current.map((member) => [member.memberAddress, member]));
  const staleMembers: NonNullable<ApplicationLaunchIssue["staleMembers"]> = [];
  for (const member of saved) {
    const currentMember = currentByAddress.get(member.memberAddress);
    if (!currentMember) {
...
export const normalizeApplicationLaunchOverride = (input: {
  ...
  if (input.launchOverride === null || input.launchOverride === undefined) return null;
