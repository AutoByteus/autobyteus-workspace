...
export const SELF_EVOLUTION_PACKAGE_TREE_MAX_DEPTH = 4;
export const SELF_EVOLUTION_PACKAGE_TREE_MAX_ENTRIES = 80;
...
export class SelfEvolutionSkillPackageTreeRenderer {
  private readonly maxDepth: number;
  private readonly maxEntries: number;

  constructor(options: { maxDepth?: number; maxEntries?: number } = {}) {
    this.maxDepth = options.maxDepth ?? SELF_EVOLUTION_PACKAGE_TREE_MAX_DEPTH;
    this.maxEntries = options.maxEntries ?? SELF_EVOLUTION_PACKAGE_TREE_MAX_ENTRIES;
  }

  async render(
    target: Pick<SelfEvolutionSkillTarget, "skillRootPath" | "skillMdPath">,
  ): Promise<string> {
