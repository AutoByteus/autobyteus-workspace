...
describe("AgentSessionManager", () => {
    it("creates sessions successfully", () => {
        const manager = new AgentSessionManager(MockAgentSession);
        const session = manager.createSession("s1", "agent-123");
        expect(session.sessionId).toBe("s1");
        expect(session.runId).toBe("agent-123");
    ...
    it("retrieves sessions for an agent", () => {
        ...
        const sessions = manager.getSessionsForRun("agent-1");
        expect(sessions).toHaveLength(2);
        expect(sessions.every((session) => session.runId === "agent-1")).toBe(true);
    });
    ...
    it("closes all sessions for an agent", () => {
        ...
        manager.createSession("s2", "agent-1");
        manager.createSession("s3", "agent-2");
        const count = manager.closeAllForRun("agent-1");
        expect(count).toBe(2);
        expect(manager.sessionCount).toBe(1);
