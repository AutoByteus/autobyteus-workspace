...
function writeManifest(appRoot: string, appId: string, manifest: Record<string, unknown>): void {
...
function writeModule(appRoot: string, appId: string, content: string, fileName = "app.mjs"): string {
  const appDir = path.join(appRoot, "applications", appId);
  fs.mkdirSync(appDir, { recursive: true });
...
describe("ApplicationService", () => {
  ...
  it("runs async entrypoint", async () => {
    ...
    const moduleUrl = writeModule(
      appRoot,
      "async_app",
      "export async function run(input) { return { status: 'ok', input }; }\n",
    );
  ...
  it("runs sync entrypoint", async () => {
    ...
    const moduleUrl = writeModule(
      appRoot,
      "sync_app",
      "export function run(input) { return { status: 'sync_ok', input }; }\n",
    );
  ...
  it("throws when app not found", async () => {
    ...
    await expect(service.runApplication("missing", {})).rejects.toThrow(
      "Application with ID 'missing' not found.",
    );
  });
  ...
  it("throws when app is team-based", async () => {
    ...
    const service = new ApplicationService(appRoot);
    await expect(service.runApplication("team_app", {})).rejects.toThrow(
  ...
  it("throws when entrypoint module is missing", async () => {
    ...
    const service = new ApplicationService(appRoot);
    await expect(service.runApplication("missing_module", {})).rejects.toThrow(
  ...
  it("throws when entrypoint function is missing", async () => {
    ...
    const service = new ApplicationService(appRoot);
    await expect(service.runApplication("missing_fn", {})).rejects.toThrow(
      "Could not load the entry point",
    );
  });
  ...
  it("propagates errors from entrypoint execution", async () => {
    ...
    const moduleUrl = writeModule(
      appRoot,
      "fail_app",
      "export function run() { throw new Error('App failed!'); }\n",
    );
