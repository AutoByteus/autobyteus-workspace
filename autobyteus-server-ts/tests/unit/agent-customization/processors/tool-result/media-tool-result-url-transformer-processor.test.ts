...
describe("MediaToolResultUrlTransformerProcessor", () => {
  ...
  it.each(["generate_image", "edit_image", "generate_speech"])(
    "transforms target tools (%s)",
    async (toolName) => {
      mockMediaStorageService.storeMediaAndGetUrl.mockResolvedValue(
        "http://server/rest/files/images/out.png",
      );

      const processor = new MediaToolResultUrlTransformerProcessor();
      const context = { agentId: "agent-1" } as AgentContext;
      ...
      expect(mockMediaStorageService.storeMediaAndGetUrl).toHaveBeenCalledWith(
        "/tmp/out.png",
        "out.png",
      );
      expect((processed.result as Record<string, unknown>).output_file_url).toBe(
        "http://server/rest/files/images/out.png",
      );
      expect((processed.result as Record<string, unknown>).local_file_path).toBe("/tmp/out.png");
    },
  );
  ...
  it("resolves relative paths using workspace", async () => {
    ...
    const event = new ToolResultEvent(
      ...
      undefined,
      undefined,
      { output_file_path: "images/out.png" },
    );

    await processor.process(event, context);

    expect(mockMediaStorageService.storeMediaAndGetUrl).toHaveBeenCalledWith(
      "/workspace/root/images/out.png",
      "out.png",
    );
