...
describe("MediaUrlTransformerProcessor", () => {
  it("derives filenames from clean URLs", async () => {
    ...
    const context = { agentId: "test_agent" } as AgentContext;
  ...
  it("emits media segment events", async () => {
    ...
    expect(
      startEvents.some(
        (event) =>
          event.segment_type === "media" &&
          event.payload?.metadata?.media_type === "image" &&
          event.payload?.metadata?.urls?.[0] === "http://server/happy_dog.png",
      ),
    ).toBe(true);
    expect(
      startEvents.some(
        (event) =>
          ...
          event.payload?.metadata?.urls?.[0] === "http://server/sound.png",
      ),
    ).toBe(true);
  });
});
