...
describe("EventBatcher", () => {
  ...
  it("unblocks a pending consumer when the batched stream is returned", async () => {
    ...
    const source: AsyncGenerator<string, void, void> = {
      next: () => new Promise<IteratorResult<string, void>>(() => undefined),
      return: async () => {
    ...
    const result = await Promise.race([
      ...
      new Promise<IteratorResult<string, void>>((_, reject) =>
        setTimeout(() => reject(new Error("Timed out waiting for batched stream return")), 100),
      ),
    ]);
