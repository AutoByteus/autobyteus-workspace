...
describe("FileExplorer", () => {
    ...
    it("throws when reading a file that exceeds max size", async () => {
        ...
        fs.writeFileSync(path.join(tempDir, filePath), content, "utf-8");
        await explorer.buildWorkspaceDirectoryTree();
        await expect(explorer.readFileContent(filePath, 1024 * 1024)).rejects.toThrow("exceeds the maximum allowed size");
    });
    ...
    it("rejects reading files outside the workspace", async () => {
        await expect(explorer.readFileContent("../outside.txt")).rejects.toThrow("Access denied: File is outside the workspace.");
    });
    it("produces shallow JSON with limited depth", async () => {
        ...
        await explorer.buildWorkspaceDirectoryTree();
        const shallowJson = explorer.toShallowJson(1);
        const shallowDict = JSON.parse(shallowJson);
        ...
        const shallowJson2 = explorer.toShallowJson(2);
        const shallowDict2 = JSON.parse(shallowJson2);
