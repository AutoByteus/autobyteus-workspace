...
vi.mock("autobyteus-ts", () => ({
  LLMFactory: {
    ...
    listModelsByProvider: (...args: unknown[]) => mockListModelsByProvider(...args),
    reloadModels: (...args: unknown[]) => mockReloadModels(...args),
  },
}));
...
describe("LlmModelProvider", () => {
  ...
  it("propagates reload errors", async () => {
    ...
    const provider = new LlmModelProvider();
    await expect(provider.refreshModelsForProvider(LLMProvider.OLLAMA)).rejects.toThrow(
      "reload failed",
    );
  });
});
