...
describe("AppConfig", () => {
  ...
  it("uses AUTOBYTEUS_LOG_DIR when provided", async () => {
    const configDir = await createTempConfigDir(
      "AUTOBYTEUS_SERVER_HOST=http://localhost:8000\nAPP_ENV=test\nDB_TYPE=sqlite\nAUTOBYTEUS_LOG_DIR=./runtime-logs\n",
    );
    const config = new AppConfig();
    config.setCustomAppDataDir(configDir);
    ...
    const expectedLogsDir = path.resolve(configDir, "runtime-logs");
    expect(config.getLogsDir()).toBe(expectedLogsDir);
