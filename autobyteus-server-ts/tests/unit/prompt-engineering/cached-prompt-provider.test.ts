...
const createProvider = () => {
  ...
  return { persistenceProvider, cachedProvider: new CachedPromptProvider(persistenceProvider as any) };
};
...
describe("CachedPromptProvider", () => {
  ...
  it("filters active prompts from cache", async () => {
    ...
    expect(new Set(activePrompts.map((prompt) => prompt.id))).toEqual(
      new Set(["1", "3", "4"]),
    );
  });
