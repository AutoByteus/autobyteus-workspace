...
const buildBinding = (): ApplicationAgentTeamBinding => ({
  ...
  runtime: {
    subject: "TEAM_RUN",
    runId: "team-run-brief-catchup-1",
    ...
    members: [
      {
        ...
        teamPath: [],
        runId: "team-run-brief-catchup-1::researcher",
      ...
      },
      {
        ...
        displayName: "Writer",
        teamPath: [],
        runId: "team-run-brief-catchup-1::writer",
        runtimeKind: "AGENT_TEAM_MEMBER",
      },
    ],
  },
...
describe("Brief persisted-artifact startup catch-up", () => {
  ...
  it("starts with valid history around an ineligible retained researcher/final revision", async () => {
    ...
    const researcherRunId = binding.runtime.members[0].runId;
    const writerRunId = binding.runtime.members[1].runId;
    ...
    try {
      expect(
        ...
        latest_run_id: binding.runtime.runId,
      });
      expect(
        verifiedDatabase.prepare(
