...
import { SelfEvolutionSkillPackageTreeRenderer } from "../../src/self-evolution/services/companion/self-evolution-skill-package-tree-renderer.js";
...
describe("SelfEvolutionSkillPackageTreeRenderer", () => {
  let tempRoot: string;
  let skillRoot: string;

  beforeEach(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "self-evolution-package-tree-"));
  ...
  it("renders a relative package tree with SKILL.md marked as the entry file", async () => {
    ...
    const tree = await new SelfEvolutionSkillPackageTreeRenderer().render(makeSkillTarget(skillRoot));
  ...
  it("reports omitted entries when depth and entry caps are reached", async () => {
    ...
    const tree = await new SelfEvolutionSkillPackageTreeRenderer({
      maxDepth: 2,
      maxEntries: 4,
