...
import {
  bindTeamMemberManagerRelayHandler,
  TeamMemberManager,
} from "../../../src/agent-team-execution/services/team-member-manager.js";
...
const createSubject = () => {
  ...
  const manager = new TeamMemberManager({
    agentRunManager,
  ...
  const unbindRelayHandler = bindTeamMemberManagerRelayHandler({
    manager,
...
describe("TeamMemberManager", () => {
  it("derives workspaceRootPath from workspaceId when creating member runtime sessions", async () => {
    const { manager, mocks } = createSubject();
    ...
    const bindings = await manager.createMemberRuntimeSessions("team-1", "team-def-1", [
      {
    ...
    expect(mocks.agentRunManager.restoreAgentRun).toHaveBeenCalledWith(
  ...
  it("defaults send_message_to capability to enabled when agent definition has no explicit tool list", async () => {
    ...
    await manager.createMemberRuntimeSessions("team-1", "team-def-1", [
      {
        memberName: "Professor",
        memberRouteKey: "professor",
        memberRunId: "member-run-1",
  ...
  it("returns deterministic error when sender member is not bound", async () => {
    ...
    const result = await manager.relayInterAgentMessage({
      teamRunId: "team-1",
  ...
  it("refreshes active Claude bindings from the active member run before returning them", () => {
    const { manager, mocks } = createSubject();
    ...
    const bindings = manager.getActiveMemberBindings("team-1");
  ...
  it("returns refreshed binding snapshots when terminating member runtime sessions", async () => {
    ...
    const result = await manager.terminateMemberRuntimeSessionsWithSnapshot("team-1");
