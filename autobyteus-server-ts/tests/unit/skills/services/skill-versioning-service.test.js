...
describeGit("SkillVersioningService", () => {
    ...
    it("createVersion throws if not versioned", () => {
        expect(() => service.createVersion(skillPath, "v1.0.0", "Test")).toThrowError(/not versioned/);
    });
    it("createVersion throws if tag exists", () => {
        ...
        expect(() => service.createVersion(skillPath, "v1.0.0", "Duplicate")).toThrowError(/already exists/);
    });
    ...
    it("diffVersions throws if not versioned", () => {
        expect(() => service.diffVersions(skillPath, "v1.0.0", "v2.0.0")).toThrowError(/not versioned/);
    });
    it("diffVersions throws if from tag not found", () => {
        service.initRepo(skillPath);
        service.createVersion(skillPath, "v1.0.0", "First");
        expect(() => service.diffVersions(skillPath, "v99.0.0", "v1.0.0")).toThrowError(/not found/);
    });
