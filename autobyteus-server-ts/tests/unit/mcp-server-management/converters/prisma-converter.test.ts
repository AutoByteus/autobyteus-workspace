...
import type { McpServerConfiguration } from "@prisma/client";

const makePrismaConfig = (overrides: Partial<McpServerConfiguration>): McpServerConfiguration => {
  return {
    id: 1,
    serverId: "server",
    transportType: "stdio",
...
describe("PrismaMcpServerConfigurationConverter", () => {
  ...
  it("converts stdio config to domain", () => {
    ...
    const stdio = domainObj as StdioMcpServerConfig;
  ...
  it("converts streamable http config to domain", () => {
    ...
    const httpConfig = domainObj as StreamableHttpMcpServerConfig;
