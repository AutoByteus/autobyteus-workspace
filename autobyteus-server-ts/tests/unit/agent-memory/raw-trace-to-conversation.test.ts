...
import { buildConversationView } from "../../../src/agent-memory/transformers/raw-trace-to-conversation.js";
...
describe("raw trace to conversation", () => {
  ...
  it("emits orphan tool results when missing call", () => {
    const entries = buildConversationView(
  ...
  it("keeps explicit tool events when collapse is disabled", () => {
    const traces = [
