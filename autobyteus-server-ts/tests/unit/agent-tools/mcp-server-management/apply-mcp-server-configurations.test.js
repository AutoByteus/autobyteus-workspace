...
describe("applyMcpServerConfigurationsTool", () => {
    ...
    it("propagates service errors", async () => {
        mockService.applyAndRegisterConfigsFromJson.mockRejectedValue(new Error("Missing transport_type"));
        const tool = registerApplyMcpServerConfigurationsTool();
        const jsonInput = JSON.stringify({
            mcpServers: { server1: { transport_type: "stdio", command: "cmd" } },
        });
        await expect(tool.execute({ agentId: "test-agent" }, { configurations_json: jsonInput })).rejects.toThrow("Missing transport_type");
    });
});
