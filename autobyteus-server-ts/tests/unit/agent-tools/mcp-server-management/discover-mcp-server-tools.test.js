...
describe("discoverMcpServerToolsTool", () => {
    ...
    it("returns JSON list of tool summaries", async () => {
        ...
        const result = await tool.execute({ agentId: "test-agent" }, { server_id: "server1" });
    ...
    it("throws when service reports missing config", async () => {
        mockService.discoverAndRegisterToolsForServer.mockRejectedValue(new Error("No configuration found"));
        const tool = registerDiscoverMcpServerToolsTool();
        await expect(tool.execute({ agentId: "test-agent" }, { server_id: "missing" })).rejects.toThrow("No configuration found");
    });
});
