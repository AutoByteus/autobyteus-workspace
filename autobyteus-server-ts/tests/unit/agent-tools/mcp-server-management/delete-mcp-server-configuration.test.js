...
describe("deleteMcpServerConfigurationTool", () => {
    ...
    it("returns not found message when delete fails", async () => {
        ...
        const tool = registerDeleteMcpServerConfigurationTool();
        const result = await tool.execute({ agentId: "test-agent" }, { server_id: "missing" });
    ...
    it("propagates errors from the service", async () => {
        ...
        const tool = registerDeleteMcpServerConfigurationTool();
        await expect(tool.execute({ agentId: "test-agent" }, { server_id: "server1" })).rejects.toThrow("Database connection lost");
    });
});
