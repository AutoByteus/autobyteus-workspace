...
describe("activatePromptTool", () => {
  ...
  it("reports prior active prompt when deactivated", async () => {
    ...
    const result = await tool.execute({ agentId: "test-agent" } as any, { prompt_id: "101" });
  ...
  it("throws when prompt is not found", async () => {
    ...
    const tool = registerActivatePromptTool();
    await expect(
      tool.execute({ agentId: "test-agent" } as any, { prompt_id: "999" }),
    ).rejects.toThrow("Prompt with ID 999 not found.");
  });
});
