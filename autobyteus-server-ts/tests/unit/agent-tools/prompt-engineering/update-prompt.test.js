...
describe("updatePromptTool", () => {
    ...
    it("updates a prompt by creating a new revision", async () => {
        ...
        const result = await tool.execute({ agentId: "test-agent" }, { prompt_id: "1", new_content: "New content" });
    ...
    it("throws when prompt_id is missing", async () => {
        const tool = registerUpdatePromptTool();
        await expect(tool.execute({ agentId: "test-agent" }, { prompt_id: "", new_content: "New content" })).rejects.toThrow("prompt_id is a required");
    });
    it("throws when new_content is missing", async () => {
        const tool = registerUpdatePromptTool();
        await expect(tool.execute({ agentId: "test-agent" }, { prompt_id: "1", new_content: "" })).rejects.toThrow("new_content is a required");
    });
    it("propagates prompt not found errors", async () => {
        mockGetPromptById.mockRejectedValue(new Error("Prompt not found"));
        const tool = registerUpdatePromptTool();
        await expect(tool.execute({ agentId: "test-agent" }, { prompt_id: "999", new_content: "New content" })).rejects.toThrow("Prompt not found");
    });
});
