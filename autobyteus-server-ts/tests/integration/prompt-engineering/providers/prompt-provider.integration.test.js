...
describe("SqlPromptProvider", () => {
    ...
    it("updates prompts through provider", async () => {
        ...
        const created = await provider.createPrompt(new Prompt({ name, category: "Provider", promptContent: "Original" }));
    ...
    it("finds prompts by name/category with suitable model filter", async () => {
        ...
        await provider.createPrompt(new Prompt({
            name,
            category: "Provider",
        ...
        const gptResults = await provider.findAllByNameAndCategory(name, "Provider", "gpt-4o");
