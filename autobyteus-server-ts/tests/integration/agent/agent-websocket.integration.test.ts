...
class FakeAgentManager {
  ...
  getAgentRun(agentId: string): FakeAgent | null {
    return agentId === this.agent.agentId ? this.agent : null;
  }

  getAgentEventStream(agentId: string): FakeEventStream | null {
...
describe("Agent websocket integration", () => {
  it("streams events and handles client messages", async () => {
    ...
    const handler = new AgentStreamHandler(new AgentSessionManager(), manager as unknown as any);
