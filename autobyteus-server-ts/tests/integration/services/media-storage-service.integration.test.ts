...
describe("MediaStorageService integration", () => {
  ...
  it("saves media from a remote URL", async () => {
    downloadFileFromUrlMock.mockImplementation(async (_url: string, destinationDir: string) => {
  ...
  it("propagates download failures and leaves no files", async () => {
    ...
    await expect(service.storeMediaAndGetUrl(TEST_REMOTE_URL, "remote_image")).rejects.toThrow(
      "HTTP 404",
    );
  ...
  it("throws for missing media files", () => {
    expect(() => service.deleteMediaFile("images", "missing.png")).toThrow(
      MediaFileNotFoundError,
    );
  });

  it("throws for invalid categories", () => {
    expect(() => service.deleteMediaFile("aliens", "file.txt")).toThrow(
      InvalidMediaCategoryError,
    );
  });

  it("throws for path traversal attempts", () => {
    expect(() => service.deleteMediaFile("images", "../../../etc/passwd")).toThrow(
      InvalidMediaPathError,
    );
  });
});
