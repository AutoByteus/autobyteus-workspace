...
describe("Team run history GraphQL e2e", () => {
  ...
  it("lists team run history, returns resume config/projection, and deletes history", async () => {
    const { teamRunId, memberRouteKey, memberAgentId } = await seedTeamRunHistory();
    ...
    expect(projectionResult.getTeamMemberRunProjection.runId).toBe(memberAgentId);
  ...
  it("continues an offline team run for an existing teamRunId", async () => {
    ...
    const continueSpy = vi
      .spyOn(TeamRunContinuationService.prototype, "continueTeamRun")
      .mockResolvedValue({
        teamRunId,
        restored: true,
      });
    ...
    expect(continueSpy).toHaveBeenCalledWith({
      teamRunId,
      targetMemberRouteKey: "super_agent",
      userInput: {
        content: "hello continuation",
        contextFiles: [],
      },
  ...
  it("persists team member memory and restores it after terminate/continue", async () => {
    ...
    const memoryDir = appConfigProvider.config.getMemoryDir();
    const memberMemoryDir = path.join(memoryDir, "agent_teams", teamRunId, memberAgentId);
    const rawTraceFile = path.join(memberMemoryDir, "raw_traces.jsonl");
