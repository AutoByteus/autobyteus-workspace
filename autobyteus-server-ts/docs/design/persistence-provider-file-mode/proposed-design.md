...
Introduce a first-class file persistence profile in `autobyteus-server-ts` so the server can switch persistence behavior using configuration (`PERSISTENCE_PROVIDER=file`), compile/run without Prisma when needed, and use one consistent persistence resolution pattern: `registry + proxy`.
...
- Make Prisma optional in build/install via explicit build profiles.
...
| --- | --- | --- | --- | --- | --- |
| UC-001 | Startup in file mode skips Prisma migrations | Yes | N/A | Yes | `proposed-design-based-runtime-call-stack.md` -> UC-001 |
| UC-002 | Prompt CRUD via file provider | Yes | Yes | Yes | `proposed-design-based-runtime-call-stack.md` -> UC-002 |
| UC-003 | Agent definition + mapping via file providers | Yes | Yes | Yes | `proposed-design-based-runtime-call-stack.md` -> UC-003 |
| UC-004 | Agent team definition via file provider | Yes | Yes | Yes | `proposed-design-based-runtime-call-stack.md` -> UC-004 |
| UC-005 | MCP config + startup load/register via file provider | Yes | Yes | Yes | `proposed-design-based-runtime-call-stack.md` -> UC-005 |
| UC-006 | External-channel ingress+callback flows via file providers | Yes | Yes | Yes | `proposed-design-based-runtime-call-stack.md` -> UC-006 |
| UC-007 | Token usage persistence/statistics via file provider | Yes | Yes | Yes | `proposed-design-based-runtime-call-stack.md` -> UC-007 |
| UC-008 | Artifact create/read via file provider | Yes | Yes | Yes | `proposed-design-based-runtime-call-stack.md` -> UC-008 |
| UC-009 | File build profile skips Prisma | Yes | N/A | Yes | `proposed-design-based-runtime-call-stack.md` -> UC-009 |
| UC-010 | Single pattern across domains (`registry + proxy`) | Yes | N/A | Yes | `proposed-design-based-runtime-call-stack.md` -> UC-010 |
...
- Phase 4: Android/Termux validation with file profile artifacts.
...
| --- | --- | --- | --- |
| C-001..C-004 | startup/profile foundation | startup integration tests | Planned |
