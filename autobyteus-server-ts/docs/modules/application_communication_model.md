...
When the frontend calls `client.backend.command(name, input)`, the request flows through the backend API gateway and engine host to the app backend handler. The handler receives `publishNotification` and the named orchestration capabilities, but none are invoked unless handler code explicitly calls them. Frontend request/response is a synchronous transport boundary, not an execution trigger. Once accepted, application work stays correlated through both host-to-worker and worker-to-host JSON-line RPC until the real handler/capability result or error returns. A transport-local deadline must not report failure while a live remote operation continues; only lifecycle-control paths may time out after terminating and awaiting the worker.

### 5. Standard application-agent communication is direct and separate

`applicationClient.agentCommunication.connect(address)` is the framework-standard bidirectional path for a frontend to communicate with an application-bound agent, whole team, or selected static member. Its public address is exactly `{ bindingId, memberAddress }`: `memberAddress: null` selects the bound Agent or Team root, while a canonical rooted non-root value such as `/reviewer` selects one configured Team member. Physical Agent/Team run IDs are not public selectors. `ApplicationAgentTargetAuthorizationService` authorizes the binding and performs the sole logical-to-physical translation; input and streaming consume its private resolved target without reloading or reinterpreting the public address. The trusted desktop host supplies the fixed application-scoped endpoint. Communication owns readiness/input/network state, Streaming owns exact provider-neutral event projection and per-consumer FIFO delivery, and Orchestration owns target authorization and binding lifecycle. This path does not invoke application backend handlers.
