...
state. Tool results are canonical JSON payloads or canonical task-delegation
error payloads, and normal lifecycle projection still owns the visible
transcript/Activity state. The old task-plan polling and creation tool names are
not exposed through this MCP surface.
