...
- Nested team events expose `source_path` / `source_route_key`; agent events
  ...
  `task_id`. Display-only aliases, if present, are not routing identity and are
  not accepted as command targets.
