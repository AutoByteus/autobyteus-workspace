...
- optional member segmented archive: stored beside the member memory artifacts in that root-hierarchical team/member directory, for example `memory/agent_teams/<rootTeamRunId>/<...teamRunPath>/<memberRunId>/raw_traces_archive_manifest.json` plus `raw_traces_archive/*.jsonl`
...
- new team runs use `<team_definition_name_slug>_<uuid-without-dashes>` generated by `TeamRunService`; child subteam wrappers store their generated child `teamRunId` as wrapper `memberRunId` / `childTeamRunId`
...
- team member routing identity is path-based and separate from `memberRunId`:
  - `memberPath` is the canonical nested path array
  - `memberRouteKey` is the normalized slash-delimited route key derived from that path
...
- `LocalMemoryRunViewProjectionProvider` reads the complete raw-trace corpus
  ...
  `memory/agent_teams/<rootTeamRunId>/<...teamRunPath>/...`, rather than
...
team-member `agent_teams/<rootTeamRunId>/<...teamRunPath>/<memberRunId>/file_changes.json`,
...
explicit `memoryDir`, so a team-member replay reads `<memberRunId>` inside the
resolved root-hierarchical team directory, for example
`memory/agent_teams/<rootTeamRunId>/<...teamRunPath>/...`, rather than confusing
...
3. `memberRunId` identifies the persisted child run/storage subtree, but it is not a route key and may be an allocator-generated opaque id.
...
5. `agent_team` nodes record the child `teamRunId` and child tree used to
   restore nested mixed-team handles on demand. Child `teamRunId` values form the
   root-relative `teamRunPath` used for nested memory directories.
