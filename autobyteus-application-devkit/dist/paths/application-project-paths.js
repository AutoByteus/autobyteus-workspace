...
const assertDirectChild = (parentPath, targetPath, fieldName) => {
...
const assertInside = (rootPath, targetPath, fieldName) => {
...
const resolveProjectRelativePath = (projectRoot, rawPath, fieldName) => {
...
const resolveOutputPath = (projectRoot, rawPath, fieldName) => {
    const normalized = rawPath.trim();
    if (!normalized) {
        throw new Error(`${fieldName} is required.`);
    }
...
const assertNoOverlap = (leftPath, rightPath, message) => {
    if (isSameOrInside(leftPath, rightPath) || isSameOrInside(rightPath, leftPath)) {
        throw new Error(message);
    }
};
const resolveOptionalChildPath = (parentPath, rawPath, fieldName) => {
    if (!rawPath) {
        return null;
    }
    const normalized = rawPath.trim();
...
export const resolveApplicationProjectPaths = (input) => {
    ...
    const outputPackageRoot = resolveOutputPath(projectRoot, input.outputPackageRootOverride ?? input.config.output.packageRoot, 'output.packageRoot');
    ...
    const sourceAgentTeamsRoot = resolveProjectRelativePath(projectRoot, input.config.source.agentTeamsDir, 'source.agentTeamsDir');
    ...
    return {
        ...
        sourceFrontendRoot,
        sourceFrontendEntryPoint: resolveOptionalChildPath(sourceFrontendRoot, input.config.frontend.entryPoint, 'frontend.entryPoint'),
...
//# sourceMappingURL=application-project-paths.js.map