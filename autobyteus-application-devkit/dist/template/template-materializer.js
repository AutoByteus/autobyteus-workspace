...
const TEMPLATE_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'templates', 'basic');
const SOURCE_TEMPLATE_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..', 'templates', 'basic');
const isTextTemplateFile = (filePath) => {
    const extension = path.extname(filePath).toLowerCase();
    return ['.json', '.mjs', '.ts', '.html', '.css', '.md', '.svg', '.gitignore', ''].includes(extension)
        || path.basename(filePath) === '.gitignore';
};
const toSafePackageName = (applicationId) => (applicationId
    .toLowerCase()
    .replace(/[^a-z0-9._~-]+/g, '-')
    .replace(/^-+|-+$/g, '')
    || 'custom-autobyteus-application');
const assertTargetDirectoryReady = async (targetDirectory, force) => {
    let entries = [];
    try {
...
const renderTemplateContent = (content, variables) => (Object.entries(variables).reduce((nextContent, [key, value]) => nextContent.replaceAll(`__${key}__`, value), content));
const resolveExistingTemplateRoot = async () => {
    try {
...
export const materializeApplicationTemplate = async (options) => {
...
//# sourceMappingURL=template-materializer.js.map