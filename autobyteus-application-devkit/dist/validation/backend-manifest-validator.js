...
import { isObjectRecord, pathExists, pushExistingPathDiagnostic, pushVersionDiagnostic, pushUnknownKeyDiagnostics, readJsonFile, validateManifestPath, validateSupportedExposures, } from './validation-helpers.js';
const validateBackendRuntimeFields = (diagnostics, rawManifest) => {
...
const validateBackendSdkCompatibility = (diagnostics, rawManifest) => {
    ...
    pushUnknownKeyDiagnostics(diagnostics, sdkCompatibility, ['backendDefinitionContractVersion', 'frontendSdkContractVersion'], 'sdkCompatibility', 'INVALID_BACKEND_MANIFEST');
    pushVersionDiagnostic(diagnostics, sdkCompatibility.backendDefinitionContractVersion, APPLICATION_BACKEND_DEFINITION_CONTRACT_VERSION_V6, 'sdkCompatibility.backendDefinitionContractVersion');
    pushVersionDiagnostic(diagnostics, sdkCompatibility.frontendSdkContractVersion, APPLICATION_FRONTEND_SDK_CONTRACT_VERSION_V6, 'sdkCompatibility.frontendSdkContractVersion');
};
...
export const validateBackendManifest = async (input) => {
    ...
    try {
        ...
        input.diagnostics.push(errorDiagnostic('INVALID_JSON', `backend.bundleManifest could not be read as JSON: ${String(error)}`, input.manifestRelativePath));
        return;
    }
    ...
    pushUnknownKeyDiagnostics(input.diagnostics, rawManifest, ['contractVersion', 'entryModule', 'moduleFormat', 'distribution', 'targetRuntime', 'sdkCompatibility', 'supportedExposures', 'migrationsDir', 'assetsDir'], 'backend', 'INVALID_BACKEND_MANIFEST');
...
//# sourceMappingURL=backend-manifest-validator.js.map