...
import { createValidationResult, errorDiagnostic } from './validation-result.js';
import { pathExists, statIfExists } from './validation-helpers.js';
export const validateApplicationPackage = async (packageRoot) => {
    const diagnostics = [];
    ...
    if (await pathExists(path.join(resolvedPackageRoot, 'application.json'))) {
        diagnostics.push(errorDiagnostic('PACKAGE_ROOT_NOT_IMPORTABLE', 'Package root appears to be a source/application root. Import dist/importable-package instead.', resolvedPackageRoot));
    }
