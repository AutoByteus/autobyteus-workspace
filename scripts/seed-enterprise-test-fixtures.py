...
This script is intentionally external to backend source code and can be run
after Docker startup to prepare repeatable test data.
"""
...
class GraphqlClient:
    ...
    def execute(self, query: str, variables: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if errors:
            messages = "; ".join(
                str(item.get("message", "unknown GraphQL error")) for item in errors if isinstance(item, dict)
            )
...
def ensure_prompt(client: GraphqlClient, *, name: str, content: str) -> None:
    ...
    """
    prompts_data = client.execute(prompts_query, {"isActive": True})
...
def ensure_agent(
    ...
    *,
    ...
    """
    definitions = client.execute(query_agent_definitions).get("agentDefinitions") or []
...
def ensure_team(client: GraphqlClient, professor_agent_id: str, student_agent_id: str) -> None:
    ...
    """
    teams = client.execute(list_teams_query).get("agentTeamDefinitions") or []
    ...
    nodes_normalized = sorted(
        [
            (
                node.get("memberName"),
                str(node.get("referenceId")),
                node.get("referenceType"),
                node.get("homeNodeId") or "embedded-local",
            )
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Seed Docker test fixtures via GraphQL.")
    parser.add_argument(
        "--graphql-url",
        required=True,
        help="GraphQL endpoint URL (e.g., http://127.0.0.1:63362/graphql)",
    )
    parser.add_argument("--retries", type=int, default=60, help="Backend readiness retry count")
...
def main(argv: List[str]) -> int:
    ...
    wait_for_backend(client, retries=args.retries, delay_seconds=args.retry_delay)
    log(f"Backend ready at {args.graphql_url}")
    ...
    professor_agent_id = ensure_agent(
        client,
        name=PROFESSOR_AGENT_NAME,
        role="Professor",
        description="Fixture professor agent for communication tests.",
        system_prompt_name=PROFESSOR_PROMPT_NAME,
    )
    student_agent_id = ensure_agent(
        client,
        name=STUDENT_AGENT_NAME,
        role="Student",
        description="Fixture student agent for communication tests.",
        system_prompt_name=STUDENT_PROMPT_NAME,
    )

    ensure_team(client, professor_agent_id, student_agent_id)
