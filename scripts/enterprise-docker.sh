...
COMPOSE_FILE="${ROOT_DIR}/docker/compose.enterprise-test.yml"
...
usage() {
  cat <<USAGE
Usage:
  ./scripts/enterprise-docker.sh <command> [options]
...
Commands:
  ...
  sync-remotes Run full sync from embedded node to discovered remote node(s)

Options (all commands):
  -p, --project <name>   Compose project name (default: ${DEFAULT_PROJECT})

Options (up only):
  -r, --remote-nodes <n> Number of remote server-only containers (default: 1)
  --no-build             Skip Docker image build
  --fresh-ports          Pick a new set of host ports
  --seed-test-fixtures   Seed Professor/Student fixture data after startup (default: on)
  --no-seed-test-fixtures Disable fixture seeding during startup
  --sync-remotes         Run post-start full sync to discovered remote node(s) (default: on)
...
run_seed_fixtures() {
  local env_file="$1"
  local seed_script="${ROOT_DIR}/scripts/seed-enterprise-test-fixtures.py"
...
run_remote_full_sync() {
  local env_file="$1"
  local expected_remote_count="$2"
  local sync_script="${ROOT_DIR}/scripts/run-enterprise-remote-sync.py"
  ...
  log "Running full sync to discovered remotes via ${graphql_url} (expected remotes >= ${expected_remote_count})"
  python3 "${sync_script}" \
...
case "${cmd}" in
  up)
    ...
    if [[ "${sync_remotes}" -eq 1 ]]; then
      if [[ "${remote_nodes}" -gt 0 ]]; then
        run_remote_full_sync "${env_file}" "${remote_nodes}"
      else
  ...
  sync-remotes)
    ...
    run_remote_full_sync "${env_file}" "${expected_remote_count}"
    ;;
