...
const supportsPreview = computed(() => {
  ...
  if (props.disableRichTextPreview) return false;
...
const fetchContent = async () => {
  ...
  try {
    const response = await authorizedFetch(props.contentUrl, { cache: 'no-store' });
    ...
    if (currentToken === fetchToken) isFetchingContent.value = false;
  }
};
...
const handleKeydown = (event: KeyboardEvent) => {
  if (event.key === 'Escape' && isMaximized.value) isMaximized.value = false;
};
...
watch(
  () => [props.contentUrl, props.reference.referenceId, props.reference.path, props.reference.type, props.reference.updatedAt, props.refreshSignal ?? 0],
