<template>
  <div class="mx-auto flex w-full max-w-4xl flex-col gap-6 px-6 py-8">
    <header class="space-y-2">
      ...
      <h2 class="text-3xl font-semibold text-gray-900">{{ $t('settings.components.settings.ExtensionsManager.optional_capabilities') }}</h2>
      <p class="max-w-2xl text-sm leading-6 text-gray-600">{{ $t('settings.components.settings.ExtensionsManager.install_voice_input_without_increasing_the') }}</p>
    </header>

    <div v-if="!extensionsStore.isElectron" class="rounded-xl border border-gray-200 bg-gray-50 p-5 text-sm text-gray-600">{{ $t('settings.components.settings.ExtensionsManager.extensions_are_available_only_in_the') }}</div>
    ...
    <VoiceInputExtensionCard
      ...
      @enable="extensionsStore.enableExtension('voice-input')"
      @disable="extensionsStore.disableExtension('voice-input')"
