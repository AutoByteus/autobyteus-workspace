...
**Requirements scope approved; design corrected after CR-001 scope review; package pending architecture/source-review reroute.**
...
- **Application-agent contract ownership (CR-001):** At the source-review point, `autobyteus-server-ts/src/application-agent-streaming/services/application-agent-stream-event-projector.ts` correctly forwarded the safe provider message and returned the existing `{ type: "ERROR", message }` shape. The code-review finding came from an upstream design/supplement interpretation that incorrectly required native provider metadata through the application SDK, while the normative application communication contract remained message-only. The user clarified that the application SDK is provider-neutral and must not expose platform transport code, provider status/code, request IDs, or diagnostic details. The corrected design keeps the public `ERROR` shape message-only and changes only the stale generic-message rule to safe original-message passthrough. Native agent/team/web transport may continue carrying canonical `code` and optional safe metadata for platform diagnostics; those fields stop at the application projector. Raw provider objects, stacks, causes, headers, credentials, session/thread/item IDs, and semantic provider classification remain excluded.

## Downstream Source-Review Finding And Scope Resolution

Source: `/Users/normy/autobyteus_org/autobyteus-worktrees/provider-catalog-pricing-error-messaging/code-review-report.md`, revision `/Users/normy/autobyteus_org/autobyteus-worktrees/provider-catalog-pricing-error-messaging/code-review-revision-record.md`, finding `CR-001`.

- The supported application-agent path is reachable through `ApplicationAgentStreamProjector` and `ApplicationAgentStreamEvent`; its source shape is intentionally provider-neutral and message-only.
- The source review correctly identified a contradiction between the provisional SR-012 design text and the existing application contract/source. The contradiction is a solution-package scope error, not evidence that the SDK needs provider metadata.
- User-approved resolution: retain `{ type: "ERROR"; message: string }`; pass through the safe original provider message; remove the stale generic fallback rule from the normative contract. Do not add `code`, provider status/code, request ID, or diagnostic details to the application SDK or its public wire contract.
- Native/platform/team transport remains responsible for canonical required `code` and optional safe provider metadata because those fields repair the Docker/team event path and support platform diagnostics. The application projector is the intentional narrowing boundary.
- Current source evidence already matches the narrowed SDK shape: `application-agent-stream-event-projector.ts` returns `{ type: "ERROR", message }` and `application-agent-events.ts` declares only `message`. Downstream work must verify this source against the corrected contract rather than implement the superseded metadata extension.
...
| `tickets/done/application-agent-streaming/application-agent-communication-contract.md` | Normative application-agent binding, stream, and public ERROR contract. | Reconciled during CR-001 scope review to preserve the original safe message in the existing message-only ERROR shape while retaining the focused five-variant stream. | REQ-007–REQ-010; AC-011, AC-014–AC-015. | Intended behavior; implementation and generated SDK output must remain synchronized. |
...
The requirements scope is approved. Following ARCH-REV-001, ARCH-REV-002, ARCH-REV-003, and CR-001, the package was reworked without expanding the user-visible scope: stable IDs were reconciled, persisted-model validation/reselection was assigned to the application configuration/run-binding path with the factory scoped to AutoByteus catalog ownership, provider evidence was mapped through native runtime/team/web boundaries, and the DeepSeek schedule was projected onto the existing pricing structures. The application-agent public contract remains the existing five-variant, message-only stream; only the stale generic error-message rule is replaced by safe original-message passthrough. The new runtime-aware rule remains explicit: AutoByteus selections use the AutoByteus catalog guard; Claude/Codex selections remain with their external runtime factories. The key confirmed root cause for the screenshot is the local event field mismatch, while the balance hypothesis and Docker build identity remain unproven. The corrected package requires architecture/source-review confirmation before API/E2E proceeds.
