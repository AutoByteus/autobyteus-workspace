...
| B-009 | The web error segment renders the received message under a generic “An Error Occurred” heading, and the malformed team message hides the original cause. | Render the original provider error message, after secret redaction only, in the existing error segment. Do not replace a meaningful provider message with a locally invented category/action message. | Existing successful tool cards, error-segment layout, and genuine validation meaning remain intact. | REQ-009, REQ-010; AC-014, AC-015 |
...
| REQ-007 | Provider failures must preserve the original provider message and safe provider status/code/request ID through the runtime and UI. The application must not semantically reclassify or replace provider messages. Only absent/blank API-key configuration is translated into the explicit `missing_api_key` setup message required by REQ-006. |
| REQ-008 | Agent error events must use a canonical non-empty protocol `code` field from `autobyteus-ts` through stream parsing, AgentRun mapping, team adaptation, websocket projection, and client contract, while keeping `message` as the original safe error text. A malformed event must not rewrite a valid provider error into `code is required`. |
| REQ-009 | User-visible provider errors must show the original provider message after redacting credentials or other sensitive material. Diagnostics/logs may include safe status/code/request identifiers but must not include keys, authorization headers, or full sensitive payloads. |
...
| AC-011 | Provider authentication/rate/request response | Representative provider messages reach the user unchanged after safe redaction, with safe status/provider code/request ID available as supplemental metadata. |
...
| AC-014 | Team/Docker node error event | The original provider error message reaches the team websocket/client contract, including the Docker-node path, and renders in the UI; `Rejected ERROR: code is required` is absent for provider failures. |
...
- `autobyteus-ts`, server/backend, team stream contracts, and web rendering are separate package boundaries and must be changed as one contract.
