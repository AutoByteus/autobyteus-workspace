...
describe('CompactionResponseParser', () => {
  it('parses the facts-only compactor result JSON', () => {
    const result = new CompactionResponseParser().parse(JSON.stringify({
      episodic_summary: 'Earlier work was compacted.',
    ...
    expect(result.episodicSummary).toBe('Earlier work was compacted.');
