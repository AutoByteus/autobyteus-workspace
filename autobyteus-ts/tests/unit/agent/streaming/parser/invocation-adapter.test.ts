...
describe('ToolInvocationAdapter basics', () => {
  it('creates invocation for complete tool segment', () => {
    ...
    const events = [
      SegmentEvent.start(TURN_ID, 'seg_5', SegmentType.TOOL_CALL, { tool_name: 'read_file' }),
      SegmentEvent.content(TURN_ID, 'seg_5', '<arguments><path>/src/main.py</path></arguments>'),
      new SegmentEvent({
  ...
  it('uses segment_id as invocation id', () => {
    const adapter = new ToolInvocationAdapter();
    const start = SegmentEvent.start(TURN_ID, 'my-unique-id-123', SegmentType.TOOL_CALL, { tool_name: 'write_file' });
  ...
  it('ignores non-tool segments', () => {
    const adapter = new ToolInvocationAdapter();
    const events = [
      SegmentEvent.start(TURN_ID, 'seg_1', SegmentType.TEXT),
      SegmentEvent.content(TURN_ID, 'seg_1', 'hello'),
      SegmentEvent.end(TURN_ID, 'seg_1')
    ];
  ...
  it('creates write_file invocation', () => {
    const adapter = new ToolInvocationAdapter();
    const events = [
      SegmentEvent.start(TURN_ID, 'seg_2', SegmentType.WRITE_FILE, { path: '/test.py' }),
      SegmentEvent.content(TURN_ID, 'seg_2', 'code'),
      SegmentEvent.end(TURN_ID, 'seg_2')
    ];
  ...
  it('creates run_bash invocation', () => {
    const adapter = new ToolInvocationAdapter();
    const events = [
      SegmentEvent.start(TURN_ID, 'seg_3', SegmentType.RUN_BASH),
      SegmentEvent.content(TURN_ID, 'seg_3', 'mkdir -p project && cd project && pwd'),
      SegmentEvent.end(TURN_ID, 'seg_3')
    ];
  ...
  it('decodes XML entities in realistic chained run_bash segment content', () => {
    const adapter = new ToolInvocationAdapter();
    const events = [
      SegmentEvent.start(TURN_ID, 'seg_3_encoded', SegmentType.RUN_BASH),
      SegmentEvent.content(TURN_ID, 
        'seg_3_encoded',
        'mkdir -p test_folder &amp;&amp; cd test_folder &amp;&amp; printf &quot;hello&quot; &gt; note.txt &amp;&amp; cat note.txt'
      ),
      SegmentEvent.end(TURN_ID, 'seg_3_encoded')
    ];
  ...
  it('creates run_bash invocation with background metadata', () => {
    ...
    const events = [
      ...
      SegmentEvent.content(TURN_ID, 'seg_3_bg', 'npm run dev'),
      SegmentEvent.end(TURN_ID, 'seg_3_bg')
    ];
  ...
  it('creates run_bash invocation with timeoutSeconds metadata alias', () => {
    const adapter = new ToolInvocationAdapter();
    const events = [
      SegmentEvent.start(TURN_ID, 'seg_3_timeout_alias', SegmentType.RUN_BASH, { timeoutSeconds: 12 }),
      SegmentEvent.content(TURN_ID, 'seg_3_timeout_alias', 'echo alias'),
      SegmentEvent.end(TURN_ID, 'seg_3_timeout_alias')
    ];
  ...
  it('uses metadata.command when run_bash segment has empty body', () => {
    const adapter = new ToolInvocationAdapter();
    const events = [
      SegmentEvent.start(TURN_ID, 'seg_3_command_meta', SegmentType.RUN_BASH, { command: 'pwd' }),
      SegmentEvent.end(TURN_ID, 'seg_3_command_meta')
    ];
  ...
  it('uses metadata.cmd fallback when run_bash segment has empty body', () => {
    const adapter = new ToolInvocationAdapter();
    const events = [
      SegmentEvent.start(TURN_ID, 'seg_3_cmd_meta', SegmentType.RUN_BASH, { cmd: 'whoami' }),
      SegmentEvent.end(TURN_ID, 'seg_3_cmd_meta')
    ];
  ...
  it('content command takes precedence over metadata.command', () => {
    const adapter = new ToolInvocationAdapter();
    const events = [
      SegmentEvent.start(TURN_ID, 'seg_3_precedence', SegmentType.RUN_BASH, { command: 'metadata command' }),
      SegmentEvent.content(TURN_ID, 'seg_3_precedence', 'content command'),
      SegmentEvent.end(TURN_ID, 'seg_3_precedence')
    ];
  ...
  it('parses JSON tool call', () => {
    const adapter = new ToolInvocationAdapter();
    const events = [
      SegmentEvent.start(TURN_ID, 'seg_json', SegmentType.TOOL_CALL),
      SegmentEvent.content(TURN_ID, 'seg_json', '{"name": "search", "arguments": {"query": "autobyteus"}}'),
      SegmentEvent.end(TURN_ID, 'seg_json')
    ];
  ...
  it('creates multiple tool invocations', () => {
    const adapter = new ToolInvocationAdapter();
    const events = [
      SegmentEvent.start(TURN_ID, 'seg_1', SegmentType.TOOL_CALL, { tool_name: 'tool_a' }),
      SegmentEvent.content(TURN_ID, 'seg_1', '<arguments><x>1</x></arguments>'),
      new SegmentEvent({
        event_type: SegmentEventType.END,
        segment_id: 'seg_1',
        turn_id: TURN_ID,
      ...
      SegmentEvent.start(TURN_ID, 'seg_2', SegmentType.TOOL_CALL, { tool_name: 'tool_b' }),
      SegmentEvent.content(TURN_ID, 'seg_2', '<arguments><y>2</y></arguments>'),
      new SegmentEvent({
        event_type: SegmentEventType.END,
        segment_id: 'seg_2',
        turn_id: TURN_ID,
  ...
  it('ignores write_file without path', () => {
    const adapter = new ToolInvocationAdapter();
    const events = [
      SegmentEvent.start(TURN_ID, 'seg_x', SegmentType.WRITE_FILE),
      SegmentEvent.content(TURN_ID, 'seg_x', 'code'),
      SegmentEvent.end(TURN_ID, 'seg_x')
    ];
...
describe('ToolInvocationAdapter state', () => {
  it('tracks active segments', () => {
    const adapter = new ToolInvocationAdapter();
    adapter.processEvent(SegmentEvent.start(TURN_ID, 'seg_1', SegmentType.TOOL_CALL, { tool_name: 'test' }));
  ...
  it('reset clears state', () => {
    const adapter = new ToolInvocationAdapter();
    adapter.processEvent(SegmentEvent.start(TURN_ID, 'seg_1', SegmentType.TOOL_CALL, { tool_name: 'test' }));
    adapter.processEvent(SegmentEvent.start(TURN_ID, 'seg_2', SegmentType.TOOL_CALL, { tool_name: 'test' }));
...
describe('ToolInvocationAdapter edge cases', () => {
  it('ignores end without start', () => {
    const adapter = new ToolInvocationAdapter();
    const result = adapter.processEvent(SegmentEvent.end(TURN_ID, 'unknown_seg'));
    expect(result).toBeNull();
  });

  it('ignores content without start', () => {
    const adapter = new ToolInvocationAdapter();
    const result = adapter.processEvent(SegmentEvent.content(TURN_ID, 'unknown_seg', 'data'));
    expect(result).toBeNull();
  });
  ...
  it('keeps incomplete segments active', () => {
    const adapter = new ToolInvocationAdapter();
    adapter.processEvent(SegmentEvent.start(TURN_ID, 'seg_1', SegmentType.TOOL_CALL, { tool_name: 'test' }));
    adapter.processEvent(SegmentEvent.content(TURN_ID, 'seg_1', 'content'));
