...
const TURN_ID = 'turn_0001';

describe('SegmentEvent', () => {
  it('builds start/content/end helpers with expected payloads', () => {
    const start = SegmentEvent.start(TURN_ID, 'seg-1', SegmentType.TEXT, { foo: 'bar' });
    ...
    const content = SegmentEvent.content(TURN_ID, 'seg-1', 'hello');
    ...
    const end = SegmentEvent.end(TURN_ID, 'seg-1');
