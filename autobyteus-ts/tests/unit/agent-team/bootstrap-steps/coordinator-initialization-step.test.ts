...
const makeAgentConfig = (name: string, initialCustomData: Record<string, unknown> | null = null): AgentConfig => {
  ...
  return new AgentConfig(name, name, `${name} description`, llm, null, null, true, null, null, null, null, null, null, null, initialCustomData);
};
...
const makeContextWithPlacement = (isLocalToCurrentNode: boolean): AgentTeamContext => {
  const node = new TeamNodeConfig({
    nodeDefinition: makeAgentConfig('Coordinator', {
      teamMemberPlacement: {
        homeNodeId: isLocalToCurrentNode ? 'node-local' : 'node-remote',
        isLocalToCurrentNode
      }
    })
  });
  const config = new AgentTeamConfig({
...
describe('CoordinatorInitializationStep', () => {
  ...
  it('skips coordinator initialization when coordinator is non-local', async () => {
    context = makeContextWithPlacement(false);
    const mockManager: any = context.teamManager;
    mockManager.ensureCoordinatorIsReady = vi.fn(async () => ({ agentId: 'coordinator-1' }));
    ...
    expect(success).toBe(true);
    expect(mockManager.ensureCoordinatorIsReady).not.toHaveBeenCalled();
  });
});
