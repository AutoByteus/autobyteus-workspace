...
export class EventEmitter {
  ...
  constructor(turnId: string, segmentIdPrefix?: string) {
    this.turnId = turnId;
  ...
  emitSegmentStart(segmentType: SegmentType, metadata: Record<string, any> = {}): string {
    ...
    const event = SegmentEvent.start(this.turnId, segmentId, segmentType, metadata);
  ...
  emitSegmentContent(delta: any): void {
    ...
    const event = SegmentEvent.content(this.turnId, this.currentSegmentId, delta);
    this.eventQueue.push(event);
  }
