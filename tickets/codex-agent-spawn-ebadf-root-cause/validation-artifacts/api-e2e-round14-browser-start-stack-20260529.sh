...
DATA_DIR="$ARTIFACT_DIR/api-e2e-round14-browser-server-data-20260529"
WORKSPACE_ROOT="$ARTIFACT_DIR/api-e2e-round14-browser-workspace-20260529"
BACKEND_LOG="$ARTIFACT_DIR/api-e2e-round14-browser-backend-20260529.log"
FRONTEND_LOG="$ARTIFACT_DIR/api-e2e-round14-browser-frontend-20260529.log"
META_JSON="$ARTIFACT_DIR/api-e2e-round14-browser-stack-20260529.json"
BACKEND_PID_FILE="$ARTIFACT_DIR/api-e2e-round14-browser-backend-20260529.pid"
FRONTEND_PID_FILE="$ARTIFACT_DIR/api-e2e-round14-browser-frontend-20260529.pid"
...
# Browser Round 14 Workspace

This README confirms that browser-level FileExplorer validation can open file content after the TDZ fix.
EOW
cat > "$WORKSPACE_ROOT/src/main.ts" <<'EOW'
export const browserRound14 = 'file explorer TDZ regression validation';
EOW
cat > "$WORKSPACE_ROOT/src/nested/deep-note.md" <<'EOW'
# Deep Note

Search target: browser-round14-live-search.
EOW
cat > "$WORKSPACE_ROOT/docs/search-target.md" <<'EOW'
Browser Round 14 search target document.
EOW
DB_URL="file:${DATA_DIR}/db/production.db"
cat > "$DATA_DIR/.env" <<EOW
APP_ENV=production
LOG_LEVEL=INFO
...
AUTOBYTEUS_INTERNAL_SERVER_BASE_URL=${BASE_URL}
...
PRISMA_QUERY_ENGINE="$REPO_ROOT/node_modules/.pnpm/@prisma+engines@5.22.0/node_modules/@prisma/engines/libquery_engine-darwin-arm64.dylib.node"
PRISMA_SCHEMA_ENGINE="$REPO_ROOT/node_modules/.pnpm/@prisma+engines@5.22.0/node_modules/@prisma/engines/schema-engine-darwin-arm64"
(
  ...
  export PRISMA_QUERY_ENGINE_LIBRARY="$PRISMA_QUERY_ENGINE"
  export PRISMA_SCHEMA_ENGINE_BINARY="$PRISMA_SCHEMA_ENGINE"
  nohup node dist/app.js --host 127.0.0.1 --port "$BACKEND_PORT" --data-dir "$DATA_DIR" > "$BACKEND_LOG" 2>&1 &
  echo $! > "$BACKEND_PID_FILE"
)
BACKEND_PID="$(cat "$BACKEND_PID_FILE")"
for i in {1..180}; do
  if curl -fsS "$BASE_URL/rest/health" >/dev/null 2>&1; then break; fi
...
(
  ...
  export NUXT_PUBLIC_GRAPHQL_BASE_URL="$BASE_URL/graphql"
  export NUXT_PUBLIC_REST_BASE_URL="$BASE_URL/rest"
  export NUXT_PUBLIC_WS_BASE_URL="ws://127.0.0.1:${BACKEND_PORT}/graphql"
...
cat > "$META_JSON" <<EOW
{
  ...
  "prismaQueryEngineLibrary": "${PRISMA_QUERY_ENGINE}",
  "prismaSchemaEngineBinary": "${PRISMA_SCHEMA_ENGINE}"
}
EOW
echo "$META_JSON"
