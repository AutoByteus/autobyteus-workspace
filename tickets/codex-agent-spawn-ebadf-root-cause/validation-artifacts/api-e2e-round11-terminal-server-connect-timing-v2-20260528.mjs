...
const summaryPath = path.join(artifactDir, 'api-e2e-round11-terminal-server-connect-timing-v2-20260528.json');
const eventLogPath = path.join(artifactDir, 'api-e2e-round11-terminal-server-connect-timing-v2-20260528.log');
const serverLogPath = path.join(artifactDir, 'api-e2e-round11-terminal-server-connect-timing-v2-20260528-server.log');
const finalLsofPath = path.join(artifactDir, 'api-e2e-round11-terminal-server-connect-timing-v2-20260528-final-lsof.log');
...
async function main() {
  ...
  await fsp.writeFile(path.join(workspaceRoot, '.terminal_probe_marker'), 'ROUND11_ACTUAL_COMMAND_OUTPUT\n');
  ...
  for (let i = 1; i <= 8; i += 1) {
    ...
    const outputMarker = 'ROUND11_ACTUAL_COMMAND_OUTPUT';
