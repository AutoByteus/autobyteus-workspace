...
const summaryPath = path.join(artifactDir, 'api-e2e-round10-embedded-server-high-churn-v2-20260524.json');
const serverStdoutPath = path.join(artifactDir, 'api-e2e-round10-embedded-server-high-churn-v2-20260524-server.stdout.log');
const serverStderrPath = path.join(artifactDir, 'api-e2e-round10-embedded-server-high-churn-v2-20260524-server.stderr.log');
...
async function writeWorkspace(root, fileCount) {
  ...
  await fsp.writeFile(path.join(root, '.terminal_probe_marker'), '__AB_ROUND10_TERMINAL_CWD_OK__\n');
...
async function main() {
  ...
  const terminalMarker = '__AB_ROUND10_TERMINAL_CWD_OK__';
  const terminalOutputPromise = waitForTerminalOutput(
    terminalSocket,
    (output) => output.includes(terminalMarker) && output.includes(path.basename(terminalRoot)),
    terminalMarker,
  );
  sendTerminalInput(
    terminalSocket,
    'cat .terminal_probe_marker; pwd\n',
  );
  const terminalOutput = await terminalOutputPromise;
  assert(terminalOutput.includes(terminalMarker), 'terminal cwd marker output missing');
