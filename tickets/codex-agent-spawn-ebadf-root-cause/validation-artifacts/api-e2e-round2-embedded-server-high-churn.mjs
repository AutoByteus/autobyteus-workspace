...
const summaryPath = path.join(artifactDir, 'api-e2e-round2-embedded-server-high-churn.json');
const serverStdoutPath = path.join(artifactDir, 'api-e2e-round2-embedded-server-high-churn-server.stdout.log');
const serverStderrPath = path.join(artifactDir, 'api-e2e-round2-embedded-server-high-churn-server.stderr.log');
...
async function main() {
  ...
  const createWorkspaceMutation = `mutation CreateWorkspace($input: CreateWorkspaceInput!) { createWorkspace(input: $input) { workspaceId name absolutePath isTemp fileExplorer } }`;
  ...
  for (let workspaceIndex = 0; workspaceIndex < 3; workspaceIndex += 1) {
    ...
    log('workspace_created', { workspaceIndex, workspaceId: workspace.workspaceId, root, fileExplorerBytes: workspace.fileExplorer?.length ?? 0 });
  ...
  const summary = {
    result: 'pass',
    scenario: 'embedded server macOS high-churn file-explorer websocket and codex activation lifecycle',
