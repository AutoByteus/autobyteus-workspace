...
`resolveExecutionCwd` currently uses `workspaceRootPath` for three separate purposes: omitted-cwd defaulting, relative-cwd anchoring, and authorization of every explicit cwd. It lexically and physically rejects an absolute directory outside the workspace before the shell or background manager is reached. The first two purposes remain correct; the third is the limitation being removed.
...
The target design keeps the agent workspace as the default and relative-path anchor, but treats an explicit absolute cwd as a direct local directory target. Relative input remains lexically and physically contained by the configured workspace. Sandbox implementation, OS isolation, command authorization, and other tool categories are explicitly outside this design.
...
relative cwd + workspace
  ...
  -> retain lexical and physical workspace containment
  -> resolve the physical path, verify it is an existing directory, and preflight cwd access

relative cwd + no workspace
  -> actionable error requiring an absolute cwd or configured workspace
```
...
tools. After it classifies the input and applies the approved absolute or
relative anchoring rule, it must complete this sequence before returning a cwd
to an execution owner:

1. Resolve the candidate's physical path using the existing ancestor/symlink
   normalization. For a relative candidate, perform the existing lexical and
   physical workspace-containment checks before accepting the result. For an
   absolute candidate, do not apply workspace containment.
2. Check that the physical candidate exists and is a directory. Preserve the
   existing working-directory validation messages for missing and
   non-directory targets.
3. Check the host filesystem access required to use the physical directory as
...
4. Convert an access/preflight failure for an existing directory to the
...
The focused test matrix must exercise both cwd forms for both public tools:

| Tool | Existing inaccessible cwd fixture | Required assertion |
| --- | --- | --- |
| `run_bash` | Absolute external directory | Working-directory accessibility error; `ShellCommandExecutor.execute` and target spawn are not called. |
| `run_bash` | Existing workspace-relative directory | Same error and no-spawn assertion after relative anchoring. |
| `start_background_process` | Absolute external directory | Working-directory accessibility error; `BackgroundProcessManager.startCommand` and target spawn are not called; no PID/record is created. |
| `start_background_process` | Existing workspace-relative directory | Same error and no-spawn assertion after relative anchoring. |

On POSIX, a temporary directory with search/execute permission removed may
provide the integration fixture. Because permission behavior varies by root
and Windows ACLs, unit coverage must also inject/mock the resolver's host
accessibility failure so the no-spawn contract is deterministic on every
supported host. The tests must not launch a probe process and must retain the
existing success, missing, non-directory, symlink, and lifecycle coverage.
...
Update both LLM-facing schemas and `autobyteus-ts/docs/terminal_tools.md` so the runtime contract is explicit. Keep all process execution and lifecycle owners unchanged.
...
| --- | --- | --- | --- | --- | --- | --- |
| BEH-001 | Contract | External absolute foreground cwd; REQ-001, REQ-005, REQ-008; AC-001, AC-007 | Agent model invokes `run_bash` with an absolute project/worktree cwd. | `run-bash.ts` -> `execution-cwd.ts`; external absolute cwd currently fails with `FILE_TOOL_PATH_OUTSIDE_AUTHORIZED_ROOT`. | Accept an existing external absolute directory and execute there; preserve command/result/timeout behavior. | `DS-001`: tool entry -> cwd resolver -> foreground executor -> shell -> result. |
| BEH-002 | Contract | External absolute background cwd; REQ-002, REQ-005, REQ-008; AC-002, AC-007 | Agent model invokes `start_background_process` with an absolute project/worktree cwd. | `start-background-process.ts` -> `execution-cwd.ts` -> `BackgroundProcessManager`; external cwd currently fails before spawn. | Accept an existing external absolute directory and retain it in managed process metadata. | `DS-002`: tool entry -> cwd resolver -> background manager -> shell -> PID record/result. |
| BEH-003 | Contract | Relative cwd remains workspace-anchored and traversal-contained; REQ-003, REQ-006; AC-003, AC-004, AC-006 | Agent model supplies a relative cwd with a configured workspace. | Current resolver joins relative input to workspace and performs lexical/physical containment. | Preserve workspace anchor, traversal rejection, and no process/shell cwd fallback. | `DS-001`, `DS-002`, `DS-003`. |
...
| BEH-005 | System | Directory validation, cwd accessibility, and physical identity; REQ-005; AC-006, AC-007 | Tool call supplies a missing, non-directory, inaccessible, or symlinked cwd. | Current resolver validates directories and normalizes physical paths after containment, but does not explicitly preflight the access needed to use an existing directory as a process cwd. | In `resolveExecutionCwd`, resolve and validate the candidate before either target process owner is called. Map missing, non-directory, and access failures to the existing working-directory validation error family; allow valid external symlink targets; preserve normalized `effectiveCwd`. On Windows, perform host-path preflight before WSL conversion. | `DS-001`, `DS-002`. |
| BEH-006 | Contract | Schema/documentation alignment; REQ-007, REQ-009; AC-008, AC-009 | Model receives tool schema and terminal documentation. | Tool descriptions say absolute paths are allowed while docs say agent terminal cwd is workspace-contained. | State absolute external support, relative workspace anchoring, defaults, and per-call semantics consistently. | Schema/docs off-spine concerns serving `DS-001` and `DS-002`. |
...
- Evidence: `execution-cwd.ts` is already the terminal-specific authority used by both cwd-bearing tools. It conflates workspace defaulting/relative anchoring with absolute-path authorization. Downstream process owners accept a resolved cwd without duplicating that policy. The server/web interactive terminal already has a separate external-root precedent.
- Design response: retain the existing terminal cwd owner, split absolute and relative resolution semantics inside that owner, and leave shell/process lifecycle code unchanged.
...
- **Workspace-anchored relative cwd:** A non-absolute cwd resolved under the configured workspace and still subject to lexical and physical containment.
...
- Preserve the relative traversal rejection because it is part of the approved relative-path invariant, not a legacy absolute-cwd path.
...
| `resolveExecutionCwd` | Owns omitted-cwd defaults, absolute-vs-relative classification, workspace-relative containment, physical normalization, directory/access preflight, host/WSL boundary ordering, and actionable working-directory errors. |
...
| --- | --- | --- | --- | --- |
| Absolute-cwd branch that requires workspace containment in `execution-cwd.ts` | External project/worktree cwd is now approved behavior. | Direct absolute resolution inside `resolveExecutionCwd`. | In This Change | Relative input retains workspace containment. |
...
| `docs/terminal_tools.md` statement that agent terminal cwd remains workspace-contained | It contradicts approved target behavior. | Explicit absolute/relative/default cwd contract. | In This Change | Keep interactive terminal distinction. |
...
| --- | --- | --- | --- | --- | --- |
| LLM-facing parameter schemas/descriptions | DS-001, DS-002 | `runBash` / `startBackgroundProcess` | Explain absolute external cwd, relative workspace anchoring, defaults, and per-call scope. | Models need an accurate contract to choose worktree roots correctly. | Schema wording would become a hidden policy authority or drift from the resolver. |
...
| Focused unit/integration tests | DS-001–DS-004 | All affected owners | Protect external/relative/default/validation/lifecycle behavior. | Makes the boundary change durable. | Tests that only assert resolver internals would miss real shell/background behavior. |
...
`workspaceRootPath` remains context data used by the cwd owner for defaults and relative input. It is not passed to `ShellCommandExecutor` or `BackgroundProcessManager` as an authorization policy. The normalized directory returned by the cwd owner is the only cwd value those process owners consume.
...
| --- | --- | --- | --- | --- |
| `resolveExecutionCwd(context, cwd)` | absolute/relative classification, workspace anchor, relative containment, physical normalization, directory/access preflight, host error mapping | `runBash`, `startBackgroundProcess` | Tool calls joining paths themselves; executor/manager checking workspace or access directly; platform resolver applying a different policy. | Strengthen this resolver's explicit absolute/relative/access contract, not add caller-side exceptions. |
...
| `resolveExecutionCwd(context, cwd?)` | Terminal cwd target | Apply default/absolute/relative resolution, physical directory/access preflight, and working-directory error mapping. | `AgentContextLike` + optional cwd string/null. | One canonical cwd policy boundary for both tools; host validation completes before platform conversion. |
...
| Terminal cwd resolution | Defaults, absolute/relative resolution, containment, physical normalization, host directory/access validation, and error mapping | DS-001–DS-004 | `resolveExecutionCwd` | Extend | Absolute external paths bypass workspace containment; relative paths do not; all candidates fail before spawn when unusable. |
...
| Terminal documentation/tests | Contract evidence and durable guidance | DS-001–DS-004 | Terminal subsystem | Extend | Replace stale rejection/containment claims. |

## Draft File Responsibility Mapping

| Candidate File | Owning Subsystem / Capability Area | Owner / Boundary | Concrete Concern | Why This Is One File | Reuses Shared Structure? |
| --- | --- | --- | --- | --- | --- |
| `autobyteus-ts/src/tools/terminal/execution-cwd.ts` | Terminal cwd resolution | `resolveExecutionCwd` | Absolute/relative/default resolution, relative containment, physical normalization, directory/access preflight, and validation error mapping | One file already owns this policy for both tools; keeping the new invariant here avoids caller and platform drift. | Existing `isWithin`, physical resolution, and validation helpers plus Node fs access primitive. |
...
| `autobyteus-ts/tests/unit/tools/terminal/run-bash.test.ts` | Terminal tests | Resolver/foreground behavior | External absolute, no-workspace absolute, relative traversal, defaults, validation. | Existing unit coverage for resolver through `runBash`. | Existing temp-directory helpers. |
...
| --- | --- | --- | --- | --- |
| `AgentContextLike.workspaceRootPath` | Yes | Yes | Low | Retain as default/relative anchor; do not reinterpret it as external cwd authority. |
...
| `cwd` tool parameter | Yes | Yes | Low | Apply identical absolute/relative/default semantics in both tools. |

## Final File Responsibility Mapping

| File | Owning Subsystem / Capability Area | Owner / Boundary | Concrete Concern | Why This Is One File | Reuses Shared Structure? |
| --- | --- | --- | --- | --- | --- |
| `autobyteus-ts/src/tools/terminal/execution-cwd.ts` | Terminal cwd resolution | `resolveExecutionCwd` | Canonical cwd resolution: absolute external, relative contained, omitted default, physical normalization, host access preflight, and working-directory error mapping. | Existing authoritative owner; no split required. | Existing physical/validation helpers plus Node filesystem access primitive. |
...
| `autobyteus-ts/docs/terminal_tools.md` | Terminal documentation | Terminal subsystem | External cwd contract, defaults, relative traversal, and interactive distinction. | Existing canonical guide. | Existing architecture/testing descriptions. |
...
- Existing terminal-specific resolver pattern: retain one owner for cwd defaults, anchoring, normalization, accessibility preflight, and validation.
...
| `autobyteus-ts/src/tools/terminal/execution-cwd.ts` | File | `resolveExecutionCwd` | Canonical cwd/default/relative/absolute policy, physical directory/access preflight, and working-directory error mapping. | It already owns this concern for both tools; the new invariant remains at the same boundary. | Shell spawning, PID lifecycle, persistent agent state, WSL authorization. |
...
| Relative cwd | `run_bash(cwd="packages/api") -> resolve under workspace -> retain containment` | `run_bash(cwd="../other-project")` silently escapes the workspace | Absolute external support must not turn relative input into an ambiguous escape mechanism. |
| No workspace | `run_bash(cwd=/tmp/project, command="pwd")` succeeds; `cwd=relative/path` fails clearly | Resolve relative cwd from server process cwd | Avoids hidden dependence on how the server was launched. |
...
| Add a separate `run_bash_external` tool | Could avoid changing existing semantics. | Rejected | Keep one `run_bash` contract with explicit absolute/relative rules. |
...
| Retain workspace containment for absolute paths and document an exception elsewhere | Could hide the behavior change. | Rejected | Remove the absolute containment branch from the canonical resolver and align docs/schema. |
...
1. Update the approved requirements and supplement to record user approval, relative traversal containment, and sandbox deferral.
2. Change `resolveExecutionCwd` so omitted cwd behavior remains unchanged; absolute cwd resolves directly without workspace containment; relative cwd still requires a workspace and retains lexical/physical containment.
3. Preserve physical normalization and directory validation, then add resolver-owned host accessibility preflight. Map access failures for existing directories to the working-directory validation error path before either process owner is called; symlinked external project/worktree roots still report the actual effective directory.
4. Update `run_bash` and `start_background_process` schema/description text with the same absolute/relative/default/per-call semantics.
5. Replace the stale unit/integration rejection expectations with external absolute success and preserved relative/default/validation coverage. Add no-spawn coverage for inaccessible absolute and workspace-relative cwd values through both tools, plus a background lifecycle case that starts, queries, reads output from, and stops an external-cwd process.
6. Update `docs/terminal_tools.md`; retain the distinction between agent non-interactive terminal tools and interactive server/web terminal backends.
7. Run focused unit/integration terminal checks, package typecheck/build, and downstream runtime/package validation after dependencies are provisioned.
...
- **Safety of relative paths:** relative input remains deterministic and workspace-contained, avoiding accidental process-cwd dependence.
...
- Distinguish absolute input before applying workspace-relative resolution. Do not use `path.resolve(workspaceRoot, absoluteInput)` as a way to reintroduce containment.
- For relative input, preserve current lexical and physical workspace containment, including symlink traversal rejection.
...
- Ensure the preflight completes before `ShellCommandExecutor.execute` or `BackgroundProcessManager.startCommand`; test both absolute and workspace-relative inaccessible paths for both tools with no-spawn assertions.
- Preserve `os.tmpdir()` fallback when cwd is omitted and no workspace exists.
- Use an actionable no-workspace error for relative cwd; do not use process cwd as a hidden anchor.
...
- Keep schemas and docs synchronized with implementation. State that explicit absolute cwd can target an external project/worktree and that cwd does not change workspace identity or persist.
