...
- FR-037: Every standard `/workspace` state must preserve a visible right-edge tools affordance whenever the right tools are not docked. The right strip may consume `50px` in non-narrow flow or use an edge-overlay treatment when that width cannot fit; the center work surface must remain mounted, and no separate top `Tools` control may be used as a replacement.
...
- AC-025: In every state where right tools are not docked, a visible right-edge strip is present and has an accessible name. In a wide user-collapsed state whose right panel fits, activating it re-docks the full right panel; in constrained/narrow or responsive-yield states, activating it opens the temporary right-tools drawer, preserves the selected run, and exposes the full available catalog. No standard `/workspace` state renders a separate top `Tools` trigger or a drawer-only replacement for the strip.
...
- AC-036: Boundary tests prove the right-tools fallback order is docked -> consuming strip -> edge-overlay strip. If left docked plus center plus the `50px` strip fit, the strip consumes that width; otherwise it remains visible as an overlay. All states have no top `Tools` trigger; a user-origin strip that can re-dock activates `redock-panel`, while a constrained/responsive strip activates `open-drawer`.
