...
The target is therefore not merely “make the four buttons fit.” The target is to preserve the personal-branch workspace mental model: left navigation/history owns selection and run creation, the center is the Work surface, and right-side tabs own Files and tools. Responsive states use symmetric left and right strips as the compact affordances. At a wide viewport, a strip created by explicit user collapse re-docks its full panel when activated; when capacity is constrained or narrow, the strip opens a transient drawer instead. They do not introduce a hamburger, breadcrumb trigger, top `Agents & teams`/`Tools` button, or duplicate generic top navigation bar. The scenario-level contract is defined in `workspace-responsive-ui-ux-spec.md` and requires architecture re-review before implementation resumes.
...
8. Preserve the original wide workspace mental model and use symmetric side ownership: no generic top-level `Work / Runs / Files / Tools` bar, hamburger, breadcrumb trigger, top `Agents & teams`, or top `Tools` control; at wide manual collapse, strip activation re-docks the corresponding panel, while constrained/narrow strip activation opens a temporary drawer.
...
- Preserve existing responsive header/navigation behavior on non-workspace routes that use `layouts/default.vue`; the no-header rule is limited to `/workspace`.
