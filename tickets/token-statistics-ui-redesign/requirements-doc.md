...
- Status: `Draft — RV-006 Terminology And Detail Decision Pending`
...
- Date: 2026-08-29
- Approval state and reference: Not yet approved. The user explicitly removed all visible prior-period comparison messaging, the standalone contributor presentation, and all visible `driver` terminology. The user also required an obvious labeled chart axis and removal of confusing short vertical point guides. RV-006 reflects those decisions and retains exact identity evidence only as secondary `Detailed usage`; `DEC-007` terminology and `DEC-008` secondary grouping placement remain unresolved.
...
| BEH-003 | User | Four equal summary cards precede two equal chart cards. Usage over time is a bar chart; pace is a cumulative current/prior line chart or a large `No comparable data` panel. A ranked horizontal breakdown precedes an exhaustive 12-column table. | Make the primary hierarchy answer total tokens/spend, daily tokens/spend, and cache-aware token composition. Within-month daily buckets use a point-marked line with explicit labeled X/Y axes and no short vertical point stems. Show no visible prior-period comparison, standalone contributor panel, or `driver` terminology. Retain exact runtime/model identity evidence only as secondary `Detailed usage`, with final grouping placement governed by `DEC-008`. | Exact accounting, cost/coverage truth, current selection coherence, chronological buckets, accessible text evidence, and CSV remain traceable to one result. Cache-read tokens stay a subset of total input; no unavailable cache rate is rendered as 0%. The comparison contract may remain unused by this UI. | Supplied captures; current components; RI-001; RV-005 user-directed removals; RV-006 axis clarification. |
...
| REQ-015 | For a within-month trend returned at daily granularity, Usage over time shall render one line/curve through the chronological buckets with a visible point marker for every daily bucket, explicit X- and Y-axis lines, an explicit Y-axis title that reflects Tokens or authoritative Cost currency, legible Y-scale labels, aligned UTC date ticks, and exact accessible values. It shall use no vertical usage bars, area fill, or short vertical guide/stem attached to individual points; accessible text shall expose the complete daily bucket series so the connecting line is not the sole evidence or mistaken for continuously measured accounting volume. | BEH-003 | Must | Direct user preference: point-marked line with an obvious scale and without confusing dotted-looking stems; supported by current `trendBuckets` and DAY range policy. | RV-004 and RV-006 clarifications; validation `VAL-016` |
...
| AC-015 | REQ-015 | Open `This month` with a 29-day daily-bucket USD result at representative desktop width | Usage over time renders exactly one chronological trend line, 29 visible point markers, explicit X/Y axis lines, a Y title that switches between `Tokens` and `Cost (USD)`, three legible Y-scale labels, five aligned UTC date ticks, no vertical usage bars, no area fill, zero short vertical point stems, and an accessible label/text equivalent containing all 29 daily buckets. | Narrow widths may reduce nonessential tick density while keeping axes, unit, chronological meaning, point markers, and exact accessible evidence clear. Other range granularities retain exact chronological bucket evidence. | Deterministic chart fixture and accessibility inspection (`VAL-016`) |
...
- Prototype revision or commit: Current visualizer is `RV-006`: content commit `0e4a0778ee499e9dc9ea6cb13b33b7f3bb987e9e`, review metadata commit `3bd5300a3e0bee3efed331e02ed2218c74a7e30e`. It preserves RV-005 removals and adds explicit X/Y axes, metric-aware Y units/labels, aligned UTC ticks, and zero point stems; browser validation remains 18/18. RV-003 feasibility remains intact. Accepted prototype baseline implementation `6ba98942c669329f70ba902db4a2880375ad52ad`; its verified fast-forward integration tip on the prototype repository's `origin/personal` is `16638137bdb8ebe627507dac6c3c8bdbc5edf9d6`.
- UI/UX user-confirmation reference: The user selected the daily point-marked line; required an obvious labeled Y scale and removal of the confusing short vertical point guides; removed all visible prior-period comparison messaging, the standalone contributor presentation, and all visible `driver` terminology; and kept monthly Tokens/Cost, daily Tokens/Cost, estimated cost/status, and input/output composition primary. RV-006 reflects those decisions. No overall UI/UX or requirements approval is claimed.
...
| `/home/autobyteus/workspace/autobyteus-web-prototype-worktrees/REQPKG-TSUI-001/tickets/in-progress/REQPKG-TSUI-001/requirements-visualization-review.md` | RV-006 review URLs, user-directed removals, axis/stem clarification, covered states, validation, limitations, and remaining terminology/detail decision | REQ-001–REQ-016; AC-001–AC-016 | Awaiting focused user decision | RV-006 is exploratory, not final or approved; 18/18 Chromium checks pass |
| `/home/autobyteus/workspace/autobyteus-web-prototype-worktrees/REQPKG-TSUI-001/tickets/in-progress/REQPKG-TSUI-001/validation/browser-validation.json` | Deterministic RV-006 browser evidence, including explicit axes/unit/ticks, daily-line/point/stem, and forbidden-term checks | REQ-004–REQ-006, REQ-009, REQ-011, REQ-015; AC-003–AC-006, AC-015 | `PASS` 18/18 | Supporting evidence; no final approval |
| `/home/autobyteus/workspace/autobyteus-web-prototype-worktrees/REQPKG-TSUI-001/tickets/in-progress/REQPKG-TSUI-001/visual-references` | Direction A/B and state captures `VIS-001`–`VIS-008`, updated through RV-006 | REQ-001–REQ-016; DEC-001–DEC-008 | Retained exploratory evidence | User-directed removals and explicit daily-axis treatment are usable clarification evidence; exact final hierarchy is still non-normative |
...
| REQ-015 | BEH-003 | AC-015 | SCN-001 | RV-004/RV-006 review record, `VIS-001`/`VIS-002`, browser validation `VAL-016` |
...
- Prototype and supplemental evidence is integrated consistently: `Yes, including RI-001 and RV-006; exploratory evidence is explicitly non-normative`
...
- Remaining blocker: User resolution of `DEC-007` input terminology and `DEC-008` Detailed-usage placement, followed by a focused Product revision that exposes the required cache-aware composition. Control disclosure (`DEC-002`) and Run-details unification (`DEC-004`) also remain open before final approval. RV-006 is exploratory and must not be treated as the implementation direction.
