...
- Date: 2026-08-24
- Approval state and reference: The previously reviewed UI/UX remains approved under PPA-001 and the user’s 2026-08-22 **“approved”** message, but terminal completeness is reopened. On 2026-08-24 the user reported that Agent Teams **Run** does not work in the prototype and explicitly requires the source-supported journey: run a selected team and see that team in the left workspace tree. RER-009 confirms the blocker and requires correction in the existing task worktree, not directly on `personal`.
...
| BEH-005 | User / System | The approved source pin supports selecting **Run** on an Agent Team, preparing its team launch draft in `/workspace`, launching the configured team, registering/selecting its live context, and projecting the team under its workspace in the left tree. The prototype instead stays on the catalog route and raises `inFlightDrafts.keys is not a function`; its prototype adapter also replaces `agentTeamRun.launchDraft` with a no-op. | The complete source-supported launch journey runs deterministically: catalog **Run** opens a valid team draft; **Run Team** creates one synthetic team run without production access; the run appears immediately under the selected workspace in the left tree and remains selectable/expandable with its members. | Existing team-card/config/tree presentation, source pin, deterministic fixtures, isolation, other journeys, final references, repository-root project placement, and unrelated prototype projects remain unchanged except where this focused behavior and its evidence must be corrected. | `agent-team-run-parity-analysis.md`; SRC-018–SRC-021; controlled reproduction evidence |
...
**Acceptance outcome:** AC-001 through AC-009 remain historically met. AC-010 through AC-013 are open and block terminal completeness. The controlled RER-009 reproduction confirms AC-010 currently fails before navigation, and RC-002 shows AC-011 would remain unsatisfied even if the first error were fixed alone.
...
- Applicable: `Yes`
- Linked UI/UX or interaction supplement for RER-009 correction: `/home/autobyteus/workspace/.codex/worktrees/initial-prototype-baseline/autobyteus-web-prototype/ui-ux-spec.md`. PPA-001 remains authoritative for the previously reviewed visual baseline, but any statement that the complete Agent Team launch interaction already passes is reopened until AC-010–AC-012 are corrected, reviewed, and approved.
- Linked runnable prototype and applicable support artifacts for the correction: Worktree root `/home/autobyteus/workspace/.codex/worktrees/initial-prototype-baseline/autobyteus-web-prototype`; the Product Prototyper shall provide the review URL from this root. The synchronized `personal` copy remains the previously approved historical package and must not be modified during RER-009.
...
- Requirements-defining visual or interaction details: The approved `ui-ux-spec.md` and final visual references continue to define unchanged appearance at source commit `8ef282ba77705180d985e7000d801f0e0068cdc1`; REQ-010–REQ-012 and the corrected controlled journey define the reopened Agent Team launch interaction. Any existing `ui-ux-spec.md` claim that this run interaction already passes is unsubstantiated until Product Prototyper correction and user review.
...
- Required screens, states, transitions, feedback, responsive behavior, or accessibility outcomes: Preserve `ROUTE-001`–`ROUTE-041`, `CFG-001`–`CFG-011`, `STATE-001`–`STATE-013`, `HOST-001`–`HOST-008`, `WKS-001`–`WKS-021`, `MOB-001`–`MOB-014`, 239 locale/responsive matrix rows, `JRN-001`–`JRN-049`, and `DISC-001`–`DISC-017`; add one new stable journey ID that enforces SCN-005 from the Agent Teams catalog through the live left-tree result.
...
| --- | --- | --- | --- | --- |
| `/home/autobyteus/workspace/.codex/worktrees/initial-prototype-baseline/tickets/in-progress/initial-prototype-baseline/investigation-notes.md` | Evidence base and decision record | All | Current through RER-009 | Requirements basis for focused correction |
| `/home/autobyteus/workspace/.codex/worktrees/initial-prototype-baseline/tickets/in-progress/initial-prototype-baseline/requirements-revision-record.md` | Chronological requirements-round index | All | Current through RER-009 | Informational traceability artifact |
| `/home/autobyteus/workspace/.codex/worktrees/initial-prototype-baseline/autobyteus-web-prototype/ui-ux-spec.md` | Product Prototyper-owned UI/UX supplement and requirements-defining/illustrative boundary | All | PPA-001 retained; BEH-005 completeness reopened | Existing approval retained except for the corrected interaction evidence pending review |
...
| `/home/autobyteus/workspace/.codex/worktrees/initial-prototype-baseline/tickets/in-progress/initial-prototype-baseline/agent-team-run-parity-analysis.md` | Controlled Agent Team launch parity analysis, source behavior, root causes, correction boundary, and evidence-gap finding | REQ-010–REQ-013; AC-010–AC-013 | Confirmed / requirement-defining for RER-009 | Requirements Engineering evidence; correction not yet approved |
...
| REQ-010 | BEH-005 | AC-010 | SCN-005 | RER-009 analysis; controlled reproduction JSON/screenshot; pinned-source `AgentTeamList.vue` and `RunConfigPanel.vue` behavior |
| REQ-011 | BEH-005 | AC-011 | SCN-005 | Pinned-source `agentTeamRunStore.ts` and `runHistoryReadModel.ts`; corrected controlled launch journey and left-tree evidence required |
| REQ-012 | BEH-005 | AC-012 | SCN-005 | RER-009 journey-gap analysis; future stable source/prototype journey ID, raw evidence, and final-package assertion |
| REQ-013 | BEH-005 | AC-013 | SCN-005 | Worktree/base/branch verification and correction commit provenance |
...
- User approval received: `Yes` for the required BEH-005 outcome and previously approved visual/unrelated baseline; corrected runnable interaction approval remains pending Product Prototyper review.
- Architecture-ready: `N/A` — this ticket is explicitly prototype-only and authorizes no architecture or production engineering.
- Remaining blocker: AC-010 through AC-013. The prototype currently throws before catalog-to-workspace navigation, the launch action is adapted to a no-op, and no terminal controlled journey enforces the user-required left-tree outcome.

## Completion Classification

- Outcome: `Draft / Prototype Needed — Agent-Team Run Parity Correction`
- Approved observable baseline: Previously reviewed appearance and unrelated interactions remain approved under PPA-001, but completeness is reopened for BEH-005 and AC-010–AC-012. The corrected runnable journey requires Product Prototyper review and explicit user approval before terminal closure.
- Correction deliverable: `/home/autobyteus/workspace/.codex/worktrees/initial-prototype-baseline/autobyteus-web-prototype` on `codex/initial-prototype-baseline`, based on `origin/personal` `52b4be02ea793f2071fe5a63a94664ab25196433`. No RER-009 work may modify, merge, or push `personal` without separate user authorization.
...
- Architecture handoff applicability: `No`; DEC-002 explicitly excludes architecture and production engineering from this ticket. The next stage is Product Prototyper correction and review, not architecture.
