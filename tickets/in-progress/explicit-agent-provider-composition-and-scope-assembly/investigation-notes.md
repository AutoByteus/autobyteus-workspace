...
- No design unknown remains for AR-001–AR-004 or CR-001. Downstream must prove the exact process input/mapping, K0–K8 ownership cuts, old-symbol allowlists, all fifteen prior direct-constructor obligations, the required Mixed Team production callback, and exact general/application root-to-member identity rather than reinterpret them.
...
Review SR-005 as the bounded ARCH-REV-004 correction. The accepted Host/Authority/provider/kernel architecture and SR-004 releaser/cache/default-manager clean cut remain unchanged. The correction recognizes the root-owned `createTeamManager` callback as a required production construction boundary: it consumes the backend factory's fixed releaser and binds the complete general or application execution-family dependencies for every root, configured child, and task Team. Exact root-to-member identity and occurrence guards are normative. No new behavior, migration, provider policy, execution multiplicity, or logical-addressing scope is introduced.
