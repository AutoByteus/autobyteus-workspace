...
- Trigger: Delivery-stage documentation sync after post-validation code review pass for server-owned task delegation / runtime tool MCP unification.
...
- Integrated base reference used for docs sync: `origin/personal` at `a96a8bdaac3dd042d084eab1fff9cd38f59fb783` (`feat(ts): stage large Autobyteus media`, fetched 2026-05-29).
- Post-integration verification reference: local merge commit `0054d2c9b481a96accae091579ae778f4bfe9c28` plus delivery-owned docs edits in the working tree.

## Why Docs Were Updated

- Summary: The implementation removed the legacy model-facing task-plan tool workflow and introduced the server-owned task-delegation surface (`delegate_tasks` / `update_task_status`) with task work packets, canonical task-delegation events, coordinator terminal notifications, and idle/run-id guarded member settlement. Long-lived docs still described the previous `assign_task_to` / `create_tasks` / polling model in several places.
- Why this should live in long-lived project docs: Future runtime/tool authors need the canonical boundary before adding MCP transports or runtime projections, and users of the TypeScript examples need to know that legacy task-plan tools are no longer available as model-facing tools.
...
| `autobyteus-server-ts/docs/modules/agent_team_execution.md` | Canonical team-run backend/lifecycle docs. | Updated | Added task-delegation lifecycle, events, work packets, and settlement behavior. |
...
| `autobyteus-server-ts/docs/modules/codex_integration.md` | Codex runtime/dynamic tool docs. | Updated | Added Codex task-delegation dynamic-tool projection and history mapping notes. |
...
| `autobyteus-ts/examples/agent-team/*` source files and prompts | Ensure examples no longer import/instruct deleted task-plan tools. | No change | Current source grep found no deleted task-tool import paths. Existing examples use `send_message_to` for free-form communication. |
...
| `autobyteus-server-ts/docs/modules/agent_team_execution.md` | Lifecycle documentation | Added task-delegation flow from tool context through ledger, work-packet activation, events, coordinator notifications, dependent activation, and idle/run-id guarded settlement. | Team runtime authors need to understand where delegation state and member settlement are owned. |
...
| `autobyteus-server-ts/docs/modules/codex_integration.md` | Runtime projection note | Documented task-delegation dynamic tools and clarified legacy task-plan names are not exposed. | Codex dynamic-tool behavior is a primary supported projection. |
...
- Notes: Docs sync completed after the latest-base merge and post-integration verification. Delivery remains at user-verification hold; no finalization, push, archive, release, or deployment has been run.
