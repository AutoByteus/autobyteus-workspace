...
- REQ-008: `update_task_status` has two caller-bound modes and must reject ambiguous use. In task-agent execution mode, the service must resolve the task from the caller's bound task-agent instance/run identity, reject model-facing task selectors, and allow only execution statuses such as `in_progress`, `completed`, or `failed` plus optional `message` and `reference_files`. In original-delegator acceptance mode, the service may accept `status: "accepted"` with the system-generated `task_id` from the completion notification; authorization must prove that the caller is the original delegator for that task. There is no model-facing `task_name` update selector.
...
- REQ-011: For self-reported `completed` work, task-agent auto-exit must not run merely because the task-agent says it is complete. The record enters an awaiting-acceptance state, and the task-agent remains addressable for `send_message_to` revisions. Auto-exit for successful work must run only after the original delegator accepts the task via `update_task_status(status: "accepted", task_id)` and after the current task-agent turn/tool call can safely complete. Failure handling may settle according to explicit failure policy, but must not interrupt active tool-result delivery.
...
- REQ-015: Every runtime/backend path that exposes `delegate_tasks`/`update_task_status` as a supported delegation workflow must support task-agent instance settlement and must settle/exit a task-agent instance after its task is accepted by the original delegator, once that instance is idle and has no work bound to it. A backend whose per-instance/per-member settlement returns unsupported must gate task-delegation exposure off or implement settlement before being considered supported.
...
- REQ-029: The task-delegation workflow must not add a separate review/acceptance tool. Delegator acceptance must reuse the existing `update_task_status` surface in original-delegator acceptance mode. If the delegator needs changes before acceptance, the existing `send_message_to` tool must be able to target the live task-agent identity supplied in the completion notification so the same task-agent can revise and report `completed` again.
...
- AC-007: A task-agent instance that reports `completed` remains visible/addressable in an awaiting-acceptance state after its tool result is delivered and original-delegator notification appears; after the original delegator accepts that `task_id` through `update_task_status(status: "accepted")`, the task-agent is settled/exited once idle.
...
- AC-010: Invalid or unbound task tool calls fail clearly without mutating any internal delegation state, including task-agent `update_task_status` calls that try to pass task selectors and delegator `accepted` calls whose `task_id` is missing, unknown, not awaiting acceptance, or not owned by that delegator. `task_name` selectors are never accepted.
...
- AC-012: Tests cover task delegation, internal ledger correlation, runnable activation with work-packet content, status update, original-delegator notification, multi-task activation, and auto-settlement gating.
...
- AC-029: No separate review/acceptance tool is exposed. The delegator accepts completed work through `update_task_status(status: "accepted", task_id)`. If the delegator requests changes instead, `send_message_to` can target the completion notification's task-agent identity, the same task-agent receives the revision request, and it can report `completed` again.
