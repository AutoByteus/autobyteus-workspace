...
- User schema feedback supersedes the earlier richer-field design: keep `delegate_tasks` model-facing input minimal for reliable LLM tool calls. The target schema is `member_name`, required rich `description`, and optional `reference_files`; do not expose separate `task_name`, `dependencies`, `completion_criteria`, or `expected_deliverables` fields. `update_task_status` exposes no task selector and uses caller task-agent instance context to identify the task.
