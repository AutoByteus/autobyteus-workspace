...
- Date: 2026-08-30
- Approval state and reference: User confirmed on 2026-08-30 that the LLM-facing contract must be intuitive, straightforward, and unambiguous, preferred the earlier `Ordinary Communication` / `Dedicated Task Execution` structure with explicit spawned-instance language, selected DEC-002 Option A so successful `send_message_to` output identifies the exact existing receiver AgentRun, and requested an explicit Agent-versus-AgentTeam four-case distinction; requirements visualizer `SMDS-RV-001` / `VIS-R04` is ready at `http://127.0.0.1:4179`; DEC-001 and explicit approval of the complete intended behavior remain pending
...
| BEH-008 | Contract | Successful Team message delivery internally resolves and returns the receiving configured AgentRun identity, but the public `send_message_to` result mapper currently discards it and exposes `result:null`. Successful `delegate_task` directly exposes `task_id`, `status:"active"`, and the fresh task ingress `target_agent_run_id`. | Successful public `send_message_to` output shall use its existing `result` field to expose the exact existing AgentRun that accepted the message as `target_agent_run_id`, so message and delegation feedback reinforce existing-instance versus spawned-instance identity. | A task Agent delegation returns its fresh AgentRun ID; a task AgentTeam delegation returns its fresh coordinator AgentRun ID, not the task TeamRun ID. Message failures continue to expose no successful receiver identity. | SRC-023–SRC-025; DEC-002 Option A |
...
- UC-009: On successful `send_message_to`, expose the exact existing receiving AgentRun ID in the already-present public `result` field; on successful delegation, continue exposing the newly spawned task ingress AgentRun ID.

### Out Of Scope

- Changing `send_message_to` or `delegate_task` inputs, target resolution, persistence, routing, execution creation, task state, result/review, notification, or settlement behavior. The only approved output change is the narrow successful `send_message_to` `result.target_agent_run_id` expansion in REQ-014.
...
| REQ-014 | A successful public `send_message_to` call shall return `result: { target_agent_run_id: <exact receiving AgentRun ID> }`. For `recipient_address`, this shall be the resolved mounted Agent execution or mounted AgentTeam coordinator ingress that accepted the message; for an accepted exact-run call, it shall confirm that exact target. A failed call shall not expose a successful receiver ID. Native and MCP projections shall be identical. | BEH-002, BEH-006, BEH-008 | Must | The existing generic `result` slot can carry the already-known receiver identity. This makes message feedback parallel to `delegate_task` without falsely calling an existing receiver a task Agent. | User decision on 2026-08-30; DEC-002 Option A; SRC-023–SRC-025 |
...
| AC-014 | REQ-001, REQ-007, REQ-008, REQ-014 | Successful logical Agent message, logical AgentTeam message, and exact-run message calls are executed through native and MCP projections | Each success result contains the exact accepting existing AgentRun under `result.target_agent_run_id`; AgentTeam logical delivery identifies its configured coordinator ingress; native and MCP envelopes agree | Rejected delivery retains a typed failure and no successful receiver ID; no new task ID or task execution is implied | Contract/unit/integration tests plus provider-parity assertions |
...
| --- | --- | --- | --- | --- |
| QR-001 | Compatibility | Existing public tool names, inputs, selectors, routing, and task/message lifecycle effects remain unchanged. Output remains unchanged except for the approved successful `send_message_to` result identity in REQ-014. | All three runtimes and Team-bound contexts | Contract/diff tests |
...
| DEC-002 | Should successful public `send_message_to` output expose the exact existing receiving AgentRun as `target_agent_run_id`, aligning its feedback with the fresh task ingress ID returned by `delegate_task`? | The backend already resolves and carries the receiving AgentRun ID for Team delivery, while the existing generic public `result` slot is currently null. | **Selected Option A:** use the existing `result` field to return `target_agent_run_id`; for an AgentTeam logical address this is its configured coordinator ingress. Rejected Option B would have preserved `result:null`. | User | Decided 2026-08-30 — Option A; captured in REQ-014 and AC-014 |
