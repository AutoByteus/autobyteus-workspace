...
`Design-ready` — approved by the user on 2026-08-26 as the separate second simplification ticket.
...
| BEH-003 | Authorization resolves a binding, but input reloads/reinterprets the public target and streaming inspects it again. | Authorization returns one private discriminated exact descriptor used directly by input and streaming; downstream does not reload or reinterpret the public selector. | Same Agent/Team input dispatch, root Team semantics, member filtering, lease, and terminal behavior. | REQ-003, REQ-004; AC-003, AC-006, AC-007 |
...
| BEH-005 | Every Team binding member carries constant application-role `runtimeKind`; every producer carries a role derivable from event/binding subject. | Remove both application-role `runtimeKind` fields; producer remains exact `{ agentRunId, displayName }`. | Provider launch `runtimeKind` such as AutoByteus/Codex/Claude remains untouched. | REQ-006; AC-011–AC-013 |
...
- Producer application role is fully determined by enclosing runtime subject; no production consumer makes a distinct decision from `producer.runtimeKind`.
...
Adopt one root/member logical address, one authorization-owned exact descriptor, and smaller current binding/producer shapes. Use current-schema projectors for existing JSON and keep the physical member role column as an implementation constant.
...
- UC-003: application input authorization/dispatch uses the resolved exact descriptor.
- UC-004: frontend/backend event streaming uses the same logical address and resolved descriptor.
...
- Provider composition remains a preceding ticket and is not reopened here.
...
- **REQ-004:** Input and streaming consumers shall use the private descriptor directly and shall not reload the binding or branch on/reinterpret the public address. The descriptor shall carry a current public binding snapshot needed for input return semantics.
...
- **AC-007:** stream runtime source uses descriptor exact subject/team/member IDs; Team root observes all member events and logical member observes exactly its bound member.
...
- **AC-012:** producers contain only `agentRunId` and `displayName`; event `runtimeSubject` and binding runtime subject remain the classification authority.
...
- Base `0811503a6c547698e7b77e1064d98890101acc1b`.
...
- Architecture review must validate the exact private descriptor and current-schema projector ownership.
...
Approved by the user on 2026-08-26 as a separate ticket after provider composition. The user requested a simple spine, clear authoritative boundary, explicit dependencies, and removal of redundant attributes rather than a compatibility-preserving rewrite.
