...
- Investigation Goal: Determine why SVG files do not render from the workspace
  File Explorer or the central Event Monitor, verify whether both surfaces share
  a renderer/policy/loader, and identify the smallest safe implementation
  boundary.
- Scope Classification (Small/Medium/Large): Small.
- Scope Classification Rationale: Both requested journeys already converge on a
  shared pure file-type policy, shared File Explorer store, shared FileViewer,
  and existing image transport boundaries. The defect is one missing extension
  in the image allowlist; focused tests and docs are the expected follow-up.
- Scope Summary: Add SVG to the established Image family so workspace selection
  and opt-in Event Monitor path activation render via the existing right-side
  Files surface.
- Primary Questions Resolved:
  ...
  5. Durable coverage: focused policy/action/viewer/store tests exist; API/E2E
...
The phrase “same renderer or something” is confirmed by the trace: the two
surfaces share the policy, store, FileViewer, and ImageViewer; Event Monitor
adds only a scoped activation/panel-launch boundary.
...
- Source implementation status: No source code has been changed in the task
  worktree. Only the solution artifacts in the task folder are currently
  untracked.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| /Users/normy/autobyteus_org/autobyteus-worktrees/svg-file-preview/tickets/in-progress/svg-file-preview/svg-preview-ui-ux-spec.md | UI journey/state contract for direct File Explorer and Event Monitor SVG preview | Existing right-panel, read-only, loading/error, keyboard/focus, responsive/runtime, and security-boundary behavior; explicitly rejects a new renderer | requirements doc, design spec | REQ-002–REQ-005; AC-002–AC-007 | Requirements-ready | Intended behavior is derived from the explicit user request/screenshot; no additional product choice introduced | Keep synchronized if architecture review changes observable behavior |
...
| 2026-08-02 | Other | User report, screenshot reference path above | Establish observed failure and requested surfaces | SVG selected; right pane reports unsupported; user explicitly requests File Explorer and Event Monitor/right-side rendering | No |
...
| --- | --- | --- | --- |
| fileTypePolicy.ts | A single allowlist owns image classification and omits .svg; the classifier is pure and case-normalizes paths | One authoritative extension addition fixes both entrypoints without policy duplication | Implementation adds .svg and unit regression |
...
| --- | --- | --- | --- | --- |
| 2026-08-02 | Setup | git status --short --branch in task worktree | Branch is isolated; before artifact creation source tree was clean; after artifact creation only task folder is untracked | No source implementation or unrelated working-tree changes are hidden |
| 2026-08-02 | Trace | nl -ba reads of policy, store, viewer, Event Monitor, and boundary files listed in Source Log | Both user journeys converge on the same classifier/store/viewer; .svg is rejected before URL/rendering | A single policy change is coherent |
...
Artifact, team-reference, and mobile read-only file viewers also reuse the shared
policy and FileViewer. They may gain SVG rendering as a consequence of fixing the
authoritative policy. This is preferred to special-casing only the two requested
surfaces; downstream coverage must verify that no consumer relies on SVG being
unsupported.
...
- Current stored subject, location, representative shape, and approximate
  volume: No persisted subject; open-file state is transient UI state and SVG
  bytes remain in the existing workspace/local file source.
...
- Normal readers and writers, including unknown/extra-field behavior: Existing
  File Explorer store reads transient state; no persisted reader/writer changes.
- Representative direct-read or compatibility evidence: FileViewer already
  consumes type Image; existing media tests prove the state shape.
...
- Concrete benefit, cost, and risk of migration if it remains a candidate: No
  migration candidate; changing an extension allowlist does not transform data.
...
- Live browser/Electron decode behavior for a malformed or feature-rich SVG is
  ...
  fallback.
- Whether all downstream artifact/team/mobile consumers should be explicitly
  covered with SVG fixtures is a downstream coverage decision; shared policy
  inheritance is known and intentional.
...
This is a small, design-ready bug fix. The key finding is that the user-visible
failure is caused by one omission in the authoritative shared image extension
allowlist, not by two independent renderers. The proposed runtime change is
limited to adding .svg to IMAGE_EXTENSIONS; existing File Explorer media
branches, right-side Files surface, Event Monitor launch coordination, trusted
content boundaries, and ImageViewer are reused unchanged. Focused source/test
and docs mappings are in the design spec. Requirements and the UI supplement
are ready for architecture review based on the user's explicit request and
screenshot. No source implementation has begun; no persisted data or migration
is involved.
