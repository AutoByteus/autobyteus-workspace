...
- Investigation status: Requirements investigation and architecture investigation complete; SR-001 requirements approved on 2026-09-22 and SR-002 design ready for implementation handoff.
...
| 2026-09-22 | Code | `autobyteus-server-ts/src/llm-management/services/codex-model-catalog.ts`; `autobyteus-server-ts/src/agent-execution/backends/codex/codex-app-server-model-normalizer.ts` | Trace Codex selection identity | Codex `model/list` rows remain exact runtime IDs and are not owned by the AutoByteus static catalog | Price support must recognize the exact emitted ID without altering discovery |
...
| 2026-09-22 | Code | `autobyteus-server-ts/src/llm-management/services/claude-model-catalog.ts`; `autobyteus-server-ts/src/runtime-management/claude/client/claude-sdk-model-normalizer.ts`; `.../claude-sdk-client.ts` | Trace Claude selection identity | Claude SDK discovery preserves the exact descriptor identifier and does not use the static catalog as the runtime availability authority | Pricing must recognize `claude-fable-5-1` when the runtime emits it |
...
| 2026-09-22 | Code | `autobyteus-ts/src/llm/supported-model-definitions.ts:16-77,196-287` | Decide the exact catalog shape and helper reuse | The GPT-5.6-only helper already implements the same cache proportions and 272K tier needed by Astra but hardcodes the GPT-5.6 effective date; the OpenAI reasoning-schema factory is already reusable | Rename/generalize the local helper and pass an explicit effective date; add an Astra-specific direct-API effort schema without affecting Codex runtime schemas |
| 2026-09-22 | Code | `autobyteus-ts/src/llm/api/openai-llm.ts`; `.../openai-responses-llm.ts`; `tests/unit/llm/api/provider-native-request-payloads.test.ts` | Verify direct Astra catalog exposure is safe | The curated OpenAI factory path already uses the generic Responses adapter and maps `reasoning_effort`/`reasoning_summary` without model-family gates | Reuse the adapter and add a mocked Astra request-shape regression; no production adapter change |
| 2026-09-22 | Code | `autobyteus-ts/src/llm/api/anthropic-llm.ts:45-137`; `tests/unit/llm/api/anthropic-llm.test.ts` | Decide direct Fable 5.1 configuration and safety | Family-prefix policy already classifies `claude-fable-5-1` as current adaptive, removes sampling fields, and drops provider-invalid manual enabled/disabled thinking shapes; the existing Fable 5 schema exposes a toggle that would be misleading for Fable 5.1's always-on behavior | Give Fable 5.1 no static thinking toggle/schema and prove default plus invalid override sanitization with mocked tests; no production adapter change |
| 2026-09-22 | Code | `autobyteus-server-ts/src/token-usage/pricing/token-price-config-provider.ts`; `.../token-cost-calculator.ts`; their unit tests | Locate the smallest server validation boundary | Existing provider resolves exact shared catalog data and the calculator selects inclusive `maxInputTokens` tiers from `accounting_input_tokens` | Add exact synthetic policy assertions and a 272,000/272,001 boundary calculation; no server production change |
| 2026-09-22 | Code / Docs | `autobyteus-ts/docs/provider_model_catalogs.md`; `.../llm_module_design.md`; `.../llm_module_design_nodejs.md`; `autobyteus-server-ts/docs/modules/token_usage.md` | Identify durable documentation ownership | Model catalog docs own identities/metadata and the server token-usage doc owns observation-time pricing semantics | Update existing docs only; do not create a parallel pricing source or UI documentation path |
...
| --- | --- | --- | --- | --- | --- | --- |
| `/Users/normy/autobyteus_org/autobyteus-worktrees/astra-fable-pricing-support/tickets/in-progress/astra-fable-pricing-support/requirements-doc.md` | Solution Designer | Canonical intended behavior and acceptance criteria | This package | All | Approved at SR-001 | Explicitly approved 2026-09-22 |
| `/Users/normy/autobyteus_org/autobyteus-worktrees/astra-fable-pricing-support/tickets/in-progress/astra-fable-pricing-support/design-spec.md` | Solution Designer | Implementation-ready technical design | This package | All | Ready at SR-002 | Implements unchanged approved SR-001 intent; no new behavior approval required |
| `/Users/normy/autobyteus_org/autobyteus-worktrees/astra-fable-pricing-support/tickets/in-progress/astra-fable-pricing-support/solution-revision-record.md` | Solution Designer | Cumulative solution-round index | This package | All | Current at SR-002 | Approval and design history recorded |

## Assumptions, Unknowns, And Risks

| ID | Type | Description | Why It Matters | Resolution / Owner | Status |
| --- | --- | --- | --- | --- | --- |
| RISK-001 | Risk | Codex Fast, OpenAI Batch/Flex, Claude Batch, inference-geo, partner, private-contract, subscription-credit, or negotiated pricing differs from Standard | Applying a variant without a captured variant identity would create false precision | Explicitly out of scope; preserve Standard-only catalog behavior | Controlled by approved SR-001 boundary |
| RISK-002 | Risk | Adding a built-in row also makes the model available through the AutoByteus direct provider catalog | Shared catalog ownership has both pricing and direct-model effects | Existing generic Responses adapter is valid for Astra; existing Anthropic family policy covers Fable 5.1; add mocked request-shape regressions and omit a misleading Fable 5.1 toggle schema | Resolved in SR-002 design |
...
| UNK-001 | Unknown | Exact implementation file set and whether the GPT-5.6 pricing helper should be renamed/generalized | Affects design cleanliness, not intended behavior | Generalize the file-local long-context pricing helper, retain existing GPT-5.6 effective date, and add only focused existing-file tests/docs | Resolved in SR-002 design |

## Architecture Investigation Findings

- Change posture is a narrow bug fix: two official exact IDs are absent from the single authoritative built-in catalog. Exact-match and fail-closed behavior are healthy and must remain unchanged.
- The only production source change belongs in `autobyteus-ts/src/llm/supported-model-definitions.ts`. No dynamic catalog, usage adapter, server calculator, transport, persistence, or frontend production change is justified.
- The existing `createOpenAIGpt56Pricing` behavior is reusable for Astra, but its name and hardcoded effective date are too narrow. Generalize it locally to a long-context OpenAI pricing constructor with an explicit effective date and remove the old helper name; do not duplicate the tier policy.
- Astra's static AutoByteus/direct schema should advertise the official direct API effort set `low`, `medium`, `high`, `xhigh`, `max` with default `medium`. Codex may continue advertising `ultra` dynamically because its App Server metadata is a separate runtime-owned contract.
- Fable 5.1 should not receive the existing optional adaptive-thinking toggle schema: official behavior is always-on adaptive thinking and this package excludes the effort beta. The direct adapter already sanitizes invalid fixed/disabled thinking and sampling options through family-prefix matching, so default provider behavior plus regression coverage is the cleanest target.
- The existing server pricing boundary can prove trusted base/cache dimensions and the calculator can prove Astra's inclusive 272,000 versus 272,001 tier selection using synthetic payloads. No paid provider request is needed.
- Existing stored observations remain untouched; new observations resolve the new rows and snapshot their prices through the same current path. Persisted-data decision is `Not Affected`.
- Completed solution classification is `task_size=Small`, `architectural_risk=Low`: one production catalog file is changed, plus focused tests and docs inside existing boundaries; no shared interface, API, persistence, security, concurrency, deployment, or ownership-boundary change is introduced.
...
The authoritative technical plan is now in `design-spec.md`. Implementation should change the centralized definitions, focused tests, and existing documentation only. Any need to change runtime discovery, pricing interfaces, stored schemas, frontend code, or variant identity is a Design Impact and must return to Solution Design rather than expanding scope.
