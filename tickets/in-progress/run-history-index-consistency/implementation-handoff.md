...
- Added explicit migration/repair tooling and operator README; full metadata directory scans are isolated to this script/README path.
- Updated cleanup script to use safe run ID/path validation and atomic V2 index writes before deleting matched run directories.
...
- Updated focused server unit tests and relevant lifecycle tests for the V2 catalog model.
...
- Legacy/orphan standalone metadata runs that are absent from `run_history_index.json` are intentionally not auto-discovered on normal startup/list; operators must run the migration/repair script.
...
- Users who do not run the explicit migration/repair script will not see legacy orphan metadata directories that are missing V2 index rows.
- Cleanup against legacy/minimal indexes must now run after the explicit migration/repair script; this order is documented in `autobyteus-server-ts/scripts/run-history-index-migration.md`.
...
- Changed source implementation files stayed within proactive size-pressure guardrails (`>500` avoided; `>220` assessed/acted on): Yes. Largest changed source file is `agent-run-history-catalog-service.ts` at 415 non-empty lines; changed large existing lifecycle files stayed below 500 non-empty lines.
- Notes: V1 index parsing is not a normal compatibility path; invalid/legacy V1 catalog files return an empty catalog with warning and are repaired only via script. Cleanup also refuses legacy/minimal indexes instead of preserving arbitrary old row shapes as V2.

## Environment Or Dependency Notes

- Worktree: `/Users/normy/autobyteus_org/autobyteus-worktrees/run-history-index-consistency`
- Branch status during handoff: `codex/run-history-index-consistency...origin/personal [behind 3]`
- Server checks used temporary local `node_modules` symlinks to the superrepo dependency installs for `autobyteus-server-ts` and `autobyteus-ts`; symlinks were removed after checks.
- Web checks used temporary local `node_modules` and `.nuxt` symlinks to the superrepo web dependency/generated-type installs; symlinks were removed after checks.
...
- Passed: server source typecheck
  - `tsc -p autobyteus-server-ts/tsconfig.build.json --noEmit --pretty false`
- Passed after code-review local fixes: focused server unit/lifecycle tests, `14` files / `55` tests
  - `vitest run tests/unit/run-history/store/agent-run-metadata-store.test.ts tests/unit/run-history/agent-run-metadata-service.test.ts tests/unit/run-history/store/agent-run-history-index-store.test.ts tests/unit/run-history/services/agent-run-history-index-service.test.ts tests/unit/run-history/services/agent-run-history-catalog-service.test.ts tests/unit/run-history/services/agent-run-history-service.test.ts tests/unit/agent-execution/agent-run-status-projection-service.test.ts tests/unit/agent-execution/agent-run-provisioning-service.test.ts tests/unit/agent-execution/agent-run-create-service.test.ts tests/unit/agent-execution/agent-run-restore-service.test.ts tests/unit/agent-execution/agent-run-termination-service.test.ts tests/unit/agent-execution/agent-run-command-coordinator.test.ts tests/unit/agent-execution/compaction/server-compaction-agent-runner.test.ts tests/unit/scripts/cleanup-codex-e2e-run-history.test.ts`
- Passed after code-review local fixes: targeted regression subset, `4` files / `21` tests
  - `vitest run tests/unit/scripts/cleanup-codex-e2e-run-history.test.ts tests/unit/run-history/services/agent-run-history-catalog-service.test.ts tests/unit/run-history/services/agent-run-history-service.test.ts tests/unit/agent-execution/agent-run-status-projection-service.test.ts`
- Passed: whitespace check
  - `git diff --check`
- Passed: migration script smoke tests
  - dry-run/apply against legacy metadata + V1 index;
  - verified V2 output, backup creation, old fields omitted;
  - verified deterministic fallback from legacy index `lastActivityAt` when no `createdAt`/`preparedAt` exists.
- Passed: targeted standalone web store checks, `9` passed / `36` skipped in file
...
- Passed: targeted mobile/history web checks, `2` files / `10` tests
...
- Run the migration script against copied legacy memory data in dry-run and apply modes; confirm backup and deterministic `createdAt` report.
- Run cleanup only after migration on copied legacy memory data; cleanup should reject legacy/minimal indexes with migration guidance.
