...
- Triggering rework report and revision record:
  ...
  - `/Users/normy/autobyteus_org/autobyteus-worktrees/token-usage-one-row-per-agent-run/tickets/in-progress/token-usage-one-row-per-agent-run/delivery-evidence/12-delivery-rework-audit-dr004.log`
  ...
  - `/Users/normy/autobyteus_org/autobyteus-worktrees/token-usage-one-row-per-agent-run/tickets/in-progress/token-usage-one-row-per-agent-run/delivery-evidence/14-exact-root-cause-handoff-audit-dr004.log`
...
- Triggering finding IDs: no separate finding ID; reachable premise `MP-004` and delivery rework `DR-004`. `CR-001`–`CR-006`, `APIE2E-F001`–`APIE2E-F002`, and historical `AR-001`–`AR-004` remain resolved.
...
| BEH-005 | Expand to current records, deterministically transport nullable source scalars, consolidate legacy rows once, keep failed-interval new-run rows disjoint, reject overlap, validate, then clean up atomically. | Prisma schema/migration; `src/app-data-migrations/migrations/token-usage-run-records-v1/*`; shared record/pricing/cache fold; migration registry; canonical allocator/current-schema admission paths. | One transaction spans preflight, DS-009 typed source projection/strict decode, normalized fold/import, target round-trip, independent normalized scalar aggregate validation, and source deletion. All 15 closed paths are parameterized and their aliases come only from the closed field set. Only canonical nonnegative `integer:` digits within SafeInt reach exact BigInt checkpoints; wrong types/grammar/range abort before cleanup. Released unknown-input/local semantics remain intact, existing disjoint current rows are preserved, and any run-ID intersection aborts before mutation. |
...
- Migration-only historical boundary: both repaired 20260730 files, moved TeamRun V1 token repository, and `src/app-data-migrations/migrations/token-usage-run-records-v1/`. The latter owns DS-009's closed-field, parameterized JSON type/exact-text projection, untrusted string-or-null transport parser, released unknown-input normalization, and independently matching scalar validation; no current owner imports it.
...
- SQLite semantic type, computed-expression affinity, Prisma result metadata, and JavaScript runtime representation are separate contracts. Migration-derived scalars require a deterministic SQL transport and exact validation rather than a TypeScript raw-query annotation.
...
- `API-REV-004` and `CRR-010` passed the post-merge integrated lifecycle delta, but they predate DS-009 and the production-shaped Electron failure. The new migration source and durable tests require source review, renewed API/E2E coverage/execution, proportional test review, delivery rebuild, and renewed user Electron verification before acceptance.
...
- The prior Electron artifact was built but failed explicit user verification because the pre-DS-009 migration rejected valid legacy scalars at the nullable Prisma boundary. It is not accepted. Delivery owns rebuilding only after this source/test state passes all required review gates.
...
- Design-spec decision reference: `Persisted Data / State Transition Decision`, `Degraded Forward-Only Capability Gate`, and `Migration Plan` in `design-spec.md` (`REQ-001`–`REQ-027`; `AC-001`–`AC-026`).
...
- Migration implementation and focused checks:
  ...
  - Consolidation performs scalar overlap preflight, bounded `(run_id,id)` reads, deterministic DS-009 JSON type/exact-text projection and strict BigInt/SafeInt parsing, released-shape normalization and legacy `(observed_at,id)` folding, target insert/round-trip and independently normalized scalar aggregate validation, and source deletion inside one real SQLite transaction.
  - A disposable real Prisma/SQLite fixture preserves the production result shape: four leading `NULL` values and later JSON integers `28826658`/`28987545` in the same ordered batch. The actual repository query yields exact tags, the transaction creates one validated checkpoint row, and cleanup occurs only after validation. Real wrong-type, negative, and out-of-range fixtures prove source preservation, empty target state, bounded reasons, and ordinary retryability.
...
- `IR-007` server TypeScript check: `./node_modules/.bin/tsc -p tsconfig.build.json --noEmit --pretty false` — passed.
- `IR-007` focused DS-009 Vitest: real Prisma/SQLite transport/transaction plus focused decoder grammar — 2 files / 32 tests passed. The success case uses one six-row ordered run with four leading `NULL` values followed by JSON integers `28826658` and `28987545`; it verifies exact `integer:` transport, one current record, totals/checkpoint, and validated cleanup.
- `IR-007` migration regression selection: DS-009 adapter/decoder, released unknown-input transaction, cleanup rollback/retry/freelist, and legacy fold semantics — 4 files / 43 tests passed. Real-adapter rejection cases cover JSON real/text/boolean/array/object, negative integer, and `9007199254740992`; every failure retains all source rows, leaves the target empty, and repeats through ordinary retry with field-specific type/grammar/range evidence. Focused decoder cases reject untagged, malformed, signed, leading-zero, fractional, exponent, whitespace, and wrong-type transport while accepting zero through `Number.MAX_SAFE_INTEGER` exactly.
- `IR-007` static checks: closed cumulative field list drives both parameterized JSON paths and closed aliases; current runtime has no legacy transport import; changed source files remain below 500 effective lines; `git diff --check` passed. No dependency, schema, current runtime, API, GraphQL, or frontend file changed in this rework.
...
`IR-007` changes only the registered migration adapter and adds no frontend or interaction delta. The prior Electron package failed migration verification and is not accepted; delivery owns a corrected rebuild and renewed user verification after source/API-E2E/test-review gates pass.

## Downstream Coverage Hints / Suggested Scenarios

- Source-review DS-009 as the only current delta: closed-field/parameterized SQL projection, migration-only string-or-null transport, exact grammar, BigInt-before-narrowing, truthful bounded errors, and no current-runtime legacy dependency.
- Re-run the real disposable Prisma/SQLite leading-`NULL` sequence through API/E2E with the actual migration boundary; preserve the exact four `NULL`, `28826658`, and `28987545` order and validate source cleanup/current checkpoint.
- Recheck wrong-type/negative/out-of-range rollback and retry, plus capability degradation while a failed consolidation remains incomplete. The existing released-scale/lifecycle/API/browser evidence can be reused only where API/E2E's fresh coverage investigation establishes no impact.
- After successful execution, route both new durable test files through proportional `/code_reviewer` review before delivery rebuilds Electron and requests renewed user verification.

## API / E2E / Executable Coverage Investigation And Execution Still Required

`API-REV-004` and `CRR-010` remain the prior integrated baseline but predate `SR-007`, `IR-007`, and the failed Electron verification. After source review, `/api_e2e_engineer` must update its coverage investigation for DS-009, execute the real adapter/migration/degraded-state plan, and return the two new durable coverage files for proportional review. Delivery must not reuse the failed package as acceptance evidence; it owns a fresh base check, rebuild, integrity verification, and explicit renewed user verification afterward.
