...
| SR-008 | `/delivery_engineer`; `delivery-requirement-gap.md`; `DR-006` after successful live one-row verification | No new AR finding; closes the terminal-state reachability gap in REQ-014/REQ-025; AR-001–AR-004 preserved | `Requirement Gap / Design Impact` | Bounded current status and terminal audit-compaction package ready for architecture re-review |
| SR-009 | `/architecture_reviewer`; `design-review-report.md`; `ARCH-REV-008` / round 8 | AR-005, MP-005; AR-001–AR-004 preserved | `Design Impact` | Reachable startup-scheduled/nonfatal audit compactor package ready for architecture re-review |
...
### SR-008 — Bounded terminal migration audit evidence

- Triggering role, report path, and round: `/delivery_engineer`; `/Users/normy/autobyteus_org/autobyteus-worktrees/token-usage-one-row-per-agent-run/tickets/in-progress/token-usage-one-row-per-agent-run/delivery-requirement-gap.md`; `DR-006` after the corrected `SR-007` implementation passed live verification.
- Triggering finding IDs: No architecture finding ID. This is a downstream `Requirement Gap / Design Impact` against the already-approved bounded-evidence requirements `REQ-014` and `REQ-025`. Preserve resolved `AR-001`–`AR-004`, historical `MP-003`, reachable `MP-004`, and passed `ARCH-REV-007`.
- Prior authoritative result: `SR-007` passed `ARCH-REV-007`, completed implementation/code-review/API-E2E/delivery reroutes, and succeeded live: attempt 6 consolidated 158,025 legacy rows into 1,283 unique current run rows with healthy current statistics and in-place updates. Delivery could not finalize because two older terminal audit bodies remained supported current API inputs.
- Current authoritative result: Requirements, investigation, analysis, convention, and design now define one exact bounded current read boundary plus a separate at-rest compaction transition for the two known already-terminal released token migration records/logs. Ready for architecture re-review; implementation/finalization remain blocked until pass and the required downstream reroute.
- Why this revision is recorded: `AppDataMigrationRunner` intentionally skips `SUCCEEDED`/`SUCCEEDED_WITH_WARNINGS`, so repaired same-ID business definitions cannot rewrite old completed summaries. The current record repository, runner, and exact frontend GraphQL query nevertheless materialize two 13,964,274/14,318,058-byte summaries, yielding a reachable 31,387,995-byte response. A writer-only evidence cap did not close this supported terminal state.
- Resolution:
  - Added derived `REQ-028` / `AC-027` while clarifying `REQ-014` / `REQ-025`: every scheduling/prerequisite/status/API/UI record read receives a uniform summary no larger than 64 KiB, projected in SQL before raw oversized JSON can enter Node.
  - DS-010 makes `AppDataMigrationRecordRepository` the generic current bounded-status owner. Valid oversized uniform summaries yield exact four aggregate counts plus one truthful omission marker; invalid/unsupported bodies yield a bounded unavailable marker. The repository contains no token migration IDs, historical detail semantics, or mutation-on-read.
  - Added/registers `20260819_token_usage_migration_audit_compaction_v1` under DS-011. It alone knows the two 20260730 IDs and regular-log shape, validates through scalar SQL, preserves migration identity/name/original terminal status/attempt/timestamps/error/four counts, and replaces only row-linear details plus an owned oversized regular log.
  - The provider-name log inside the configured directory is eligible for canonical bounded replacement; the missing custom-provider `/tmp` log is not reconstructed. Missing/outside/unwritable/invalid source remains intact and produces bounded `SUCCEEDED_WITH_WARNINGS`; DS-010 already keeps the application/status API healthy.
  - Compaction is idempotent, not a prerequisite for already-validated token consolidation, never reruns/relabels either business migration, and never touches `token_usage_ledger_events`, `token_usage_run_records`, or the successful consolidation record. No manual production mutation, `VACUUM`, backup/recovery state machine, or runtime legacy support is introduced.
- Approved behavior or requirement IDs affected: `BEH-004`; clarified `REQ-014` and `REQ-025`; new `REQ-028`; clarified `AC-014` and `AC-024`; new `AC-027`. `REQ-001`–`REQ-013`, `REQ-015`–`REQ-024`, `REQ-026`–`REQ-027`, `AC-001`–`AC-013`, `AC-015`–`AC-023`, and `AC-025`–`AC-026` retain their approved meaning.
- Canonical artifacts and sections updated:
  - `requirements.md`: successful live baseline, terminal-state gap, in-scope read/compaction behavior, `REQ-028` / `AC-027`, constraints, risks, coverage, and approval state.
  - `investigation-notes.md`: `DR-006` sources, exact sizes/counts/API/log evidence, read-only SQL proof, findings 31–37, persisted audit subject, owner split, and reviewer guidance.
  - `design-spec.md`: SR-008 status, DS-010/DS-011, 64 KiB SQL projection, closed two-ID/log compactor algorithm, owners/interfaces/files/dependencies/removals/sequence/examples/risks/coverage.
  - `data-migration-conventions.md`: terminal audit/current bounded-read rule, separate-migration ownership, warning disposition, default pattern, and implementation-mechanics impact.
- Supplemental artifacts updated, added, or removed: `token-usage-data-model-analysis.md` records successful consolidation plus exact terminal summary/detail/count/API/log evidence and the bounded-reader/separate-compactor conclusion. No new supplement added; `delivery-requirement-gap.md` and evidence 19/20 remain the triggering downstream records.
- Downstream and architecture-review impact: Architecture review must verify that DS-010 protects every current consumer before compactor scheduling, remains format-generic/current-only, and cannot transfer an oversized body; that DS-011 preserves audit truth and has a closed mutation/path boundary; and that warning/idempotence/test behavior is exact. On pass, implementation must resume from the reviewed current worktree and the full source/test/document package must repeat implementation, code review, API/E2E, and delivery. Finalization stays held.
- Next recipient or routing: `/architecture_reviewer` for round-8 re-review with the complete cumulative package including `DR-006` and evidence 19/20.
- Remaining gaps or risks: Malformed/unsupported old summary shapes and unowned/unwritable logs remain preserved warning residue; the current read envelope prevents availability or result-size impact. General migration audit retention and physical SQLite shrink remain outside scope. No residual affects verified token totals or the one-row invariant.

### SR-009 — Reachable startup scheduling separated from audit failure criticality

- Triggering role, report path, and round: `/architecture_reviewer`; `/Users/normy/autobyteus_org/autobyteus-worktrees/token-usage-one-row-per-agent-run/tickets/in-progress/token-usage-one-row-per-agent-run/design-review-report.md`; `ARCH-REV-008`, review round 8.
- Triggering finding IDs: `AR-005`, with material premise `MP-005`. Preserve resolved `AR-001`–`AR-004`, historical `MP-003`, reachable `MP-004`, and the sound DS-010/DS-011 ownership confirmed by the reviewer.
- Prior authoritative result: Architecture review `Fail` / `Design Impact`. SR-008 defined a correct compactor but declared `requiredOnStartup=false` plus `STARTUP_ONLY`; actual `runPending()` skips false definitions, `runMigration()` rejects STARTUP_ONLY, and ServerRuntime calls only `runPending()`, so no supported production entrypoint could execute DS-011.
- Current authoritative result: Requirements, investigation, analysis, convention, and design now align compactor scheduling, failure criticality, retry states, and acceptance coverage with the real runner. Ready for architecture re-review; implementation/finalization remain blocked.
- Why this revision is recorded: Scheduling inclusion and startup fatality are distinct in current code despite the `requiredOnStartup` field name. A noncritical cleanup migration still needs `requiredOnStartup=true` to enter `runPending()`. Noncriticality is expressed through explicit ServerRuntime gates and prerequisites, not by making the definition unreachable.
- Resolution:
  - Compactor metadata is now `requiredOnStartup=true`, `executionPolicy="STARTUP_ONLY"`; ordinary ServerRuntime -> `runPending()` is its sole supported production path. No runner API or second orchestrator is added, and public/manual `runMigration()` remains rejected.
  - The compactor ID is explicitly absent from `TokenUsageRunRecordsV1AppDataMigration.prerequisiteMigrationIds` and every ServerRuntime fatal-status lookup/gate. A failed/warning compactor result does not change token readiness or global application health.
  - Retry semantics now follow actual statuses: `FAILED` and stale `RUNNING` retry on later ordinary startup; `SUCCEEDED` and `SUCCEEDED_WITH_WARNINGS` are terminal/skipped. Unsupported/unowned evidence is a final bounded warning rather than a false retry promise.
  - Partial progression is exact: log-success/database-summary failure throws and becomes `FAILED`; later `runPending()` treats the bounded log as a no-op and completes the summary. Compacted old source plus compactor terminal-status persistence failure leaves `FAILED` or stale `RUNNING`; later `runPending()` recognizes already-bounded source.
  - `AC-027` must exercise the registered definition through the actual registry/repository/`runPending()` path. Direct migration-object `execute()` or manual `runMigration()` cannot prove reachability.
- Approved behavior or requirement IDs affected: Clarified `REQ-028` and `AC-027`; no new requirement/acceptance IDs. `BEH-004` production path is corrected. Token behavior, DS-009, DS-010/DS-011 ownership, and all other requirements/criteria retain their meaning.
- Canonical artifacts and sections updated:
  - `requirements.md`: SR-009 status, exact metadata/caller/fatality/retry contract in `REQ-028`, ordinary-runner and partial-progression coverage in `AC-027`, approval alignment.
  - `investigation-notes.md`: ARCH-REV-008/AR-005/MP-005 source evidence, runner/ServerRuntime path, findings 38–41, constraints, reviewer guidance.
  - `design-spec.md`: SR-009 status/mechanics, current path, migration plan, DS-011 start/owner, metadata, status retry rules, dependencies/files/examples/rejections/sequence/risks/guidance.
  - `data-migration-conventions.md`: reusable scheduling-versus-criticality rule and current runner application.
- Supplemental artifacts updated, added, or removed: `token-usage-data-model-analysis.md` adds the verified runner scheduling/fatality/retry analysis. No artifact added or removed.
- Downstream and architecture-review impact: Review must verify that DS-011 is now reachable from ServerRuntime's ordinary `runPending()`, remains nonfatal without a hidden prerequisite/gate, and that retry/coverage claims match `FAILED`/stale `RUNNING` versus terminal warnings. On pass, implementation resumes through the full downstream reroute. Finalization stays held.
- Next recipient or routing: `/architecture_reviewer` for round-9 re-review with `ARCH-REV-008` and the complete cumulative package.
- Remaining gaps or risks: The overloaded metadata name remains a maintenance smell but changing the generic runner API is disproportionate here; explicit production-path tests lock its current meaning. General audit retention, physical SQLite shrink, and unsupported historical audit source remain outside/noncritical. No risk reopens token totals or the one-row invariant.
