...
| --- | --- | --- | --- | --- | --- |
| `tickets/in-progress/right-panel-resize-collapse/ui-ux-spec.md` | UI/UX journey and state-transition contract | R-001–R-005 | AC-001–AC-006 | `Requirements-ready`; intended behavior based on user request | Defines docked/strip/drawer states, explicit collapse versus responsive yield, and the reported interaction sequence |
...
- Requirement or scope impact: Modify the existing responsive policy and add focused regression coverage; no new subsystem, API, persisted state, or broad refactor.
