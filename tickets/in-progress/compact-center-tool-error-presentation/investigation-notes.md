...
- Investigation status: Coherent baseline complete; ready for approval, not approved or routed

## Initial Request And Clarifications

- Original request: Investigate why a failed command shows a very long, mostly normal-looking output in the center event-monitor area, then improve the UI so the middle area does not display detailed error content; existing red failed treatment is enough because the right-side Activity item already provides expandable Error detail.
- Clarifications received: The user emphasized that the center stream should not show detailed error content at all and then requested a new ticket bootstrap.
- User-supplied facts and constraints: Screenshots identify the center flooding and the existing Activity detail surface; raw traces under AutoByteus memory were suggested as the evidence source.
...
| --- | --- | --- | --- | --- | --- |
| 2026-09-02 | User | Current request and two supplied screenshots | Establish desired presentation and evidence target | Center detail is unwanted; existing red state is enough; Activity is the detailed surface | Capture as target behavior and preserve Activity |
...
| 2026-09-02 | Code | `autobyteus-web/components/progress/ToolActivityItem.vue:34-145` | Verify detailed surface | Activity has expandable Error section with full multiline string; outer card/error default expanded | Preserve behavior, including default expansion |
...
| `BEH-002` | User | User clicks/keyboard-activates a non-approval center tool card or opens Activity | Frontend selects progress/Activity tab and highlights matching invocation; Activity card exposes independently toggleable detail sections | Exact arguments/error are available in a diagnostic surface | Screenshot, components, tests | High |
...
| `components/progress/ToolActivityItem.vue` | Renders detailed arguments/logs/result/error and status | Becomes the sole normal detailed error surface for tool failures | No redesign required |
...
- Runtime modules, shared interfaces, routes, APIs, persistence boundaries, security/concurrency controls, deployment configuration, or ownership boundaries: Frontend segment/activity state projection, shared tool-card presentation interface, center component, Activity component, navigation/highlight store calls, browser/component tests.
- Existing structural surfaces that can support the approved behavior: Activity already stores/renders full detail independently; center card already navigates to exact Activity item and already has red failed styling.
...
| User | Right Activity already provides expandable Error detail | Direct | Preserve full detail and navigation in Activity | Do not change default expansion in this ticket |
...
- User's requested outcome, in the user's own terms: Middle area should not show detailed error content; red failure treatment is enough; detail is available from Activity.
- Requirement / behavior IDs involved: `BEH-001`, `BEH-002`; `REQ-001`–`REQ-004`
...
- Critical journey and states: Failed center card, navigation to matching Activity item, expanded Activity Error detail.
- Known constraints and non-goals: No new surface, excerpt, backend change, or Activity redesign.
- Relevant existing-product or frontend context supplied or established: Two screenshots and current production components.
...
- Explicit user-confirmation reference: Target behavior stated in the 2026-09-02 request; formal requirements approval pending.
...
- Product decisions supported by evidence: Compact center; detailed Activity.
- Alternatives rejected or still open: Center excerpt/collapsed center details rejected in proposed `DEC-001`.
...
- The existing center red border/icon and Activity navigation already implement most desired behavior; the unwanted behavior is the inline body and its supporting presentation/tests.
...
- Very large payload validation is mandatory; testing only a one-line error would miss the reported failure mode.
...
- Keep the right Activity error string and trace/replay contract authoritative. Do not solve the UI issue by changing backend enrichment or discarding `segment.error` before Activity has it.
...
- Retain center navigation/highlighting and keyboard activation for failed cards.
- Update the existing `codex-command-failure-detail` fixture/probe to assert intentionally different center and Activity content roles.
- Run with a deterministic very large error fixture and inspect DOM/accessibility text, card height, vertical/horizontal scroll behavior, and Activity completeness at desktop and narrow widths.
