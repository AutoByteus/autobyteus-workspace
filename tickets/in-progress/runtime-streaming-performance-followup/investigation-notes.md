...
- Notes For Downstream Agents: Do not use the user's shared `personal` checkout for authoritative edits. This follow-up design is based on `origin/personal @ c2ae6634d`; the task branch has no implementation yet.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `/Users/normy/autobyteus_org/autobyteus-worktrees/runtime-streaming-performance-followup/tickets/in-progress/runtime-streaming-performance-followup/performance-evidence.md` | Evidence-only live installed-process observation, prior event-shape calculation, current code/settings-path analysis, and accumulated-Markdown scaling probe. | Establishes the 500 ms choice, backend shaping benefit/boundary, timer-only insufficiency, and existing settings integration path. | Requirements and future design spec. | FR-001, FR-002, FR-003, FR-006, FR-007, FR-008 / AC-001, AC-002, AC-003, AC-006, AC-007, AC-008 | `Current` | `N/A — evidence only` | Add realistic candidate before/after evidence downstream. |
...
| BEH-003 | System | Runtime emits fine-grained content events through an active `AgentRun`. | Runtime backend source-event batch -> `AgentRun.publishSourceEvents` -> canonical event processing/dispatch -> standalone/team and non-WebSocket subscribers -> server mapper -> WebSocket send -> client parse/dispatch. | Native AutoByteus commonly supplies one content event per batch. Every canonical consumer sees token-sized frequency; exact accumulated content/boundary semantics are the real invariant. | Current source inspection; historical event shape. |
...
- Candidate root cause classification: `Boundary Or Ownership Issue` plus bounded `Local Implementation Defect`
- Refactor posture evidence summary: Refactor is required. Cadence currently begins only after every raw event reaches the renderer, while server agent/team WebSocket egress has no shared pacing owner; active text presentation also routes every revision through a full accumulated rich-render pipeline. A clean target should add one shared WebSocket-egress coalescer, remove the frontend timer, leave internal run events unthrottled, and use completion-aware active/final rendering.
...
- Every intervening non-content message is also a merge barrier. A routine non-terminal status may be sent without draining the pending window, but it seals the current tail group so content that arrives afterward cannot be concatenated across that status.
...
- A non-blocking companion that passes without a flush must still seal the pending aggregate tail; “no flush” is not permission to merge content from opposite sides of the companion.
...
- Requirements are approved. Review the design against the 500 ms configurable default, exclusive WebSocket-egress cadence ownership, and live-plain/final-rich presentation change.
- Treat the user's residual report as a gap in the historical solution. The refined evidence and user clarification support a configurable server WebSocket-egress cadence owner plus a rendering-lifecycle split, not a constant-only patch or internal event-bus throttling.
