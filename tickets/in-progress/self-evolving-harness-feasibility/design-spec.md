...
- **Run evidence / traces**
  ...
  - Current run metadata records `agentDefinitionId`, `workspaceRootPath`, runtime/model, `skillAccessMode`, and memory directory. The MVP must add a run-start snapshot of the **effective self-evolution config** so later manual actions use the run's launch-time eligibility. Exact resolved skill path/hash snapshots remain deferred; the MVP resolves current configured skill paths at evolution time and records that limitation.
...
- **Skill resolution / files**
  ...
  - Runtime skill loading caches skill objects; active runs may not automatically re-read an edited `SKILL.md`.
...
```text
Manual trigger -> visible self-evolver agent -> autoExecuteTools true -> run_bash edits target SKILL.md directly -> Git-backed inspect/revert -> target notification/reload
```
...
- The service supplies exact absolute target `SKILL.md` paths because skills may live outside the target workspace root.
- MVP direct edits are scoped to target skill files; non-skill mutation remains out of scope.
...
7. `SelfEvolutionService` resolves target run evidence, target agent definition, configured skill targets, and exact absolute `SKILL.md` paths.
...
11. The evolver task message instructs the agent to inspect traces/skills and directly edit only the supplied target skill files if an improvement is warranted.
12. After the evolver finishes, the service records provenance, update-production metrics, and optional Git status/diff/changing path information.
13. If skill changes are detected or assumed, the service emits a target skill-update notification/reload request. Future runs naturally load the edited skill; active run refresh remains best-effort/queued.
14. The metrics path links later target runs back to completed evolution records so reports can distinguish harness-updating success from downstream harness-benefit signals.
...
- No custom `emit_skill_change`, `apply_skill_update`, or diff/proposal tool.
- No mutation of tools, MCP config, model weights, repository code outside explicit target skill files, agent/team instructions, or memory lessons.
...
- Current design issue found (`Yes`/`No`/`Unclear`): Yes, for self-evolution orchestration. The substrate exists, but there is no authoritative owner that connects run evidence, skill path resolution, visible evolver launch, provenance, and target notification.
...
- Intentional deferrals and residual risk, if any:
  - Direct skill edits bypass service-mediated validation. This is accepted for MVP because the feature is off by default, manually triggered, visible, and intended for Git-backed testing first.
  - Active-run skill cache reload is best-effort/queued; next-run correctness is the MVP correctness baseline.
  - Effective self-evolution config snapshots are required at run launch. Exact resolved skill binding snapshots at original run start are still deferred; the MVP resolves current configured skill files and records this historical-accuracy limitation.
  - Stricter service-mediated proposal/apply may be added later as another `evolverStrategy` or apply mode after testing proves the direct-edit loop valuable.
...
- **Target skill file**: an exact absolute `SKILL.md` path resolved from the target agent definition's configured skills.
...
- **Evolution run record**: durable provenance record linking target run, evolver run, skill paths, runtime/model, status, post-run change summary, update-production metrics, and downstream benefit metrics.
...
- **Harness-updating metrics**: metrics about whether an evolution run produced a persistent harness update.
- **Harness-benefit metrics**: metrics about whether later target runs appear to use and benefit from the updated harness.
...
| DS-002 | Primary End-to-End | User clicks `Improve from this run` on standalone run | Visible evolver run edits target skill files and target is notified | `SelfEvolutionService` | Core MVP business flow. |
| DS-003 | Primary End-to-End | User clicks `Improve this member from this run` on team run | Visible evolver run edits selected member skill files and target/member is notified | `SelfEvolutionService` | Same core flow but with explicit team/member identity. |
...
| DS-005 | Return-Event | Evolver run completes/terminates | Evolution record finalized and target update notification emitted | `SelfEvolutionService` | The side effect occurs through file edits, so post-run observation matters. |
...
| DS-007 | Bounded Local | Post-run changed-file recording | Git status/diff summary attached to evolution record | `SelfEvolutionChangeRecorder` | Provides inspection/revert evidence without a custom patch/apply pipeline. |
...
| DS-009 | Return-Event | Completed evolution record and later target runs | Metrics report separating harness-updating from harness-benefit | `SelfEvolutionMetricsService` | Satisfies REQ-010 and preserves the paper's distinction between producing updates and benefiting from them. |
...
`Run detail UI -> SelfEvolutionResolver.startAgentRunSelfEvolution -> SelfEvolutionService -> TargetRunContextResolver -> SkillTargetResolver / EvidenceBuilder -> SingleAgentEvolverStrategy -> AgentRunService.createAgentRun -> EvolverAgent(run_bash edits SKILL.md) -> ChangeRecorder -> TargetNotificationService`

### DS-003 Team member self-evolution spine

`Team run detail UI -> SelfEvolutionResolver.startTeamMemberSelfEvolution -> SelfEvolutionService -> TeamMemberTargetContextResolver -> SkillTargetResolver / EvidenceBuilder -> SingleAgentEvolverStrategy -> AgentRunService.createAgentRun -> EvolverAgent(run_bash edits selected member SKILL.md) -> ChangeRecorder -> TargetNotificationService`
...
### DS-009 Metrics / benefit observation spine

`SelfEvolutionService finalizes record -> SelfEvolutionMetricsService records harness-updating metrics -> later run-history/metadata query -> SelfEvolutionMetricsService links post-evolution target runs -> GraphQL metrics report -> UI evolution report`
...
| DS-002 | A user manually starts evolution for a standalone run. The service validates global/target eligibility, resolves run metadata and configured skill paths, builds an evidence task, launches a visible single evolver agent with run_bash and auto-exec, records post-run state, and notifies/reloads target if skill files changed. | Manual action, self-evolution service, target context, skill paths, evolver run, change record, notification | `SelfEvolutionService` | Evidence reading, settings resolution, Git snapshotting, helper-run event collection |
| DS-003 | A user manually starts evolution for a selected team member. The service uses explicit `teamRunId + memberRunId`, resolves member metadata and that member's configured skills, then follows the same visible evolver launch and post-run recording path. | Team member action, self-evolution service, member context, skill paths, evolver run, notification | `SelfEvolutionService` | Team metadata flattening, member memory layout, member-scoped UI |
...
| DS-005 | The evolver edits files directly, so the service does not apply a patch. Instead, after final output/termination it records status/diff summaries and emits a notification/reload event. | Evolver completion, change recorder, run record finalization, notification | `SelfEvolutionService` | Git commands, active-run lookup, SenderType.SYSTEM message construction |
...
| DS-007 | Before and after the evolver run, the service captures lightweight Git/file status for supplied skill targets when possible. This gives visibility without building custom patch tooling. | Pre snapshot, post snapshot, diff summary, record attachment | `SelfEvolutionChangeRecorder` | Child-process git invocation, path grouping by Git root |
...
| DS-009 | The system records update-production metrics immediately after the evolver run and computes/refreshes downstream benefit metrics by linking later runs for the same target and overlapping skills. Reports explicitly show `not_enough_data` until later runs/feedback exist. | Evolution record, update metrics, linked later runs, benefit metrics report | `SelfEvolutionMetricsService` | Run-history lookup, raw trace activation proxies, UI reporting |
...
- `SelfEvolutionChangeRecorder`
- `SelfEvolutionMetricsService`
...
| `SelfEvolutionService` | End-to-end self-evolution request lifecycle, eligibility checks, strategy selection, provenance record creation/finalization, metrics finalization callout, target notification sequencing. |
...
| `SelfEvolutionSkillTargetResolver` | Resolving configured target skills to exact absolute `SKILL.md` paths and target metadata. |
| `SelfEvolutionEvidenceBuilder` | Building the concise evolution task evidence package and references from run memory/projections. |
...
| `SelfEvolutionChangeRecorder` | Optional pre/post Git/file snapshots for target skill paths; no mutation authority. |
| `SelfEvolutionMetricsService` | Harness-updating metric calculation, downstream benefit linkage, skill activation/adherence proxy extraction, and metrics report construction. |
| `SelfEvolutionTargetNotificationService` | Best-effort active target notification/reload request after direct skill-file changes. |
...
| Any planned custom evolver patch/proposal tools from earlier investigation | MVP direct-edit strategy intentionally uses existing `run_bash`. | `SingleAgentEvolverStrategy` + exact skill paths + Git-backed change recording. | In This Change | Do not add `emit_skill_change`, `apply_skill_update`, or custom diff schema. |
...
`Evolver AgentRun events -> SingleAgentEvolverStrategy completion collector -> SelfEvolutionService finalizes record -> SelfEvolutionChangeRecorder post snapshot -> SelfEvolutionTargetNotificationService -> active target run SenderType.SYSTEM message / next-run fallback`

Narrative:

The evolver run is a normal visible run. The self-evolution service should not parse every tool call as an apply contract. It only needs to know whether the run completed, failed, or timed out. After completion, it compares target skill paths before/after where practical. If changes are found or if change detection is inconclusive but the run completed, it records the result and sends a concise target notification.
...
| `SelfEvolutionChangeRecorder` | `group skill paths by Git root -> capture before status -> capture after status/diff stat -> normalize changed paths` | Gives auditability without custom patch tooling. |
...
| Trace/evidence reading | DS-002, DS-003 | Evidence builder | Build concise evidence/task context from memory. | Keeps prompt construction bounded and reusable. | Evolver prompt assembly would sprawl across service/resolver. |
| Skill path resolution | DS-002, DS-003 | Skill target resolver | Convert target definition's configured skills into exact `SKILL.md` paths. | Direct-edit MVP requires exact absolute paths. | Evolver might edit wrong files or workspace symlinks. |
...
| Git/file snapshots | DS-005, DS-007 | Self-evolution service | Record before/after status. | Provides inspect/revert evidence. | Main service becomes shell-command details owner. |
| Target notification | DS-005 | Self-evolution service | Notify active/inactive targets after skill file changes. | Separates mutation observation from runtime adoption. | Evolver might message target as teammate or mutate team topology. |
...
| Metrics / benefit observation | DS-009 | SelfEvolutionService and UI reports | Separate update-production metrics from linked downstream-benefit signals. | Required by REQ-010 and paper framing. | Change recorder would be mistaken for benefit measurement. |
...
| Post-run file change view | Existing run file change projection | Deferred / Create narrow Git recorder | Existing projection listens to file-change events from tools like `write_file`; `run_bash` edits may not emit those events. | A narrow Git snapshot recorder is needed for direct shell edits. |
...
| Effective config snapshot | Agent/team/run config and metadata | Extend | Existing config/metadata are the right persistence surfaces; add one resolver to avoid duplicated precedence. | A new independent settings table would not match target/run identity. |
| Harness-updating metrics | Evolution run record/change recorder | Extend | Update metrics derive from evolver status and change summary. | Metrics owner is still needed to name/update fields coherently. |
| Harness-benefit metrics | Run history / raw traces / future validation feedback | Create New narrow service | Benefit linkage crosses evolution records and later target runs. | Run history alone does not own self-evolution semantics. |

## Subsystem / Capability-Area Allocation

| Subsystem / Capability Area | Owns Which Concerns | Related Spine ID(s) | Governing Owner(s) Served | Decision (`Reuse`/`Extend`/`Create New`) | Notes |
| --- | --- | --- | --- | --- | --- |
| `self-evolution` | Capability, settings, effective config resolution, manual trigger request creation, request lifecycle, target resolution, strategies, run records, metrics, notification. | DS-001, DS-002, DS-003, DS-005, DS-006, DS-007, DS-008, DS-009 | `SelfEvolutionService` | Create New | New top-level server subsystem. |
...
| `src/self-evolution/services/self-evolution-skill-target-resolver.ts` | self-evolution | Skill target resolver | Resolve configured skill paths and eligibility. | Direct-edit MVP requires exact paths. | Yes |
| `src/self-evolution/services/self-evolution-evidence-builder.ts` | self-evolution | Evidence builder | Build prompt/evidence package. | Keeps prompt context separate from orchestration. | Yes |
...
| `src/self-evolution/services/self-evolution-metrics-service.ts` | self-evolution | Metrics owner | Computes update-production metrics and downstream benefit reports. | Metrics are neither Git recording nor generic run history. | Yes |
| `src/self-evolution/services/self-evolution-change-recorder.ts` | self-evolution | Change observation | Git status/diff summaries for target paths. | Shell/Git concern split from lifecycle. | Yes |
...
| Skill target info | `self-evolution/domain/models.ts` | self-evolution | Used by task message, Git recorder, record store. | Yes | Yes | Full `Skill` clone with unrelated fields. |
...
| Update/benefit metrics | `self-evolution/domain/models.ts` | self-evolution | Used by run store, metrics service, GraphQL, and UI. | Yes | Yes | Generic analytics schema for all runs. |
...
| `SelfEvolutionSkillTarget` | Yes | Yes | Low | Include `skillName`, `skillRootPath`, `skillMdPath`, `isWritable`, `gitRootPath`; no duplicated relative/absolute path pairs except where needed. |
| `SelfEvolutionRunRecord` | Yes | Yes | Low | Separate `target`, `evolverRun`, `skillTargets`, `changeSummary`, `updateMetrics`, and `benefitMetrics`; do not flatten every field into top-level strings. |
| `SelfEvolutionMetricsReport` | Yes | Yes | Low | Separate update-production and downstream-benefit sections; mark unavailable benefit fields as `not_collectible` or `not_enough_data`, not absent. |
...
| `autobyteus-server-ts/src/self-evolution/services/self-evolution-skill-target-resolver.ts` | self-evolution | Skill target resolver | Use `SkillService`/definitions to produce exact skill paths and eligibility. | Direct-edit path needs one trustworthy resolver. | Yes |
| `autobyteus-server-ts/src/self-evolution/services/self-evolution-evidence-builder.ts` | self-evolution | Evidence builder | Build task prompt/evidence/reference package. | Prevents prompt construction from bloating service. | Yes |
...
| `autobyteus-server-ts/src/self-evolution/services/self-evolution-metrics-service.ts` | self-evolution | Metrics service | Persist/update update metrics, link later runs, produce benefit report. | REQ-010 owner. | Yes |
| `autobyteus-server-ts/src/self-evolution/services/self-evolution-change-recorder.ts` | self-evolution | Change recorder | Pre/post Git/file snapshot around target skill paths. | Direct shell edits require observation. | Yes |
...
- `SelfEvolutionService` is authoritative for starting an evolution run. GraphQL/UI must not directly assemble target context, skills, evidence, metrics, and helper run creation.
...
- `SelfEvolutionSkillTargetResolver` is authoritative for skill file paths. The evolver prompt receives paths from this resolver; it must not be told to discover arbitrary skills by scanning the filesystem.
- `SingleAgentEvolverStrategy` owns helper-run creation mechanics; `SelfEvolutionService` owns lifecycle sequencing around it.
- `SelfEvolutionChangeRecorder` observes file/Git state only. It must not mutate files.
- `SelfEvolutionMetricsService` is authoritative for harness-updating vs harness-benefit metric calculation and report construction. Change recording is one input to metrics; it is not the metrics owner.
...
| `SelfEvolutionSkillTargetResolver` | Agent definition lookup, configured skill resolution, path normalization, Git/writable classification | SelfEvolutionService | Evolver prompt or UI scanning skill directories itself | Add resolver output fields. |
...
| `SelfEvolutionMetricsService` | Update metrics, downstream run linkage, activation/adherence proxies | SelfEvolutionService, GraphQL metrics/report query | UI/run store interpreting raw traces as benefit metrics independently | Add report fields or refresh method. |

## Dependency Rules

Allowed:

- `SelfEvolutionResolver` may depend on `SelfEvolutionService`, `SelfEvolutionCapabilityService`, and `SelfEvolutionMetricsService` for report queries only.
- `SelfEvolutionService` may depend on self-evolution config resolver, trigger strategy, evolver strategies/stores/metrics, `AgentRunService`, run metadata services, `SkillService`, `AgentDefinitionService`, `TeamRunMetadataService`, and `AgentRunManager`/team run manager for notifications through dedicated owned services.
...
- `SelfEvolutionChangeRecorder` may execute read-only Git/status commands against exact target paths.
- `SelfEvolutionMetricsService` may read self-evolution records, run metadata/history, raw trace projections for activation proxies, and explicit feedback/validation stores when present.
...
- UI must not compute metrics or benefit linkage by scraping records; it calls metrics/report APIs.
- Direct edit prompts must not say “edit any relevant files”; they must supply exact target `SKILL.md` paths and instruct the evolver to stay within them.
...
| `startAgentRunSelfEvolution(input)` | Standalone self-evolution command | Start manual evolution for one run. | `runId`, optional target config override | Returns `evolutionRunId`, `evolverRunId`. |
...
| `getSelfEvolutionRunRecord(evolutionRunId)` | Evolution provenance | Return recorded status/paths/run refs/change summary and embedded metrics. | `evolutionRunId` | Can power UI inspection. |
| `getSelfEvolutionMetricsReport(evolutionRunId)` | Evolution metrics | Return harness-updating and downstream-benefit report; refresh benefit links if needed. | `evolutionRunId` | Report shows `not_enough_data` until later runs exist. |
...
| `getSelfEvolutionMetricsReport` | Yes | Yes | Low | Metrics subject is one evolution run. |
| Agent/team/run config update surfaces | Yes | Yes | Low | Existing subject-specific update/launch APIs carry `selfEvolution`; no generic config mutation. |
...
| Metrics owner | `SelfEvolutionMetricsService` | Yes | Low | Keeps benefit observation separate from change recording. |
...
interface SelfEvolutionSkillTarget {
  skillName: string;
  skillRootPath: string;       // absolute root directory
  skillMdPath: string;         // absolute .../SKILL.md path supplied to the evolver
...
interface SelfEvolutionEvidencePackage {
  ...
  rawTracePaths: string[];        // references, not necessarily inlined in prompt
  runHistorySummary: string;      // minimized digest
...
interface SelfEvolutionChangeSummary {
  detectionMode: "git" | "file_timestamp" | "assumed_after_completed_run" | "none";
...
interface SelfEvolutionUpdateMetrics {
...
type BenefitSignalAvailability = "observed" | "not_observed" | "not_collectible" | "not_enough_data";
...
interface SelfEvolutionBenefitMetrics {
...
- `SelfEvolutionSkillTarget.skillMdPath` is the path the evolver may edit. If a skill root has supporting files, the MVP still treats `SKILL.md` as the primary editable artifact unless a future design broadens target paths.
- `rollbackMode: "unversioned"` is informational, not a second rollback implementation. Git remains the recommended rollback mechanism for MVP testing.
- Metrics must stay split into `updateMetrics` and `benefitMetrics`; changed files are not proof of downstream benefit.
...
- `requested/resolving_target -> failed` when capability is disabled, the target is missing, strategy is not implemented, no skill targets exist, or all target skill paths are unwritable.
...
- `recording_changes -> completed` even if Git detection fails, as long as the evolver run itself completed; attach warnings to `changeSummary`.
...
## Metrics And Benefit Observation Contract

This resolves AR-001 and preserves the paper's central distinction.

### Metrics owner

`SelfEvolutionMetricsService` owns metric calculation and reporting. `SelfEvolutionRunStore` persists the metrics inside/alongside `SelfEvolutionRunRecord`, but it does not interpret them. `SelfEvolutionChangeRecorder` supplies changed-file evidence but does not decide benefit.

### Harness-updating metrics

Recorded immediately after the evolver run and change recorder finish:

- evolver run completed / failed / timed out;
- no-op versus changed;
- changed skill path count and names;
- Git-backed versus unversioned target count;
- warning/error counts;
- notification status;
- evidence summary hash presence.

These answer: “Did the system produce a persistent harness update?”

### Harness-benefit metrics

Computed initially as `not_enough_data`, then refreshed by explicit metrics query or by a future run-completion hook. The MVP can implement query-time refresh first to avoid a background worker.

Benefit linkage algorithm:

1. Load the completed `SelfEvolutionRunRecord`.
2. Search later run metadata for the same target subject:
   - standalone: same `agentDefinitionId` as the evolved run;
   - team member: same member agent definition within the same team definition when available, or same member run identity lineage when available.
3. Keep candidate later runs whose configured/effective skills overlap the evolved `skillTargets` by skill name/path when resolvable.
4. Record linked run IDs with `linkMethod: "target_identity_and_skill_overlap"`.
5. Inspect linked run histories/raw trace projections for available proxies:
   - `load_skill` tool calls for evolved skill names where global/discovery mode exposes the tool;
   - configured preloaded skill availability when `PRELOADED_ONLY` makes activation non-observable but guaranteed available;
   - direct references to evolved skill names/headings in assistant messages when practical;
   - explicit user feedback/review/validation signals if existing stores expose them.
6. Produce `assessment` from available signals, defaulting to `not_enough_data` when no linked runs or feedback exist.

Benefit fields that cannot be collected yet must be represented as `not_collectible` or `null`, not silently omitted. This lets UI distinguish “no downstream evidence yet” from “benefit measured and neutral.”

### Metrics UI/reporting

The evolution detail UI should show two sections:

```text
Harness update
  - evolver status
  - no-op or changed files
  - changed skill count/paths
  - Git rollback availability
  - warnings/errors

Harness benefit
  - linked post-evolution runs
  - activation/adherence proxy status
  - user/evaluator/validation signal counts if available
  - assessment: not_enough_data / positive / negative / mixed / neutral
```

The UI must not claim improvement from changed files alone. If no later linked runs exist, it displays `No downstream runs observed yet`.
...
7. At least one configured skill resolves to an exact `SKILL.md` path.
8. At least one target skill path is writable.
...
| Skill not Git-backed | eligible with rollback warning | Record `rollbackMode: unversioned`; UI warns Git revert is unavailable. |
...
`SelfEvolutionEvidenceBuilder` should treat traces as sensitive experience, not as durable skill content.

Evidence package contents:

- target identity and source run IDs;
- target role/name/description and configured skill names;
- exact target skill paths;
- run-history digest: user goal, major turns, tool successes/failures, explicit user corrections, review feedback, final outcome if discoverable;
- references to raw trace files when needed for deeper inspection;
- explicit privacy instruction: do not copy secrets, user-specific details, one-off file paths, credentials, private messages, or proprietary content into durable skills unless the target skill already intentionally owns that domain-specific context.

Evidence package constraints:

- Prefer a concise digest plus file references over inlining entire `raw_traces.jsonl` into the prompt.
- If raw trace paths are included, they are read-only evidence. They are not editable targets.
- When evidence comes from team runs, include only the selected member's trace context plus minimal team-level context needed to understand coordination failures.
- Record an `evidenceSummaryHash` when practical so later review can connect the evolution record to the evidence digest without storing another full copy of sensitive traces.

## Evolver Task Prompt Contract

The default self-evolver agent template and the service-created task message should agree on the same direct-edit contract. The task message should be more specific than the agent template because it includes the exact target run and path context.

Required task-message clauses:

```text
You are improving durable skills for a target AutoByteus agent from prior run evidence.
...
5. Do not copy secrets, transient file paths, personal data, or one-off user requests into durable skills.
6. Prefer general reusable strategy, activation guidance, checklists, and failure-avoidance rules over task-specific details.
```

The default built-in self-evolver `agent.md` can include broader role guidance, but it must not weaken the task message's exact-path restrictions.
...
type SelfEvolutionUpdateMetrics {
  evolverRunCompleted: Boolean!
  noOp: Boolean!
  changedSkillCount: Int!
  changedSkillPaths: [String!]!
  gitBackedTargetCount: Int!
  unversionedTargetCount: Int!
  warningCount: Int!
  errorCount: Int!
}

type SelfEvolutionBenefitMetrics {
  linkedPostEvolutionRunIds: [ID!]!
  linkMethod: String!
  completedLinkedRuns: Int!
  failedLinkedRuns: Int!
...
type SelfEvolutionMetricsReport {
  evolutionRunId: ID!
...
type Query {
  ...
  getSelfEvolutionMetricsReport(evolutionRunId: ID!): SelfEvolutionMetricsReport!
}
...
- If backend eligibility returns warnings, the UI should display them near the action, especially non-Git or partially read-only skill targets.
- Starting evolution should link the user to the visible evolver `AgentRun` and later to the evolution run record/change summary/metrics report.
...
- If the target is inactive or completed, record `next_run_only`; future runs load the changed files.
...
```text
Your configured skill files were updated by self-evolution run <evolutionRunId>.
Changed skills: <list>.
...
- **Adapter/recorder**: `SelfEvolutionChangeRecorder` adapts Git/status information into a self-evolution change summary without owning mutation.
- **Metrics/reporting service**: `SelfEvolutionMetricsService` separates update-production metrics from downstream benefit reports.
...
| `autobyteus-server-ts/src/self-evolution/services/self-evolution-service.ts` | File | Main owner | Start/finalize evolution requests. | Spine owner. | Git command implementation details. |
...
| `autobyteus-server-ts/src/self-evolution/services/self-evolution-skill-target-resolver.ts` | File | Skill target resolver | Configured skill paths. | Direct-edit owner. | Evidence summarization. |
| `autobyteus-server-ts/src/self-evolution/services/self-evolution-evidence-builder.ts` | File | Evidence builder | Evolution task evidence. | Prompt input owner. | Run creation. |
...
| `autobyteus-server-ts/src/self-evolution/services/self-evolution-run-store.ts` | File | Run record persistence | Store/read/index evolution records. | Durable audit owner. | Business orchestration. |
| `autobyteus-server-ts/src/self-evolution/services/self-evolution-metrics-service.ts` | File | Metrics owner | Build/update metrics reports. | Separates update vs benefit. | Git command implementation. |
| `autobyteus-server-ts/src/self-evolution/services/self-evolution-change-recorder.ts` | File | Change recorder | Git/file status snapshots. | Direct-edit observation. | File mutation. |
| `autobyteus-server-ts/src/self-evolution/services/self-evolution-target-notification-service.ts` | File | Notification | Target update message/reload request. | Runtime adoption concern. | Skill path resolution. |
| `autobyteus-server-ts/src/api/graphql/types/self-evolution.ts` | File | GraphQL transport | Capability/strategy/start/record/metrics endpoints. | Existing GraphQL type layout. | Service internals. |
...
| Direct edit task prompt | `Only edit these exact files: /abs/path/A/SKILL.md` | `Improve any relevant skills in this repository` | Prevents broad repository mutation. |
...
| Config precedence | `default -> agent -> run -> run metadata snapshot` | Start mutation re-reads current agent config and changes old run eligibility | Historical run actions must be stable. |
...
| Metrics | `updateMetrics.changedSkillCount` separate from `benefitMetrics.assessment` | Treat changed files as proof of improved performance | Matches harness-updating vs harness-benefit distinction. |
...
Frontend UI/store
  ...
  -> target/evidence/skill/settings/effective-config/trigger/evolver/change/metrics/notification services
...
| --- | --- | --- |
| Add | `autobyteus-server-ts/src/self-evolution/` | New self-evolution domain, services, trigger/evolver strategies, effective config resolver, store, metrics service, change recorder, notification owner. |
| Add | `autobyteus-server-ts/src/api/graphql/types/self-evolution.ts` | Typed GraphQL capability, strategy catalog, eligibility, start, record, and metrics surfaces. |
...
| Add | `SelfEvolutionMetricsService` and metrics GraphQL/report fields | Distinguish harness-updating from downstream harness-benefit. |
...
1. **Add domain model and settings constants**
   ...
   - Define `SelfEvolutionRunConfigOverride`, `SelfEvolutionEffectiveConfig`, strategy descriptors, trigger request types, target refs, skill targets, run records, and update/benefit metrics.
...
7. **Implement single-agent evolver strategy**
   ...
   - Post a task message with evidence, exact absolute target `SKILL.md` paths, and direct-edit instructions.
...
8. **Implement run record, change recorder, and metrics owner**
   - Store records under `memory/self_evolution/evolution_runs/<evolutionRunId>/record.json` plus an index.
   - Capture before/after Git status/diff summary where Git roots are discoverable.
   - Record target skill paths regardless of Git availability.
   - Populate `updateMetrics` from evolver status and change summary.
   - Implement `SelfEvolutionMetricsService` with query-time benefit refresh over later run metadata/history.

9. **Implement target notification**
   - For inactive/future runs: record notification only; next run sees files.
   - For active idle standalone native run: post `SenderType.SYSTEM` notification with summary and changed skill paths.
...
10. **Add GraphQL transport**
    - Add capability, strategy catalog, eligibility, start, record, and metrics report queries/mutations.
...
11. **Add frontend UI**
    ...
    - Evolution run/history: link to visible evolver run, changed file summary, and metrics report.
...
12. **Validation and tests**
    ...
    - Unit-test target resolution and skill path resolution.
    - Unit-test built-in agent bootstrap default-setting generalization.
    - Unit-test update metrics and benefit report `not_enough_data`/linked-run cases.
...
2. **Evolver may edit unrelated files through `run_bash`.**
   - Mitigation: prompt exact paths; post-run Git summary; scope tests; future stricter strategy if needed.

3. **Skill paths may be outside workspace root.**
   - Mitigation: service supplies absolute `SKILL.md` paths; `run_bash` supports absolute paths.

4. **Some target skills may not be Git-backed.**
   - Mitigation: classify `gitRootPath`; expose warning/status. Design detail: implementation can disable direct-edit button for no-Git targets or allow with explicit warning. Recommended initial default: allow only writable targets and record Git availability; UI labels non-Git rollback risk.
...
9. **Metrics may be mistaken for proof of quality.**
   - Mitigation: UI separates update-production from downstream benefit and uses `not_enough_data` until later linked runs/feedback exist.
...
- The evolution task message should include a clear, concrete direct-edit contract, for example:

```text
You are improving the target agent's configured skills from a past run.

You may directly edit ONLY the following files:
1. /absolute/path/to/skill-a/SKILL.md
2. /absolute/path/to/skill-b/SKILL.md

Use run_bash to inspect traces and edit these files if improvement is warranted.
Do not edit agent.md, agent-config.json, tool config, MCP config, source code, or unrelated files.
Do not copy private one-off trace details into durable skills.
If no general improvement is warranted, make no file changes and explain why.
```

- Prefer a short evidence digest plus file paths to raw trace files. If raw trace paths are included, explicitly warn the evolver not to copy secrets or one-off user data into skills.
- Start mutations must fail fast with clear messages when:
  ...
  - target skill paths are not writable.
- Change recording should be best-effort and must not block basic evolution if Git is unavailable unless implementation chooses to enforce Git-backed targets.
- Metrics recording is required: `updateMetrics` must be available after evolver completion, and `benefitMetrics` must at least return `not_enough_data` with a clear link/availability explanation.
