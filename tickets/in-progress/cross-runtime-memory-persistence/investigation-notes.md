...
9. External runtime compaction signals are not equivalent to native Autobyteus compaction.
   - Native Autobyteus compaction is local-memory-aware: it emits `COMPACTION_STATUS`, creates compacted memory/snapshot state, and prunes known raw trace ids into `raw_traces_archive.jsonl`.
   ...
   - Therefore provider compaction signals/items must not drive local semantic compaction or deletion. The safe uses are non-destructive provenance markers or active-file rotation boundaries, pending schema/product decision.
...
- Must not prune, archive, rewrite, or compact local Codex/Claude memory files based solely on provider-internal compact-boundary/status events.
...
- Current `autobyteus-ts` memory primitives already define the raw trace item and snapshot serializer, but there is no explicit direct-memory-directory store facade; implementation should add one so the server does not duplicate file names or JSON field serialization.
...
- If provider compact-boundary markers are later persisted, the schema should preserve trigger/status metadata without overloading tool fields or pretending a local compaction plan occurred.
- If long-running external-runtime storage becomes a near-term requirement, add a separate rotation/snapshot-retention design instead of folding it into semantic compaction.

## Notes For Architect Reviewer

Requirements are refined and approved by the user as of 2026-04-30. The key clarified constraints are: Codex/Claude persistence is storage-only recording, not runtime memory management; and the common memory file structure/format should be made reusable from `autobyteus-ts` for server import instead of duplicated. Recommended design direction is a server-owned `AgentRunMemoryRecorder` under `agent-memory`, attached by `AgentRunManager`, with an internal `AgentRun` accepted-command observer for user messages and a normalized event accumulator for assistant/reasoning/tool output. Native Autobyteus memory should remain owned by `autobyteus-ts` and be skipped by the server recorder; only the low-level file-format/store kit should be shared. Corrected compaction finding: native compaction is local-memory destructive/archive-aware; Codex does expose compaction item/notification paths (`type=compaction`, `thread/compacted`) but the current server integration misses/drops them; Claude SDK exposes compact-boundary/status messages. External compaction signals/items are safe as non-destructive provider metadata or future rotation boundaries, not as local pruning triggers.
