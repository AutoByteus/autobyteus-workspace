...
async function uiSnapshot(page) {
  ...
  return {
    ...
    protocolErrors: [...body.matchAll(/Rejected SEGMENT_(?:START|CONTENT|END):[^\n]*/g)]
      .map((match) => match[0]),
    body,
  };
}
...
try {
  ...
  const teamHeading = page.getByRole('heading', { name: 'Nested Classroom Test Team', exact: true });
  await teamHeading.waitFor({ state: 'visible', timeout: 120000 });
  const teamCard = teamHeading.locator('xpath=ancestor::div[.//button[normalize-space()="Run"]][1]');
  ...
  const resultConditions = {
    ...
    noTeamProtocolErrors: beforeRefresh.protocolErrors.length === 0 && afterRefresh.protocolErrors.length === 0,
