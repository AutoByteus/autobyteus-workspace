...
const serverBase = 'http://127.0.0.1:60236';
const gqlEndpoint = `${serverBase}/graphql`;
const outDir = new URL('./browser/', import.meta.url).pathname;
const referencePath = new URL('./api36-mobile-desktop-reference.txt', import.meta.url).pathname;
const expectedReply = 'API36_REFERENCE_REPLY_AUTOBYTEUS';
const expectedContent = 'API36_REFERENCE_CONTENT_AUTOBYTEUS';
...
async function verifyDesktopReference(page, screenshotPrefix) {
  ...
  const referenceRow = page
    ...
    .filter({ hasText: 'api36-mobile-desktop-reference.txt' })
    .first();
  ...
  return {
    ...
    exactReferencePathVisible: viewerText.includes('api36-mobile-desktop-reference.txt'),
...
async function createMobileSession(deviceName) {
  ...
  const advertisedServerBaseUrl = 'http://192.168.2.158:60236';
  const pairing = await rest('/rest/remote-access/pairing-sessions', {
    method: 'POST',
    body: JSON.stringify({
      serverBaseUrl: advertisedServerBaseUrl,
      serverName: 'API REV 036 Disposable Browser Node',
  ...
  return {
    ...
    session: {
      version: 1,
      nodeId: 'api36-mobile-paired-node',
...
try {
  ...
  const mobileActive = await verifyMobileReference(context, 'API REV 036 Active Mobile Browser', 'active');
  const conditions = {
