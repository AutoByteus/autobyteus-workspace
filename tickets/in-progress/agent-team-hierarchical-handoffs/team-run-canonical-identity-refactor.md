...
- Scope: SR-018 comprehensive TeamRun address, recipient/handoff-tool, released-data persistence and token migration ownership/atomicity, direct forward-only application SDK V5 cut, exact Team event/initial-status/pre-run-status stream contract, immutable frontend topology, valid concrete execution projection, and live validation
- Status: `Refined — SR-018 aligned; complete architecture re-review pending DR-005/DR-006`
- Approval applicability: This supplement defines intended behavior. The user approved the rooted target model, recipient/handoff protocol, V5 application boundary, and SR-011 live-validation contract. SR-013 corrected TeamRun predecessor interpretation/sequencing and passed complete ARCH-REV-008. SR-015 corrected token migration ownership/atomicity. SR-016 responds to CRR-050 Design Impact by closing the Team domain-to-wire boundary and giving one frontend aggregate exclusive ownership of concrete execution lifecycle while keeping immutable Team topology separate. SR-017 records the user's explicit application rollout constraint: the unused application framework is a direct forward-only target cut, so no application migration, predecessor decoder, compatibility adapter, quarantine/upgrade workflow, dual reader, or fallback is permitted. ARCH-REV-010 passed that structure and SR-018 closes only the two reachable Agent-status producers plus UC-019's stale application-conversion wording.
- Related requirements: R-011 through R-013, R-021, R-026, R-028 through R-043, R-047, and R-049 through R-052
- Related acceptance criteria: AC-012, AC-013, AC-019, AC-023 through AC-039, AC-043, and AC-045 through AC-048
- Supersession rule: SR-010 retains SR-008's rooted TeamRun model and SR-009's `recipient_address` naming. SR-008 superseded SR-007's persisted `MountedTeamTopology` + `TeamAgentLaunchProfile` + `TeamRunBindingSet` normalization; SR-009 made public recipient-oriented tools use `recipient_address`, with request-only `RecipientAddressExpression` resolving to `AgentTeamAddress`. SR-010 changes only the LLM-facing handoff seam: Team runtime intrinsically exposes the collaboration tools and filesystem-like completion instruction, and `get_handoff_rules` projects compiled edges to ordered `{when,recipient_address}` rows without a generic success envelope. SR-011 adds required live verification. SR-012 introduced the exact backend-definition/frontend-SDK V5 contract, while SR-017 supersedes SR-012's speculative application compatibility/database-migration rollout with one direct target rewrite of project-owned source, generated/vendor/importable artifacts, fixtures, and fresh databases. SR-013 corrects the supported TeamRun predecessor conversion: historical `memberName` is display-only, route/path fields are structural, one migration-only decoder owns flat interpretation, and the separately pending canonical migration owns final v3 conversion whether the stable prerequisite runs now, already produced a predecessor tree, or is terminal-with-warning while a repaired flat item remains. SR-015 additionally assigns token semantic conversion to that pending canonical aggregate, retires the terminal historical token ID from current registry authority, and gives one migration store the transactional batch. SR-016 delegates the exact Team event/wire/frontend execution boundary to [team-stream-execution-projection-contract.md](./team-stream-execution-projection-contract.md); SR-018 completes that boundary for non-event initial status and correlated pre-run status without changing its one binding/wire/frontend authority. SR-017 preserves all released Team/task/token migration work but places no application item in that migration, and SR-018 corrects UC-019 to match. The comprehensive target runtime model, physical-storage preservation for released data, authored handoff schema, direct-current-Team task eligibility, send delivery envelope, and exact-run behavior remain unchanged.
...
16. connection status calls the shared exact status projector directly without a fake TeamRun event, while pre-run status uses one correlated status-event constructor and is replaced by the first matching real status.
...
27. Run implementation checks; complete source review must include application-producer construction seams, real producer-to-consumer Team contract seams, strict invalid-input no-mutation, immutable-topology/concrete-execution lifecycle convergence, all migration/application/storage/provider proofs, and removal/allowlist scans.
28. Resume API/E2E only after full source Pass; run focused/affected/deterministic coverage and the imported nested-classroom live matrix under its isolated contract, then reconcile durable docs.

No accepted checkpoint may contain dual current runtime behavior. Historical types remain only inside migration input packages/fixtures under the exact allowlist. API/E2E remains paused until SR-018 complete architecture review, implementation, and full cumulative source review pass.
...
- **Dedicated wire-contract package vs server-local DTOs:** a small browser-safe package adds one workspace boundary but eliminates handwritten parallel DTOs, generic payloads, casts, and partial validation. It owns only exact Team wire shape and strict runtime schemas, not domain or frontend policy.
- **Identity-free launch draft vs provisional AgentContext reuse:** one explicit success-time focus/input transfer replaces synthetic TeamRun/AgentRun/conversation IDs and cross-index rebasing; this is a larger frontend cut but a much smaller steady-state model.
- **One frontend aggregate vs fragmented composables:** centralized lifecycle ownership is intentionally stronger; bounded pure transitions and view projectors remain testable off-spine concerns, while no second mutable owner or public map is permitted.

Main risks are missed identity/event/status producers, a non-exhaustive domain-to-wire branch, an initial snapshot left on generic Team egress or disguised as a fake event, an overlay rebuilding/guessing identity before AgentRun materialization, wrong/missing genuine task-Team AgentRun ID, status replacement keyed only by logical address, browser mutation before full validation, misclassifying historical display names as structural identity, relying on changed code under either terminal predecessor migration ID, duplicating flat/token semantics, per-row commits leaving partial token conversion, dishonest pre-commit migrated counts, long transaction time over material token volume, requiring an unavailable post-listen recovery API, genuine route/path contradiction, task-chain conversion, frontend persistent/task collisions, accidental topology mutation or empty/provisional-ID materialization, draft ID leakage or pending-input loss, raw serialized-key coupling, stale project-generated application artifacts, a non-current application declaration admitted under changed types, or speculative application migration/compatibility code surviving the direct cut. Mitigations are correlated discriminated unions, one Team Agent binding/status constructor set, direct snapshot plus correlated overlay spines, full execution-address overlay keys and replacement proof, one exact strict Zod wire package, exhaustive mapping, mutation-free parse failure, identity-free draft success/failure seams, one private lifecycle aggregate, valid concrete execution variants, selected-root-plus-exact-execution focus checks, one pending `20260801...` released-data owner, one TeamRun flat decoder, one strict token planner/index, one transaction-owning batch store, forced-later-failure rollback proof, structural-only address derivation, exact source/removal allowlists, exact V5 target artifact/fresh-database consistency, three-level runtime coverage, one execution-address serializer, and byte/row/path preservation tests for supported released data.
...
- The user-approved target basis remains authoritative. SR-013 passed ARCH-REV-008; SR-015 corrected CRR-025 token ownership/atomicity; ARCH-REV-010 passed SR-017's rooted/migration/application/frontend structure and returned only DR-005/DR-006. SR-018 closes those gaps and must pass complete architecture re-review before implementation resumes.
