...
- Scope: SR-024 comprehensive TeamRun address, recipient/handoff-tool, released-data persistence and token migration ownership/atomicity, direct forward-only application SDK V5 cut, exact Team event/status/segment/error stream contract, immutable frontend topology, valid concrete execution projection, and live validation
- Status: `Refined — SR-024 aligned; architecture re-review pending DR-012`
- Approval applicability: This supplement defines intended behavior. The user approved the rooted target model, recipient/handoff protocol, V5 application boundary, and SR-011 live-validation contract. ARCH-REV-017 resolves DR-001–DR-011 and reconfirms the cumulative rooted structure. SR-024 preserves the sole downstream diagnostic and SR-023 first-thread-boundary correction while contracting Codex admission to the exact four current segment-producing families. Exact ownership is delegated to [agent-segment-lifecycle-contract.md](./agent-segment-lifecycle-contract.md).
...
- The user-approved target basis remains authoritative. ARCH-REV-017 resolves DR-001–DR-011 and reconfirms the complete rooted/migration/application/Team-status/frontend/common-lifecycle structure. SR-024 corrects only DR-012 by applying the accepted first-boundary Codex policy to the exact four current segment-producing families and deleting the unsupported unknown-event/exemption machinery. Complete architecture re-review must pass before implementation resumes.
