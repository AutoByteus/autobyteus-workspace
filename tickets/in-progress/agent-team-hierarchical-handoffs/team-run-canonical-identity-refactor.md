...
- Scope: SR-020 comprehensive TeamRun address, recipient/handoff-tool, released-data persistence and token migration ownership/atomicity, direct forward-only application SDK V5 cut, exact Team event/status/segment/error stream contract, immutable frontend topology, valid concrete execution projection, and live validation
- Status: `Refined — SR-020 aligned; architecture re-review pending DR-007/DR-008`
- Approval applicability: This supplement defines intended behavior. The user approved the rooted target model, recipient/handoff protocol, V5 application boundary, and SR-011 live-validation contract. ARCH-REV-011 passed the cumulative SR-018 structure. ARCH-REV-012 accepts SR-019's run-owned segment lifecycle; SR-020 completes only its processor/listener and diagnostic projection cuts. Exact ownership is delegated to [agent-segment-lifecycle-contract.md](./agent-segment-lifecycle-contract.md).
- Related requirements: R-011 through R-013, R-021, R-026, R-028 through R-043, R-047, and R-049 through R-056
- Related acceptance criteria: AC-012, AC-013, AC-019, AC-023 through AC-039, AC-043, and AC-045 through AC-051
- Supersession rule: SR-010 retains SR-008's rooted TeamRun model and SR-009's `recipient_address` naming. SR-017 supersedes speculative application compatibility/database migration with a direct target rewrite. SR-016 delegates Team event/wire/frontend execution to [team-stream-execution-projection-contract.md](./team-stream-execution-projection-contract.md); SR-018 completes all Agent-status producers; SR-019 delegates lifecycle to [agent-segment-lifecycle-contract.md](./agent-segment-lifecycle-contract.md); SR-020 makes that canonical barrier complete and adds exact runtime diagnostic evidence. The target runtime model, migration, application cut, physical storage, handoff schema, task eligibility, send envelope, and exact-run behavior remain unchanged.
...
18. one exact error-evidence authority distinguishes turn diagnostic from runtime diagnostic with null turn, and strict Team/standalone projections preserve both without terminal effects. File/history/output/application/browser consumers receive only canonical facts and own no lifecycle repair.
...
45. `AgentRunErrorEvidence` alone classifies turn/runtime diagnostic/terminal semantics. A missing-turn malformed candidate becomes `RUNTIME_DIAGNOSTIC` with null turn; no consumer borrows an active turn or terminalizes for either diagnostic.
...
- turn-attributed and missing-turn runtime diagnostics both cross exact Team/standalone wire/browser evidence while preserving status/message/segment/tool/application/external/command/output state; terminal/unclassified behavior remains;
...
Main risks are missed identity/event/status/segment consumers, a non-exhaustive domain-to-wire branch, segment lifecycle cached on the pipeline or placed after a processor, file end/replay still depending on type/reset, memory/output fallbacks surviving, runtime diagnostic being mistaken for terminal, snapshot pre-cleanup rejecting final content, provider/browser defaults surviving, or any previously recorded identity/migration/frontend/application risk. Mitigations are DS-017A–G complete consumer/diagnostic proofs, one error-evidence authority, exact required wire fields, correlated unions, one Team binding/status set, exact source/removal scans, and all previously accepted migration/application/storage/frontend proofs.
...
- The user-approved target basis remains authoritative. ARCH-REV-011 passed the complete SR-018 rooted/migration/application/Team-status/frontend structure; ARCH-REV-012 accepts SR-019's common lifecycle owner. SR-020 changes only DR-007/DR-008's consumer/diagnostic seam and must pass architecture re-review before implementation resumes.
