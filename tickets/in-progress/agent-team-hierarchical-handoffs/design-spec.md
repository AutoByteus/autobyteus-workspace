...
Nested TeamRuns currently use child-local member paths and a parent-boundary callback. The target design preserves lazy child lifecycle, local execution selectors, task/token/memory ownership, and event prefixing, but gives every member a small shared addressing binding containing its root run identity, absolute member path, and immediate Team path. Collaboration composes outgoing handoffs and delivery wiring around that binding; task delegation composes caller/task lifecycle identity around the same binding. Both operations canonicalize recipient input once and ask the root topology owner for the same typed placement. Message delivery may traverse anywhere in the root; task delegation accepts the placement only when its owner is the caller's immediate Team, then uses existing local activation.
...
6. Replace the flat communication roster and generic `MemberTeamContext.members` with one `MemberLogicalAddressContext` shared by collaboration and task delegation; keep outgoing handoffs/delivery and task caller/lifecycle data in their own composed contracts.
7. Interpret both `send_message_to.recipient_name` and `delegate_task.recipient_name` exclusively as `/...` or `./...`, canonicalize against the caller's immediate Team, and resolve once against the root `memberTree` to one minimal immutable `ResolvedTeamLogicalPlacement` containing only root-canonical subject coordinates, structural owner-local coordinates, and exact Team ingress.
...
10. Change `delegate_task` cleanly from `{target:{kind,name}}` to `recipient_name`; infer Agent versus Team from the resolved placement, enforce existing direct-target/current-TeamRun eligibility, and map the eligible root placement to exact local task identity/real ingress before existing activation.
11. Replace roster instructions with the caller's absolute Agent address, immediate Team address, shared path grammar, operation-specific Team behavior, and read-before-handoff guidance.
...
| --- | --- | --- | --- | --- |
| [agent-team-addressing-handoff-contract.md](./agent-team-addressing-handoff-contract.md) | Normative shared message/task grammar, schema, mounting, tool result, error, and scenario contract | R-001–R-027 / AC-001–AC-022 | Defines the public behavior and coordinate-only shared-placement boundary that the spines/examples below implement. | User-approved intended behavior through SR-004; boundary-aligned in SR-005; `Design-ready` |
...
- Evidence: `TeamDefinitionTopologyPlanner`/`TeamRunConfig` already own truthful recursive placement identity, but flat communication and flat task `{kind,name}` resolution create two competing public policies; representative fields feed task-Team ingress; provider adapters drop typed result codes; and current `stripMemberPathPrefix` adds a nested identity inconsistency by localizing descendant routes without nested Team coordinator routes.
- Design response: establish the shared address domain and provider envelope; add one strict TeamRun subteam-topology localizer; bind one member logical-address context into collaboration and task contracts; make root topology placement resolution the only logical-route authority; make task delegation enforce current direct-owner eligibility and exact local mapping after resolution; then delete both flat projections and representative data. No consumer guesses target kind or whether a route is root- or child-local.
...
- Intentional deferrals and residual risk: native AgentOrg, dynamic reconciliation, frontend authoring UI, and external Agent package configuration remain out of scope. External packages that still emit bare names will fail until their separately owned definitions are updated; no runtime fallback mitigates that intentional clean cut.
...
- **Member collaboration context**: the focused per-Agent projection containing root run identity, effective member/Team paths, outgoing edges, and Team delivery wiring. It is composed by, but not synonymous with, `MemberTeamContext`.
- **Member logical-address context**: the tight per-Agent root/caller/immediate-Team identity shared by any recipient-oriented operation; it contains no handoff rules, target roster, delivery mechanism, or task lifecycle state.
- **Resolved Team logical placement**: the single typed result of canonical root traversal: Agent or Team placement, absolute/root selector coordinates, structural owning-Team/owner-local coordinates, and exact Team coordinator ingress when applicable; it contains no configs, handles, runtime settings, or run/member lifecycle IDs.
...
| DS-003 | Primary End-to-End | BEH-002, BEH-003, BEH-009 | `send_message_to.recipient_name` | Exact recipient AgentRun input and code-preserving provider envelope | Root `MixedTeamManager` / delivery coordinator | Consumes the shared typed placement, then preserves lazy nested message transport and the public result span. |
...
| DS-009 | Bounded Local Shared | BEH-002, BEH-003, BEH-011 | `/...` or `./...` plus caller address context | Typed `ResolvedTeamLogicalPlacement` with canonical address, exact Team ingress, and owning-Team route | `TeamLogicalPlacementResolver` | Makes message and task recipient identity literally identical while leaving operation policy outside the resolver. |
| DS-010 | Primary End-to-End | BEH-011 | `delegate_task.recipient_name` | Existing current-TeamRun task-Agent/task-Team lifecycle for an eligible resolved placement | `TaskDelegationService` with `TaskDelegationTargetMapper` | Applies direct-target eligibility and exact local identity mapping after DS-009, then reuses existing activation/lifecycle without flat lookup. |
...
- **DS-009 — shared logical placement:** `message delivery or task tool -> root MixedTeamManager.resolveLogicalPlacement (direct internal call or root TeamRun/backend facade) -> strict AgentCollaborationAddress parser -> caller-relative normalization from MemberLogicalAddressContext -> root TeamRunConfig.memberTree traversal -> typed Agent/Team placement + exact Team ingress + owning-Team route`
- **DS-010 — hierarchical task delegation:** `Runtime task tool/MCP -> shared MemberLogicalAddressContext -> DS-009 -> TaskDelegationTargetMapper direct-owner eligibility + current-local identity/ingress -> current TeamRun task-Agent/task-Team activation -> existing ledger/submission/review/settlement lifecycle`
...
| DS-009 | The resolver parses once, expands `./` from the caller's immediate Team, walks the root `memberTree`, and returns an Agent or Team placement without applying an operation. It converts its private config traversal cursor into deeply immutable root subject coordinates, a structural owner/local coordinate, and exact Team ingress; no config object or lifecycle run/member ID crosses the result boundary. | Caller address context, absolute address, root topology, typed placement | `TeamLogicalPlacementResolver` | strict syntax errors, root selector derivation, owner/local structural pairing |
| DS-010 | The task tool accepts only `recipient_name`. Before task ID reservation or ledger mutation, the mapper consumes DS-009, requires a non-null owner whose Team path equals the caller's immediate Team path, rejects the caller Agent/root Team, and pairs the root placement with the exact direct config in the current canonical local `memberTree`. It constructs typed task identity/ingress without parsing or name lookup, then existing activation/lifecycle runs unchanged. | Resolved placement, current local run config, task target identity, task execution | `TaskDelegationTargetMapper` with `TaskDelegationService` | tool schema/parser, task reference files, task IDs, lifecycle notifications |
...
| `MemberLogicalAddressContext` | Caller-specific root/member/immediate-Team identity shared by message and task tools; not a resolver or operation context. |
...
- `MemberLogicalAddressContext` owns only the current member's root TeamRun ID, absolute member path/address, and immediate Team path/address. `MemberCollaborationContext` composes it with filtered outgoing edges and logical delivery wiring. Task tool context composes the same address identity with the current TeamRun ID and task caller/instance state. `MemberTeamContext` owns neither address resolution nor target lists.
...
- Root `MixedTeamManager` owns the single active-runtime call into `TeamLogicalPlacementResolver` and the private post-policy conversion from an effective Agent coordinate into config/participant/registry-handle delivery data; `TeamMemberDeliveryCoordinator` owns delivery sequencing. Non-root managers only forward root-bound message intents and do not resolve partial namespaces.
- `TeamLogicalPlacementResolver` consumes the shared caller address context and root run config privately. It returns a typed placement containing only immutable logical/selector coordinates and owning-Team/local pairing; for Team placements it validates and includes the exact real coordinator ingress coordinate. It does not return config references or lifecycle IDs, decide message/task self policy, create/start runs, publish events, or inspect handoffs.
...
- `TaskDelegationTargetMapper` consumes `ResolvedTeamLogicalPlacement`, caller address/task identity, and the current canonical local `TeamRunConfig.memberTree`. It owns direct-owner/self eligibility and conversion to `TaskDelegationMemberIdentity` or `TaskDelegationTeamIdentity`; it pairs the direct config only when both `placement.owner.localMemberPath` and `localMemberRouteKey` match exactly and does not parse addresses, traverse root topology, or inspect communication/handoff state. The structural logical owner path is the eligibility join, so persistent and task-scoped instances of the same logical Team use the same address without any shared run-ID claim.
...
- **DS-009 parent owner:** `TeamLogicalPlacementResolver`.
  - Chain: `Runtime recipient input -> strict parse -> caller-relative root normalization -> private root segment walk -> intermediate-Team enforcement -> coordinate-only typed subject -> exact direct coordinator coordinate if Team -> structural owner/local coordinate -> deep immutable construction`.
...
- **DS-010 bounded task owner:** `TaskDelegationTargetMapper` under the current TeamRun task service.
  - Chain: `Resolved placement -> direct-owner/self eligibility -> exact current-local config pairing -> task identity/ingress -> existing activation`.
...
Task delegation preserves its current TeamRun ownership. `placement.owner` is used first as an eligibility fact: it must be non-null and its `teamPath` must equal `MemberLogicalAddressContext.immediateTeamPath`. Only then may the mapper pair `owner.localMemberRouteKey` with the current local config. No task lifecycle component follows the root placement into another manager in this ticket.
...
| `TeamLogicalPlacementResolver.resolve` | private config traversal, strict parse/normalization, coordinate derivation, Team ingress, owner/local pairing | root `MixedTeamManager.resolveLogicalPlacement` only | returning config references, handles, settings, or run/member lifecycle IDs | convert the private traversal cursor through `resolved-team-logical-placement.ts` constructors |
...
| `TaskDelegationTargetMapper.fromPlacement` | direct-owner eligibility and current-local Agent/Team execution identity mapping | current-TeamRun `TaskDelegationService` | task resolver traversing root topology or matching `memberName` | enrich the resolved placement/task mapper contract |
...
6. `MemberTeamContext` composes one `MemberLogicalAddressContext` with collaboration and task lifecycle fields. Generic `members`, direct target lists, and representative-bearing descriptors are removed. Both operations may consume the address context; neither may consume the other's operation state.
7. Root `MixedTeamManager.resolveLogicalPlacement` is the only active-runtime invocation boundary for placement and delegates to `TeamLogicalPlacementResolver`, the sole parser/traversal owner. Root message delivery calls that manager method internally; task tooling reaches it only through root `TeamRun.resolveLogicalPlacement` and its backend delegate. The resolver may traverse root `TeamRunConfig.memberTree` privately but must convert the cursor into a deeply immutable coordinate-only placement before return; neither the result value nor its type file imports/carries configs, handles, settings, or run/member lifecycle IDs. It may not create handles, start runs, publish events, read definitions, consult handoffs as ACLs, or apply operation-specific self policy. Task adapters must not read `TeamRun.config` directly.
...
9. `TaskDelegationTargetMapper` may consume `ResolvedTeamLogicalPlacement`, task caller identity, and the caller's current local `TeamRunConfig` for one exact direct path-and-route match. It must not import communication roster/event/provider types, parse addresses, traverse the root config or descendants, match leaf names, or accept caller-supplied kind.
...
11. `TaskDelegationTargetMapper` must reject `owner:null` or an `owner.teamPath` that differs from `callerAddressing.immediateTeamPath`. For an eligible placement it may inspect only the current TeamRun's direct local configs using exact equality on both `owner.localMemberPath` and `owner.localMemberRouteKey`; it must not traverse into another manager, strip/retry prefixes, or repair topology.
...
| `MemberLogicalAddressContext` | current Agent recipient-address origin | Root TeamRun ID, absolute member path/address, immediate Team path/address | canonical root/path arrays | Shared by message and task tools; contains no target list. |
...
| `TeamLogicalPlacementResolver.resolve(config,input,caller)` | root placement target | Strict parse/normalize/traverse privately and derive subject/ingress/owner coordinates | runtime address + `MemberLogicalAddressContext` | Returns the minimal `ResolvedTeamLogicalPlacement`; applies no operation and returns no config reference. |
| `ResolvedTeamLogicalPlacement` | cross-operation logical placement | Carry immutable subject, structural owner/local, and exact Team ingress coordinates | Agent `{kind,subject,owner}` or Team `{kind,subject,owner,ingress}` | Exhaustive field boundary; no config/handle/setting/definition/run/member lifecycle identity. |
| `TaskDelegationTargetMapper.fromPlacement(placement,caller,currentConfig)` | task execution target | Enforce direct-owner/self eligibility and construct exact current-local Agent/Team identity | resolved placement + caller address/task identity + current local config | No root parsing/traversal/name lookup. |
...
| `ResolvedTeamLogicalPlacement` | Yes | Yes | Low | Coordinate-only discriminated union; config-independent deep constructors/clones enforce the exhaustive field set. |
...
type MemberLogicalAddressContext = {
  ...
  memberAddress: string;          // canonical absolute Agent placement
  memberPath: string[];           // root-canonical segments
  immediateTeamAddress: string;   // "/" or canonical Team placement
  immediateTeamPath: string[];
};
```

It is immutable and contains no target roster, handoffs, delivery callback, task list, or provider data. `MemberCollaborationContext` composes it with outgoing handoffs and message delivery. `TaskDelegationToolContext` composes it with the current TeamRun ID and caller/task-instance identity only.
...
```ts
type ResolvedLogicalAgentCoordinate = Readonly<{
  absoluteAddress: string;                 // public canonical Agent identity
  memberRouteKey: string;                  // root-canonical selector key
}>;

type ResolvedLogicalTeamCoordinate = Readonly<{
  absoluteAddress: string;                 // "/" for root Team
}>;

type ResolvedPlacementOwnerCoordinate = Readonly<{
  teamPath: readonly string[];             // logical parent Team path
  localMemberPath: readonly [string];       // one direct path in that TeamRun config
  localMemberRouteKey: string;             // exact direct route in that config
}>;
...
type ResolvedAgentTeamPlacement = Readonly<{
  ...
  owner: ResolvedPlacementOwnerCoordinate | null; // null only for root Team
...
These fields are exhaustive for the shared result. `absoluteAddress` is the canonical placement identity used for operation-neutral equality/self comparison. `memberRouteKey` exists only on an Agent coordinate because the root manager concretely needs that exact selector to materialize a private delivery endpoint; Team delivery uses the ingress Agent's selector. `owner` is the structural join into the caller's current local task config, and `ingress` is the real coordinator's minimal root-canonical Agent coordinate. Root subject `memberPath`, Team route key, and all derivable/display/runtime fields are deliberately omitted. The value contains no `TeamRunConfig`, member config, handle, provider/runtime setting, role/description, Team definition ID, `memberRunId`, `teamRunId`, or owning/template run ID.

`resolved-team-logical-placement.ts` owns constructors/cloners that clone and freeze every object/array. It imports no TeamRun config type. The resolver may hold config nodes only in a private traversal cursor and must discard them when constructing the result. The resolver:

1. parses `recipientName` with the strict collaboration address parser;
2. expands `./` from `callerAddressing.immediateTeamPath` and leaves `/` root-based;
3. walks the root `memberTree` once, requiring every intermediate placement to be a Team;
4. returns the final Agent or Team subject without collapsing a Team into an Agent;
5. for a Team, resolves `coordinatorMemberRouteKey` to exactly one direct Agent in that Team and attaches only that Agent's canonical address plus exact root route key as `ingress`;
6. derives one `owner` from the paired root nodes, containing the logical parent Team path and a one-segment direct owner-local member path plus its exact route key; the root Team has `owner:null` so task policy cannot mistake it for a direct child; and
7. applies no message/task self rule and performs no lifecycle operation.

Message/task placement equality is deep structural equality over this exhaustive union: `kind`, `subject`, nullable `owner`, and Team `ingress`. Tests must compare those values and prove the forbidden fields are absent; config object identity, active/static run identity, and private delivery-endpoint identity are never part of equality.

Message policy consumes the result as follows: Agent -> `subject`; Team -> `ingress`; then reject if the effective Agent address equals the caller. Inside the root mixed manager only, a private delivery-endpoint lookup converts that Agent coordinate's exact root route key into the config, runtime participant fields, registry handle, and downward selector required for delivery. That transient endpoint never crosses `TeamRun.resolveLogicalPlacement` and is not a second address resolver or operation-visible placement shape.
...
`TaskDelegationTargetMapper.fromPlacement(placement, callerAddressing, currentConfig)` performs no string parsing or root-topology search:

1. Require `placement.owner` and exact equality between `placement.owner.teamPath` and the caller's immediate Team path; otherwise reject it as outside current direct-target eligibility.
2. Agent placement -> reject when `placement.subject.absoluteAddress` equals the caller's logical Agent address; otherwise pair `placement.owner.localMemberPath` and `localMemberRouteKey` with the exact direct current-local Agent config and construct `TaskDelegationMemberIdentity`.
3. Team placement -> pair it with the exact direct current-local Team config and construct `TaskDelegationTeamIdentity` with that config's exact localized coordinator ingress.
4. Preserve task-specific runtime IDs, role/description, and Team definition ID from the current-local config. Missing required IDs or ingress fail before activation.
5. Never accept a caller-supplied kind, retry a local/root route, search leaf names, or inspect handoffs.

`TaskDelegationToolRunRouter` keeps current service routing for review/submit. For delegation it exposes a distinct route containing (a) the service for the caller's active current TeamRun and (b) the root TeamRun resolved from `callerAddressing.rootTeamRunId`. The root facade verifies that bound root ID, then produces DS-009. `TaskDelegationService.delegateTask` receives the immutable placement and invokes `TaskDelegationTargetMapper.fromPlacement(placement, callerAddressing, currentConfig)` before reserving a task ID or mutating the ledger. The mapper owns the post-resolution bridge:

1. require non-null `placement.owner`, with `placement.owner.teamPath` equal to `callerAddressing.immediateTeamPath`; otherwise fail with `TASK_DELEGATION_TARGET_NOT_ELIGIBLE`;
2. for an Agent, reject the caller's same logical placement and find exactly one direct current-local Agent config whose path and route equal `placement.owner.localMemberPath` and `localMemberRouteKey`;
3. for a Team, find exactly one direct current-local Team config by that paired path/route, then resolve its already localized `coordinatorMemberRouteKey` to exactly one direct Agent child;
4. construct task identities from the current local runtime IDs/config plus `placement.subject.absoluteAddress`, keeping task execution identity separate from the common placement value; and
5. invoke the existing current-TeamRun activation coordinator, task registries, ledger, submission/review/notification, and settlement paths unchanged.

The mapper never strips prefixes, retries root/local shapes, searches leaf names, or repairs configs. DS-009 produced the root/owner-local pair structurally, and DS-011 guarantees the current local Team/config coordinator invariant. A resolved deeper/cross-branch placement is a valid address but an ineligible task target; Ticket 1 does not introduce cross-TeamRun task execution routing.
...
| Shared resolved value | `ResolvedTeamLogicalPlacement` | Yes | Medium | Keep only `subject`, nullable structural `owner`, and Team `ingress` coordinates; never grow configs or lifecycle identity into it. |
...
| --- | --- | --- | --- | --- | --- |
| `agent-collaboration/domain` | strict address grammar/formatting, handoff value, typed errors | DS-001–DS-005, DS-008–DS-009 | Definition, run, communication, tools | Create New | Future AgentOrg may reuse without an AgentOrg dependency. |
...
| `agent-team-execution/domain/member-logical-address-context.ts` | Execution | shared per-member address origin | root run/member/immediate-Team canonical identity | Tight cross-operation projection | collaboration address |
...
| `agent-team-execution/services/team-logical-placement-resolver.ts` | Execution | shared runtime placement resolver | walk root `memberTree`, typed Agent/Team result, ingress, owning-Team route | No operation/lifecycle/event duties | Yes |
| `agent-team-execution/task-delegation/task-delegation-target-mapper.ts` | Task delegation | task target mapping | direct-owner/self eligibility and exact current-local identity/ingress from resolved placement | One post-resolution mapping boundary | placement + local config + task identities |
...
| Caller root/member/immediate-Team identity across message/task tools/prompts | `member-logical-address-context.ts` | Execution | One recipient-origin meaning | Yes | Yes | Target roster or operation context |
| Typed target placement across message/task policies | `resolved-team-logical-placement.ts` beside resolver | Execution | Same immutable subject/ingress/owner coordinates for both operations | Yes | Yes | Config snapshot, lifecycle identity carrier, task/message kitchen sink, or persisted duplicate topology |
...
| `MemberLogicalAddressContext` | Yes | Yes | Low | Store root ID/path arrays and canonical displays only; no targets or operations. |
| `ResolvedTeamLogicalPlacement` | Yes | Yes | Low | One coordinate-only typed placement. `subject`, nullable `owner`, and Team `ingress` have distinct meanings; no config, handle, setting, definition, member-run, or TeamRun identity is carried. |
...
| `src/agent-collaboration/domain/collaboration-logical-address.ts` | Collaboration | address grammar | segment invariant, strict definition/runtime parse, absolute format/rebase | Every consumer needs identical rules | error contract |
...
| `src/agent-team-execution/domain/member-logical-address-context.ts` | Execution | shared caller origin | root TeamRun ID, absolute member path/address, immediate Team path/address, immutable clone | Same recipient origin for message and task | collaboration address |
...
| `src/agent-team-execution/task-delegation/task-delegation-target-mapper.ts` | Task delegation | post-resolution target owner | direct-owner/self eligibility; exact current-local Agent/Team config and ingress mapping from resolved placement | Explicit DS-010 mapping owner | placement + local config + task identities |
...
| `src/agent-team-execution/services/team-logical-placement-resolver.ts` | Execution | shared root placement resolver | strict parse/relative normalization, private root-config traversal, and coordinate-only subject/ingress/owner construction | Explicit DS-009 owner | address/error + TeamRun config internally; minimal placement output |
| `src/agent-team-execution/services/resolved-team-logical-placement.ts` | Execution | shared placement value | coordinate types plus deep clone/freeze constructors for `subject`, nullable `owner`, and Team `ingress` | Shared result imports no TeamRun config or operation type | collaboration address/path primitives only |
...
| `src/agent-execution/backends/autobyteus/autobyteus-managed-team-context-builder.ts` | AutoByteus runtime | native Team context adapter | clone `memberTeamContext.addressing` and task caller/instance state; remove target members/representative | Existing native context owner | address context |
| `src/agent-tools/task-delegation/task-delegation-autobyteus-context.ts` | Task tools | untrusted native parser | accept root/member/immediate-Team address context plus task caller identity; no target list/representative | Clean task context boundary | address + task caller identity |
...
| `tests/unit/agent-team-execution/team-logical-placement-resolver.test.ts` | Execution tests | DS-009 common identity seam | same coordinate-only message/task placement for absolute/relative Agent/Team paths; owner/ingress, deep immutability, forbidden-field absence, and typed failures | New focused shared resolver seam | address/root config input; coordinate-only output |
...
| `src/agent-team-execution/domain/member-logical-address-context.ts` | File | shared per-member addressing | root/member/immediate-Team canonical identity | One recipient origin for both operations | Targets, handoffs, delivery, task lifecycle |
...
| `src/agent-team-execution/task-delegation/task-delegation-target-mapper.ts` | File | Task target policy | Direct-owner/self eligibility and exact current-local identity/ingress mapping from shared placement | Existing task subsystem owns execution meaning | Root address parsing/provider/cross-Team routing |
...
| `src/agent-team-execution/services/team-logical-placement-resolver.ts` | File | shared root placement resolution | Strict parse/normalize/private config traversal, then coordinate-only Agent/Team result, ingress, and structural owner/local pairing | Domain-level runtime resolver | Message/task policy, startup/events, or config references in its return value |
| `src/agent-team-execution/services/resolved-team-logical-placement.ts` | File | shared placement value | Coordinate-only result variants plus deep clone/freeze constructors | Prevent operation-specific parallel shapes without leaking resolver internals | TeamRun/member configs, handles, settings, roles, definition IDs, or run/member lifecycle IDs |
| `src/agent-team-execution/domain/team-run.ts`; `backends/team-run-backend.ts` | Files | root placement query facade | expose the coordinate-only `resolveLogicalPlacement` through existing run/backend ownership | Prevent task tools bypassing run boundary | Config-bearing return values, handles, settings, active/template lifecycle IDs, or task/message operation policy |
...
| --- | --- | --- | --- |
| Runtime path canonicalization | Caller binding `{teamPath:["research_team"], memberPath:["research_lead"]}` + `./field_team` -> canonical target `/research_team/field_team` before forwarding. | Every child manager interprets one segment and rewrites sender/target incrementally. | One root target and one sender identity survive arbitrary nesting. |
...
| Minimal shared placement | `/design_team` returns `{kind:"team", subject:{absoluteAddress:"/design_team"}, owner:{teamPath:[],localMemberPath:["design_team"],localMemberRouteKey:"design_team"}, ingress:{absoluteAddress:"/design_team/team_lead",memberRouteKey:"design_team/team_lead"}}`. | Attach root subject paths/Team route keys, `teamConfig`, `memberConfig`, `memberRunId`, `owningTeamRunId`, provider settings, or a handle to the shared result. | Demonstrates the exact cross-operation field boundary and keeps task-scoped active-run identity truthful. |
| Task-owned execution | For root caller `/product_manager`, the resolved `/design_team` has owner path `/`, matching the caller's immediate Team. `TaskDelegationTargetMapper` pairs it with the root TeamRun's exact direct local config/ingress, then existing task-Team activation runs. | Keep `SubTeamRepresentativeDescriptor` or derive task target from a flat communication/task roster. | Removes false identity while preserving current direct-target task ownership. |
...
For caller `/research_team/research_lead`, DS-009 resolves `/research_team/field_team` to that Team placement and root ingress `/research_team/field_team/field_lead`, plus `owner:{teamPath:["research_team"],localMemberPath:["field_team"],localMemberRouteKey:"field_team"}`. DS-010 verifies that owner path equals the caller's immediate Team and pairs `field_team` with the current `research_team` run's already localized direct config, whose coordinator is exactly `field_team/field_lead`. If `field_team` is started as either a persistent/restored child or a task Team child, the same factory invokes the same localizer on the parent-local `field_team` entry and returns:
...
The restore runtime context and task-team identity are lifecycle inputs applied after this config transformation; neither changes route localization. At no point does the localizer, shared placement resolver, task mapper, or tool run router try both `research_team/field_team/field_lead` and `field_team/field_lead`; DS-009 pairs root and owner-local identity structurally before lifecycle routing.
...
8. Add `MemberLogicalAddressContext` and derive it for every persistent, restored, task-Agent, and task-Team AgentRun. Refactor `MemberCollaborationContext` to compose that identity with outgoing handoffs and message delivery; keep task/token state outside both values.
9. Add coordinate-only `ResolvedTeamLogicalPlacement` types/constructors, `TeamLogicalPlacementResolver`, root `MixedTeamManager.resolveLogicalPlacement`, and the root `TeamRun`/backend facade. Unit-test identical Agent/Team structural results, exact ingress, nullable root-Team owner, owner-local path/route, deep clone/freeze, and collaboration error codes for absolute/relative inputs from root and three-level nested callers. Assert the result and type imports contain no derivable root subject path/Team route, config, handle, settings, definition/member-run, or TeamRun lifecycle identity; prove root message delivery calls the manager method and task tooling uses the TeamRun facade rather than reading config.
10. Tighten `InterAgentMessageDeliveryIntent` to logical Team address only. Make message delivery consume DS-009, apply Team-to-ingress/effective-self policy, then convert the selected Agent coordinate to config/participant/handle mechanics privately inside the root manager and reuse root/child handles. Remove address traversal from the old sender-specific message resolver; no private delivery endpoint crosses the shared facade.
...
12. Add `TaskDelegationTargetMapper`. Require `placement.owner` with a Team path equal to the caller's immediate Team path; reject the caller Agent and root Team; pair both `owner.localMemberPath` and `localMemberRouteKey` with exactly one direct config in the current canonical local `memberTree`; derive an exact Team ingress; never localize, retry, or search names.
...
14. Switch AutoByteus managed Team context, Agent Tools MCP task context, and task wrappers to clone only shared caller addressing and task-instance identity. Remove generic target members, Team/subteam target conversion, `representative` fallback, `TASK_TARGET_KIND_*` handling, and direct `memberName` lookup.
...
16. Add task lifecycle coverage before deleting representatives: direct peer Agent and direct child Team by relative/equivalent absolute `recipient_name`, identical DS-009 placement equality with send, three-level current-local task-Team ingress, existing result submission/review/settlement and task-Agent caller chaining, non-direct/deeper/cross-branch eligibility rejection, and every legacy selector/self/error rejection.
...
No temporary compatibility route may remain after steps 16–17 or 20. DS-011 is connected before current-local Team ingress mapping; DS-009 supplies the only root placement and exact root/owner-local pair; the task mapper never compensates for a mixed route shape, old selector, or ineligible cross-Team target. Common resolution and task mapping are proven before representative/target-list deletion; the public communication result mapper is connected before message-only provider behavior is deleted. The reviewable result contains one owner for each parse, placement, operation policy, topology localization, local task mapping, and provider projection.
...
5. **Shared address-context completeness:** every AgentRun construction path, including persistent/restored child members, task Agents, and task-Team members, must receive the same root/member/immediate-Team identity. A missing or wrongly rooted context would make message and task behavior diverge despite a shared resolver.
6. **Task eligibility/local pairing:** DS-009 returns root placement identity while child task services operate on DS-011-localized configs. The mapper must verify immediate-Team ownership before exact local pairing; accepting a non-direct placement or using a root route directly would either expand scope or fail activation.
...
9. **Placement-boundary regression:** the resolver necessarily touches rich root configs, and message delivery necessarily needs runtime participant/handle data. If either convenience leaks into `ResolvedTeamLogicalPlacement`, task tooling would regain config/lifecycle coupling and task-scoped owner identity would become misleading. Type-import checks, forbidden-field tests, and root-manager-private endpoint coverage are required.
...
- In `MemberLogicalAddressContext`, store canonical root/member/immediate-Team identity once and derive display strings through the shared formatter. Both message and task contexts must clone this same value; do not independently reconstruct caller roots in provider adapters.
...
- Build the final recipient participant from the exact Agent run config only inside the root manager's private delivery-endpoint conversion, while use of the top-level subteam runtime context is only the transport handle choice. Never attach that config, participant, handle, or its run ID to `ResolvedTeamLogicalPlacement`; do not overwrite the leaf participant with wrapper-team identity during sender enrichment.
...
- Keep `TeamLogicalPlacementResolver` operation-neutral: return the coordinate-only Team subject plus real ingress rather than collapsing it; apply message effective-self and task direct-owner/self eligibility in their own consumers. Private traversal cursors/configs must be discarded before the shared value is constructed.
...
- Preserve current TeamRun task service/activation ownership. A resolved placement owned by another Team path must fail eligibility before task ID activation; do not route it to another manager in this ticket.
...
- Switch server task context, AutoByteus managed context, and MCP task context to shared caller addressing before deleting generic members. Delete old target-object validation and direct member lookup; retain defensive validation only for task execution identities that can arrive from untrusted native input.
...
- Focused implementation tests must cover: `localizeSubTeamRunTopology` on the three-level source tree, path-outside-prefix and invalid-direct-coordinator rejection, persistent child create, restored nested child, and task-child use of the same factory; identical coordinate-only DS-009 message/task Agent and Team placements for absolute/relative paths; deep clone/freeze and explicit absence of config/handle/settings/definition/member-run/TeamRun fields; exact root ingress plus current-owner-local `field_team/field_lead` then `field_lead` with no fallback; root-manager-private delivery endpoint resolution from the shared Agent coordinate; task mapper direct Agent/Team identities sourced from current local config, self rejection, and non-direct/deeper/cross-branch eligibility rejection; existing submit/review/settlement after all representative/target-list types are absent; rejection of every old `{target:{kind,name}}` form; result mapper supplied-code preservation and both absent-code defaults; AutoByteus send/handoff JSON success and rejection; MCP send/handoff text-vs-`structuredContent` equality and `isError`; and `target_agent_run_id` success/grant/inactive codes unchanged through both provider forms.
