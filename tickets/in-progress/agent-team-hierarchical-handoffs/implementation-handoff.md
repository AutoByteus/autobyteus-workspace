...
- Downstream lineage: `code-review-report.md`, `code-review-revision-record.md`, `api-e2e-coverage-investigation.md`, `api-e2e-execution-coverage-report.md`, `api-e2e-revision-record.md`, `api-e2e-test-review-report.md`, `delivery-revision-record.md`, and `delivery-integration-blocker.md` in the same ticket directory. `CRR-036` passed IR-020 source; API-REV-017 then exposed `CR-F-020` / `API-F-011` and `CR-F-021` / `API-F-012`, and `CRR-037` confirmed both as bounded frontend implementation defects. SR-015 and `ARCH-REV-009` remain current design authority; API/E2E remains paused at its API-REV-017 checkpoint.

## Current Implementation State

- Implementation revision: `IR-021`
- Implementation cycle: `Local Fix` for `CR-F-020` and `CR-F-021` from `CRR-037`
...
- Triggering findings: `CR-F-020` / `API-F-011` and `CR-F-021` / `API-F-012`. No requirement gap, design impact, or unclear finding is open; `CR-F-012` through `CR-F-019` remain resolved.
...
- Current source: `f8092a0dda94e9b4d0e4358bd7ae14a0593f92fa` (`fix(web): materialize delegated task executions`). The GraphQL hydration boundary strips only Apollo's exact expected `__typename` metadata before the unchanged strict domain parser. Exact persistent task records and streaming events now create distinct task Agent/task AgentTeam projections with complete execution addresses, task-scoped children, detail/timeline state, exact focus, restore, and terminal cleanup.
- Projection coverage: a focused current-contract probe passed the real three-record Apollo-decorated response, direct task Agent projection without stable-node mutation, task Team root plus independently focusable task-scoped children, and wrong-owner/wrong-parent rejection before projection. The isolated Nuxt production build and web/localization guards passed.
- Current review state: integrated IR-021 source awaits focused code re-review. API/E2E remains paused at API-REV-017 and owns fresh durable coverage plus browser/provider execution after source Pass.
- Durable coverage: implementation edited no API/E2E-owned durable test or fixture. Existing staged API-REV-016/017 coverage and evidence were preserved without modification.
- Operational safety: IR-021 ran no server startup, app-data migration, live-provider, or browser scenario. The user-requested manual stack remains untouched on `127.0.0.1:60004` and `127.0.0.1:31004` against `manual-user-20260810-2`; no action targeted `/Users/normy/.autobyteus/server-data/db/production.db`.
...
IR-021 changes rendered interaction by adding exact transient task Agent/task Team rows through the existing workspace tree and task-detail surfaces; it intentionally adds no new visual component language. The isolated Nuxt production build proves the rendered bundle compiles, and the current-contract probe proves the exact row/tree/focus data paths. Direct browser interaction was not performed because the user-requested manual server/frontend on ports 60004/31004 must remain available and untouched; implementation did not repoint, restart, terminate, or mutate that stack. Therefore visual state quality for count/details, active/awaiting-review/accepted transitions, refresh/restore, and all three runtimes remains an explicit downstream uncertainty rather than a claimed pass.

IR-006 through IR-018 otherwise do not change a rendered frontend surface. IR-020 changes backend contextual command selection only. IR-019 resolves web Team streaming transport against latest-base server-owned cadence/coalescing without an intentional layout change. IR-015 changes model-facing system/developer instruction text only; IR-016 removes a dead internal Claude input; IR-017 and IR-018 correct backend routing only. The Activity label noted in CRR-028 and preserved by CRR-030 remains unchanged. The cumulative IR-005 frontend result remains:
...
- `CR-F-020` and `CR-F-021` are source-corrected locally but not yet reviewer-accepted or independently exercised through the real browser/provider environment. API/E2E remains paused at API-REV-017 until source Pass.
- After code review, API/E2E owns fresh durable coverage and browser proof for task count/details, distinct task Agent and nested task-Team rows, exact selection, active/awaiting-review/accepted lifecycle, refresh/restore, cleanup, and all three runtimes. Existing API-REV-014/015 evidence predates latest-base integration and cannot substitute.
- The IR-019 implementation selection exposed one stale latest-base memory-integration block that instantiates removed flat TeamRun config. IR-021 did not edit it; coverage validity and correction remain API/E2E-owned. Any durable edit/removal returns through proportional code review.
- The mandatory operational disclosure remains: API-REV-014 mutated `/Users/normy/.autobyteus/server-data/db/production.db` before containment and no automatic rollback occurred. IR-021 did not target it. Renewed execution must prove an exact disposable root/database and fail closed on mismatch.
...
- Reviewed change posture: bounded Local Fix for `CR-F-020` and `CR-F-021` after API-REV-017 failure-origin review.
- Root cause: Apollo transport metadata crossed directly into the strict domain parser, while task event helpers created contexts but returned the persistent node and had no production owner for distinct visible task execution nodes.
- Refactor decision: `Focused Boundary And Projection Owners`; one exact GraphQL DTO projector owns transport stripping, and one exact recursive task-execution tree owns transient task nodes/contexts, focus, restore, and cleanup. The stable rooted tree/index and strict domain parser remain authoritative and unmodified.
- Implementation matched the reviewed assessment: `Yes`.
- New Design Impact or Requirement Gap found during implementation: `No`.

## Routing

Route this cumulative SR-015 / IR-021 integrated package to `code_reviewer` for focused `CR-F-020` / `CR-F-021` source re-review while preserving resolved `CR-F-012` through `CR-F-019`, IR-019 integration choices, current API-REV-017 artifacts, the untouched manual stack, and the operational safety disclosure. On Pass, route to `api_e2e_engineer` to resume fresh coverage investigation/execution; any durable coverage edit/removal must return through proportional code review before delivery re-enters.
