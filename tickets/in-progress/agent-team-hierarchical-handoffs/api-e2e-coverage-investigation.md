...
- Current API/E2E Revision ID: `API-REV-015` (`Pass / 96%`; prior `API-REV-014` completed `Pass / 96%`). API-REV-014's product/runtime result remains authoritative and is not rewritten.
- Current Investigation Round: `15` (CRR-032 bounded durable-coverage correction and reissue).
- Trigger: `CRR-032` finds no product or implementation regression and preserves CRR-031/API-REV-014 runtime evidence, but returns the cumulative durable-coverage package for local correction under `TR-F-002` and `TR-F-003`. Seven capability-gated runtime E2Es carry removed create-run/schema-v2 inputs, two excluded duplicate XML-patch tests import deleted source, and two live-E2E support edits lack reconciled ownership and focused isolation evidence. Current source basis remains IR-018 commit `035fba611e6895187f7f6d4644993e22efd8c38c`; current artifact HEAD remains `6b829e3d9aed63a24cd2f1e50c37a3457f9c0718`.
- Prior Investigation Reviewed: round 4 / `API-REV-004`, which proves `SR-006` only. It is historical context, not SR-012 evidence.
- Latest Authoritative Investigation: this file. API-REV-015 completes the CRR-032 coverage-package correction as `Pass / 96%`; it does not reopen the successful product/runtime result or API-F-010 resolution.
