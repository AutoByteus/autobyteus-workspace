...
const runtimeRoot = path.join(testRuntimeRoot, 'api-rev-028-live-20260812-1');
const databasePath = path.join(serverRoot, 'db', 'api-rev-028-live-20260812-1.db');
...
if (path.resolve(databasePath) === path.resolve(operationalPath)) throw new Error('OPERATIONAL_TARGET_MATCH');
if (fs.existsSync(runtimeRoot) || fs.existsSync(databasePath) || fs.existsSync(`${databasePath}.secret.key`)) {
  throw new Error('OWNED_TARGET_NOT_FRESH');
}
const sanitized = createSanitizedTestEnvironment();
if ('DATABASE_URL' in sanitized || 'DATABASE_URL_TEST' in sanitized) throw new Error('CHILD_DATABASE_SELECTOR_PRESENT');
const materialized = materializeTestRuntime({
  runtimeRoot,
  databaseUrlOverride: databaseUrl,
  serverUrlOverride: 'http://127.0.0.1:60228',
});
const fixed = Object.fromEntries(fs.readFileSync(materialized.runtimeEnvironmentPath, 'utf8')
