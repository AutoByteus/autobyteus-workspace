...
async function expandTeamPanels(page) {
  ...
  if (await taskHeader.count() && await taskHeader.first().getAttribute('aria-expanded') !== 'true') {
    await taskHeader.first().click();
  }
  const messageHeader = page.locator('[data-test="team-messages-header"]:visible');
  const messageList = page.locator('[data-test="team-communication-message-list"]:visible');
  if (await messageHeader.count() && await messageHeader.first().getAttribute('aria-expanded') !== 'true') {
    await messageHeader.first().click();
  }
...
try {
  ...
  for (let iteration = 0; iteration < 900; iteration += 1) {
    ...
    if (isAccepted && submissionExact && persistentReply && referenceExact &&
      snapshot.body.includes(expectedComplete)) {
  ...
  const configBack = page.locator('[data-test="run-config-back-to-events"]');
  await configBack.waitFor({ state: 'visible', timeout: 120000 });
  const selectedConfigTitle = await page.locator('body').innerText();
  const selectedRuntime = await page.locator('#team-run-runtime-kind').inputValue();
  const selectedRuntimeDisabled = await page.locator('#team-run-runtime-kind').isDisabled();
  const selectedAutoApproveDisabled = await page.locator('#team-auto-execute').isDisabled();
  const selectedRunButtonCount = await page.getByRole('button', { name: 'Run Team', exact: true }).count();
  const selectedReadOnlyVisible = selectedConfigTitle.includes('Selected team run configuration is read-only')
    || selectedConfigTitle.includes('read-only');
  const selectedTeamConfigEvidence = {
    titleVisible: selectedConfigTitle.includes('Team Configuration'),
    runtime: selectedRuntime,
    runtimeMatches: selectedRuntime === runtime,
    runtimeDisabled: selectedRuntimeDisabled,
    autoApproveDisabled: selectedAutoApproveDisabled,
    runButtonAbsent: selectedRunButtonCount === 0,
    readOnlyNoticeVisible: selectedReadOnlyVisible,
  };
  await page.screenshot({ path: `${outDir}/${slug}-selected-team-config-read-only.png`, fullPage: true });
  await configBack.click();
  await page.getByPlaceholder('Type a message...').waitFor({ state: 'visible', timeout: 120000 });

  const settings = await rest('/rest/remote-access/settings', {
    method: 'PUT', body: JSON.stringify({ phoneAccessEnabled: true }),
  });
  const advertisedServerBaseUrl = 'http://192.168.2.158:60230';
  const pairingSession = await rest('/rest/remote-access/pairing-sessions', {
    method: 'POST', body: JSON.stringify({
      serverBaseUrl: advertisedServerBaseUrl,
      serverName: 'API REV 030 Disposable Browser Node',
      trustedPrivateHttpAcknowledged: true,
    }),
  });
  const pairingExchange = await rest('/rest/remote-access/pairing-exchanges', {
    method: 'POST', body: JSON.stringify({
      pairingCode: pairingSession.payload.pairingCode,
      deviceName: 'API REV 030 Mobile Browser',
      serverBaseUrl: advertisedServerBaseUrl,
    }),
  });
  const mobileSession = {
    ...
    serverBaseUrl,
    credential: pairingExchange.credential,
    device: pairingExchange.device,
    pairedAt: new Date().toISOString(),
  };
  const mobilePage = await context.newPage();
  await mobilePage.setViewportSize({ width: 390, height: 844 });
  await mobilePage.addInitScript(({ key, value }) => localStorage.setItem(key, JSON.stringify(value)), {
    key: 'autobyteus.remote_access.mobile_session.v1', value: mobileSession,
  });
  await mobilePage.goto(`${base}/mobile`, { waitUntil: 'domcontentloaded', timeout: 120000 });
  await mobilePage.locator('[data-testid="mobile-home"]').waitFor({ state: 'visible', timeout: 120000 });
  const recentTeamRow = mobilePage.locator('[data-testid="mobile-readable-work-row"]')
    .filter({ hasText: 'Nested Classroom Test Team' }).first();
  await recentTeamRow.waitFor({ state: 'visible', timeout: 120000 });
  await recentTeamRow.click();
  await mobilePage.locator('[data-testid="mobile-work-shell"]').waitFor({ state: 'visible', timeout: 120000 });
  await mobilePage.locator('[data-testid="mobile-tab-activity"]').click();
  await mobilePage.locator('[data-testid="mobile-open-team-messages"]').waitFor({ state: 'visible', timeout: 120000 });
  await mobilePage.locator('[data-testid="mobile-open-team-messages"]').click();
  const mobileReferenceRow = mobilePage.locator('[data-testid="mobile-team-reference-row"]')
    .filter({ hasText: `${slug}-reference.txt` }).first();
  await mobileReferenceRow.waitFor({ state: 'visible', timeout: 120000 });
  await mobileReferenceRow.click();
  const mobileViewer = mobilePage.locator('[data-testid="mobile-team-reference-viewer"]');
  await mobileViewer.waitFor({ state: 'visible', timeout: 120000 });
  await mobileViewer.getByText(`REFERENCE_CONTENT_${upper}`, { exact: false }).waitFor({ state: 'visible', timeout: 120000 });
  const mobileViewerText = await mobileViewer.innerText();
  await mobilePage.screenshot({ path: `${outDir}/${slug}-mobile-reference-content.png`, fullPage: true });
  await mobilePage.locator('[data-testid="mobile-team-reference-back"]').click();
  await mobileViewer.waitFor({ state: 'hidden', timeout: 120000 });
  const mobileReferenceEvidence = {
    phoneAccessEnabled: settings.settings?.phoneAccessEnabled === true,
    pairedCredentialIssued: typeof pairingExchange.credential === 'string' && pairingExchange.credential.startsWith('mra_'),
    selectedExactActiveTeam: (await mobilePage.locator('[data-testid="mobile-context-title"]').innerText()).includes('Nested Classroom Test Team'),
    referenceRowRestoredAfterBack: await mobileReferenceRow.isVisible(),
    exactReferencePathVisible: mobileViewerText.includes(`${slug}-reference.txt`),
    exactReferenceContentVisible: mobileViewerText.includes(`REFERENCE_CONTENT_${upper}`),
  };
  await mobilePage.screenshot({ path: `${outDir}/${slug}-mobile-reference-back.png`, fullPage: true });
  ...
  const resultConditions = {
    ...
    taskPeerDeliveredObservation: taskPeer.length === 1,
    taskPeerReplyDeliveredObservation: taskPeerReply.length >= 1,
    referenceDelivered: referenceMessages.length >= 1,
    observedTaskSummaryObservation: observedTaskSummary,
    observedTaskTeamSummaryObservation: observedTaskTeamSummary,
    observedTransientTaskExecutionObservation: observedTransientTaskExecution,
    observedActiveTaskUiObservation: observedActiveTaskUi,
    observedAwaitingReviewUiObservation: observedAwaitingReviewUi,
    observedAcceptedTaskUiObservation: observedAcceptedTaskUi,
    observedSelectedDetailObservation: observedSelectedDetail,
    observedTeamCommunicationUiObservation: observedTeamCommunicationUi,
    observedReferenceUiObservation: observedReferenceUi,
    taskDetailVisibleObservation: [...beforeRefresh.taskDetails, ...afterRefresh.taskDetails]
      .some((detail) => detail.includes(expectedPeer) && detail.includes(expectedResult)),
    teamCommunicationVisibleObservation: allVisibleText.some((text) =>
      text.includes(expectedReply) || text.includes(expectedPeer) || text.includes(expectedResult)),
    ...
    selectedTeamConfigReadOnly: Object.values(selectedTeamConfigEvidence).every((value) => value === true || value === runtime),
    mobileReferenceContentPath: Object.values(mobileReferenceEvidence).every(Boolean),
    refreshRetainedTaskHeaderCount: afterRefresh.taskHeader.some((text) => /1\s+task\b/i.test(text)),
    refreshRetainedVisibleTaskCountObservation: afterRefresh.taskEntries === 1,
    refreshRetainedTaskTeamSummaryObservation: afterRefresh.taskTeamEntries === 1,
  ...
  const blockingConditions = Object.fromEntries(Object.entries(resultConditions).filter(([key]) => !key.endsWith('Observation')));
  const passed = Object.values(blockingConditions).every(Boolean);
  rowResult = {
    schemaVersion: 2, slug, runtime, model, startedAt, completedAt: new Date().toISOString(), passed,
    resultConditions, blockingConditions, effectiveLaunchForm, rootRun, resumeConfig, finalTaskRecords, finalCommunications,
    observedStatuses: [...observedStatuses], beforeRefresh, afterRefresh, samples,
    consoleEvents, graphqlResponseSummaries, referencePath, selectedTeamConfigEvidence, mobileReferenceEvidence,
  };
} catch (error) {
