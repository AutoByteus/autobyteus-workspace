...
const gqlEndpoint = 'http://127.0.0.1:60230/graphql';
const outDir = new URL('./browser/', import.meta.url).pathname;
const agentName = 'API REV 030 Browser Agent';
...
async function ensureAgentDefinition() {
  ...
  const created = await gql(
    ...
    { input: {
      name: agentName,
      role: 'assistant',
      description: 'Disposable API-REV-030 standalone browser/provider agent.',
...
try {
  ...
  const resume = await gql(`query($id:String!){getAgentRunResumeConfig(runId:$id){runId isActive metadataConfig { runtimeKind llmModelIdentifier agentDefinitionId workspaceRootPath autoExecuteTools llmConfig skillAccessMode runtimeReference { runtimeKind sessionId threadId metadata } }}}`, { id: runId });
  const projection = await gql(`query($id:String!){getRunProjection(runId:$id){runId summary lastActivityAt conversation}}`, { id: runId });
  fs.writeFileSync(`${outDir}/standalone-${slug}-run-projection.json`, `${JSON.stringify({ data: projection }, null, 2)}\n`);
  ...
  const agentContainer = agentGroup.locator('xpath=../..');
  const runRow = agentContainer.getByRole('button').filter({ hasText: expectedToken }).first();
  ...
  const conditions = {
    ...
    persistedProjectionRunMatches: projection.getRunProjection?.runId === runId,
    persistedAssistantTokenExactlyOnce: projection.getRunProjection?.conversation?.filter((item) => item.role === 'assistant' && item.content === expectedToken).length === 1,
  ...
  result = { schemaVersion: 1, slug, runtime, model, agentDefinitionId, runId, expectedToken, startedAt,
    ...
    effective, resume: resume.getAgentRunResumeConfig, projection: projection.getRunProjection, consoleEvents };
} catch (error) {
  result = { schemaVersion: 1, slug, runtime, model, agentDefinitionId, runId, expectedToken, startedAt,
