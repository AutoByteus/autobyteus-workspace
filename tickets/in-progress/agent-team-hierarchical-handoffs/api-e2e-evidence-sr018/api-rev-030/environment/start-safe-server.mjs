...
const runtimeRoot = path.join(workspaceRoot, 'autobyteus-server-ts/tests/.tmp/api-rev-030-live-20260812-1');
const databasePath = path.join(workspaceRoot, 'autobyteus-server-ts/db/api-rev-030-live-20260812-1.db');
...
process.stdout.write(`API_REV_030_SAFE_SERVER_READY ${JSON.stringify(marker)}\n`);
let closing = false;
const close = async () => {
  if (closing) return;
  closing = true;
