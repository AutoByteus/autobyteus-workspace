...
const agentName = 'API REV 029 Browser Agent';
...
async function ensureAgentDefinition() {
  ...
  const created = await gql(
    ...
    { input: {
      name: agentName,
      role: 'assistant',
      description: 'Disposable API-REV-029 standalone browser/provider agent.',
...
try {
  ...
  const resume = await gql(`query($id:String!){getAgentRunResumeConfig(runId:$id){runId isActive metadataConfig}}`, { id: runId });
  ...
  const runRow = page.locator(`[data-test="workspace-agent-run-${runId}"]`).first();
