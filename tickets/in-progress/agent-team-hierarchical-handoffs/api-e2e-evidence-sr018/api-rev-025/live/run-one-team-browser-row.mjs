...
const base = 'http://127.0.0.1:31225';
const gqlEndpoint = 'http://127.0.0.1:60225/graphql';
...
async function expandTeamPanels(page) {
  const taskHeader = page.locator('[data-test="team-delegated-tasks-header"]');
  if (await taskHeader.count() && (await taskHeader.first().getAttribute('aria-expanded')) !== 'true') {
    await taskHeader.first().click();
  }
  const messageHeader = page.locator('[data-test="team-messages-header"]');
...
async function uiSnapshot(page) {
  ...
  return {
    ...
    transientTexts: await page.locator('[data-test="workspace-team-transient-execution-row"]').allInnerTexts(),
    taskDetails: await page.locator('[data-test="delegated-task-task-body"]').allInnerTexts(),
    messageRows: await page.locator('[data-test="team-communication-message-row"]').allInnerTexts(),
    messageMarkdown: await page.locator('[data-test="team-communication-message-markdown"]').allInnerTexts(),
    referenceRows: await page.locator('[data-test="team-communication-reference-row"]').allInnerTexts(),
...
try {
  ...
  for (let iteration = 0; iteration < 900; iteration += 1) {
    ...
    if (snapshot.taskEntries > 0 && !observedSelectedDetail) {
      await page.locator('[data-test="team-delegated-task-summary-row"]').first().click();
