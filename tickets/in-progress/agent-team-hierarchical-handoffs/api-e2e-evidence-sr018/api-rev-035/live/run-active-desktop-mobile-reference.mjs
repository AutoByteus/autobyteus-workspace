...
const serverBase = 'http://127.0.0.1:60235';
const gqlEndpoint = `${serverBase}/graphql`;
const outDir = new URL('./browser/', import.meta.url).pathname;
const referencePath = new URL('./api35-mobile-desktop-reference.txt', import.meta.url).pathname;
const expectedReply = 'API35_REFERENCE_REPLY_AUTOBYTEUS';
const expectedContent = 'API35_REFERENCE_CONTENT_AUTOBYTEUS';
...
async function verifyDesktopReference(page, screenshotPrefix) {
  ...
  const referenceRow = page
    ...
    .filter({ hasText: 'api35-mobile-desktop-reference.txt' })
    .first();
  ...
  return {
    ...
    exactReferencePathVisible: viewerText.includes('api35-mobile-desktop-reference.txt'),
...
async function createMobileSession(deviceName) {
  ...
  const advertisedServerBaseUrl = 'http://192.168.2.158:60235';
  const pairing = await rest('/rest/remote-access/pairing-sessions', {
    method: 'POST',
    body: JSON.stringify({
      serverBaseUrl: advertisedServerBaseUrl,
      serverName: 'API REV 035 Disposable Browser Node',
  ...
  return {
    ...
    session: {
      version: 1,
      nodeId: 'api35-mobile-paired-node',
...
try {
  ...
  const mobileActive = await verifyMobileReference(context, 'API REV 035 Active Mobile Browser', 'active');
  const conditions = {
