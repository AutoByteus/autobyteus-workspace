...
- Replaced the three duplicate hover-only cost detail toggles with one explicit, always-visible `Details` control in the `Total Cost` cell.
- Added `aria-expanded`, `aria-controls`, and localized show/hide labels for the Total Cost details control.
- Preserved non-`estimated` `apiCostStatus` visibility by rendering an inline status badge near Total Cost and keeping the existing expanded cost breakdown status/details.
- Updated the expanded detail row colspan from `11` to `9` for the reduced column count.
- Added English and Chinese localization for Details, sort actions, and cost detail show/hide labels.
- Updated focused task-table tests for the new column set, visible sort affordances, accessible sort/detail semantics, inline non-complete status preservation, and detail-row colspan.
...
- The intended visible replacement for the removed Status column is exception-only status visibility in/near Total Cost; normal `estimated` rows stay quiet in the main row.
...
- Notes: `TokenUsageTaskStatisticsTable.vue` is 306 total lines / 285 effective non-empty lines after the change. The source implementation diff for that file stayed under the `>220` changed-line split/escalation signal. Test/localization files are outside the hard source implementation file-size guardrail.
...
- Verify visually that each sortable task-table header shows a persistent neutral indicator and the active Created Time/selected sort column shows the correct active direction.
...
- Verify Input Cost and Output Cost are plain values and that only Total Cost owns the row cost-details toggle.
