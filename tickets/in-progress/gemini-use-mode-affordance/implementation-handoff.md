...
- Triggering rework finding: `SR-005`, against the color ambiguity observed in the rendered Activate/Active comparison
...
- Triggering finding IDs: `SR-005`; exploratory visual iteration selected blue Activate and green Active treatments

The current implementation uses a visible localized `Activate` text button for configured, non-active Gemini rows with a blue outlined treatment and a visible green/emerald `Active` badge on the active row. While activation is pending, the same blue button remains disabled and shows the existing spinner plus visible localized `Activating` text. The superseded plain-check icon-only contract is removed. The existing configured dot, edit/configure control, active row styling, title/ARIA label, test IDs, activation event payload, live announcement, and state/API/persistence behavior remain unchanged.

## Approved Behavior Implementation Trace

| Behavior ID | Approved Change / Preserved Outcome | Implemented Production Path / Key Files | Result / Notes |
| --- | --- | --- | --- |
| `BEH-001` / `UC-001` | Use visible localized `Activate` action text with a blue outlined treatment; preserve metadata and semantics. | `GeminiConfigurationOptionCard.vue`: `configured && !active` button with localized `activate_mode` text and blue border/background/text colors. | Implemented. Text action expands beyond 44px while retaining minimum 44px height; no icon carries activation meaning. |
...
| `BEH-002` | Preserve pending spinner, disabled behavior, focus ring, hover treatment, hit area, and visible pending status using the blue variant. | Same `activating`/`actionsDisabled` paths in `GeminiConfigurationOptionCard.vue`; focused assertions in `GeminiSetupForm.spec.ts`. | Implemented. Pending button remains disabled, shows spinner plus visible localized `Activating`, and omits `Activate`. |
| `BEH-003` / `UC-003` | Replace radio-like active marker with visible green/emerald `Active` text/badge; omit activation action. | Existing `active` branch and `v-if="configured && !active"` guard in `GeminiConfigurationOptionCard.vue`. | Implemented. Existing `data-testid`, title, active row background/left accent, and screen-reader status remain. |
| `BEH-006` | Separate action and state through visible `Activate` versus `Active` text and distinct blue/green treatments; retain configured dot and edit control. | Card status/action cluster in `GeminiConfigurationOptionCard.vue`. | Implemented; Activate uses blue outlined styling while Active uses the green/emerald badge. |
...
- If challenged, routed as `Design Impact`: `Yes` — the initial icon-only implementation and temporary text-button implementation were returned upstream and corrected through the user-authorized exploratory revisions culminating in `SR-005`.
...
- `pnpm test:nuxt components/settings/providerApiKey/__tests__/GeminiSetupForm.spec.ts --run` — **passed**, 1 file / 7 tests; assertions cover visible `Activating...`, absence of idle `Activate` while pending, blue Activate classes, and green Active classes.
...
- Approved UI/UX, interaction, requirement, and design references: revised `requirements.md`, `design-spec.md`, `ui-ux-spec.md`, and `solution-revision-record.md` (`SR-005`).
...
- Visual or interaction issues found and corrected: Removed checkmark ambiguity, then separated the remaining blue-on-blue action/state treatment. Activate now uses a blue outlined button (`border-blue-200 bg-blue-50 text-blue-700`, blue hover) while Active uses a green/emerald badge (`border-emerald-200 bg-emerald-100 text-emerald-700`).
- Supporting evidence and remaining unverified states or limitations: Prior action/state screenshot `/Users/normy/.autobyteus/browser-artifacts/526f15-1785231690937.png`; final SR-005 screenshot `/Users/normy/.autobyteus/browser-artifacts/8a0e34-1785233706633.png` shows the selected blue Activate/green Active treatment in the live Settings surface. Narrow-width wrap, hover/focus, and pending browser visuals remain for downstream validation.
...
- Verify the blue outlined text button remains readable/wraps without clipping at narrow card widths; verify keyboard focus/hover treatment and contrast.
