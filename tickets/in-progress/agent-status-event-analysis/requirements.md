...
Design-ready — user confirmed existing `AGENT_STATUS`, coarse `idle/running/error` API status, `can_interrupt`, and no backward compatibility.
...
- Refined API-level design should use coarse canonical `AGENT_STATUS.status` values (`idle`, `running`, `error`) plus `can_interrupt`; detailed runtime phases can remain internal or optional diagnostics.
...
4. Ensure terminal transitions publish a final non-working status (`idle` or `error`) for completion, interruption, stop, and error.
...
6. Remove duplicated frontend lifecycle policy after status becomes authoritative: lifecycle events can complete messages, but they should not be the owner of status/work/interrupt state.
...
- REQ-002: A status payload MUST include the current canonical coarse `status` (`idle`, `running`, or `error`). Team-scoped messages MUST include `agent_id` or `agent_name` when the envelope does not otherwise identify the member.
...
- REQ-004: Completion, interruption, runtime stop, shutdown completion, and error paths MUST publish or snapshot a final non-working status.
...
- REQ-011: `TEAM_STATUS` MUST use the coarse `{ status }` payload contract with `status` in `idle`, `running`, or `error`; it MUST NOT use `new_status`/`old_status`.
...
- REQ-013: Frontend hydration, active recovery, local termination, history/read-model mapping, status enums, status visuals, and team handlers MUST be migrated to the coarse status model without API-visible `uninitialized`, `processing`, `shutdown_complete`, or detailed phase statuses.
...
- AC-010: Frontend active recovery/hydration/history paths only produce coarse statuses and active placeholders use `running` with interrupt unavailable until the authoritative snapshot arrives.
...
- Must map native AutoByteus detailed internal statuses and Codex/Claude coarse runtime statuses into the API-level `idle/running/error` contract.
...
| REQ-011 | UC-002 |
| REQ-012 | UC-002 |
| REQ-013 | UC-001, UC-002, UC-003, UC-004 |
...
Pending user approval to proceed from analysis into implementation design.
