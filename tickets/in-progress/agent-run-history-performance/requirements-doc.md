...
5. Preserve existing bottom-follow behavior. Drive it from a cheap explicit visible-presentation revision, not `conversation.updatedAt`. When the user has scrolled upward inside the recent window, an actual visible append/update/eviction must not force the viewport downward and must expose `New activity · Jump to latest` until the user returns to the bottom. Non-visible protocol traffic must not expose the action.
...
- `REQ-005` When the Event Monitor is at the bottom, an actual visible presentation change must continue to follow the bottom. When the user scrolls upward, an actual retained visible append, content/lifecycle change, center-compaction change, or eviction-only presentation change must not force-scroll the viewport and must expose `New activity · Jump to latest` until latest is reached. Protocol messages that do not alter the bounded center presentation must not expose the action. This behavior must be driven by an explicit visible-presentation change/revision contract, not the generic conversation timestamp.
...
- `AC-005` While pinned to the bottom, a real visible append/update leaves the latest change visible. While scrolled upward, a real visible append, retained segment/card update, center-compaction change, or eviction-only presentation change does not force the viewport to the bottom, increments the explicit visible-presentation revision once, displays the localized `New activity · Jump to latest` control, and activating it moves to latest and clears the control. `CONNECTED`, `TURN_STARTED`, accepted no-op command/status messages, and other non-visible protocol traffic do not increment the revision or show the control.
...
- The visible-presentation revision is ephemeral per `AgentRunState`. It starts/resets at `0` on initial historical hydration or run/context replacement; increments once after a dispatch/submission that actually changes the bounded center presentation (including an eviction-only result); and does not increment for no-op/non-visible messages. Run selection or revision reset establishes a new feed baseline and clears unseen state rather than showing the jump action.
