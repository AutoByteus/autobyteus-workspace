...
`Refined for architecture round 12` — the underlying active-only/latest/paging model and round-10 direct-input gate remain unchanged. `CR-007` / `MP-CR-007` placement and ownership remain resolved. This revision resolves `AR-011`: cursor expiry uses the exact same centered quiet-white/neutral-border/simple-dark arrow treatment as ordinary unseen/browse states, and the current expired-only amber branch is removed.
...
On the frontend, historical conversion, live stream dispatch, local user submission, Activity retention, and final feed presentation each own part of the observable Event Monitor state, but no shared recent-window invariant connects them in the pre-change system. `AIMessage` groups multiple visible segments inside one message, so message count is not a valid UI bound. Center compaction rows are sourced from Activity state and merged by the feed, which means the final presentation must account for them as visual events too. Some events remain mutable across protocol messages: streamed text/Thinking, nonterminal tool cards, and started compactions. Bottom-pinning belongs to `AgentConversationFeed`; the ticket candidate added unseen/browse controls there, but the implementation renders a sticky padded top row containing `Load 50 earlier` and a wide centered text pill for Jump/Return. Hands-on Electron review proved those controls expose internal mechanics, reserve/obscure feed space, and violate the desired clean visual hierarchy. Both live dispatchers also update `conversation.updatedAt` for every parsed message, including messages that do not visibly change the center feed, so that timestamp cannot serve as the unseen signal.
...
Round-10 source commit `aa9705a28057b369fd63ed7199c1f1f5c655df0e` implemented the approved zero-layout behavior with the arrow at `bottom-2 right-2` and an expired-only `border-amber-300 text-amber-700` conditional. A later live screenshot established a reachable conditional state omitted from the placement premise: `AgentEventMonitor` composes `SkillImprovementComposerCta` immediately above the input, and that CTA renders right-aligned when capability and run eligibility allow it. The user chose the centered reference direction and one quiet neutral base treatment. This revision changes only feed-owned arrow placement/styling and coexistence/style-equivalence verification; `SkillImprovementComposerCta`, composer layout, expiry behavior, browse behavior, API, and state ownership remain unchanged.
...
- The user approved the 2026-07-21 refinement as a local UI/interaction change: no persistent page-size label, top control row, beginning marker, or wide jump pill may remain. Normal feed layout must be indistinguishable from a feed without pagination machinery.
- The user-approved placement is bottom-center relative to the Event Monitor feed, not the application viewport or residual space beside another action. The existing conditional **improve skills** CTA retains its lower-right composer-context placement. No responsive fallback may move the arrow into that zone.
...
6. `AgentConversationFeed` receives the explicit presentation revision, uses the derived bounded presentation, keeps existing bottom-follow behavior, and exposes a compact icon-only downward-arrow overlay at bottom-center of its own scroll region only when a post-baseline visible revision arrives while the user is non-pinned or an active browse snapshot requires explicit exit. The former wide text pill is removed; the independently conditional lower-right **improve skills** composer action remains unchanged.
...
8. Normal projection reports whether active-trace events precede the latest window. A dedicated active-trace page provider reconstructs lifecycle-correct replay events from the same active source, never archives, and exposes an internal server-fixed 50-event earlier cursor contract through explicit standalone and team-member queries. The fixed count is never presented in UI or accessibility copy.
9. The first guarded user-originated top-threshold transition establishes a server-consistent browse snapshot containing the current latest 100 plus up to 50 immediately preceding events. Later deliberate top transitions prepend at most 50 events. The opaque cursor is path-free, stable across append, and expires on active-file rewrite/compaction. Mount, selection, automatic bottom pinning, anchor restoration, and continued residence inside the top threshold cannot trigger or chain requests.
10. Frontend browse pages remain separate from `AgentRunState.conversation` and the Activity store. The browse controller freezes its presentation while live state continues independently, caps resident/mounted browse content at 300, releases farthest newer pages as necessary, and clears on run change, cursor recovery, or activation of the compact downward arrow. Paging-ready and beginning states render no visible chrome; loading/error/expiry use transient zero-layout presentation.

## Active-Trace Earlier-Paging Design

### Approved Behavioral Boundary

`latest mode (<=100) -> deliberate guarded scroll transition into top threshold -> active-trace browse snapshot -> repeated internal <=50 earlier pages -> beginning of raw_traces_active.jsonl`.

Archived segments are not a continuation source. `hasEarlier=false` means the active trace has begun even if the manifest lists archives. The feed simply stops at that beginning; it renders no persistent boundary marker or archive-related copy.
...
On the first successful load, the server-consistent <=150-entry snapshot becomes the displayed browse source. Live conversation/Activity handlers continue updating the normal latest-100 state behind it. Presentation-revision changes while browsing only preserve visibility of the compact downward-arrow action; they do not patch or force-scroll the frozen browse snapshot. Activating the arrow clears browse state and reveals current live truth.

The browse merge indexes only by `eventId`, preserves chronological order, and converts the dedicated visuals through the page-only linear converter above. It does not call `enforceRecentConversationWindow` or any normal hydration/dedupe converter. The presentation groups adjacent assistant visuals by stable `turnGroupId` solely for current avatar/layout continuity; assistant row key is `browse-assistant:<turnGroupId>` and every nested visual is keyed by `visualId`. User/compaction row keys are their `visualId`. Every visual wrapper also exposes `data-event-monitor-visual-key=visualId`, so anchoring never uses an ordinal row key. If central visual count would exceed 300, complete farthest-newer page blocks are released until the result is <=300. The compact downward arrow remains the sole visible exit to live truth; no lower-boundary banner is added. This page-turnover design is chosen over a new dynamic-height virtualizer because the repository has no virtualization capability and Event Monitor cards contain asynchronous markdown/media/disclosures.

`AgentConversationFeed` remains scroll authority. Before emitting a load request it captures `(scrollHeight, scrollTop, firstVisibleVisualId/offset)` from the first intersecting `[data-event-monitor-visual-key]`; after the successful prepend and `nextTick`, it finds that same escaped visual ID and restores its offset, with scroll-height delta only as a fallback. Stable visual Vue keys ensure a retained Thinking/tool disclosure component cannot be rebound to an equal-content neighbor after prepend or turnover. The feed consumes browse lifecycle props/events but maps them into the zero-layout state machine below; ready/beginning/released states do not create rows, padding, or banners.

### Zero-Layout Feed Control Design

The clean state contains only event content. Pagination exists as behavior, not as a persistent visual object. Remove `showBoundaryControl`, the sticky `top-0 ... px-3 py-2` wrapper, every count-bearing earlier label, the inline beginning/newer-released paragraphs, the expired-return top button, and the wide centered Jump/Return text pill.

`AgentConversationFeed` adds one bounded **user scroll-intent session**, not a position-only armed flag. Scroll events observe a session; they can never create one.

```text
gateMode = idle | intent(session) | blocked
session = {
  id,
  ...
  lastDirectInputAt,
  startScrollTop,
  lastQualifiedScrollTop,
  sawAway,
  consumed
}
scrollWorkEpoch = monotonically increasing integer

idle -> intent:
  only a trusted direct upward input on/for this scroll region may start a session

intent -> request:
  session is current, unconsumed, within lifetime/effect window,
  captured scrollWorkEpoch still matches,
  sawAway is true,
  a qualified upward scroll sample crosses <= TOP_TRIGGER_PX,
  canLoadEarlier and browseState is not loading
  => consume session before capture/emit, increment epoch, enter blocked

blocked:
  mount, subject reset, jump, auto-bottom, request, prepend, and anchor restore
  invalidate any session; scroll/input samples cannot arm or fire

blocked -> idle:
  request and anchor work are complete,
  nextTick plus two animation frames observe stable restoration,
  no touch/scrollbar pointer remains active,
  and no relevant direct input has occurred for POST_WORK_INPUT_QUIET_MS

idle -> next intent:
  requires a fresh direct user interaction after that quiet boundary
```

Direct-input adapters are explicit:

- `wheel`: a trusted `wheel` with `deltaY < 0` on the feed starts/refreshes upward intent. Wheel events during `blocked` only extend the quiet boundary; they cannot pre-arm the next request.
- `touch`: a trusted `touchstart` followed by downward finger movement (content moving toward earlier/top) starts the session. It remains active only until `touchend`/`touchcancel` plus the short effect window; momentum during `blocked` is ignored until quiet and a new touch sequence.
- `keyboard`: when focus is within the feed scroll region, trusted `ArrowUp`, `PageUp`, `Home`, or `Shift+Space` starts/refreshes upward intent. The starting position may satisfy `sawAway`; the resulting scroll must arrive inside the effect window.
- `scrollbar`: a trusted pointer press in the computed native vertical-scrollbar gutter starts a pending scrollbar session. Only subsequent upward thumb movement/scroll-position decrease while that pointer is active (or inside its short release effect window) qualifies. `pointerup`, `pointercancel`, blur, or lost capture ends it. Chromium/Electron browser coverage must prove the gutter event path; a position-only fallback is forbidden.

Use constants colocated with the existing bottom threshold: `TOP_TRIGGER_PX=24`, `TOP_REARM_PX=96`, `DIRECT_INPUT_EFFECT_MS=200`, `WHEEL_IDLE_MS=250`, `POST_WORK_INPUT_QUIET_MS=250`, and `MAX_INTENT_SESSION_MS=5000`. A session starts with `sawAway=true` only when its direct upward interaction begins at or beyond 96 px; otherwise the user must end that interaction, move away, and start a fresh upward interaction from outside the re-arm distance. Every qualifying scroll sample must move upward relative to `lastQualifiedScrollTop` and remain causally bounded to its session (within 200 ms of the latest wheel/keyboard input or while the touch/scrollbar pointer is active/recently released). Expiry, blur, visibility loss, subject/run change, any feed-owned scroll write, request start, retry, jump, or cursor reset invalidates the session.

Every feed-owned programmatic `scrollTop` write runs through one `beginScrollWork()` helper that increments `scrollWorkEpoch`, invalidates intent, and enters `blocked` **before** writing. The blocked lifetime extends beyond the synchronous call through queued scroll delivery, anchor restoration, two stable animation frames, and the input-quiet boundary. A later media/markdown resize or CSS scroll-anchoring adjustment may emit scroll events, but without a new direct-input session those events have no authority. The browse controller's existing in-flight coalescing remains a second safety boundary, not the origin proof.

Visual states are deliberately minimal:

- `latest/ready` and `beginning`: render no control, message, divider, placeholder, padding, reserved block, or boundary announcement.
- `loading`: set `aria-busy` on the scroll region immediately. After a short presentation delay (target 150 ms to avoid flashing on fast requests), show only three neutral 4 px animated dots in a pointer-inert absolute overlay centered 8 px below the scroll-region top. The overlay has no normal-flow box and no effect on `scrollHeight` or the first event's offset; reduced-motion uses opacity rather than translation.
- recoverable `error`: keep content and anchor intact; replace the dots in the same absolute top slot with a compact retry glyph button. It has a 32–36 px visible circle inside a minimum 44 px hit target, a concise localized non-visual accessible name, no explanatory tooltip, and visible focus. Successful retry returns to the loading state; no error banner is inserted.
- `expired`: keep resident browse content and show the exact same centered return arrow as ordinary unseen/browse states: identical >=44 px target, 8 px inset, quiet-white 36 px surface, subtle neutral border/shadow, simple dark 16 px glyph, focus treatment, and accessible name. No amber/warning branch, tooltip, status text, expired banner, or automatic archive/latest fetch occurs.
- `browse`, unseen live activity, or released newer browse pages: show exactly one return arrow. Never stack a separate new-activity pill, release notice, or browse-exit control.

The return action is a real button absolutely anchored at `bottom: 0.5rem; left: 50%; transform: translateX(-50%)` relative to the `AgentConversationFeed` root, outside document flow and above—not over—the composer boundary. Its visible surface is a quiet white 36 px circle with a 16 px simple dark `heroicons:arrow-down` glyph and subtle neutral project border/shadow; its hit target is at least 44 px. It has no badge, pulse, count, visible text, or explanatory tooltip. A localized `aria-label` supplies only the non-visual accessible name. Centering is against the feed box itself and remains unchanged whether `SkillImprovementComposerCta` is absent, resolving, disabled, or visible at lower-right in the following composer-context sibling. Target geometry and base visual classes/computed treatment are also invariant across ordinary unseen, frozen browse, released-page, and expired states; state may affect behavior/visibility only, never arrow color, icon, surface, or border. The arrow fades in/out without moving layout, honors reduced motion, and has no responsive side-placement fallback. It is visible only when browse state must be discarded or a net presentation revision exists while non-pinned; merely being able to scroll is not sufficient.
...
| --- | --- | --- | --- | --- |
| `/Users/normy/autobyteus_org/autobyteus-worktrees/agent-run-history-performance/tickets/in-progress/agent-run-history-performance/history-window-ui-ux-spec.md` | Defines exact latest window, active-trace paging, centered return-arrow/coexistence, one neutral base treatment including expiry, scroll/recovery, disclosure, label, and copy-removal behavior | `REQ-001`–`REQ-012`; `AC-001`–`AC-015` | Constrains latest/browse/expiry presentation and interaction; no archive affordance or state-specific warning treatment may be added | `Refined for round 12`; zero-layout, centered quiet-white arrow, and one neutral treatment user-approved 2026-07-21 |
| `/Users/normy/autobyteus_org/autobyteus-worktrees/agent-run-history-performance/tickets/in-progress/agent-run-history-performance/integrated-live-validation-plan.md` | Defines safe same-candidate snapshot topology, latest/page measurement schema, ordinary/browse/expired arrow-style equality, and failure classification | `REQ-001`–`REQ-003`, `REQ-009`–`REQ-012`; `AC-001`–`AC-003`, `AC-008`–`AC-010`, `AC-012`–`AC-015` | Validation supplement; separates bootstrap, backend, hydration, usability, page, style-equivalence, and bound evidence without changing production UX | `Design-ready for round 12`; approval `N/A` |

## Task Design Health Assessment (Mandatory)

- Change posture: `Performance`, `Behavior Change`, `Feature`, `Cleanup`
- Current design issue found: `Yes`
- Root cause classification: original performance work `Missing Invariant`; zero-layout control refinement `Local Implementation / Presentation Defect`; `CR-007` / `MP-CR-007` `Cross-Feature Layout Premise Omission`
- Refactor needed now: `Yes`
- Evidence: The provider reads an unbounded archive corpus; GraphQL returns it; frontend conversation and Activity hydration use quadratic scans; historical/live state and mounted components have no duration-independent bound. A real run produced a 47.54 MB response and 27.9 seconds of client dedupe. Even the largest observed active-only file produced 609 conversation entries, 379 activities, 9.09 MB, and ~906 ms of current client dedupe. For paging, current replay drops some raw IDs, normal conversion semantically collapses equal entries and recursively walks toolResult, and actual row/segment keys are ordinal. The original candidate rendered count-bearing/wide controls; round-10 source corrected those, but placed the arrow at lower-right without accounting for the reachable eligible **improve skills** CTA already aligned at the adjacent lower-right composer boundary. The user's live screenshot and centered reference establish the conflict and approved correction.
- Design response: Put active-only/recent selection in the shared run-history provider; keep one lifecycle-aware frontend Event Monitor window capability with completed-first eviction and a deterministic hard fallback; make the authoritative mutation boundary compare bounded pre/post presentation witnesses before revising run state; reset every conversation-replacement baseline including team reopen; cap Activity using the same completion policy; and defend the final merged presentation in the feed. Add active-trace paging as a sibling provider policy with identity assigned during replay construction, a closed central visual projector, isolated ID-only view-state/conversion, and stable actual DOM keys, so optional older content cannot bypass archive exclusion, expand canonical live/Activity state, collapse equal events, or transport hidden result detail. Preserve that model while using a direct-input-authorized top-scroll gate, transient zero-layout dots, and one feed-centered quiet-white return arrow. Leave the lower-right skill CTA and composer composition unchanged; prove both conditional actions together.
- Refactor rationale: Scattering `slice(-100)` across resolvers, handlers, and templates would encode different meanings (raw record, message, segment, activity, feed row) and would inevitably drift. The new capability makes the user-approved visual-event meaning explicit and testable. The 2026-07-21 UI correction does not justify backend/state refactoring: the feed already owns scroll and anchor behavior, so the clean fix stays local to that owner plus localization/tests.
...
| `AR-008` | The executable validation supplement still clicked the removed visible earlier-history control, so it could pass without exercising the proposed zero-layout interaction. | Phase 5A and `PAGE-001` now start at a fresh trusted direct-input session and deliberate top-threshold crossing. `PAGE-GATE-001` separately verifies no mount/selection/programmatic/queued-scroll/layout/momentum chain, one new request only after a fresh away/re-enter session, delayed dots, retry/expiry/arrow behavior, no visible count/status/tooltip, and identical normal-flow geometry. | Real Chromium/Electron input-path execution plus DOM geometry, request-count, trigger-to-stable, anchor, accessibility, and screenshot evidence. |
| `AR-009` / `MP-AR-009` | A position-only disarmed/armed flag plus synchronous programmatic-depth guard could be re-armed by a queued API scroll and later fired by CSS scroll anchoring or media reflow without a new user gesture. | Scroll events no longer create authority. A bounded feed-local session begins only from a trusted upward wheel/touch/keyboard/native-scrollbar interaction, carries source/lifetime/causal position and scroll-work epoch, is consumed before dispatch, and is invalidated by subject change, programmatic work, request lifecycle, interaction end/idle, blur/cancel, or expiry. `blocked` extends through request, anchor restoration, queued delivery, two stable frames, and post-input quiet; a fresh post-quiet interaction from outside the re-arm distance is required for another page. | Component adapter/session tests plus a real-browser held-response scenario that delivers queued programmatic scroll and delayed media/card-height anchoring after completion while request count remains one. |
| `AR-010` | The UI/UX supplement implied that manually reaching bottom always cleared the return arrow, conflicting with the frozen browse snapshot invariant. | Manual near-bottom clearing is explicitly latest-mode-only. In browse mode, reaching the resident bottom leaves the snapshot, pages, cursor, and arrow intact; explicit arrow activation alone discards browse state and reveals current live truth. | State-transition and browser checks distinguish latest manual bottom, browse manual bottom, and explicit browse-exit arrow activation. |
| `CR-007` / `MP-CR-007` | The round-10 lower-right arrow premise omitted the reachable conditional **improve skills** action already occupying the adjacent lower-right composer-context zone, producing a competing action cluster and possible narrow-layout crowding. | The user approved a feed-centered treatment. `AgentConversationFeed` keeps the arrow absolute at bottom-center with a 44 px target inset 8 px from the feed bottom and quiet-white 36 px surface; `SkillImprovementComposerCta` remains independently right-aligned below it. No prop/coordination contract or responsive right-edge fallback is added. | Component and real-browser evidence with capability eligible/ineligible at representative wide and 390 px narrow widths proves center alignment to the feed, 8 px inset, unchanged lower-right CTA placement, no intersection/stack/overflow/composer overlap, and the reviewed icon/accessibility/zero-layout behavior. |
| `AR-011` | The approved UI/UX basis defined one neutral return-arrow treatment, but the design guidance and round-10 source retained an expired-only warning/amber branch. | Ordinary unseen, frozen browse, released-page, and cursor-expired states now share one exact base treatment: the same target and centered geometry, quiet-white surface, neutral border/shadow, dark down glyph, focus treatment, and accessible name. Expiry may change recovery behavior/visibility only. The source amber conditional and its state-specific test expectation are explicitly removed. | Component and `PAGE-COEXIST-001` evidence compare the same computed-style signature in ordinary unseen, browse, and expired states at CTA-visible/absent wide and 390 px layouts. Any state-dependent base color, surface, border, shadow, icon, dimension, or accessible-name divergence fails. |
...
- **Active-trace browse mode:** A frozen, ephemeral, server-consistent ordered view entered only after a guarded deliberate top-threshold transition; its source range is the current active trace and its resident central presentation is at most 300.
- **Top paging intent session:** Feed-local bounded authority created only by a direct upward wheel, touch, supported keyboard, or native-scrollbar interaction. Scroll events may advance it within its causal/lifetime window but cannot create it. It is consumed before one request and invalidated by programmatic work, request lifecycle, interaction end/idle, blur/cancel, or subject reset.
...
| `BEH-003` | Preserve bottom follow and expose only a compact icon-only downward arrow for a net bounded visible change or required browse exit; the control consumes no layout height. | Begin witness -> handler/enforce -> post witness/revision -> feed pin/unseen state -> zero-layout overlay. | `DS-003`, `DS-004`, `DS-007` | `REQ-005`, `REQ-011`; `AC-005`, `AC-011`, `AC-013`, `AC-014` |
...
| `BEH-006` | Add internal fixed-50 earlier traversal only within current active trace, triggered once per consumed direct user scroll-intent session, with no visible page count/persistent top chrome, <=300 resident visuals, and stable source-to-DOM event/subvisual identity. | Direct input session + qualified top crossing -> explicit run/team page query -> identity-bearing active replay -> dedicated central page visual DTO -> isolated ID merge/turnover -> keyed feed DOM. | `DS-006`, `DS-007` | `REQ-010`–`REQ-012`; `AC-012`–`AC-015` |
| `BEH-007` | Keep cursors valid across append and expire safely across active rewrite/compaction. | Active generation/anchor validation -> typed page result -> the exact same neutral centered return arrow used for ordinary unseen/browse recovery; no state-specific warning treatment. | `DS-006`, `DS-007` | `REQ-011`; `AC-013` |
...
| `BEH-010` | Place the conditional return arrow at feed bottom-center using the user-approved quiet-white/dark-arrow treatment while preserving the independent lower-right **improve skills** action. | Presentation revision, browse, released-page, or expired state -> `AgentConversationFeed` one centered absolute arrow with one invariant base treatment; eligible run/member -> existing composer-context `SkillImprovementComposerCta` right-aligned below the feed. No state or prop coupling joins the paths. | `DS-004`, `DS-007` | `REQ-005`; `AC-005`, `AC-011`, `AC-013` |
...
| `DS-004` | `Bounded Local` | Conversation + compaction props + presentation revision | Completed-first ≤100 rendered feed + zero-layout pin/unseen controls | `AgentConversationFeed` | Enforces exact visual bound and truthful clean recovery UX |
...
| `DS-006` | `Primary End-to-End` | Guarded deliberate top-threshold transition | Identity-bearing closed central-visual page response | Agent/team page resolver -> run-history page provider/projector | Adds useful earlier context without visible batch mechanics or archive/full-history/result payload work |
...
- `DS-006`: `fresh bounded direct-input session qualifies an upward top-threshold crossing -> consume intent, capture anchor, and emit one load -> explicit standalone/team GraphQL page query -> existing source resolution -> active-only memory view -> full active lifecycle reconstruction with eventId/turnGroupId -> validate generation/anchor -> select newest-100-plus-50 event page or preceding-50 page -> project closed typed central visuals with stable visualId -> response`
...
| `DS-007` | Validate/index the typed response by event/visual ID, build a frozen stable-keyed page-only presentation in linear time, preserve the exact visual anchor, release farthest newer pages until <=300, and keep live state behind the direct-input session and zero-layout arrow boundary. | Browse subject, page blocks, event/visual maps, cursor, visual DOM anchor, top paging intent session | Active-trace browse controller + page-only presentation converter + feed | Localization, retry/expiry, dynamic-height tolerance, run-change cancellation |
...
- `AgentConversationFeed` owns bottom proximity, latest-mode unseen clearing, compact bottom-center return-arrow behavior, the top paging input session/scroll-work epoch, and use of the final bounded presentation/revision. Manual bottom is meaningful only for latest-mode unseen state; frozen browse exit remains an explicit arrow action. The feed centers against its own box and does not inspect skill eligibility or coordinate with composer actions. It must not infer activity from generic timestamps, merge pages, or call GraphQL directly.
...
- `AgentConversationFeed` additionally owns visual-ID prepend anchor capture/restoration and zero-layout browse overlays. It renders no persistent boundary row, page count, beginning/release banner, or wide jump pill, and still performs no GraphQL call or page merge itself.
- `AgentWorkspaceView` / `AgentEventMonitor` own composition only and no longer own conversation serialization. Their existing feed-then-composer-context sibling structure supplies the geometric boundary; they do not calculate arrow coordinates or pass skill-action state into the feed.
- `SkillImprovementComposerCta` continues to own skill eligibility/action presentation and its right-aligned composer-context layout. It does not reposition, hide, or reserve space for the Event Monitor arrow.
...
| Round-10 expired-only `browseState === 'expired' ? 'border-amber-300 text-amber-700' : ...` arrow branch | Contradicts the user-approved single neutral arrow and lets expiry create a second visual language | One state-invariant neutral return-arrow class/computed-style path in `AgentConversationFeed.vue` | `In This Rework` | Remove the amber conditional rather than retaining a compatibility/state variant; update the expired-state component assertion to require exact equality with ordinary unseen and browse |
...
- `DS-006` initial browse: `direct upward input starts session outside re-arm distance -> causally qualified scroll crosses top trigger -> consume session/enter blocked/capture anchor -> page query without cursor -> identity-bearing current active replay sequence -> latest 100 + preceding <=50 -> closed central visual projection -> frozen typed response -> keyed anchored presentation -> settle/quiet -> idle`
- `DS-006` continuation: `prior input ends + post-work quiet -> fresh direct upward input session starts outside re-arm distance -> qualified top crossing consumes session -> opaque before cursor -> validate subject + active generation + event anchor -> preceding <=50 -> stable event/visual page response -> ID-only prepend/turnover`
- `DS-007` live while browsing: `live commit -> normal capped state/revision -> browse snapshot unchanged -> one compact arrow remains visible -> activate arrow -> clear pages -> reveal current live latest-100`
...
- Parent owner: `AgentConversationFeed`
  - Chain: `run/revision reset -> establish baseline and clear unseen; revision increment -> if latest+pinned guarded auto-scroll else mark unseen; latest-mode manual bottom clears unseen/pins; arrow clears unseen and, when browsing, explicitly discards browse state; browse-mode manual bottom changes neither state nor arrow`
  - Why: Keeps scrolling local to the actual scroll container.
- Parent owner: `AgentConversationFeed` top paging intent session
  - Chain: `idle -> trusted upward wheel/touch/key/scrollbar input starts bounded session -> only causally adjacent upward scroll samples may establish away/crossing -> consume before request -> blocked through request/restore/queued scroll/two stable frames/input quiet -> idle -> fresh interaction required`
  - Why: Makes paging discoverable through normal scrolling without exposing internal page chrome, while queued API scroll, CSS anchoring, media reflow, momentum, and position-only samples have no authority to arm or chain.
...
- Parent owner: active-trace browse controller/page converter/feed
  - Chain: `guarded top crossing -> capture first visible visualId/offset -> request/coalesce -> validate subject still selected -> validate/index eventId+visualId -> merge only by eventId -> release farthest newer page blocks until visual count<=300 -> linear page presentation -> render actual visualId Vue/data keys -> guarded restore of same visualId; error/expired preserve current view -> arrow clears`
...
| --- | --- | --- | --- | --- | --- |
| Localization | `DS-004`, `DS-007` | Feed UX | English/zh-CN non-visual arrow/retry accessible names; remove visible batch/status/tooltip copy | Assistive-technology naming without visible explanation | Hard-coded text or stale count-bearing/status generated keys |
| Accessibility | `DS-004`, `DS-007` | Feed UX | Real arrow/retry buttons, focus, `aria-busy`, concise non-visual names, no feed live region | Keyboard/screen-reader parity while visual chrome stays minimal | Stream handlers become UI-aware or icons become unnamed |
...
| Zero-layout loading/recovery | `DS-007` | Browse controller/feed | Delayed three-dot overlay, compact retry glyph, silent beginning, and one state-invariant neutral return-arrow treatment including expiry; no status text/tooltips/rows/banners | Bounded navigation with no explanatory or state-divergent feed chrome | Network layer rendering strings, template adding flow height, or expired-only warning classes |
...
| Scroll pinning and guarded top paging | Agent conversation feed | `Extend` | Existing scroll element, bottom threshold, input events, and anchor logic already live here; add one colocated bounded direct-input session plus scroll-work epoch/settling lifecycle | N/A |
...
| Web workspace components | Feed pin/arrow UX, guarded top paging, zero-layout browse status, visual-anchor states, typed browse rendering, and copy removal | `DS-004`, `DS-007` | Feed/workspace view | `Extend` | Reuse leaf components and `heroicons:arrow-down`; disclosure behavior unchanged and keyed by visualId |
...
| Existing feed/store/view | Web UI/state | Local owners | Compact arrow/presentation, guarded scroll gate, zero-layout status, cap, copy removal | Existing responsibility locations | Window capability/constant |
...
| `.../AgentConversationFeed.vue` | Web workspace UI | Feed/presentation | Consume latest or typed browse presentation/revision; actual stable row/visual keys; `data-event-monitor-visual-key`; bounded wheel/touch/keyboard/scrollbar intent session; scroll-work epoch/blocked settling; delayed dot/retry overlays; silent beginning/expiry; visual-ID prepend anchor; latest-only manual-bottom clearing; explicit browse-exit arrow with one invariant neutral base treatment in ordinary/browse/expired states | Existing scroll owner | Latest selector + browse controller props/events + existing Icon capability |
...
| `autobyteus-web/components/workspace/agent/AgentConversationFeed.vue` | File | Feed | Latest/browse presentation, bounded direct-input session and scroll-work settling, zero-layout loading/retry overlays, prepend anchor, latest-only manual-bottom clearing, feed-centered explicit browse-exit arrow | Owns scroll container/input lifecycle and arrow geometry | History fetch, page merge, store mutation, or skill eligibility |
...
| `autobyteus-web/components/workspace/skill-improvement/SkillImprovementComposerCta.vue` | File | Existing composer action | Preserve conditional eligibility, right-aligned placement, and action behavior unchanged | Existing skill-action owner; included only in coexistence validation | Event Monitor arrow positioning or browse state |
...
| Compaction during browse | Active rewrite changes generation; page returns `EXPIRED`; UI preserves view without a status announcement and keeps the same neutral compact return arrow available | Read archive to continue, silently reset, or add warning styling | Keeps source boundary, avoids duplicate/missing content, and preserves the single approved arrow treatment |
...
Web explicit browse boundary
  ...
  -> feed guarded top-threshold gate / visualId prepend-anchor restoration
  -> zero-layout dots/retry states + one compact centered downward arrow
  -> arrow discards browse state and reveals canonical live <=100
```
...
| Earlier page | Armed user scroll crosses top threshold -> stable anchored browse render | Per-request TTFB/total/bytes/event+visual IDs, closed-schema/result-log exclusion, linear conversion, actual DOM visual key/anchor delta, zero archive opens, no mount/programmatic/chained request, zero control-row height, <=2.0 s under `AC-015` |
...
7. Refactor parent prop flow and `AgentConversationFeed` to consume bounded presentation plus explicit revision and add baseline/unseen/compact-arrow scroll state. Preserve current disclosure components and bottom threshold. Scope manual near-bottom clearing to latest mode; in browse mode only explicit arrow activation clears pages/cursors and reveals live truth. Place the arrow at bottom-center relative to the feed with a quiet-white 36 px surface inside a >=44 px target. Delete the round-10 expired-only amber class conditional so ordinary unseen, browse, and expired states use one exact neutral class/computed-style path. Add component tests for completed-first >100 selection, pinned/non-pinned revision behavior, no-op revision absence, reset, latest-manual-bottom versus browse-manual-bottom/arrow behavior, exact ordinary/browse/expired base-style equality, zero layout height, feed-center geometry, no composer or eligible **improve skills** overlap at wide/narrow widths, non-visual accessible naming with no tooltip, reduced motion, and keyboard semantics.
...
13. Add the typed browse assistant row and extend `AgentConversationFeed`/parents with stable carried row/visual Vue keys, `data-event-monitor-visual-key`, the bounded direct-input session/scroll-work epoch defined above, 24 px/96 px top hysteresis inside that session, visual-ID prepend anchor restoration, delayed three-dot loading, compact retry, silent beginning/expiry states, and one localized non-visually-named icon-only downward arrow centered at the feed's lower edge. Remove the sticky top wrapper, count-bearing labels, beginning/newer-release rows, expired top button, visible status/tooltip copy, former wide text jump pill, and current expired-only amber class/test branch. Preserve `SkillImprovementComposerCta` unchanged at lower-right in composer context. Prove no mount/selection/automatic-bottom/anchor-restoration/queued-scroll/layout-reflow request, no continued-momentum/near-top chaining, one request per consumed fresh wheel/touch/keyboard/scrollbar session, post-work quiet/settling, zero normal-flow height across states, exact ordinary/browse/expired arrow-style equality, centered-arrow/skill-action coexistence at wide and 390 px narrow widths, retained Thinking/tool disclosure identity across prepend/turnover, and no ordinal browse keys. Keep GraphQL/merge out of the feed and canonical conversation/Activity untouched.
...
- **Internal active-trace paging vs an archive/history product:** Server-fixed 50-event pages recover occasionally useful earlier context without reopening archive-scale work, while the UI intentionally conceals the count and persistent controls. The beginning of the active trace is final even when archives exist and is represented by the natural end of upward scrolling, not a banner.
...
- **Guarded automatic top paging vs a visible load button:** The guarded threshold makes normal scrolling the interaction and removes irrelevant implementation chrome. It adds a small feed-local hysteresis/programmatic-scroll state machine, but that complexity is bounded, testable, and safer than a naive `scrollTop <= threshold` watcher that can cascade after mount or prepend.
- **One compact return arrow vs no explicit recovery control:** Ordinary scroll does not discard the frozen browse snapshot or reveal live state after newer pages are released. The arrow preserves that required state transition while reducing its visual footprint to the conventional chat-style scroll-to-bottom affordance.
...
- Browser scroll anchoring and asynchronous media/layout changes can emit or alter scroll geometry after prepend. Every feed-owned programmatic scroll write must increment the scroll-work epoch and enter blocked state before writing; queued scroll delivery, restoration, two stable frames, and input quiet complete that work. Later layout events still cannot fire without a fresh bounded direct-input session. Real Chromium/Electron coverage must prove the `MP-AR-009` delayed-scroll/dynamic-layout path does not issue a follow-on request.
- An icon-only arrow must remain operable without adding visible explanation. Keep the standard dark downward glyph, localized non-visual `aria-label`, visible focus, minimum 44 px hit target, quiet-white surface, and neutral border/shadow identical across ordinary unseen, browse, released-page, and expired states; do not add a tooltip, badge, pulse, count, text pill, or state-specific warning treatment.
- The conditional **improve skills** action and the return arrow are owned by different siblings. Preserve that separation: center the arrow against the feed using project tokens, leave the CTA right-aligned in composer context, add no eligibility prop or DOM measurement between them, and prove their bounding rectangles do not intersect at wide or 390 px narrow widths. The user reference is visual direction only; do not copy external CSS, assets, or implementation details.
...
- The browse controller owns network state, cursors, Map/Set event/visual ID validation, first-snapshot freeze, 300-visual page-block turnover, and reset. `AgentConversationFeed` owns scroll measurement/restoration, the bounded direct-input session/scroll-work epoch, latest-only manual-bottom clearing, explicit browse-exit arrow, and zero-layout presentation. Preserve the first visible `visualId`/offset across prepend after `nextTick`, with scroll-height delta only as a fallback.
- Coalesce activation while an earlier request is in flight. On network failure keep all resident content and expose only the compact retry glyph in the absolute top slot; on `hasEarlier=false` render no visual marker and leave the gate permanently unavailable for that snapshot; on `EXPIRED` keep current content silently and keep the same neutral centered return arrow available. Do not add a feed live-region announcement, warning color, alternate icon, or flow row.
- Add server tests for standalone/team identity/authorization; raw/tool/orphan/legacy event-ID stability; equal-content/equal-timestamp distinctness; deterministic multi-visual IDs; exact 100+50 and subsequent 50 pages; 275-event traversal; append/rewrite/foreign cursor; archive-open spies; no GraphQLJSON/result/log/Activity fields; explicit tool status/summary/error; and byte-identical multi-megabyte-result vs result-null page projection. Add web tests for generated-type validation, O(E+V) conversion/no recursive getter access, duplicate structural ID error, distinct equal content, subject reset, visual-ID prepend anchor, retained disclosure DOM identity, live separation, latest-manual-bottom versus browse-arrow reset, localized non-visual ARIA names with no tooltip/status copy, delayed loading dots/reduced motion, retry/expiry, exact ordinary/browse/expired arrow class/computed-style equality, zero flow-height/first-offset equality, bottom-center geometry and eligible/ineligible **improve skills** coexistence at wide/narrow widths, no mount/selection/programmatic/queued-scroll/layout-reflow request, direct-input session expiry/consumption for wheel/touch/keyboard/scrollbar, one request per fresh session, >500-event turnover <=300, and no gaps/duplicates among resident blocks.
