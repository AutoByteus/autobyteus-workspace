...
- Related code-review revision IDs: `CRR-001–CRR-004` (current trigger `CRR-004`)
- Related API/E2E revision IDs: `API-REV-001–API-REV-003` (failure trigger `API-REV-003`)
...
| BEH-009 | Preserve loading/error/retry/locked/accessibility/responsive behavior for nested Team controls. | Address-scoped store/readiness state; `TeamScopeConfigEditor.vue`; `WorkspaceSelector.vue`; `RunConfigPanel.vue`; localized EN/ZH labels. | Canonical-address feedback, accessible disclosure, disabled/read-only state, retry/reset controls, responsive layouts, and workspace-inventory failure recovery were exercised. |
...
- Delivery still owns refreshing the ticket branch against the latest tracked base. Recovery deliberately reconstructed the implementation on its recorded historical base. The recovery safety stash remains available through review.
- API/E2E investigation/execution reached API-REV-003 and validly exposed API-E2E-F-001. IR-003 does not claim downstream closure: repeat source review must pass before the API/E2E owner reruns the strengthened 4/4 production-upgrade coverage and returns its durable-test delta for proportional review.
...
- Notes: normal runtime is V2-only. IR-002 removes the full-create missing-runtime fallback, retains no compatibility branch, and corrects current-path V1 labels while preserving legitimate migration-owned V1 language. Exact V1 types/validation/building remain solely in the approved migration-owned folder. No authored implementation source exceeds 500 effective non-empty lines; the generated GraphQL client is excluded from the authored-source guardrail. Six `>220` change-delta files were assessed: they are cohesive replacements/extractions (intent store, compact form rewrite, V1 schema/index, V2 migration, and return-view context factory), and each authored file remains below 300 effective non-empty lines.
...
- IR-003 applicability: `Not Applicable` — this revision changes only startup predecessor migration extraction and focused server unit coverage; it does not affect rendered frontend code or interaction. The prior IR-001/IR-002 rendered-result evidence below remains authoritative for the unchanged frontend.
...
The mandatory coverage investigation and API-REV-003 execution have occurred, and CRR-004 attributed API-E2E-F-001 to implementation source. IR-003 must pass repeat source review before `/api_e2e_engineer` reruns the full strengthened 4/4 production-upgrade coverage. Any resulting repository-resident durable coverage state must return through proportional code review before delivery.
