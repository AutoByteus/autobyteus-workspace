...
| IR-013 | `architecture_reviewer`; `design-review-report.md`; ARCH-REV-007 / SR-015 cleanup round | `AR-001` resolved; `CR-013` rescinded; `MP-CR-010` Not Reachable | `Design-Approved Cleanup` | `SR-015`, `ARCH-REV-007`, `CRR-021`, `API-REV-010`, `DR-004` | Unsupported CR/LF scope removed, retained history tests grounded in emitted fields, and implementation prepared for complete source review |
...
### IR-011 — Separate stored capabilities and preserve exact partial-schema history

- Triggering role, report path, and round: `architecture_reviewer`; `/Users/normy/autobyteus_org/autobyteus-worktrees/hierarchical-team-run-launch-config/tickets/in-progress/hierarchical-team-run-launch-config/design-review-report.md`; `ARCH-REV-005`, SR-013 correction after `CRR-017`
- Triggering finding IDs: `CR-011`, `CR-012`; material premise `MP-CR-009`
- Classification: `Design-Approved Rework`
- Prior authoritative result: `CRR-017` failed IR-010 because the common Team scope/Agent model carried editable authoring capabilities into stored history and whole-schema-only fallback logic hid or default-normalized explicit values under partial current-schema drift
- Current authoritative result: source commit `ab7d8eedf` establishes parallel editable/stored Team and Agent capabilities, removes stored authoring imports/sentinels, and renders every explicit historical model-config key exactly once through a pure representability classifier; ready for complete source review
- Related solution revision IDs: `SR-013` (preserving SR-012/SR-011/SR-008 where not superseded)
- Related architecture-review revision IDs: `ARCH-REV-005` (prior ARCH-REV-001–004 decisions preserved)
- Related code-review revision IDs: `CRR-017`
...
- Why this implementation revision is recorded: architecture review approved a bounded correction that keeps one shared stored/editable appearance while preventing capability conflation and preserving explicit history across current schema drift
- Approved behavior or requirement IDs affected: `BEH-010`; `R-042–R-044`; `AC-035–AC-038`; `DS-006`; `USER-UX-003`; `MP-CR-009`
- Implementation delta: introduced neutral display fields plus closed editable/stored Team scope, Agent node, member tree, and top-level models; removed stored workspace selection, override, operation, catalog, repair, and lock sentinels; updated recursive renderers to accept discriminated subjects. Added `projectHistoricalModelConfigFields`, current-control/residual presentation, exact fallback rows, stable removed-key ordering, partial and whole-schema behavior, and historical-mode sanitization/default suppression. Added direct pure/root/nested-Team/Agent coverage for representable values, `reasoning_effort=ultra`, `removed_parameter=persisted-value`, whole-schema absence, stable order, no duplication, and zero mutation.
- Changed files or areas: new `TeamRunFormDisplay.ts`, `EditableTeamRunFormModel.ts`, `StoredTeamRunFormModel.ts`, and `historicalModelConfigFields.ts`; narrowed `TeamRunFormModel.ts` and stored/editable projectors; updated shared form/tree/Team/Agent/workspace/model-config controls; added/updated eleven focused suites; IR-011 evidence and handoff artifacts
...
### IR-013 — Remove unsupported multiline scope and ground retained history coverage

- Triggering role, report path, and round: `architecture_reviewer`; `/Users/normy/autobyteus_org/autobyteus-worktrees/hierarchical-team-run-launch-config/tickets/in-progress/hierarchical-team-run-launch-config/design-review-report.md`; `ARCH-REV-007`, SR-015 producer-bounded cleanup round
- Triggering finding IDs: `AR-001` resolved; `CR-013` rescinded; `MP-CR-010` Not Reachable; `API-E2E-F-003` Out Of Scope / Non-Blocking
- Classification: `Design-Approved Cleanup`
- Prior authoritative result: committed IR-012 added CR/LF-specific classifier and residual-presentation behavior after CRR-018/019 treated an API/E2E-invented free-text schema as a current product path; CRR-021, API-REV-010, SR-014/SR-015, and ARCH-REV-007 corrected that reachability premise while preserving IR-011
- Current authoritative result: source commit `554e1baaa2e455ca8c73e0414a100e1c8e24829a` cleanly removes the unsupported predicate, branches, styling, and synthetic fixtures, retargets retained supported-history coverage to emitted `reasoning_effort` and optional `service_tier`, passes focused/build/guard/static/render validation, and is ready for complete source review
- Related solution revision IDs: `SR-015` current; `SR-014` producer-boundary basis; `SR-013` capability/classifier baseline; `SR-012` shared-form baseline
- Related architecture-review revision IDs: `ARCH-REV-007`; `ARCH-REV-006 / AR-001` resolved; prior ARCH-REV-001–005 decisions preserved where not superseded
- Related code-review revision IDs: `CRR-021`; `CRR-020` superseded; `CRR-018–CRR-019` premise correction history
- Related API/E2E revision IDs: `API-REV-010 / Pass 98%` for real current-user paths; `API-REV-009` synthetic applicability corrected
- Related delivery revision IDs: `DR-004` preserved historical delivery state; no re-entry claimed
- Why this implementation revision is recorded: the user-approved production-reachability rule rejects source machinery and acceptance obligations whose initiating state exists only through test-owned catalog mutation and arbitrary injected data. SR-015 made the exact cleanup and producer-backed test vocabulary mandatory while keeping the classifier generic and provenance-free.
- Approved behavior or requirement IDs affected: `BEH-010`; `R-043–R-044`; `AC-036–AC-038`; `DS-006`; material premise `MP-CR-009` remains Reachable and `MP-CR-010` is Not Reachable
- Implementation delta: removed `canTextInputRepresentExactly` and all CR/LF-specific classification from `historicalModelConfigFields.ts`; removed multiline-only `whitespace-pre-wrap` from `HistoricalModelConfigFallback.vue`; removed ordinary/LF/CR synthetic classifier and mounted fixtures/assertions; replaced invented removed/legacy fixture names in the direct classifier, root/nested-Team, and Agent tests with stale `reasoning_effort=ultra` and removed optional `service_tier=fast`. Retained the generic per-field schema/enum/type algorithm, compact residual renderer, stable ordering, one occurrence, zero mutation, stored/editable capability split, and shared locked UI. Added no provenance, compatibility, feature flag, provider branch, alternate fallback, or isolated-CR mechanism.
- Changed files or areas: `autobyteus-web/utils/historicalModelConfigFields.ts`; `autobyteus-web/components/workspace/config/HistoricalModelConfigFallback.vue`; `autobyteus-web/utils/__tests__/historicalModelConfigFields.spec.ts`; `autobyteus-web/components/workspace/config/__tests__/StoredTeamScopeHistoricalFields.spec.ts`; `autobyteus-web/components/workspace/config/__tests__/MemberOverrideItem.spec.ts`; IR-013 handoff/evidence artifacts
- Local validation and result: focused stored/shared cohort passed 11 files/112 tests; production Nuxt build passed with 3,732 modules/15 routes; web/localization guards passed; static/source audit passed with production files at 77/41 effective non-empty lines and all unsupported vocabulary absent. The supported real-user stored Settings journey retained eight disabled fields, read-only explanation, no Run/Reset, an operable Advanced disclosure, and no narrow overflow; the synthetic CR browser scenario was intentionally not rerun.
- Next recipient or routing: `/code_reviewer` for complete source review only
- Remaining limitations or risks: no standalone `vue-tsc` pass is claimed because the package still lacks that executable. The live stored snapshot has current representable values, so producer-backed stale/removed states remain covered in mounted root/nested-Team/Agent consumers without mutating user history. API-REV-010 is the preserved real-user baseline. No API/E2E execution, delivery re-entry, push, archival, release, deployment, tag, or cleanup is claimed.
