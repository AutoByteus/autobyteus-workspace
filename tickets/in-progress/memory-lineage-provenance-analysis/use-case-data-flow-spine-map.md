...
- Status: Refined normative supplement; SR-004 aligned to lineage-tail current authority, message-only snapshot ownership, and fail-closed startup.
...
This map does not authorize implementation by itself. The requirements and foundation contract are approved design input, and the mandatory design specification derives concrete files and APIs from these spines before architecture review authorizes implementation.
...
7. The expressions M(n) and R(n) are descriptive:
   - M(n): the bounded replacement output produced by successful compaction C(n);
...
| DF-L02 | Structured compaction proposal inside DF-P04/DF-P05/DF-P06/DF-P10 | Canonically rendered recurrent input from DF-L08 | Call configured runner -> parse response -> normalize one-to-three episode contents and accepted semantic-fact contents -> copy selected new raw IDs, retained continuation, and execution metadata into the strategy result | Valid IDless `WorkingContextCompactionProposal`; no output artifact IDs, accepted lineage/context candidate, or durable writes | Separates LLM content generation and strategy selection from application-owned output identity, accepted state, and persistence. |
...
| DF-L04 | Accepted-compaction transition inside DF-P04/DF-P05/DF-P06/DF-P10 | Valid IDless proposal, unchanged pending/baseline state, and non-empty R(n) raw refs | `MemoryManager` reuses pending `compactionId` -> verifies captured baseline lineage head -> maps that head to `previousCompactionId` -> assigns deterministic output artifact IDs -> builds/finalizes/validates accepted lineage/context candidate -> internal committer archives exactly selected raw IDs into one completed file -> persists bounded output artifacts -> appends one immutable next lineage record as the new head -> installs finalized WorkingContext -> writes message-only schema-v5 snapshot -> clears pending request | Successful compaction becomes current exactly once in normal execution | Centralizes acceptance and publication sequencing, keeps strategy IDless, and prevents store/lineage bypasses. No journal or crash-recovery state is introduced. |
...
| DF-L08 | Natural compactor conversation-history rendering inside DF-P04/DF-P05/DF-P06/DF-P10 | Planned ordered compactable logical WorkingContext prefix containing optional projected M(n-1) plus selected R(n) | Adapt visible User/Assistant content and settled call/result groups -> DF-L09 serializes/redacts/bounds values and renders each call/outcome body as `name` + `status` + `arguments` + `result`-or-`error` -> add `User`/`Assistant`/`Tool` labels in logical order -> omit reasoning/backend IDs and synthetic timestamps -> escape source-originated reserved boundary sequences -> wrap the complete prefix once in `<conversation_history>...</conversation_history>` -> build one natural compactor task user message | Bounded natural representation of the compactable context seen by the working LLM; no separate memory section, `Assistant tool call` label, or persisted prompt copy | Preserves WorkingContext semantics while keeping lineage/storage mechanics out of the LLM-facing prompt and adding no second condenser. |
...
| UC-018 — Create one episode from one bounded compaction input | Accepted compactor output contains one episode | DF-P04, DF-P05, or DF-P10 + DF-L08 + DF-L02 + DF-L04 | `MemoryManager` acceptance assigns the episode ID and the one producing `compactionId` relates it to direct inputs. |
| UC-019 — Create multiple episodes from one extraction | Accepted output contains two or three episodes | DF-P04, DF-P05, or DF-P10 + DF-L08 + DF-L02 + DF-L04 | All produced episode IDs share one producing record; no duplicate evidence references are stored per episode. |
| UC-020 — Create semantic facts from the bounded compaction input | Accepted output contains semantic facts within the limit | DF-P04, DF-P05, or DF-P10 + DF-L08 + DF-L02 + DF-L04 | `MemoryManager`-assigned IDs and one producing record establish provenance; the LLM/strategy supply content only. |
...
Coverage result: **UC-001 through UC-026 are all mapped.** UC-025 is carried by normal native-compaction primary spines and UC-026 by the supported Work Evidence primary spine; neither is justified by a local helper alone.
...
| Structured compaction strategy | Planning, canonical natural conversation-history request construction from selected WorkingContext units, and valid IDless content/selection/execution proposal construction | Assigning output IDs, reading/validating the lineage head, building accepted lineage/context candidates, or publishing raw archive/memory/lineage/snapshot |
| `MemoryManager` | Sole live WorkingContext mutation/replacement boundary; pending operation and baseline-lineage-head verification; output-ID assignment; authoritative accepted-candidate construction/validation/application | Provider wire encoding, Event Monitor projection, recursive provenance queries |
...
| Compactor conversation-history renderer | Adapt planner-selected WorkingContext constituents into visible User/Assistant values and settled Tool values; preserve logical order; omit reasoning/backend IDs and synthetic timestamps; add exactly one application-owned `<conversation_history>` boundary; escape reserved-delimiter collisions; choose the compactor bound; and compose natural task text | Raw-record reconstruction, shared redaction/serialization/omission mechanics, lineage identity, generated Work Evidence ownership, or semantic condensation |
...
| Compaction-lineage persistence boundary | Immutable direct records, append-order current-head lookup, and output-to-producing-compaction lookup | LLM content generation, raw content duplication, recursive business response assembly |
...
These rules eliminate the current mixed-level compaction write path in which `StructuredJsonCompactionStrategy` mutates the store before `PendingCompactionExecutor` validates and asks `MemoryManager` to install the result. The exact replacement API and file mapping belong in the post-approval design specification.
...
| Compactor conversation-history renderer | Structured compaction strategy / DF-L08 | Adapt planner-selected WorkingContext constituents; preserve logical User/Assistant/Tool order; omit reasoning/backend fields and synthetic timestamps; choose compactor bounds; use DF-L09; escape reserved boundary collisions; and emit exactly one natural outer conversation-history block. |
...
| Compactor runner and response parser/normalizer | Structured compaction strategy / DF-L02 | Invoke the configured compactor and convert accepted output to the bounded internal proposal shape. |
...
| `compaction_lineage.jsonl` | **New Additive Current-Schema Data** | Only successful native compactions append records. The last valid record selects the bounded current output; absent/empty means none. No separate state file, pointer, manifest, or historical synthesis exists. |
...
| Startup encounters pre-lineage episode/semantic/snapshot/manifest files | Reachable | `startConfiguredServer` invokes `AppDataMigrationRunner.runPending()` before `bootstrapBuiltInAgents`, `buildApp`, and `app.listen`; representative persisted files exist, usage is sparse, and the user explicitly approves their deletion. The current caller catches/logs migration failure and continues, so fail-closed caller propagation is a real target delta. | Requires DF-S02/DF-L06 as a required idempotent reset with raw-evidence preservation, runner-level aggregate startability enforcement, and caller-level rejection before exposure. |
| Natural compactor conversation-history rendering | Reachable on every normal native structured-compaction path | Current `AgentCompactionSummarizer` always builds one conversation-history task prompt; current builder emits `Assistant work notes`, while the planner excludes the existing compacted-memory user unit that influenced later selected history | Requires DF-L08 and direct durable coverage; this is not a synthetic-only helper premise. |
| Oversized generated Work Evidence values | Reachable on the supported generated Work Evidence path | The current product caller requests `AgentWorkTraceProjectionService`; the renderer cleans user/assistant/tool argument/result/error values, and `AgentWorkTraceRedactor` currently applies a silent 20,000-character prefix slice. Normal shell/file-write/read tool activity can exceed that bound. | Requires DF-P11 + DF-L09 coverage and changes only the rebuildable presentation body, not raw storage or file/manifest ownership. |
...
- Eleven primary spines cover activity, UI projection, normal request preparation, pre-dispatch compaction, tool deferral, recurrence, restore, origin lookup, external-runtime evidence, immediate post-response compaction, and generated Work Evidence.
...
- UC-001 through UC-026 are mapped; UC-025 is carried by the normal DF-P04/P05/P06/P10 trigger paths plus DF-L08/DF-L09, while UC-026 is carried by the supported DF-P11 path plus DF-L09 rather than by a local helper alone.

### Principle 3 — Ownership clarity and boundary encapsulation

**Target constraint: Satisfied; current implementation gap explicitly identified.**
...
- The current strategy's direct `MemoryStore.add/pruneRawTracesById` calls before executor validation are an ownership/sequencing defect to remove in implementation design.
...
- Existing archive, snapshot, reporting, tool-safety, and projection capabilities are reused or extended. Generated Work Evidence retains raw-backed authority and its envelope while adopting shared explicit head/tail omission; compaction renders its own selected WorkingContext source.
...
- Generated Work Evidence is already rebuildable, so its presentation change needs no migration.
...
- behaviorally complete for the 26 enumerated use cases;
...
The user approved the original foundation on 2026-07-30, explicitly replaced its persisted-data handling with the clean startup reset/current-only runtime policy, and then fixed the final separation: snapshot owns messages/ranges; derived stores own memory content; successful lineage records own relationships/history; the tail alone is current. SR-004 also assigns reset-result truth, aggregate required-result startability, and fail-closed server exposure to three explicit owners. The package returns through architecture review round 4. Further implementation is paused until review returns `Pass`; the already-present SR-002-derived work must then be reconciled rather than discarded.
