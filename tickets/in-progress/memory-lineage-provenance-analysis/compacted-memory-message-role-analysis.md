...
- Investigation status: Complete; SR-015 aligned. Delivered SR-010 natural-count/audit/message boundary remains unchanged. Pending work narrows migration to exact native snapshots, uses tolerant current-shape per-run conversion with omission rather than repair, preserves external raw-only behavior/ordinary startup semantics, removes raw-history restore, and relocates request recovery after durable compaction.
...
- Related requirements: REQ-007 through REQ-014
- Related acceptance criteria: AC-007 through AC-018
...
8. A valid v5 snapshot owns finalized native continuation messages/local ranges only. After external cleanup, 347 exact metadata-classified native AutoByteus snapshots remain (32,501,775 bytes; v1=1, v3=79, v4=267; zero parse failures/lineage); these are the only conversion targets. Codex/Claude are raw-only, and imported/unsupported/unclassified/conflicting locations are excluded.
...
15. Before SR-010, compactor sizing was duplicated and mechanically constrained in the system prompt, operation builder, parser, normalizer, accepted builder, and lineage record. Delivered SR-010 removed every numeric count gate, including the late lineage gate after output writes.
16. The delivered tight split is: `agent.md` owns natural task/JSON/quality policy; the operation message is renderer history only; parser/normalizer/acceptance/lineage validation enforce structure/per-entry/record safeguards without count caps; `MemoryManager` writes prompt audit version 2; readers preserve supported 1/2 records. Launch/provider configuration is unchanged.
...
19. `promptContractVersion` audits the producing contract. Value 1 truthfully identifies immutable SR-004 fixed-count/duplicated-operation records; delivered SR-010 and current writes use 2. The lineage reader preserves supported 1/2 mixed chains without structural branching or migration.
20. API-REV-007 delivered that natural-count verification through parser/acceptance, output persistence, lineage append/read, exact-head projection, and typed origin membership; SR-014 preserves it.
21. Existing-run restore without a snapshot is not a supported product state: normal creation persists snapshots and explicit restore reads them. The current last-twelve raw projector therefore has no valid premise and must be removed. Explicit restore without a snapshot fails; normal new-run initialization stays separate.
22. A migrated pre-lineage run has absent/empty lineage and no current `compacted_memory` constituent. Valid system messages may remain. A non-system logical unit or complete Tool group is retained only when its current structure is valid and its required stored references resolve truthfully to eligible active raw records. Otherwise it is omitted. Migration creates no raw evidence row, so the first later compaction can consume only retained input already backed by real active evidence.
23. Latest origin captures request recovery before `LLMRequestAssembler`, but that assembler can durably publish pending compaction. A normal provider failure can therefore restore pre-compaction context behind the lineage head. Target capture is post-compaction/pre-request, carried in `RequestPackage`, and settles only WorkingContext/pending state.
24. One exact metadata/location classifier is shared by delivered external cleanup and the native migration. It owns no snapshot action; each migration applies its own explicit runtime policy. Migration `FAILED` retries later without a ticket-global startup exception/rethrow.

## Delivered SR-010 Quality-First Output Contract Evidence
...
The exact delivered boundary is not left to downstream interpretation. `memory-compactor-prompt-content-contract.md` contains the current natural `agent.md` and requires the operation user message to byte-equal one renderer-produced history block with no static prefix or suffix. Current source matches it; this investigation remains evidence for why that owner split is required.
...
Delivered SR-010 completed the rendering delta:

1. `agent.md` is the sole stable task/JSON/quality policy;
...
### Delivered natural-cardinality and audit path

Pre-SR-010 enforcement existed in the system prompt, operation builder, parser, normalizer, accepted builder, and lineage record normalization. The last gate ran during lineage append after archive/output writes. SR-010 removed those upper count limits while preserving every structural/per-entry/lineage invariant.

Current source has no item-count maximum at any of those layers. Prompt contract value 1 remains truthful for immutable SR-004 records; current writes use value 2. Readers accept/preserve supported 1/2 mixed chains, and projection/origin behavior is version-agnostic because the relationship shape is unchanged.
...
### Observed restore failure and bounded transition

The user's built Electron follows `AgentFactory.restoreAgent -> AgentBootstrapper -> WorkingContextSnapshotRestoreStep -> WorkingContextSnapshotBootstrapper` and rejects schema v4. After the completed external cleanup, an exact metadata audit found 347 native snapshots (v1/v3/v4), all parseable and all with active raw. This is a normal continuation path; the former 2,332-file/3.35-GiB mixed scan is historical broad evidence, not the current migration scope.

The pending design keeps the runtime boundary strict: `20260731_migrate_native_working_context_snapshots_v5` converts only exact classified absent/empty-lineage AutoByteus locations, never bootstrap/external/imported/unclassified data. One typed per-run input carries classifier-derived standalone `runId` or team `memberRunId`, source bytes, and bounded same-subject eligible-active reference facts; the pure migration converter alone owns historical decode and message/ref matching. The migration retains valid current-representable system messages and complete non-system units/Tool groups only when their required stored references resolve truthfully to eligible active records. It ignores unknown optional fields and omits unsupported, incomplete, ambiguous, old-compacted-memory, or unsourced non-system units. Parse-invalid or fully omitted input becomes strict v5 with `messages: []`. It generates no recovery text, placeholder, synthetic Tool result, baseline/repair raw row, or other raw mutation. It finalizes and validates the complete candidate before replacing the snapshot, and only then deletes obsolete episode/semantic/manifest files. Every nonempty-lineage location is skipped byte-for-byte without state validation. An already-valid strict-v5/no-lineage natural snapshot is retained byte-for-byte only when it has no compacted-memory constituent and every non-system logical unit is backed by truthful eligible active records; otherwise it follows tolerant conversion. The old migration ledger ID cannot suppress the new migration.
...
9. the real startup path with delivered external cleanup first and one exact classifier: prove only AutoByteus locations reach conversion; excluded classes remain byte-for-byte untouched; representative native v1/v3/v4/natural-v5/any-nonempty-lineage/no-snapshot cases preserve/convert/skip correctly; validated v5 precedes exact cleanup; completed degradation returns warnings and restores/continues; filesystem behavior remains the existing runner's concern; isolated roots protect product ledger; strict restore never replays raw history;
...
15. pending compaction -> durable C(n) -> recovery capture -> current request -> provider failure, proving restore retains C(n) context/v5/cleared pending/archive/output/lineage and removes only request mutation; plus local assembly failure, pre-capture compactor failure, success, and retained interruption settlement.

The durable normative scenarios are SCN-003 through SCN-009, SCN-013 through SCN-016, and SCN-019 through SCN-021 in `memory-context-and-lineage-contract.md`. SCN-017 and SCN-018 remain reserved for the downstream API-REV-006 evidence already recorded in that package.
...
The structural questions are resolved in `design-spec.md`:
...
6. `RuntimeMemoryLocationClassifier` owns exact current metadata/location identity; external cleanup and exact-ID native conversion own separate actions. The native migration plus focused converter own only AutoByteus historical projection/validated v5/exact cleanup; normal bootstrap remains strict-v5-only. The destructive reset and `WorkingContextRecoveryProjector` are removed; `CompactedMemorySchemaGate`, global compacted-memory manifest authority, historical runtime dictionary access, and old snapshot readers remain absent.
...
8. `LLMRequestAssembler` owns post-compaction/pre-request recovery capture; `LlmPhase` owns provider settlement; `LlmRequestRecoveryBoundary` owns only the ephemeral WorkingContext/pending copy and one-settlement invariant.
