...
The framework's central code vocabulary must also let a developer infer concrete responsibilities without first reconstructing the dependency wiring. Server assembly, the live application runtime, process-scoped Agent Tools infrastructure, scoped session management, run supervision, shutdown coordination, and bind-once cycle breakers must use distinct familiar role nouns. This is a behavior-neutral readability correction: it must not change routes, packages, persistence, runtime identity, lifecycle order, or application behavior.
...
| BEH-004 | API-REV-009/010 prove clean Studio and standalone Brief resolve Codex/Luna defaults, create application-runtime-scoped researcher/writer runs, authenticate to the Agent Tools route, execute `publish_artifacts`/`send_message_to`, complete handoff, and project artifacts through the exact application runtime. | Preserve the complete application execution and return path while renaming its central internal owners by concrete responsibility. | Launch/readiness, capability tokens, tool projection, binding/events/artifact owners, and the external-gateway boundary remain unchanged. Runtime-internal tooling remains outside this ticket. | REQ-004, REQ-005, REQ-007, REQ-009 / AC-005, AC-006, AC-010, AC-014–AC-016, AC-018 |
| BEH-005 | Both servers register `/mcp/agent-tools/:sessionId`; one process-scoped MCP subsystem supplies the route/session registry, catalog, executor, and dispatcher, while application-runtime sessions carry the exact publisher. Shutdown/restart behavior passes. | Preserve that ownership, but replace overlapping `Composition`, `Graph`, and `Authority` code names with explicit server/runtime/manager/supervisor/coordinator roles. | One internal route, existing bearer/401/404 semantics, Studio-only `/mcp/gateway`, selected-app browser surface, and no user-auth/server fork remain unchanged. | REQ-005, REQ-007, REQ-009 / AC-003, AC-009, AC-010, AC-016, AC-018 |
...
| BEH-009 | The central implementation uses `Composition`, `Graph`, `Authority`, `Runtime`, and `Port` for materially different server assembly, runtime-service result, session lifecycle, run lifecycle, shutdown, and bind-once proxy roles. A new developer must trace implementations to discover those differences. | Use one bounded familiar vocabulary and a clean current-to-target map so each central type/factory/file exposes its concrete role and scope. Reserve `composition` for the assembly activity/root and `dependency graph` for design explanation, not returned live objects. | All passed functional behavior, exact dependency identity, public wire contracts, stored data, lifecycle order, and package bytes remain unchanged; no repository-wide mechanical rename or compatibility aliases are introduced. | REQ-009 / AC-018 |

## Investigation Findings

- The functional dual-host design is architecture-approved through `ARCH-REV-008`, implemented through `IR-015`, source-reviewed as Pass through `CRR-026`, and API/E2E-validated as Pass at 98.3% through `API-REV-010`; proportional durable-test review also passes in `CRR-027`.
- Real Studio and standalone journeys now validate Codex/Luna defaults, create application-runtime-scoped researcher/writer runs, authenticate Agent Tools sessions, execute publication and recipient-name messaging, complete handoff, project artifacts, restart cleanly, and preserve canonical package bytes.
- `CRR-028` does not reopen those behaviors. It finds a cross-cutting naming design impact: the central vocabulary does not distinguish server assembly, the live application runtime, process MCP infrastructure, scoped session management, run supervision, shutdown coordination, and bind-once proxies.
...
- Current source responsibility tracing shows that `StudioServerComposition` is a returned assembled server, `ApplicationPlatformRuntimeGraph` is a flat live-service result, `AgentToolsMcpProcessAuthority` is a process MCP subsystem, `ApplicationAgentToolsSessionAuthority` manages a scoped session collection, `GeneralProcessRunAuthority` supervises general run managers, `ApplicationRunShutdownAuthority` coordinates ordered stopping, and `DeferredPublishedArtifactPublicationPort` is a bind-once publisher proxy.
- The root package export exposes `buildStudioServerComposition`/`StudioServerComposition`, but `autobyteus-server-ts` is private and a repository-wide scan found no consumer outside the server project. The remaining central names are internal. No serialized contract, route, database schema, manifest, environment key, or package artifact contains these symbols.
- The correct response is one clean internal/public-package rename pass with import/file/test/doc updates and no aliases. It is not another runtime implementation or structural redesign.
...
| --- | --- | --- | --- | --- | --- |
| [sources/autobyteus-vertical-application-developer-experience-proposal.md](sources/autobyteus-vertical-application-developer-experience-proposal.md) | User-supplied proposal snapshot | REQ-001–REQ-009 | AC-001–AC-018 | Input source; not accepted wholesale | Original vision and claims evaluated by the task. |
| [proposal-critical-analysis.md](proposal-critical-analysis.md) | Claim/evidence/readiness assessment and revised roadmap | REQ-001–REQ-009 | AC-001–AC-018 | Approved/refined through 2026-07-30; corrected through SR-011 | Supplies the bounded dual-host recommendation and behavior-neutral naming correction. |
| [design-self-validation.md](design-self-validation.md) | Reachable scenario, spine, and canonical-principles validation | REQ-001–REQ-009 | AC-001–AC-018 | Complete through SV-016; approval `N/A` | Records use-case/principles validation and the bounded responsibility-to-name mapping without replacing requirements/design authority. |
...
- Evidence basis: the functional boundaries pass, but the same abstract suffixes describe a server result, live runtime bundle, process MCP owner, scoped session manager, general-run lifecycle owner, shutdown coordinator, and bind-once proxy. The user's direct comprehension feedback and CRR-028 source trace contradict the previous self-descriptive naming assessment.
- Required response: adopt the bounded role vocabulary and exact rename map in the design spec. Use `Server`, `Runtime`, `Manager`, `Supervisor`, `Coordinator`, `Service`, `Publisher`, `Handler`, `Registry`, `Gateway`, `Store`, and `Resolver` only for their stated roles. Retain `composition` only for top-level assembly and `dependency graph` only for architectural wiring explanations.
...
15. Break the application-runtime construction cycle with one `BindOncePublishedArtifactPublisher`: create it before runtime factories/session issuance, construct the application run manager and publication service, bind exactly once before lifecycle readiness, reject before bind/after close, revoke the scope’s sessions during stop, then close the publisher.
...
19. Apply the exact current-to-target name/file map as a behavior-neutral clean cut. Building `ApplicationPlatformRuntime` prepares managers, factories, and services but starts no new agent/team run; only an application business request or legitimate recorded-run recovery creates/restores execution.
...
- **AC-018:** The reviewed current-to-target naming map is implemented as a clean cut across symbols, filenames, imports, root exports, tests, diagrams, and affected module/developer documentation. `buildStudioServer` returns `StudioServer` with `fastify` and `applicationRuntime`; `buildApplicationPlatformRuntime` returns `ApplicationPlatformRuntime`; process Agent Tools infrastructure is `AgentToolsMcpRuntime`; scoped sessions are managed by `ScopedAgentToolMcpSessionManager` and carry `AgentToolMcpSessionExecutionCapabilities`; general runs use `GeneralProcessRunSupervisor`; application run stopping uses `ApplicationRunShutdownCoordinator`; cycle breakers are `BindOncePublishedArtifactPublisher` and `BindOnceApplicationEngineEventHandler`; internal builders return `*Services`, not `*Authorities`. A structural test proves runtime construction creates no new agent/team run, while existing business-demand and legitimate-recovery paths still create/restore runs. Old names/files are absent, no compatibility aliases remain, and TypeScript plus retained Studio/standalone behavior and route/package/persistence contracts pass unchanged.

## Constraints / Dependencies

- Reviewed implementation base: functional implementation and durable tests pass through `IR-015`, `CRR-026`, `API-REV-010`, and `CRR-027`; current naming-review HEAD is `548b55833` (`CRR-028`). The ticket branch remains behind the latest tracked `origin/personal`; delivery owns final refresh/integration after the naming loop passes.
...
- Naming changes are source-only. They must not alter serialized fields, route paths, HTTP/WebSocket/MCP payloads, environment variables, database tables/rows, manifest/package bytes, or generated application contracts.
...
- Lifecycle stop revokes application-scoped Agent Tools sessions before closing their publisher and before application-runtime publication/event owners are disposed. Restart creates a new scope; no session or bind-once publisher survives process restart.
...
The later functional implementation is passed through API-REV-010. The user then directly identified that `createApplicationPlatformRuntimeGraph` and “creates agent/team execution” were too difficult to understand and clarified the intended meaning: one host-level set of application services is prepared at startup, while actual agent/team runs are created only on business demand or legitimate recovery. CRR-028 formalizes that developer-comprehension requirement without changing product behavior. SR-011 selects the exact bounded vocabulary/rename map and requires architecture review before implementation resumes.
