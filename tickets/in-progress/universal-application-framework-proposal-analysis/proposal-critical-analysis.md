...
- Approval applicability: Approved/refined through 2026-07-30. User/account concerns are excluded; application-folder commands, complete package defaults, and optional Studio overrides remain as approved. SR-009 clarifies the internal Agent Tools/general gateway/application-owned MCP boundary and makes runtime-internal tooling explicitly out of scope.
...
The original proposal was **not implementation-ready**. The bounded architecture through complete package defaults, three-state readiness, invalid host-override handling, selected-resource sparse editing, recursive portable policy, and graph-local prompt construction has since passed review and been implemented. The current proof is not complete because standalone advertises the existing Agent Tools MCP session callback to Codex/Claude but does not register that route. The missing route blocks eligible server-owned tools such as publication and team messaging, so the real Brief run cannot finish its handoff/artifact path. Runtime-internal tooling is outside this correction. At the original investigation baseline, the proposal treated several absent or partial capabilities as small adaptations:
...
| Dual-host conformance | Partial and substantial | Selected-resource/policy/prompt/launch cases now pass. API-REV-005 proves standalone run/session creation but fails eligible server-tool dispatch and artifacts because the route is absent. CRR-016 corrects the mistaken assumption that package `toolNames` equals the Agent Tools descriptor. | Register the existing route and rerun the exact server publication/message/handoff/artifact path before declaring proof complete; do not add unrelated runtime-internal scope. |
...
Run provisioning issues a session descriptor for eligible server-owned Agent Tools and selected available MCP-origin tools at `/mcp/agent-tools/:sessionId`. Studio registers that bearer-protected callback; standalone currently advertises it but returns generic 404. This internal route is different from the general external-client `/mcp/gateway`. Runtime-internal Codex/Claude tooling is outside this correction.

**Correction:** reuse the working subsystem unchanged. Import and await the existing `registerAgentToolsMcpRoutes(app)` in standalone before static/SPA registration, exactly as Studio already does for the internal route. Preserve existing descriptor/session/catalog/dispatcher/adapters/auth/base-URL/cleanup behavior. Validate the actual descriptor/`tools/list`: `publish_artifacts` and `send_message_to` are eligible server-owned tools, while a selected `ToolOrigin.MCP` tool may appear only when available in the current host registry. Do not add a second route, aggregate runtime, new ports, publication bridge, compatibility fallback, user-auth subsystem, external gateway to standalone, or unrelated runtime-internal work.
...
16. Both hosts register the existing Agent Tools session callback before static fallback. The callback dispatches eligible server-owned tools such as `publish_artifacts` and `send_message_to` plus selected MCP-origin tools already available in the current host. Team handoff, events, and artifacts complete, while general `/mcp/gateway` remains Studio-only.
...
| Current package as universal proof artifact | Accept after bounded correction | Launch/edit/prompt boundaries pass; the proof still needs standalone registration of the existing internal Agent Tools route and the server-tool/handoff/artifact path. |
...
The implemented macro and launch/edit/prompt slices show that AutoByteus can support a second host. Completion now depends on one bounded composition fix: call the existing Agent Tools route registrar in standalone before static fallback, preserve the existing server/configured-MCP projection, and complete the real standalone publication/message/team/artifact journey. Do not redesign the working Agent Tools subsystem, expand into runtime internals, expose the general gateway, or inherit Studio MCP state. The broad-suite `APIE2E-REPO-005` diagnostic remains separate and unattributed.
