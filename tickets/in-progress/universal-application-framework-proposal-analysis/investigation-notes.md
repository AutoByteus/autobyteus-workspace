...
- Current Status: The functional foundation and SR-011 naming correction pass through `ARCH-REV-009`, `IR-016`, source `CRR-029` Pass / 97, `API-REV-011` Pass / 98.9%, and `CRR-030` Pass. `ARCH-REV-010` confirms SR-012 resolves CR-019 and CR-020 in design, but returns `Fail — Design Impact` for two bounded CR-021 lifecycle edges (`AR-008`, `AR-009`): artifact delivery must retain worker ensure/restart, and inactive-run pruning must retain exact cleanup without a reverse callback. SR-013 defines the correction; implementation and delivery remain paused pending architecture re-review.
...
- Scope Summary: Preserve every passed dual-host, package-default, route/session, publication/messaging, gateway-separation, runtime identity, lifecycle, storage, recovery/remount, shutdown, and exact `73/73` package-parity behavior. Retain the architecture-approved four runtime projections and package ownership split. Complete CR-021 through a closed artifact-delivery queue/service with launcher ensure-before-controller invoke and an early session-scope/run-resource/exact-run registry chain. No route/schema/data change, generic container/event bus/deferred handler, reverse callback, compatibility alias, host-mode builder, or repository-wide singleton rewrite.
...
- Remote Refresh Result: Task branch was created from `origin/personal` at `d6983612c5a77fb94d9266df85a9d03fe2d1c68b` on 2026-07-26 and refreshed before implementation to `6caf809303294252c109420b238588f0c68aca6a`. SR-012 is commit `0cc258659554801898e4d2ef94860143d196dcb1`; ARCH-REV-010 is recorded in reviewer-owned dirty artifacts. Delivery-owned reports/evidence and devkit output remain dirty/untracked, so solution design will stage only its own artifacts. Delivery owns final tracked-base refresh/integration after the architecture/implementation loop passes.
...
The current result remains `Fail — Design Impact` only until architecture reviews SR-013. CR-019 and CR-020 are resolved in design. The bounded CR-021 target now preserves worker ensure/restart through a closed artifact-delivery queue/service and exact cleanup through early session-scope/run-resource/registry owners. Implementation remains paused.
...
3. **The outward-boundary gap is resolved in design:** ARCH-REV-010 confirms the four exact runtime projections and private-owner encapsulation.
4. **The package temporal-coupling gap is resolved in design:** ARCH-REV-010 confirms separate registry/commands/reconciliation/refresh owners and acyclic construction.
5. **The remaining bounded CR-021 gap is artifact delivery:** SR-012's controller-only relay omitted current ensure/restart. SR-013 inserts a closed queue and late delivery service that calls launcher ensure before controller invoke.
6. **The remaining bounded CR-021 gap is run cleanup:** SR-012's later manager callback was cyclic/underspecified. SR-013 constructs an early session scope and run resource manager under an identity-checked active registry.
...
9. **Scope remains bounded:** the work restructures only central boundaries participating materially in the reviewed Studio/standalone spines. Routes, wire contracts, package/database schemas, provider behavior, Agent Tools security, business behavior, and unrelated repository structure do not change.
...
- Initial evidence baseline was `origin/personal` at `d6983612c5a77fb94d9266df85a9d03fe2d1c68b`; the implementation/design-review baseline was refreshed to `origin/personal` at `6caf809303294252c109420b238588f0c68aca6a` on 2026-07-29. The current solution baseline is SR-012 commit `0cc258659554801898e4d2ef94860143d196dcb1` plus reviewer-owned ARCH-REV-010 artifacts; fixed behavior passes through `CRR-029`, `API-REV-011`, and `CRR-030`.
- Existing Studio behavior must remain the first authoritative host path.
- The same-package/two-host structure and SR-011 vocabulary are implemented and passed. SR-013 must preserve the complete API-REV-011 characterization baseline through full source review and API/E2E, not only proportionate structural checks.
...
`ARCH-REV-010` returns only AR-008/AR-009 after confirming CR-019/CR-020 resolved in design. SR-013 is intentionally behavior-neutral:
...
- construct the application MCP session scope, queue-backed relay, `AgentRunResourceManager`, `ActiveAgentRunRegistry`, concrete publisher, scoped session issuer, then run managers; preserve identity-checked removal and exact session/file/artifact/memory cleanup without a manager callback;
- construct `ApplicationEngineController` plus the two closed queues early and `ApplicationEngineLauncher` after orchestration/streaming; the artifact-delivery service always ensures before controller invoke, while journal append precedes the application-ID event queue whose dispatcher retains ensure/ack/retry/backoff/resume/stop;
- remove both `BindOnce*` files, `ApplicationEngineHostService`, manager-owned active/disposer maps, late-assigned Studio callbacks, 19-field outward exposure, and application-path optional/global branches; do not add aliases, reverse callbacks, or another deferred container;
...
- validate exact contract keys/import boundaries, package success/failure sequencing, worker-exit-before-publication, all exact-run removal origins and duplicate identity, event timing, real Studio/standalone Codex/Luna journeys, restart/recovery/remount, cleanup, and `73/73` parity;
