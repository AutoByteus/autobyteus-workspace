...
- Current Status: SR-006 passed as `ARCH-REV-006`; its implementation progressed through IR-008/IR-009 and source review passed as `CRR-014`. API/E2E round `API-REV-005` proves clean standalone package defaults/readiness/binding/team launch but fails because standalone omits the existing Agent Tools MCP session route. `CRR-016` supersedes `CRR-015`: the gateway subsystem is already established and `CR-013` is a bounded `Fail — Local Fix`. SR-007 is withdrawn; SR-008 restores the approved boundary and routes the composition fix to implementation. SR-009 records the user’s final MCP ownership clarification and excludes all Codex/Claude runtime-internal tooling from this ticket’s requirements, source changes, and executable proof.
...
- Scope Summary: SR-008 corrected the unapproved SR-007 premise and preserved the ARCH-REV-006 design. SR-009 clarifies MCP ownership and runtime-tool exclusion without changing the Local Fix. API/E2E tests, evidence, and other roles’ artifacts remain preserved for their owners.
...
- Remote Refresh Result: Task branch was created from `origin/personal` at `d6983612c5a77fb94d9266df85a9d03fe2d1c68b` on 2026-07-26 and refreshed before implementation to `6caf809303294252c109420b238588f0c68aca6a`. Architecture passed SR-006; downstream implementation/review/API work now places task `HEAD` at `e8e06afdd` (`fix(standalone): register agent tools mcp route`). That implementation-owned result is not presumed passed here. `origin/personal` has independently advanced. API/E2E-owned uncommitted tests/evidence and other roles’ artifacts are preserved, so delivery—not solution design—owns final tracked-base refresh/integration.
...
The authoritative evidence is [design-self-validation.md](design-self-validation.md). The prior passes validated the dual-host macro architecture, package-default/readiness/prompt authority, invalid saved overrides, selected-resource sparse editing, and recursive portable policy. SV-013 corrects the withdrawn SV-012/SR-007 premise using CRR-016; SV-014 records the user-confirmed MCP ownership and runtime-internal scope boundary.
...
The current result is `ARCH-REV-006 remains the approved design; CR-013 is a Local Fix`. The existing Agent Tools subsystem remains authoritative. Standalone adds its existing route registrar before static fallback, makes no Codex/Claude runtime-internal tool change, and leaves the general external-client MCP gateway Studio-only.
...
| BEH-004 | Resource / Runtime | App resolves a required slot, starts a real team run, and its runtime receives eligible server/configured-MCP tools | Launch authority -> binding/team run -> established session descriptor -> `/mcp/agent-tools/:sessionId` for eligible server/MCP tools -> handoff/artifact | Launch/readiness pass. Studio has the route; standalone returns platform 404, so `publish_artifacts`/`send_message_to` and the product completion path are blocked. Runtime-internal tools are unchanged upstream behavior. | API-REV-005 / CRR-016; adapter providers, configured-MCP resolver, route, and standalone composition |
...
| BEH-006 | Developer / Operational | Developer runs maintained `dev`/`dev:studio`/`build`/`validate`/`start` | Devkit mapping -> package validator -> real host -> eligible server-tool callback | Defaults, portable policy, and application-folder commands pass through launch; standalone `dev` stalls only because the existing callback route is absent. Runtime-internal tools remain outside the change. | API-REV-005 / CRR-016; Brief standalone log and route regression |
...
2. **The real tool-dependent run cannot complete:** eligible server-owned `publish_artifacts` and `send_message_to` cannot use the absent route; Brief remains incomplete with zero projected artifacts.
3. **The tool boundary was misread in API-REV-005/CRR-015:** package `toolNames` are not the descriptor-enabled list. Only eligible static adapters and selected available `ToolOrigin.MCP` tools are projected into Agent Tools MCP. Runtime-internal tools are untouched and outside scope.
4. **Internal and external MCP surfaces remain distinct:** the per-session bearer callback is existing application-run infrastructure; `/mcp/gateway` is a process-level external-client view of host-configured MCP-origin tools and must not enter standalone.
5. **Studio MCP configuration is not an application dependency:** standalone does not inherit it. Application-owned MCP resource declaration/provisioning is useful follow-up work, not a hidden extension of this proof.
6. **No new design impact remains:** CRR-016 verifies the gateway subsystem is working and classifies only standalone route registration as Local Fix. Earlier SR-006/ARCH-REV-006 decisions remain authoritative.
...
- do not add `AgentToolsMcpRuntime`, new ports, a path builder, a deferred publication bridge, runtime-tool changes, compatibility aliases, or external `/mcp/gateway`;
- preserve the existing session service, registry, catalog, dispatcher, authorization gates, Codex/Claude materializers, and Studio behavior;
...
- after source review, API/E2E owns correction of the over-broad tool expectation and must inspect the actual descriptor/`tools/list`, require Brief `publish_artifacts` and `send_message_to`, and rerun the server-publish/message -> handoff/artifact journey without modifying or making a new acceptance claim about runtime-internal tools;
