...
`Refined` — approved by the user on 2026-07-28 after `repository_prisma@1.0.9` publication and revalidation against the latest `origin/personal`.
...
- `REQ-004` Secret persistence must expose model-specific `BaseRepository` subclasses for `SecretEntry` and `SecretEncryptionMetadata`. The authoritative vault persistence coordinator must compose them and preserve the existing explicit initialization repository contract, record transformations, metadata singleton validation, batch counts, domain checks, receipt ownership, and exact-row compensation.
...
- `AC-005` Source/structural review proves secret entry and metadata repositories each extend the correct `BaseRepository` model, the coordinator alone owns cross-model sequencing, and no secret runtime/service/bootstrap/provider caller accepts or retains a raw Prisma client/transaction delegate.
...
- `AC-012` Final source/package scan finds no compatibility wrapper, dual repository implementation, local pnpm patch/link, vendored library copy, token/secret raw-client factory path, obsolete transaction-delegate plumbing, or restored file-first repositories.
