...
import {info,evidence} from './api-live-utils.mjs';
const upstream=new URL(info.serverUrl);
const server=http.createServer(async(req,res)=>{
 const chunks=[];for await(const chunk of req)chunks.push(chunk);const body=Buffer.concat(chunks);
 let fault={};try{fault=JSON.parse(await fs.readFile(evidence+'/api-metadata-fault.json','utf8'));}catch{}
 ...
 if(payload?.query?.includes('workspaceMetadata(')&&payload.variables?.rootPath===fault.rootPath){
  await fs.appendFile(evidence+'/api-proxy-requests.jsonl',JSON.stringify({time:new Date().toISOString(),operation:op,rootPath:fault.rootPath,fault:true})+'\n');
 ...
 if(payload)await fs.appendFile(evidence+'/api-proxy-requests.jsonl',JSON.stringify({time:new Date().toISOString(),operation:op,variables:payload.variables})+'\n');
...
server.listen(0,'127.0.0.1',async()=>{const url='http://127.0.0.1:'+server.address().port;await fs.writeFile(evidence+'/api-proxy-url.txt',url);console.log(url);});
