...
- Purpose: provide the reviewable target architecture and data-flow spines in Mermaid for the in-process Local backend, authenticated Store/key pairing, physically separate host default/real-E2E Stores, unchanged Docker persistence, first-delivery backend configuration, LLM construction, host real E2E, Claude two-mode authentication, and future extension boundary.
- Scope: intended behavior and architecture for REQ-001 through REQ-018 and AC-001 through AC-018.
- Status: `User Approved — AR-007 / MP-002 Evidence Reassessment; Architecture Re-review Requested`.
- Approval applicability: approval required as part of the requirements/design basis.
...
7. The real-E2E Store never reads, copies, inherits, or falls back to the default Store. First-delivery setup accepts dedicated test credentials directly and is constructed with only the E2E target.
...
9. Raw credentials do not enter `LLMConfig`; `LLMFactory` creates an ephemeral `LLMConstructionContext` from effective config plus resolved authentication.
10. Storage relocation is paired with built-in file-tool denial, empty-base child environments/descriptors, and honest `LOCAL_HARDENED` assurance. Strong same-user/process isolation is deferred.
...
There is no profile, product owner, parent, inheritance, value hash, timestamp, or caller-visible storage revision. `store_id` is a random public physical-pair binding, not a product/profile identity. Each Store has an independently generated root key. Setup writes a dedicated test credential directly to the E2E target and has no source/default Store dependency.
...
sequenceDiagram
  ...
  Factory-->>Provisioning: provider ID and authentication requirement
  Provisioning->>Manager: resolveForUse(semantic consumer and credential slot)
...
A first machine, missing provider, expired/revoked key, or unavailable Store is a legitimate one-time/repair dependency. A new host worktree is not. The setup service has no default-Store input, read, copy, or fallback. Host provider execution opens the shared E2E Store read-only; secret CRUD tests receive a separate disposable writable Store. Existing Docker deployments use their own persistent node-local Store and are not changed by this E2E flow.
...
This diagram proves the first-delivery `LOCAL_HARDENED` boundary only. Built-in file tools cannot traverse, symlink, or directly address Store files, and all listed children receive empty-base environments/descriptors. The exact managed Claude child is the sole explicit provider-key exception: it receives only its catalog-authorized Anthropic key, while its parent, siblings, other runtime children, and AutoByteus tool children do not. Managed-mode setting/tool controls remove supported process/environment-inspection descendants. Because the server and an agent child may still run as the same OS user or in the same container—and the authorized Claude executable can inspect its own environment—the design does **not** claim arbitrary native/shell/process secrecy. `STRONG_AGENT_ISOLATION` remains future work and is never reported by first-delivery code.
...
sequenceDiagram
  ...
  alt auto, api-key, or unknown
    ...
    alt mode omitted or cli
      ...
      Client->>Client: build empty-base CLI/account env
...
The exact Claude consumer and native `AnthropicLLM`/metadata consumers are independently authorized to the same stored definition. CLI mode never resolves it. Managed mode loads no user/project/local settings, hooks, plugins, API-key helper, or external MCP configuration; passes `tools: []`; uses strict explicitly materialized AutoByteus in-process MCP tools only; and does not accept caller `env`. Parent, siblings, unrelated children, and AutoByteus-owned tool children remain key-free. Missing/non-ready custody, invalid binding, spawn, or provider-auth failure returns an exact value-free code without fallback. The authorized Claude process/SDK can observe and retain its own key; this is the explicit recipient trust limit.
...
- [ ] Direct dedicated E2E provisioning only, with no source/default read/copy contract, is accepted.
...
- [ ] UC-001–UC-017 spines and removal of unsupported generic scope/address/version attributes are accepted.
