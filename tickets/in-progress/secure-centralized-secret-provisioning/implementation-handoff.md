...
| `BEH-006` | Deployment-neutral early bootstrap below `serverDataDir` | `app-config.ts`; `prisma-client-factory.ts`; lazy Prisma repositories/databases; `startup/migrations.ts`; sanitized built bootstrap smoke; direct/Electron server entrypoints | Private persisted config explicitly supplies validated SQLite state to migration and in-process Prisma consumers only when runtime database work begins. Built production modules/bootstrap import with no `DATABASE_URL`, and the built direct server still starts twice from one data directory with no parent URL. Docker Compose/launcher/port/volume files remain unchanged. |
...
- Focused CR-009/CR-010 config/Prisma/app-data suite — 7 files / 44 tests passed, including persisted private SQLite configuration, non-SQLite rejection, explicit child delivery, engine override preservation, zero import/constructor acquisition across all four owners, exactly-on-first-operation acquisition, app-data migration runners, and token-usage repository access.
