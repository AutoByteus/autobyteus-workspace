...
| `BEH-006` | Deployment-neutral early bootstrap below `serverDataDir` | `app-config.ts`; `prisma-client-factory.ts`; lazy Prisma repositories/databases; shared token-usage Prisma owner; `startup/migrations.ts`; sanitized built bootstrap smoke; direct/Electron server entrypoints | Private persisted config explicitly supplies validated SQLite state only when runtime database work begins. Built modules/bootstrap import with no `DATABASE_URL`; request-scoped token-usage stores reuse one lazy process-lifetime client; migration-owned clients disconnect deterministically; and the built direct server still restarts from one data directory with no parent URL. Docker/launcher topology remains unchanged. |
...
- The local built direct-launch restart/reopen path and sanitized built-module/bootstrap smoke now pass. The independently failing Round-2 clean Docker evidence predates CR-010; clean source-image build, container start, same-volume persistence/restart/removal, cross-platform ACL/owner behavior, busy contention, and single-Pod/PVC restart/reopen still need downstream rerun/evidence.
...
- `pnpm --filter autobyteus-server-ts run build` — passed, including shared/core build, Prisma generation, TypeScript compilation, and the built-in-agent/bootstrap smoke in a sanitized child with no `DATABASE_URL`.
- Focused CR-009/CR-010/CR-011 config/Prisma/app-data suite — 7 files / 47 tests passed, including persisted private SQLite configuration, non-SQLite rejection, explicit child delivery, engine override preservation, zero import/constructor acquisition, one shared client across multiple default token-usage repository/store instances, injected-client non-ownership, migration-owned disconnect, app-data migration runners, and token-usage repository access.
- `pnpm --filter autobyteus-server-ts exec vitest run tests/integration/token-usage/providers/token-usage-store.integration.test.ts` — passed 7/7 against the migrated test database, exercising the normal token-usage store path with the bounded shared default client owner.
...
Implementation-scoped builds, focused unit checks, the sanitized built import/bootstrap regression, the bounded built-server restart regression, and frontend self-validation are complete. API/E2E-owned Round 1/2 durable changes and evidence are preserved. After source review passes, `api_e2e_engineer` must rerun the clean source-image build, container start, and same-volume restart/persistence/removal path plus the applicable matrix, then refresh execution confidence/evidence. The dedicated real-E2E Store is still unavailable, so real provider execution remains unclaimed.
