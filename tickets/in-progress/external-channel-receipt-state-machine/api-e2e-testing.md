...
- Focus: executable proof for the final authoritative external-channel architecture in `autobyteus-server-ts`
...
| `AC-002` | `R-003`, `R-004`, `R-005` | a multi-leg turn produces one final externally published reply | `AV-002`, `AV-006` | Passed | 2026-04-09 |
| `AC-003` | `R-003`, `R-004`, `R-006` | delayed or unavailable live observation resumes from durable receipt state without misbinding | `AV-003`, `AV-007`, `AV-008`, `AV-010` | Passed | 2026-04-09 |
| `AC-004` | `R-005`, `R-007` | duplicate inbound delivery plus direct/team variants remain idempotent | `AV-004` | Passed | 2026-04-09 |
...
| --- | --- | --- | --- | --- | --- |
| `DS-001` | Primary End-to-End | `ChannelIngressService -> Channel*RunFacade -> ReceiptWorkflowRuntime` | `AV-001`, `AV-002`, `AV-004`, `AV-007`, `AV-009`, `AV-010` | Passed | ingress-to-publish flow is proven with exact dispatch binding on the primary path, including repeated distinct inbound messages on the same thread/run and restore-after-termination reuse of the bound run |
| `DS-002` | Return-Event | `Channel*RunReplyBridge` | `AV-003`, `AV-004`, `AV-007` | Passed | known-turn live observation remains adapter-only and feeds the receipt runtime |
...
| `tests/integration/api/rest/channel-ingress.integration.test.ts` | API Test | Yes | `AV-001`, `AV-002`, `AV-004`, `AV-006`, `AV-009`, `AV-010` | covers the real ingress route, direct/team dispatch, final external publication behavior, repeated distinct inbound messages on the same thread/run, and restore-after-termination reuse of the bound run |
...
- Commands executed:
  ...
  - `pnpm -C autobyteus-server-ts exec vitest run tests/unit/external-channel/runtime/channel-dispatch-lock-registry.test.ts tests/unit/external-channel/runtime/channel-agent-run-facade.test.ts tests/unit/external-channel/runtime/channel-team-run-facade.test.ts tests/unit/external-channel/runtime/receipt-workflow-runtime.test.ts tests/unit/external-channel/services/channel-ingress-service.test.ts tests/unit/external-channel/services/channel-message-receipt-service.test.ts --reporter=dot`
...
- Result summary:
  - targeted ingress integration file: `1/1` file passed, `8/8` tests passed
  - retained focused slice from round 4: `6/6` files passed, `29/29` tests passed
  - broad slice rerun: `24/24` files passed, `111/111` tests passed
  - retained build from round 4: `Pass`
- Notes:
  ...
  - authoritative validation now matches the final architecture exactly: receipt runtime owns the durable workflow, external-channel facades own dispatch-scoped capture plus same-run serialization, known-turn recovery stays adapter-only, runtime/core event schemas remain generic, repeated distinct inbound messages on the same thread/run remain truthful without manual run reset, and a terminated bound run restores cleanly on the next same-thread inbound message
