...
- **Focused validation:** `git diff --check` passed. `pnpm -C autobyteus-ts exec tsc -p tsconfig.build.json --noEmit` passed. Broader server typecheck remains blocked by unrelated generated Prisma-client export errors; no changed-path diagnostics were reported.
