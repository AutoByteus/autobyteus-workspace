...
- 2026-03-08: Published prerelease tag `v1.2.26-rc2`, confirmed the artifact contains the first direct-run fix, and then isolated a second Local Fix: temp installs under `/var` still exited before `main()` because direct-run detection compared non-canonical `/var/...` against Node-resolved `/private/var/...` paths.
- 2026-03-08: Applied a realpath-based direct-run check in `autobyteus-message-gateway/src/index.ts`, bumped the desktop/package release target to `1.2.26-rc3`, reran `pnpm -C autobyteus-message-gateway build`, `node autobyteus-message-gateway/scripts/build-runtime-package.mjs --skip-build --release-tag v1.2.26-rc3`, `pnpm -C autobyteus-server-ts build:file`, and `pnpm -C autobyteus-server-ts test --run tests/e2e/messaging/managed-messaging-gateway-graphql.e2e.test.ts tests/e2e/messaging/managed-messaging-gateway-update-graphql.e2e.test.ts`.
- 2026-03-08: Pre-publish `rc3` proof passed by extracting the generated runtime tarball into a fresh temp directory and starting it through `MessagingGatewayProcessSupervisor.start()`, which now succeeds and reaches health even from `/var`-backed install paths.
- 2026-03-08: Published prerelease tag `v1.2.26-rc3`; GitHub Actions run `22826096208` completed successfully and published the gateway tarball, checksum, metadata JSON, and `release-manifest.json` for the repaired artifact.
- 2026-03-08: Remote release E2E passed against the live `v1.2.26-rc3` manifest URL by driving the real GraphQL enable/status/disable flow through `buildGraphqlSchema()` and confirming the server downloaded the published asset, started it under supervision, reported `RUNNING`, surfaced diagnostics including the bind port, and then shut it down cleanly.
...
- Current environment constraints (tokens/secrets/third-party dependency/access limits): `No third-party provider tokens were required because Stage 7 used fake runtime archives for provider-facing paths and the remote release E2E validated only the managed download/install/start/disable flow against live GitHub-hosted assets.`
- Best-available compensating automated evidence: `GraphQL E2E exercises manifest resolution, archive download, checksum verification, extraction, process startup, disable, and rollback; targeted Nuxt component/store regressions cover the UI-owned lifecycle and managed-only flow; the live rc3 GitHub release was published and then consumed by a real managed GraphQL enable/status/disable script against the published manifest URL.`
- Residual risk accepted: `Yes`
- Waiver reference (if `Yes`): `Provider-specific external auth/session handshakes remain unexercised in the live release E2E because no third-party credentials were introduced for this ticket.`
...
| 2026-03-08 | No impact | `N/A` | The release-lane rc1 through rc3 fixes only changed gateway entrypoint startup detection, supervisor health timeout handling, and release artifact versioning; the existing managed-flow README remains accurate. | Completed |
