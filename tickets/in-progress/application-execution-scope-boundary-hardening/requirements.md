...
- Normal Team binding launch reads `getExecutionTreeSnapshot()` from a returned live `RootTeamRun`, and normal Agent/Team input resolves a live run then calls `postUserMessage`/`postMessage` in orchestration. A boundary that merely wraps those returns would still expose mutable termination/task/lifecycle behavior; exact immutable commands/projections are required.
...
- `REQ-003` Callers above the owner must receive only semantically narrow capabilities required for Agent execution, Team execution, streaming, artifact access, memory lookup, tool readiness, and lifecycle; no caller receives a generic services collection, raw manager/registry/session manager, or live `AgentRun`/`RootTeamRun`. Agent/Team launch and input return only immutable use-case projections/dispositions.
...
- `AC-003` Focused tests prove exact internal identity from Agent/Team launch through scoped sessions, activation/resources, memory, streaming, publication, nested configured/task Teams, and cleanup; Agent/Team create/input expose only immutable identity/member projections or accepted/rejected/not-available dispositions, never live runs.
...
- `AC-005` Architecture guards prove application paths do not import/call `AgentRunManager.getInstance()`, `AgentTeamRunManager.getInstance()`, `getAgentRunService()`, `getTeamRunService()`, or application-ID/run-ID execution locators; outward scope contracts and consumers neither import/return `AgentRun`/`RootTeamRun` nor directly resolve/post/snapshot them; all required named construction dependencies are injected.
...
- The exact TypeScript capability signatures and immutable result shapes are normative in `application-execution-scope-contracts.md`. Implementation may relocate type-only imports but must not widen the boundary, return live run aggregates, add convenience APIs, or change discriminants/fields without a new design-impact review.
