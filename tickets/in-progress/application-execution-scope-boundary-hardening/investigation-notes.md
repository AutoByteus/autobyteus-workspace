...
| 2026-08-25 | Code/Command | exact Agent/Team service signatures; artifact/memory/stream contracts; `rg` of builders, consumers, getters, AFB obligations, defaulting constructors, and durable-test constructors | close initial AR-001/AR-002 exactness gaps | seven exact capabilities, 8-field scope input, 12-field required platform input, 12-field orchestration result, 22 migrated plus three new graph-sensitive nested obligations (25 total), and closed source/test set recorded | No |
| 2026-08-25 | Review | `design-review-report.md` / ARCH-REV-002, remaining AR-001 | second architecture review | AR-002 resolved; exact inputs, construction, lifecycle, and transition inventory accepted; live run aggregate returns still bypassed the proposed scope | Resolved in SR-003 |
| 2026-08-25 | Code/Command | `ApplicationRunBindingLaunchService`, `ApplicationOrchestrationHostService`, `AgentRunService`, `TeamRunService`, `AgentRun.postUserMessage`, `RootTeamRun.postMessage`, Team execution-tree types, and affected tests | ARCH-REV-002 remaining AR-001 live-aggregate containment | normal launch reads a Team snapshot from returned `RootTeamRun`; normal input resolves returned live Agent/Team runs and posts outside the proposed owner. Exact target is immutable Agent identity, recursively flattened configured-Team member projection, and accepted/rejected/not-available input dispositions with resolution/posting retained inside scope | No |
...
AR-002 is resolved. SR-003 completes the remaining AR-001 boundary: all seven capability contracts have one owner in the scope-contract file, but `AgentRun` and `RootTeamRun` are now excluded from every outward signature. Only non-aggregate message/address/input/result types cross via type-only imports. Restore-aware input posting and Team configured-member projection occur inside the concrete scope.
