...
- Added Linux ARM64 Electron packaging/startup support with architecture-specific Linux AppImages (`linux-x64` and `linux-arm64`) and matching updater metadata (`latest-linux.yml` and `latest-linux-arm64.yml`), including explicit Linux x64 artifact naming instead of `linux-x86_64`.
