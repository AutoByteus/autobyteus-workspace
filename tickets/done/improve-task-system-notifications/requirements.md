...
- Treat task-delegation agent-facing copy as an actionable task/review surface, not a debug dump, not sender-centered messaging, and not target-kind-centered messaging: include only task content, task id where the receiving agent must reference it, review comments/instructions, decision/status, and reference files. Visible activation wording must be the same whether the original `delegate_task` target is an individual agent or an agent team, because the immediate runtime recipient simply has a task to execute. Keep delegator/sender/reviewer/receiver names, accountable team labels, logical member labels, target kind labels, submission ids, review ids, execution kind, task-agent run ids, task-team run ids, task-team instance ids, and routing metadata out of visible text.
...
- **FR-001:** For task-delegation activation, the visible `SYSTEM_TASK_NOTIFICATION.content` must use one uniform, concise task-centered template for both individual-agent targets and agent-team targets. It should describe only the new task, task id, task description/content, and reference files. It must not mention whether the original target was a member or team, must not include `New delegated team task`, `Accountable team`, `Logical member`, delegator/sender/reviewer/receiver names, execution/run identifiers, internal lifecycle mechanics, or tool-call protocol text.
- **FR-002:** For task-team target activation specifically, the immediate recipient is the task-team ingress/coordinator runtime, so the visible notification must still read as a direct task assignment to the recipient. Team accountability, team target name, ingress identity, and child-run details may remain in metadata/events where needed for routing and diagnostics, but must not appear in visible notification content or non-actionable runtime copy.
...
- **AC-001:** When `delegate_task` targets an individual agent, the emitted `SYSTEM_TASK_NOTIFICATION.content` uses the uniform activation wording, includes the task id and task description, and does **not** mention who delegated/sent it or the target kind. It does **not** contain `target_agent_run_id`, `task_agent_run_id`, `Task-agent run`, `Execution kind`, `Lifecycle instructions`, `submit_task_result`, `review_task_result`, JSON tool-call examples, or `send_message_to` protocol warnings.
- **AC-002:** When `delegate_task` targets an agent-team, the emitted `SYSTEM_TASK_NOTIFICATION.content` uses the same uniform activation wording as individual-agent targets, includes the task id and task description, and does **not** mention who delegated/sent it, `New delegated team task`, `Accountable team`, the team target name as a target label, or the target kind. It does **not** contain `task_team_run_id`, `Task-team run ID`, `task_team_instance_id`, `Ingress coordinator`, `Execution kind`, `Lifecycle instructions`, `submit_task_result`, `review_task_result`, JSON tool-call examples, or `send_message_to` protocol warnings.
