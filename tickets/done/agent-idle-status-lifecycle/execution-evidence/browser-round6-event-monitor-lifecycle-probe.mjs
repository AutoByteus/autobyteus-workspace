...
const baseUrl = process.env.BROWSER_PROBE_URL ?? 'http://127.0.0.1:18134/__validation-round6';
...
try {
  ...
  expectEqual(await page.getByTestId('tool-result').textContent(), '{"output":"round6-delayed-result-retained"}', 'delayed tool result remains retained');
  ...
  await page.screenshot({
    path: new URL('browser-round6-event-monitor-lifecycle-final.png', evidenceDir).pathname,
    fullPage: true,
  });
  const result = { result: 'Pass', url: baseUrl, observations, browserMessages };
  await fs.writeFile(
    new URL('browser-round6-event-monitor-lifecycle-probe-result.json', evidenceDir),
