<template>
  <main data-testid="round7-harness" class="space-y-4 p-8">
    <h1>Round 7 Event Monitor and lifecycle browser validation</h1>
...
const context = reactive(new AgentContext(
  {} as never,
  new AgentRunState('round7-browser-run', {
    id: 'round7-browser-run',
    messages: [],
    createdAt: now,
    updatedAt: now,
  }),
...
const dispatchToolStartA = (): void => dispatch({
  ...
  payload: {
    invocation_id: 'round7-tool-a',
    tool_name: 'search',
    turn_id: 'round7-turn-a',
...
const dispatchToolLogA = (): void => dispatch({
  type: 'TOOL_LOG',
  payload: {
    tool_invocation_id: 'round7-tool-a',
    tool_name: 'search',
    turn_id: 'round7-turn-a',
...
const dispatchToolResultA = (): void => dispatch({
  ...
  payload: {
    ...
    turn_id: 'round7-turn-a',
    arguments: { query: 'current-base lifecycle neutrality' },
    result: { output: 'round7-delayed-result-retained' },
  },
...
const toolSegment = computed(() => context.conversation.messages
  ...
  .find((segment) => 'invocationId' in segment && segment.invocationId === 'round7-tool-a'));
