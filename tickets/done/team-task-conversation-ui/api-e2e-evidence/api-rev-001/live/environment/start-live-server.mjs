...
const runtimeRoot = path.join(workspaceRoot, 'autobyteus-server-ts/tests/.tmp/api-rev-001-team-task-live');
const databasePath = path.join(workspaceRoot, 'autobyteus-server-ts/db/api-rev-001-team-task-live.db');
const packageRoot = path.join(workspaceRoot, 'tickets/in-progress/team-task-conversation-ui/api-e2e-evidence/api-rev-001/live/fixture-root');
...
const server = await startBuiltTestServer({
  ...
  port: 60321,
  timeoutMs: 180000,
  environment: { AUTOBYTEUS_AGENT_PACKAGE_ROOTS: packageRoot },
});
...
const marker = { status: 'READY', pid: server.child.pid, serverUrl: server.serverUrl, runtimeRoot: server.runtimeRoot,
  runtimeEnvironmentPath: server.runtimeEnvironmentPath, databasePath: server.database.databasePath,
  packageRoot, lsofExactDisposableTarget: true, lsofOperationalTarget: false };
fs.writeFileSync(path.join(evidenceRoot, 'safe-server-ready.json'), `${JSON.stringify(marker, null, 2)}\n`);
process.stdout.write(`API_REV_001_SAFE_SERVER_READY ${JSON.stringify(marker)}\n`);
let closing = false;
const close = async () => {
  if (closing) return;
  closing = true;
