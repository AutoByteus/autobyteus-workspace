...
- Added the same ingress workflow-runtime parity coverage for team bindings: one-turn final publish, repeated same-thread inbound messages on the same bound team run, and terminate-then-restore on the next same-thread team message.
- Strengthened the team ingress validation harness to model a real multi-member team and prove that a channel binding with no explicit `targetNodeName` resolves to the coordinator member run, persists coordinator-owned routed receipts, and publishes through the coordinator member reply path only.
...
- Next execution step: `Hold at refreshed Stage 8 checkpoint until the user resumes post-scoreboard work`
