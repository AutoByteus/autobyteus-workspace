...
`Refined — SR-015 architecture coherence correction ready for re-review` — the user-approved SR-012 rule remains authoritative: configuration and post-configuration inspection use the same form hierarchy and controls, while stored mode consumes immutable stored truth and exposes no editing capability. SR-013's distinct editable/stored capabilities and per-field historical classifier passed `ARCH-REV-005`; IR-011 implemented that architecture. CRR-018 then introduced an unsupported multiline-string premise, and IR-012 added CR/LF-specific classifier, fallback CSS, and tests. API-REV-009 proved those fields only after inventing `ordinary_prompt`/`multiline_prompt`, mutating the browser catalog, and injecting arbitrary GraphQLJSON; the current Codex Luna catalog exposes no free-text setting. CRR-021 rescinded that premise, and API-REV-010 passes real current-user paths at 98%. ARCH-REV-006 agreed with SR-014's direction but found AR-001: the mandatory decommission table omitted the cleanup and one hard block remained unqualified. SR-015 corrects those design-spec contradictions without changing requirements or reopening product behavior.
...
| BEH-010 | In `origin/personal`, a selected existing TeamRun reused `TeamRunConfigForm` with `readOnly=true`. The ticket first introduced a separate stored-card UI; IR-010 then restored the shared form but coupled stored scopes to fabricated editable workspace/override/catalog state and hid or changed exact model-config values when only part of the current schema drifted. API-REV-009 later exercised two API/E2E-invented free-text fields that no current catalog emits. | Opening Settings for an existing TeamRun uses the same root, member disclosure, nested-Team, and Agent controls as authoring, populated from the exact stored snapshot and disabled against mutation. Stored and editable scope capabilities remain distinct. Model-config values legitimately produced through a supported catalog/launch path stay truthful: representable values use ordinary disabled controls, while genuinely historical removed keys or stale values use compact exact fallbacks in the same field region. | The stored `TeamRunConfigurationView` remains immutable, preserves stored topology/values without current-definition inference, never becomes editable intent, and never requires fabricated authoring-state sentinels. Synthetic catalog mutation, arbitrary GraphQLJSON injection, and hypothetical future/custom fields do not define current supported UI behavior. | R-042–R-044, AC-035–AC-038 |
...
- `origin/personal` routes a selected existing TeamRun through the same `TeamRunConfigForm` used for authoring, with `readOnly=true`; focused tests assert disabled controls, operable disclosures, and no stored-snapshot mutation.
- The three standalone `Stored*` configuration components are absent from `origin/personal` and were introduced by this ticket. Live browser reproduction against the user's running Electron server confirms that they produce the rejected “Stored root Team defaults”/`/`/key-value-card UI.
- The distinct immutable `TeamRunConfigurationView` is still required to preserve exact V2 stored topology and values. Data-contract separation does not require visual-form separation: one form presentation can consume the view directly in stored mode without converting it into editable intent or consulting mutable current definitions.
- IR-010 restored one shared form and deleted the rejected stored components, but its common Team scope type imports draft-owned workspace operation state and requires stored projection to fabricate editable selection, override, catalog, and idle operation values.
- CRR-017's reachable partial-schema probe proved that whole-schema fallback is insufficient: an explicit stored enum value absent from the current enum displayed as `Default`, and a persisted key removed from the current schema disappeared.
- API-REV-009's `ordinary_prompt` and `multiline_prompt` fields were not production findings: API/E2E added both to browser Pinia state and injected arbitrary values into an isolated V2 fixture. Their renderer observation is useful robustness evidence, but the current Codex Luna catalog emits no free-text model setting and the scenario is not a supported current-user path.

## Relevant Supplemental Task Artifacts

| Artifact Path | Type / Purpose | Related Requirement IDs | Related Acceptance-Criteria IDs | Status / Approval | Relationship To Requirements |
| --- | --- | --- | --- | --- | --- |
| `ui-ux-spec.md` | Dedicated visual baseline, distinct editable/stored capabilities, supported-history fallbacks, journeys, interaction states, wireframes, responsive/accessibility behavior, and explicit removed-output list | R-001–R-010, R-038–R-044 | AC-001–AC-008, AC-031–AC-038 | Refined and user-approved through SR-012; clarified through SR-014 | Makes the origin-personal-preserving single-form UI and the supported historical-value contract reviewable without promoting synthetic test fields into product UI. |
| `hierarchical-launch-configuration-behavior.md` | Intended-behavior supplement with hierarchy, UI states, stored inspection, and resolution examples | R-001–R-044 | AC-001–AC-038 | Functional behavior approved on 2026-08-24; editable and stored presentation approved on 2026-08-25 | Clarifies the configuration hierarchy and user experience without replacing this document. |
...
- Change posture (`Feature`/`Bug Fix`/`Behavior Change`/`Refactor`/`Cleanup`/`Performance`/`Larger Requirement`): completed `Behavior Change` with current `Requirement Applicability Clarification`
- Initial design issue signal (`Yes`/`No`/`Unclear`): `Yes`
- Root cause classification: original `Shared Structure Looseness`, workspace/identity ownership, capability-conflation, and supported historical-representability defects are resolved; API-REV-009 exposed `Synthetic Reachability Overclassification`, not a current product defect.
- Refactor posture: `Bounded frontend simplification required` — remove IR-012's unsupported CR/LF branch/CSS/tests and replace remaining invented historical fixture names with actual emitted `reasoning_effort`/`service_tier` production cases; do not alter IR-011 behavior.
- Evidence basis: API/E2E's correction artifact proves the browser catalog and stored free-text keys were invented after the real run loaded; the normal current Codex Luna form cannot create them. CRR-021 rescinds CR-013/MP-CR-010, and API-REV-010 passes real current-user paths. IR-011's capability split and dynamic enum/schema-drift handling remain independently grounded in the production runtime catalog and stored Settings path; IR-012's CR/LF additions have no producer.
- Requirement or scope impact: preserve the approved UI and IR-011 frontend architecture; unwind IR-012-only CR/LF code/tests and prevent hypothetical/future/custom schemas or arbitrary injected GraphQLJSON from blocking delivery without a separately approved, producer-backed requirement.
...
11. Restore origin-personal parity for selected existing TeamRuns: render the same configuration form in read-only mode from the exact stored view, remove the standalone stored card/tree/card components, and preserve stored historical topology independently of current definitions.
12. Keep editable and stored Team/Agent subject capabilities distinct; share visual fields and components without fabricated authoring selection/override/operation/catalog state.
13. Classify supported product-originated historical model configuration per persisted key/value so representable values use normal disabled controls and genuinely removed/stale values remain visible as compact exact fallbacks; keep synthetic/future schema probes non-blocking unless separately approved.
...
- `UC-012` Select an existing TeamRun, open Settings, and inspect its exact root, nested-Team, and Agent configuration through the same form used during configuration, without allowing mutation.
...
- Treating manually injected GraphQLJSON keys, browser-mutated catalogs, or hypothetical future/custom provider schemas not emitted by any current supported catalog as current acceptance paths. Defensive renderer support may remain, but it does not define this ticket's UI or block sign-off.
...
- A material premise may govern design, implementation, or blocking review only when a named supported user/system/operational trigger or established contract produces it through normal production execution. Manual hidden-state mutation, arbitrary data injection, test-only setup, or generic type/API capability cannot establish its own product reachability.
...
- `R-042` Selecting an existing TeamRun and opening Settings shall render the same Team Definition, root controls, member disclosure, nested-Team controls, and Agent controls used during configuration, with stored values shown through those controls in read-only mode. The UI shall not substitute a history-only card/key-value inspector.
- `R-043` Stored-mode rendering shall consume the immutable stored `TeamRunConfigurationView` and its persisted topology/values directly through a stored-only capability. It shall not rebuild history from the current AgentTeam definition, convert the snapshot into editable draft intent, consult or import draft-owned workspace/operation/catalog state as a stored-scope requirement, fabricate editable workspace selections, overrides, loading/error/catalog sentinels, or emit configuration/workspace/reset mutations. Visual components may be shared only through presentation fields and an explicit editable-versus-stored capability distinction.
- `R-044` Stored mode shall keep disclosures operable and actual fields legible while disabling edits and omitting editable-only actions such as Reset and Run. For model configuration legitimately accepted from a supported product catalog and persisted by a supported launch path, historical representability shall be evaluated per field and exact value at root Team, nested Team, and Agent scopes. A current control shall render only a value it can represent without substitution. A legitimately historical key removed from the current schema, or a formerly supported value no longer valid for its current field, shall remain visible with its exact key/value in a compact read-only fallback in that field's normal model-configuration region. A partially compatible schema shall render its representable fields normally and only its unrepresentable residuals as fallbacks; it shall never hide, substitute, mutate, or duplicate those supported persisted values or switch the whole screen to an alternate card design. Browser catalog mutation, arbitrary GraphQLJSON injection, and invented future/custom fields are mechanically possible robustness probes, not supported-user-path acceptance authority under this requirement.
...
- `AC-035` Repeating the `origin/personal` selected-run journey—select existing TeamRun, focus a member, open Settings—renders the same quiet form structure used for new TeamRun configuration, with controls disabled and the read-only explanation visible; “Stored root Team defaults,” root `/`, standalone key/value cards, and “Stored Team and Agent configuration” are absent.
- `AC-036` Expanding `Team Members Override (N)` and a nested Team in stored mode shows the exact stored Team and Agent values through the same disabled runtime/model/model-specific/workspace/auto-approve presentation and the same hierarchy treatment used during configuration; disclosures remain keyboard-operable, no mutation event is accepted, and no stored Team or Agent scope depends on fabricated editable override/workspace/operation/catalog state.
- `AC-037` A stored run remains inspectable from its persisted hierarchy and exact values if the current AgentTeam definition is missing, renamed, moved, or changed; the UI never silently replaces stored subjects or values with current-definition data, and the stored form model contains no draft-owned authoring capability.
- `AC-038` At root Team, nested Team, and Agent scopes, a supported TeamRun whose model configuration was valid when written remains truthful after a real catalog/schema change: every still-representable stored value uses its ordinary disabled control, and each formerly supported removed key or stale value appears once as an exact compact fallback in the same model-configuration region. No such product-originated key/value is hidden, substituted with `Default`, duplicated, mutated, or used to restore the rejected card inspector. Tests and current-path API/E2E sign-off must start from real emitted settings—use the dynamic Codex `reasoning_effort` and `service_tier` producer paths for stale-value/removed-field coverage. Invented `ordinary_prompt`, `multiline_prompt`, `removed_parameter`, or comparable fixture-only fields do not satisfy this criterion.
...
| UC-012 | R-028–R-031, R-040–R-044 |

Cross-cutting UI state and accessibility requirements R-038–R-044 apply to UC-001–UC-007, UC-011, and UC-012 where the relevant control is present.
...
| AC-035–AC-038 | Origin-personal stored-settings parity, exact read-only hierarchy/values, current-definition independence, and unavailable-catalog fallback |
...
- On 2026-08-25, the user explicitly confirmed the editable `ui-ux-spec.md` direction: retain only necessary output, keep the launch form almost identical to the original, add global configuration for each nested Team, and omit canonical-root/summary noise because the controls already display configured values. SR-011 recorded that approval.
- Continued Electron review then exposed `USER-UX-003`: the ticket-created stored TeamRun card inspector violates the same principle and regresses `origin/personal`, where existing TeamRuns reused the form read-only. The user explicitly approved SR-012's single-form rule: how the Team is configured is substantially how it is later viewed, with the same UI and locked controls.
- `CRR-017` does not introduce a new product decision. `CR-011` and `CR-012` contradict the approved SR-012 authority split and exact-history promise. SR-013 therefore strengthens R-043/R-044 and AC-036–AC-038 with distinct capability and per-field/per-value representability rules; architecture re-review is authorized before another implementation round.
