...
- Status: **Approved — SR-021 intended behavior unchanged** (SR-016 baseline). SR-023 large-Org launch/health correction passed ARCH-REV-004 and its downstream gates. The current **SR-024** technical design recovery addresses a first-turn AGY Org-member configured-skill preparation failure in the replacement 1.4.80 Electron user-verification build. No new intended behavior, renewed behavior approval or final delivery acceptance is inferred; SR-024 remains in independent architecture review before affected implementation resumes.
- Current solution revision ID: `SR-024` (design recovery; last approved behavior delta `SR-021`, baseline `SR-016`)
- Package identifier: `antigravity-cli-runtime-redesign-20260924`
- Requirements owner: Solution Designer
- Date: 2026-09-24
- Approval state and reference: The user's 2026-09-24 “niceee. i approve the ticket” approved the structured AGY direction; subsequent messages explicitly approved AGY-specific runtime-selection default-on/no-headless-manual-approval behavior and clarified conversion into existing AutoByteus raw traces, not a separate provider archive. After reviewing the live JSON experiments and mapped conversion direction, “So do you think now everything is clear? If yes, then we can start with the tickets now. You can continue, right?” approved the complete SR-016 baseline, including day-one standalone/team/org members and run-start identity retention on exact resume. After IR-001 and **further raw command-outcome experiments**, the user explicitly directed “if they really do not have return result, then we simply map done to success thats it,” approving the narrow SR-021 status change with the tested nonzero-command consequence. No old AGY AutoByteus product run history exists to migrate. Approval authorizes revised design, not bypass of applicable review/validation.
...
### SR-020 prior proposal — superseded by SR-021

IR-001 reports the user's screenshot/feedback requesting a green completed command badge like other runtimes. The screenshot also shows `AGENT_SEGMENT_LIFECYCLE_INVALID`, a **separate implementation bug** locally fixed/tested by Implementation Engineer but not yet freshly validated in the rendered UI. The green request conflicts with the approved SR-019 design's neutral gray “Completed; outcome not reported” presentation for AGY `DONE`. In a recorded CLI 1.2.10 control, `sh -c 'exit 7'` emitted AGY tool `DONE` and overall `SUCCESS` with no machine-readable command exit status. Thus green cannot truthfully mean “exit code 0” or “command succeeded” based only on AGY `DONE`.

This was the Solution Designer's unapproved suggestion: for AGY `run_command`, color terminal `DONE` green as “Completed” while keeping a neutral canonical event. The user instead explicitly chose a simpler provider-state mapping after further experiments: **AGY `DONE` → AutoByteus success**. Do not implement the SR-020 proposal.

### SR-021 approved decision — AGY tool `DONE` maps to success

The user said, after asking for further experiments, “if they really do not have return result, then we simply map done to success thats it.” Fresh AGY CLI 1.2.10 raw-JSON controls in `agy-command-outcome-matrix-probe/` establish that `run_command` exit 0, exit 8 and command-not-found/exit 127 each emitted tool `DONE` without a structured command exit-status field; even the nonzero turns had top-level `SUCCESS`. An approval-denied command emitted tool `ERROR` and `denied_actions` instead. The earlier exit-7 control agrees. The user has therefore explicitly approved treating **provider tool-step `DONE` as AutoByteus tool success**, including a green success presentation, despite the fact that for `run_command` this can also describe an underlying nonzero shell exit. It is a **product convention based on AGY state**, not evidence that the command exited zero.

Approved guardrails: map AGY tool `ERROR` or an explicit tool error payload to failed/denied, not success; do not override that with overall `result.status=SUCCESS` or CLI exit 0. Preserve exposed output/error and the original AGY state in the normalized trace; do **not** fabricate a shell exit code or suppress a textual error such as “command not found.” Apply the AGY `DONE` rule to tool steps generally, rather than command-only styling, because the user's direction is to map the provider state, not a special `run_command` color. Other runtimes keep their existing mappings. This decision supersedes the SR-020 neutral-event/green-completion proposal and the SR-019 neutral treatment of AGY `DONE`.
...
| REQ-011 | For AGY tool steps, convert terminal provider `DONE` without an explicit tool error to canonical AutoByteus tool success and green success presentation. This classifies the provider step, not the underlying shell exit; do not invent an exit code. Convert AGY `ERROR`/explicit tool error to failed or denied as appropriate, regardless of top-level turn/process success. Do not change other runtimes. | SCN-001, SCN-002, SCN-003, SCN-005 |
...
| AC-010 | REQ-011 | AGY exit-0 and exit-8 `run_command` controls, and a command-not-found control, each emit tool `DONE` and appear green/success in live and reloaded Event Monitor/raw-trace views, while any AGY-emitted output/error text remains available and no shell exit code is fabricated. A denied/explicit-error tool is failed or denied, not green, even if overall turn status is `SUCCESS`. Non-AGY mappings remain unchanged. |
...
5. Per SR-021 user approval after extra CLI experiments, AGY tool `DONE` is converted to AutoByteus success/green, even when an underlying shell command may have exited nonzero without a structured AGY exit status. This is provider-step success semantics, not a claim of exit code zero. AGY tool `ERROR`/denial remains failed or denied; preserve exposed text and never synthesize a missing exit code.
