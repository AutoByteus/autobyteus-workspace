...
OUT=pathlib.Path(__file__).with_name('agy-explicit-workspace-in-agent-tools-probe'); OUT.mkdir(exist_ok=True)
with tempfile.TemporaryDirectory(prefix='autobyteus-agy-workspace-in-agent-') as temp:
 ...
 args=['agy','--new-project','--agent','capsule-probe','--add-dir',str(real),'--model','gemini-3.8-flash-low','--dangerously-skip-permissions','--output-format','stream-json','--print','Create a file named directed-file.txt in your agent workspace with the exact contents DIRECTED-ROOT-6251. Use the write_to_file tool. Then tell me its absolute path.']
 ...
 print(json.dumps({'exit':p.returncode,'status':result.get('status'),'response':result.get('response'),'realFile':(real/'directed-file.txt').read_text() if (real/'directed-file.txt').exists() else None,'capsuleFile':(capsule/'directed-file.txt').read_text() if (capsule/'directed-file.txt').exists() else None,'toolTargets':[(e['step_update'].get('state'),e['step_update'].get('tool_name'),e['step_update'].get('tool_info',{}).get('parameters')) for e in ev if e.get('event')=='step_update' and e['step_update'].get('step_type')=='tool'],'stderrTail':p.stderr[-300:]}))
