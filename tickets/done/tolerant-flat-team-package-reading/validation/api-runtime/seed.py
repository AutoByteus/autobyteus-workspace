...
traces=[{'id':'api-old-user','ts':1788220800,'turn_id':'api-old-turn','seq':1,'trace_type':'user','content':'Remember the secret word BLUEBERRY-915 for the next message. This is test history.','source_event':'LLMUserMessageReadyEvent','file_attachments':[{'uri':uri,'file_type':'text','file_name':'retained-note.txt'}]}, {'id':'api-old-reply','ts':1788220801,'turn_id':'api-old-turn','seq':2,'trace_type':'assistant','content':'I will remember BLUEBERRY-915.','source_event':'LLMCompleteResponseReceivedEvent'}]
...
inv=json.loads((E.parent.parent/'package-inventory.json').read_text());assert all(hashlib.sha256(Path(r['path']).read_bytes()).hexdigest()==r['sha256'] for r in inv['teams'])
def snapshot(root):
 out={}
