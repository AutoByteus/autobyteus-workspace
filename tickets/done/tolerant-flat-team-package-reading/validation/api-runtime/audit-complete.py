...
E=Path(__file__).resolve().parent/'complete';W=E.parents[5];D=W/'autobyteus-server-ts/tests/.tmp/team-package-complete-api001';S=Path('/Users/normy/autobyteus_org/autobyteus-agents')
label=sys.argv[1]
def snap(root):
 out={}
...
db=sqlite3.connect('file:'+str(W/'autobyteus-server-ts/db/team-package-complete-api001.db')+'?mode=ro',uri=True);db.row_factory=sqlite3.Row;rows=[dict(r) for r in db.execute('select * from app_data_migration_records')];db.close()
