...
The public tool response should answer: "what does the calling agent need to know after this tool call?" It should not expose input echoes, caller-selected decisions, run ids, audit ids, notification booleans, settlement booleans, or raw warning objects.
...
- `review_task_result` currently/previously returned `decision`, but that only echoes the caller-selected input. The caller agent made the decision and already knows whether it accepted or requested revision; the resulting `status` is the meaningful state change.
...
- Evidence basis: Task delegation service currently returns or previously returned domain/lifecycle internals directly through public tool responses. The caller only needs task id, resulting status, and optional `message` when a successful tool call has a non-fatal lifecycle issue to report.
...
- UC-005: A reviewer accepts a submitted result and receives only task id and `accepted` status.
- UC-006: A reviewer requests revision and receives only task id and `active` status.
...
- REQ-008: `review_task_result` acceptance result must contain exactly `task_id` and `status: "accepted"`; it must not include `decision`, `review_id`, `reviewed_submission_id`, `notification_delivered`, `settlement_requested`, or `warnings: []`.
- REQ-009: `review_task_result` revision-request result must contain exactly `task_id` and `status: "active"`; it must not include `decision`, `review_id`, `reviewed_submission_id`, `notification_delivered`, `settlement_requested`, or `warnings: []`.
- REQ-010: If `review_task_result` records the review but revision notification delivery fails, the public result must include a concise non-empty `message` and must not expose `decision`, internal route keys, task-agent run ids, task-team run ids, or warning arrays.
...
- AC-007: An accepted `review_task_result` call returns exactly `{ task_id: "...", status: "accepted" }` apart from value differences.
- AC-008: A revision-request `review_task_result` call returns exactly `{ task_id: "...", status: "active" }` apart from value differences when notification delivery succeeds.
- AC-009: A revision-request `review_task_result` call whose notification delivery fails returns task id/status plus a concise `message`, and does not expose `decision`, warning-array details, or target route/run ids.
...
- Existing unit/integration/e2e tests currently assert old verbose `submit_task_result` shape and the previous `review_task_result.decision` echo; they need durable updates.
...
- Since downstream delivery/docs work had already started for the two-tool scope, docs and handoff artifacts may need another pass after the submit-result and review-decision refinement is implemented and reviewed.
...
## Approval Status
User approved the refined scope on 2026-07-01 after downstream follow-up. `submit_task_result` now receives the same meaningful public result cleanup philosophy as `delegate_task` and `review_task_result`. User additionally clarified on 2026-07-01 that `review_task_result` must not return `decision` because it only echoes the caller-selected input; ready for architecture review.
