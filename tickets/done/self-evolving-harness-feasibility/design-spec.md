...
```text
Manual trigger -> visible self-evolver agent -> autoExecuteTools true -> run_bash edits files inside exact target skill roots -> runtime-neutral target UI notification / next-run baseline -> Git-backed manual inspect/revert during testing
```
...
4. Eligible standalone runs expose a primary minimal labeled CTA near the active run composer: `Self improve` (localized equivalent such as `自我改进`). Do not add a run-history row self-evolution icon/action; keep the row in its simpler previous state.
5. Eligible team/member source runs expose the same visible CTA copy (`Self improve`) for the currently selected member source run; full-team evolution is deferred. Target scope belongs in the accessible label/tooltip, not in long visible button text.
6. Clicking the composer-adjacent CTA creates a self-evolution request using `ManualTriggerStrategy`.
...
13. The service emits a concise target skill-update UI notification after successful evolver completion. For active idle standalone target runs, this notification is a local runtime-neutral `AgentRunEventType.SYSTEM_TASK_NOTIFICATION` emitted on the target `AgentRun`; that local event is the canonical source for WebSocket `SYSTEM_TASK_NOTIFICATION` and frontend `SystemTaskNotificationSegment` rendering across AutoByteus, Codex, Claude, and future runtimes. Future runs naturally load any edited skill-root content; active runtime/model refresh remains deferred unless a separate runtime-instruction/reload capability is explicitly designed.
...
- Intentional deferrals and residual risk, if any:
  ...
  - Active-run user-visible notification is supported through a local runtime-neutral event; active-run skill cache/model reload is deferred and next-run correctness is the MVP correctness baseline.
...
| DS-002 | Primary End-to-End | User clicks composer-adjacent `Self improve` CTA for a standalone run | Visible evolver run edits files inside target skill roots and target is notified | `SelfEvolutionService` | Core MVP business flow and concise discoverable manual trigger. |
| DS-003 | Primary End-to-End | User clicks composer-adjacent `Self improve` CTA for a selected member source run | Visible evolver run edits files inside selected member skill roots and target/member is notified | `SelfEvolutionService` | Same core flow but with explicit team/member identity. |
...
| DS-005 | Return-Event | Evolver run completes/terminates | Minimal evolution record finalized and runtime-neutral target UI notification emitted where applicable | `SelfEvolutionService` | The visible helper run is the review surface; the service records linkage and notification only. |
...
| DS-009 | Return-Event | Target notification service emits local run event | `SystemTaskNotificationSegment` appears in target conversation | `SelfEvolutionTargetNotificationService` / stream dispatchers | Keeps UI notification runtime-neutral instead of depending on runtime command injection. |
...
`Run detail composer-adjacent CTA -> SelfEvolutionResolver.startAgentRunSelfEvolution/startTeamMemberSelfEvolution -> ManualTriggerStrategy.createRequest -> SelfEvolutionService.startFromEvolutionRequest -> normal target/evidence/skill/evolver path`
...
### DS-009 Runtime-neutral notification segment spine

`SelfEvolutionTargetNotificationService -> activeRun.emitLocalEvent(AgentRunEventType.SYSTEM_TASK_NOTIFICATION) -> AgentRunEventMessageMapper -> WebSocket SYSTEM_TASK_NOTIFICATION -> AgentStreamingService/TeamStreamingService shared handler -> SystemTaskNotificationSegment`

Optional runtime/model instruction, if ever supported, is a separate branch from `SelfEvolutionTargetNotificationService` and must not be the source of the UI segment.
...
| DS-002 | A user manually starts evolution for a standalone run from a labeled composer-adjacent CTA. The service validates global/target eligibility, resolves run metadata and configured skill roots, builds an anonymized evidence task, launches a visible single evolver agent with run_bash and auto-exec, records minimal helper-run linkage, and notifies/reloads the target after successful completion. | Manual action, self-evolution service, target context, skill roots, evolver run, notification | `SelfEvolutionService` | Evidence reading, settings resolution, helper-run event collection |
| DS-003 | A user manually starts evolution for a selected team member from the member-scoped labeled CTA. The service uses explicit `teamRunId + memberRunId`, resolves member metadata and that member's configured skills, then follows the same visible evolver launch, minimal record, and notification path. | Team member action, self-evolution service, member context, skill roots, evolver run, notification | `SelfEvolutionService` | Team metadata flattening, member memory layout, member-scoped UI |
...
| DS-005 | The evolver edits files directly, so the service does not apply a patch or audit changes. After final output/termination it records helper-run linkage and asks the notification owner to emit any applicable runtime-neutral target UI notification. | Evolver completion, run record finalization, notification | `SelfEvolutionService` | Active-run lookup, local event emission |
...
| DS-009 | Completion notification rendering is driven by a local `AgentRunEventType.SYSTEM_TASK_NOTIFICATION`, not by injecting a message into the target runtime. The server event mapper and websocket stream deliver the event to the same shared frontend handler used by standalone and team streams, which renders the existing system notification segment. | Local run event, websocket message, frontend notification segment | `SelfEvolutionTargetNotificationService` / stream dispatchers | Duplicate suppression if a runtime also emits notification events |
...
| `SelfEvolutionTargetNotificationService` | Runtime-neutral active target UI notification, next-run/skip/fail notification outcome recording, and any future explicitly gated runtime instruction. |
...
`Evolver AgentRun events -> SingleAgentEvolverStrategy completion collector -> SelfEvolutionService finalizes minimal record -> SelfEvolutionTargetNotificationService -> active target run local SYSTEM_TASK_NOTIFICATION event / next-run fallback`

Narrative:

The evolver run is a normal visible run. The self-evolution service should not parse every tool call as an apply contract and should not add a redundant file-change audit. It only needs to know whether the helper run completed, failed, or timed out. On successful completion it records the visible evolver run linkage and asks the notification service to emit a concise local UI notification when the target is active/idle, or record next-run-only outcome otherwise.
...
| `SelfEvolutionTargetNotificationService` | `find active target -> if idle emit local SYSTEM_TASK_NOTIFICATION -> if busy queue/skips -> else next-run only` | Active-run UI notification is runtime-neutral; active model adoption remains future/best-effort and next-run correctness remains baseline. |
...
| Target notification | DS-005, DS-009 | Self-evolution service | Notify active/inactive targets after successful evolver completion using local UI event or next-run record. | Separates helper-run completion and UI notification from runtime/model command injection. | Evolver might message target as teammate, mutate team topology, or make UI notification runtime-dependent. |
...
| Active run notification | `AgentRun.emitLocalEvent` + agent streaming event mapper | Reuse | Local run events are runtime-neutral and already feed websocket stream listeners. Do not depend on runtime-posted `SenderType.SYSTEM` messages to render UI notifications. | Full runtime cache reload/model instruction can be future work. |
...
| `src/self-evolution/services/self-evolution-target-notification-service.ts` | self-evolution | Notification | Emit local UI notification event, record next-run/skip/fail outcome, and gate any future runtime instruction. | Separates UI notification from runtime adoption concern. | Yes |
...
| `autobyteus-server-ts/src/self-evolution/services/self-evolution-target-notification-service.ts` | self-evolution | Notification | Emit local runtime-neutral notification event / record next-run notification / gate future runtime instruction. | Keeps UI notification separate from runtime adoption. | Yes |
...
- Developer/debug or evolution-record surfaces may show the visible evolver run linkage and listed editable skill packages, but the primary composer UI should not show a persistent open-run card or record ID after start.
...
10. Target is not the Skill Self-Evolver helper run itself.
11. Target source run is idle/completed or otherwise has a stable work-history snapshot, and evidence can be summarized through run-history/memory readers without retaining raw trace paths.

Eligibility output should include `eligible: boolean`, machine-readable `reasons`, `warnings`, and normalized `skillTargets`. These backend reasons are for validation, tests, and developer/debug surfaces. The primary chat UI should hide ineligible CTAs by default rather than showing technical reasons.
...
| Old run created before effective-config snapshots | hidden / ineligible; do **not** display the technical pre-snapshot reason in chat UI | Reject; old runs did not capture self-evolution consent/eligibility. |
...
| Active target is busy/running | hidden or short disabled state such as `Wait until idle` only if needed | Reject or require user to wait until the run is idle/completed. |
| Selected target is Skill Self-Evolver helper run | hidden / ineligible | Reject; avoid recursive self-evolution helper runs in MVP. |
| No meaningful work history yet | hidden by default | Reject; greeting-only/no-signal runs should not show a primary CTA unless product chooses to allow no-op review later. |
...
- Run-history/detail pages do not show self-evolution row actions or old long-form `Improve from this run` buttons.
...
- Primary manual trigger placement:
  - Show a labeled contextual CTA slightly above the active run composer/text area for the currently selected source run when backend eligibility is `eligible: true`.
  - Visible copy for standalone and team/member source runs: `Self improve` (localized equivalent such as `自我改进`).
  - Scope-specific wording such as `from this run` or `this member's skills` belongs in the accessible label/tooltip, not in the visible button text.
  - Use an icon only as supporting decoration; text is mandatory for the primary CTA.
  - Do not show a row-history sparkle/star self-evolution action; keep the run row in its simpler previous state.
- Ineligible/disabled behavior:
  - Hide all self-evolution CTAs when the global capability is disabled.
  - Do not show the CTA for the visible Skill Self-Evolver helper run itself.
  - If the selected run has no self-evolution snapshot, snapshot disabled, no editable skill roots, or insufficient/unfinished work history, hide the CTA by default. Do not display technical backend reasons such as `Run was created before self-evolution effective-config snapshots were recorded.`
  - If the action is temporarily unavailable for a user-understandable reason, use a very short disabled state only when necessary, for example `Wait until idle`; avoid long helper text under the button.
  - Backend warnings remain available to developer/debug surfaces, but the MVP composer UI should not render warning blocks unless a warning is required to prevent a confusing destructive action.
- Copy/accessibility/localization:
  - Tooltip/accessibility copy may be slightly more explicit, for example `Self improve from this run` for standalone and `Self improve this member from this run` for member scope.
  - Localized strings should keep the same concise semantics: self-improve from the current run, not generic “magic” or “optimize”.
  - Started state should be unobtrusive: a short transient toast/status such as `Self improve started` is acceptable.
  - Do **not** render a persistent green success card in the composer area, do **not** show evolution record IDs in the primary UI, and do **not** show an `Open Skill Self-Evolver run` button. The visible Skill Self-Evolver run remains discoverable through normal run/sidebar surfaces.
...
- displaying verbose old-run/config-snapshot technical eligibility reasons in the chat UI;
- displaying persistent start/open/detail cards under the composer.
...
- If the target is an active idle standalone run, regardless of runtime kind, emit a local runtime-neutral UI notification event on the target `AgentRun`:
...
- This local event is the canonical source for user-visible notification rendering. It must travel through `AgentRunEventMessageMapper` to websocket `SYSTEM_TASK_NOTIFICATION`, then through the shared frontend notification handler to `SystemTaskNotificationSegment`. Do not rely on `activeRun.postUserMessage(new AgentInputUserMessage(..., SenderType.SYSTEM))` to create the UI notification segment.
- Suggested concise message:

```text
Self improve finished. New runs will use updated skills when changes were made.
```

- The notification should not include raw skill-root paths, evolution record IDs, or long implementation details in the user-visible message. Detailed linkage remains in the evolution record and visible evolver run, not in the target chat UI.
- Runtime/model-consumed instruction is a separate optional concern:
  - The MVP should not post a runtime-consumed system message merely to show UI notification.
  - If a runtime-specific implementation later needs to tell an active model to re-read skills before continuing, that must be capability/policy gated and explicit, for example a future `SkillReloadRequestedEvent` or a backend capability such as `supportsRuntimeSkillRefreshInstruction`.
  - Because the MVP does not send the runtime instruction, duplicate visible notifications are not expected in the MVP path. If such an instruction is later sent and the runtime also emits `SYSTEM_TASK_NOTIFICATION`, the notification service or stream projection must suppress duplicate visible segments for the same self-evolution completion event, for example by using an internal completion notification dedupe key that is not shown in the segment copy.
  - Next-run correctness remains the MVP baseline; active in-run skill adoption is best-effort/future work.
- Current frontend/server finding:
  - `AgentRun.emitLocalEvent(...)` already provides a runtime-neutral way to publish local run events to active stream listeners.
  - `AgentInputPipeline` can emit `SYSTEM_TASK_NOTIFICATION` for AutoByteus `SenderType.SYSTEM` messages, and `AgentRunEventMessageMapper` maps that event to the websocket server message, but Codex/Claude/future backends do not guarantee the same conversion from a posted message.
  - `autobyteus-web` already has `SystemTaskNotificationSegment` and `handleSystemTaskNotification`.
  - `TeamStreamingService` dispatches `SYSTEM_TASK_NOTIFICATION` to the handler, but standalone `AgentStreamingService` currently lists `SYSTEM_TASK_NOTIFICATION` as a live runtime message without a switch case that calls the handler. Therefore the active standalone target may receive and respond to the system message while the frontend fails to render the notification segment.
  - Implementation should move/reuse `handleSystemTaskNotification` as a shared handler and add a standalone `AgentStreamingService` dispatch case so both standalone and team/member streams render the same compact notification segment from the local event path.
- Team-member active reload/notification remains `next_run_only` in the MVP unless a concrete team-member active-run notification path is implemented; do not imply team member active reload in UI.
- If the active target is busy, record `skipped_busy` or queue the local UI event until idle if that is simple and deterministic; otherwise rely on next-run correctness. External-runtime model reload support must not affect local UI notification eligibility for active idle standalone runs.
...
| `autobyteus-server-ts/src/self-evolution/services/self-evolution-target-notification-service.ts` | File | Notification | Build concise notification copy, emit local runtime-neutral `SYSTEM_TASK_NOTIFICATION` events for active standalone targets, record next-run/skip/fail outcomes, and gate any future runtime instruction. | Keeps UI notification ownership server-side and runtime-neutral. | Skill root resolution, evidence building, or direct runtime command injection as a UI-notification dependency. |
...
| Relevant run/detail/run-launch/composer components | Files | UI components | Show run-launch toggle and composer-adjacent labeled manual CTA when capability and backend eligibility allow. | Product surface. | Strategy implementation or backend eligibility logic. |
| `autobyteus-web/services/agentStreaming/handlers/systemTaskNotificationHandler.ts` or shared handler export | File | Notification segment projection | Project `SYSTEM_TASK_NOTIFICATION` payloads into `system_task_notification` conversation segments for standalone and team streams. | Fixes active-target notification visibility without duplicating logic. | Self-evolution business logic or activity-store projection. |
...
| Primary manual CTA | Labeled composer-adjacent `Self improve` button as the only user-facing trigger | Any run-row self-evolution icon/action or long `Improve this member's skills from this run` visible copy | Keeps the manual MVP discoverable without crowding or overexplaining the UI. |
...
| Add | frontend self-evolution stores/components | Capability store, action store, settings/config controls, composer-adjacent labeled manual CTA; no run-row self-evolution action. |
...
9. **Implement target notification**
   ...
   - For active idle standalone runs across AutoByteus, Codex, Claude, and future runtimes: emit a local runtime-neutral `AgentRunEventType.SYSTEM_TASK_NOTIFICATION` on the target `AgentRun` with concise no-path/no-ID copy.
   - Do not call `postUserMessage(... SenderType.SYSTEM ...)` as the mechanism for rendering the UI segment. If a runtime/model instruction is later needed, implement it as a separate capability/policy-gated step and suppress duplicate visible notifications.
   - Ensure frontend standalone streaming dispatches `SYSTEM_TASK_NOTIFICATION` into the existing system notification segment. Reuse the same handler path as team streaming instead of leaving the message unhandled.
...
11. **Add frontend UI**
   ...
   - Run detail/composer: show the labeled `Self improve` CTA above the composer for the currently selected eligible source run.
   - Run history row: remove/revert any sparkle/star self-evolution action; the row should not expose self-evolution.
   - Hide old/ineligible selected runs without showing technical pre-snapshot reasons in the chat UI.
   - After start, show at most a short transient toast/status; do not render a persistent green card, evolution record ID, or `Open Skill Self-Evolver run` button under the composer.
...
12. **Validation and tests**
   ...
   - Frontend/component tests or story states for composer-adjacent CTA visible, hidden when global disabled/ineligible/old pre-snapshot, hidden for Skill Self-Evolver helper run, absence of run-row self-evolution action, and absence of persistent start/open cards.
   - Server notification-service tests proving local `SYSTEM_TASK_NOTIFICATION` event emission for mocked active idle standalone runs with AutoByteus, Codex, and Claude runtime kinds, without requiring runtime-posted `SenderType.SYSTEM` messages.
   - Server/frontend tests proving duplicate visible notifications are not produced if a future/runtime path also emits `SYSTEM_TASK_NOTIFICATION` for the same completion event.
   - Frontend streaming tests for standalone and team `SYSTEM_TASK_NOTIFICATION` messages rendering a `system_task_notification` segment.
...
| Active reload vs next-run | Next-run correctness baseline plus runtime-neutral active UI notification | Current runtime caches skills/system prompts, and runtimes differ in whether posted system messages become UI events or model instructions. | Active run may not fully adopt until next run; future explicit reload/instruction capability can address this. |
...
5. **Active target run may not really reload changed skill.**
   - Mitigation: next-run correctness baseline; emit a local runtime-neutral UI notification so the user sees completion; future `SkillReloadRequestedEvent` or explicit runtime instruction capability can handle active model refresh.
...
9. **Evolver completion may be mistaken for proof of quality.**
   - Mitigation: MVP primary composer UI uses only a short start status and must not claim changed files or downstream benefit. Detailed evolution-record/debug surfaces may link the visible evolver run. Formal metrics are future work.
...
- Primary manual UX must be the single labeled composer-adjacent `Self improve` CTA for the selected eligible source run; do not add or keep a run-row sparkle/star self-evolution action, verbose ineligible reason, persistent start card, evolution record ID, or `Open Skill Self-Evolver run` button in the composer area.
