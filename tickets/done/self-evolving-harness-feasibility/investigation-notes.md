...
- Task Artifact Folder: `/Users/normy/autobyteus_org/autobyteus-worktrees/self-evolving-harness-feasibility/tickets/done/self-evolving-harness-feasibility`
...
- External repos, samples, or artifacts cloned/downloaded for investigation:
  - PDF: `/Users/normy/autobyteus_org/autobyteus-worktrees/self-evolving-harness-feasibility/tickets/done/self-evolving-harness-feasibility/external-paper/arxiv-2605.30621.pdf`
  - Extracted paper text: `/Users/normy/autobyteus_org/autobyteus-worktrees/self-evolving-harness-feasibility/tickets/done/self-evolving-harness-feasibility/external-paper/arxiv-2605.30621.txt`
...
This earlier handoff note is superseded by later user refinements on 2026-06-05 and the DI-001 notification refinement on 2026-06-06. The current design is centered on a single `SelfEvolutionService` that launches a visible direct-edit evolver run, provides anonymized work-history evidence and skill-root context, records only minimal run provenance, and emits runtime-neutral target UI notification. Dedicated change-recording/audit and metrics services are no longer MVP scope.

## Additional Architecture Analysis — Skill Evolution Bridge — 2026-06-04

The user clarified that the important architectural question is how to connect two existing storage worlds: run traces/history in app-data memory and skill files primarily living in agent packages. Additional analysis produced `/Users/normy/autobyteus_org/autobyteus-worktrees/self-evolving-harness-feasibility/tickets/done/self-evolving-harness-feasibility/skill-evolution-architecture-analysis.md`.
...
Earlier recommendation, later superseded by code-review DI-001: MVP considered using existing run-posting semantics for the LLM-visible part:
...
The existing AutoByteus input pipeline recognizes `SenderType.SYSTEM` and can surface system task notifications, but this is not runtime-neutral across Codex/Claude/future backends. The revised design uses local `AgentRunEventType.SYSTEM_TASK_NOTIFICATION` emission as the canonical UI notification path, and treats runtime/model-consumed instruction as a separate optional future concern. Longer term, add a dedicated `SkillUpdatedEvent` / `SkillReloadRequestedEvent` runtime control event so internal reload and LLM-visible instruction are separate.
...
visible compaction-style SkillEvolutionRunner
  ...
  + target run local SYSTEM_TASK_NOTIFICATION UI event
  + next-run correctness by default; active-run model reload/instruction deferred unless an explicit capability is designed
```
...
The user pushed back that the MVP should not expose separate approval and notification strategies. Refined recommendation: keep only essential user-facing configuration: self-evolution enabled, trigger strategy (manual/scheduled), and default evolver agent. For manual trigger, the user's click should count as consent to run the visible direct-edit evolver. The evolver may edit files inside target skill roots directly via `run_bash`; the feature is off by default and rollback/testing is Git-backed. Notification should be default system behavior after apply, not a configurable strategy: record a skill-updated notification outcome, emit a runtime-neutral local UI notification for an active idle standalone target run, queue/skip if busy, and rely on next-run updated skills if the target is not active. Runtime/model-consumed reload instruction is separate future/gated behavior.
...
run configuration enables self-evolution
  ...
  -> when idle/completed, UI shows Self improve
...
- whether future active-run reload needs a dedicated `SkillReloadRequestedEvent` or capability-gated runtime instruction;
...
`/Users/normy/autobyteus_org/autobyteus-worktrees/self-evolving-harness-feasibility/tickets/done/self-evolving-harness-feasibility/design-spec.md`
...
default runtime-neutral target UI notification attempt; next-run correctness baseline
```
...
Architecture review round 1 failed the design on three blocking items, recorded in `/Users/normy/autobyteus_org/autobyteus-worktrees/self-evolving-harness-feasibility/tickets/done/self-evolving-harness-feasibility/design-review-report.md`:
...
- The simplified flow is: manual user trigger -> visible evolver run -> anonymized work-history digest + exact skill roots -> direct skill-root edits via run_bash -> minimal source/evolver run provenance -> default runtime-neutral target UI notification / next-run baseline.
...
## Implementation-Loop UX Design Impact — Discoverable Manual CTA — 2026-06-05

Implementation reported user feedback that the current run-history row sparkle/star self-evolution action is hard to discover and ambiguous. The user noticed the icon but did not understand what it did, and expected many users would have the same issue. Screenshot context showed the sparkle/star icon embedded in run rows without action text, while the active run composer area has space for a clearer contextual action.

Design decision: this should be included in the current MVP, not deferred, because manual triggering is the core MVP interaction. If users cannot discover or understand the manual trigger, the feature is effectively inaccessible.

Updated UX direction:

- The primary self-evolution entrypoint is a labeled contextual CTA placed slightly above the active run composer/text area for the currently selected eligible source run.
- Initial standalone copy considered: `Improve skills from this run`.
- Initial team/member copy considered: `Improve this member's skills from this run`.
- The sparkle/star row action should be removed/reverted; the run row should remain in its simpler previous state without a self-evolution action.
- The CTA uses backend eligibility and the run/member metadata snapshot. Manual start still accepts no config override.
- Hide all CTAs when the global capability is disabled.
- Do not show the CTA for the Skill Self-Evolver helper run itself.
- For ineligible selected runs, the initial thought was hide-or-disable with a short backend reason; this was later superseded by the minimal-UI decision to hide old/ineligible runs by default and avoid technical reason text in the chat UI.
- Helper/tooltip text should explain that a visible Skill Self-Evolver run will review this run's work history and may update configured skill packages, or may make no changes if no reusable improvement is found.
- Copy must avoid implying guaranteed improvement from helper completion.

The requirements and design spec were updated and should return through architecture review before implementation changes.

### UX Refinement Update — Composer CTA Only, No Row Shortcut — 2026-06-05

The user further clarified that the run-row sparkle/star action should not remain even as a secondary shortcut. The row already has other controls such as stop, and adding self-evolution there is too crowded and ambiguous. Updated final UX decision:

- Remove/revert the run-history row self-evolution action.
- The only user-facing manual self-evolution entrypoint is the labeled contextual CTA near the active run composer/text area.
- The run row should return to its previous simpler state without self-evolution controls.
- The CTA remains governed by backend eligibility and run/member metadata snapshot.

Requirements and design spec were updated accordingly while architecture review was pending.

### UX Refinement Update — Minimal CTA And No Start Card — 2026-06-05

The user reviewed the implemented composer CTA screenshots and requested a much cleaner UI:

- Remove the verbose disabled/ineligible reason `Run was created before self-evolution effective-config snapshots were recorded.` from the chat UI. Old/pre-snapshot runs should simply not show the CTA.
- Replace long visible CTA copy such as `Improve this member's skills from this run` with the short label `Self improve` (localized equivalent such as `自我改进`).
- Remove the persistent green start/result card under the composer, including evolution record ID text and `Open Skill Self-Evolver run`.
- Keep the visible Skill Self-Evolver helper run discoverable through normal run/sidebar surfaces rather than adding a second open/run link near the composer.
- Use at most a short transient toast/status such as `Self improve started`.

Design impact: the primary UI should use backend eligibility to decide whether the CTA is present, but it should not surface internal config-snapshot diagnostics or evolution-record plumbing in the main chat area.

### Notification Segment Investigation — 2026-06-05

The user also asked why the active target agent's self-evolution system notification was not shown in the UI as a notification segment even though the frontend has a notification segment component.

Relevant current-code findings:

- `autobyteus-server-ts/src/self-evolution/services/self-evolution-target-notification-service.ts` posts active standalone target notifications with `activeRun.postUserMessage(new AgentInputUserMessage(message, SenderType.SYSTEM))` when the target run is active and idle.
- `autobyteus-ts/src/agent/pipelines/agent-input-pipeline.ts` detects `SenderType.SYSTEM` and calls `notifyAgentDataSystemTaskNotificationReceived({ sender_id, content })`.
- `autobyteus-ts/src/agent/streaming/streams/agent-event-stream.ts` maps that runtime event to `StreamEventType.SYSTEM_TASK_NOTIFICATION`.
...
- `autobyteus-web/components/conversation/segments/SystemTaskNotificationSegment.vue` and `autobyteus-web/services/agentStreaming/handlers/teamHandler.ts::handleSystemTaskNotification` already exist and project such payloads into `system_task_notification` conversation segments.
- `autobyteus-web/services/agentStreaming/TeamStreamingService.ts` dispatches `SYSTEM_TASK_NOTIFICATION` to `handleSystemTaskNotification`.
- `autobyteus-web/services/agentStreaming/AgentStreamingService.ts` includes `SYSTEM_TASK_NOTIFICATION` in `LIVE_RUNTIME_ACTIVITY_MESSAGE_TYPES`, but its dispatch switch has no `SYSTEM_TASK_NOTIFICATION` case. Therefore standalone target notifications can be emitted by the backend/runtime but dropped by the standalone frontend stream dispatcher for UI rendering.
- The self-evolution notification service currently returns `next_run_only` for non-`agent_run` targets, so team-member active notification rendering is not exercised by the MVP target-notification path unless a team-member active notification path is later implemented.

Conclusion at this stage: the missing UI notification segment was not because the segment component was absent. It exposed a standalone frontend dispatch gap. Later code review found an additional design issue: relying on runtime command injection as the source of that event is not runtime-neutral.

### Code Review DI-001 — Runtime-Neutral Local Notification Event — 2026-06-06

Code review rerouted the design because the previous notification contract still conflated two separate concerns:

1. User-visible UI notification that self-improve finished for this run.
2. Runtime/model-consumed instruction asking an active agent to re-read relevant skills.

Current implementation used:

```ts
activeRun.postUserMessage(new AgentInputUserMessage(message, SenderType.SYSTEM))
```

That can work for AutoByteus because its input pipeline can emit `SYSTEM_TASK_NOTIFICATION`, but Codex app-server, Claude Agent SDK, and future runtime backends accept posted messages as runtime input and do not guarantee a corresponding Web UI notification event. Therefore, runtime command injection must not be the canonical UI notification source.

Relevant current-code facts:

- `autobyteus-server-ts/src/agent-execution/domain/agent-run.ts` already has `AgentRun.emitLocalEvent(event)`, which publishes local `AgentRunEvent` instances to active stream listeners without going through a runtime backend.
- `autobyteus-server-ts/src/agent-execution/domain/agent-run-event.ts` already defines `AgentRunEventType.SYSTEM_TASK_NOTIFICATION`.
- `autobyteus-server-ts/src/services/agent-streaming/agent-run-event-message-mapper.ts` maps `AgentRunEventType.SYSTEM_TASK_NOTIFICATION` to websocket `ServerMessageType.SYSTEM_TASK_NOTIFICATION`.
- Frontend notification segment rendering should then use the shared `SYSTEM_TASK_NOTIFICATION` handler and `SystemTaskNotificationSegment`.

Revised design direction:

```text
SelfEvolutionTargetNotificationService
  -> activeRun.emitLocalEvent(AgentRunEventType.SYSTEM_TASK_NOTIFICATION, concise payload)
  -> AgentRunEventMessageMapper
  -> websocket SYSTEM_TASK_NOTIFICATION
  -> shared frontend notification handler
  -> SystemTaskNotificationSegment
```

This local event must be emitted for active idle standalone targets regardless of whether the target runtime is AutoByteus, Codex, Claude, or a future runtime. Runtime/model-consumed instruction is separate:

- MVP must not call `postUserMessage(... SenderType.SYSTEM ...)` merely to show a UI notification.
- If active in-run skill refresh is later required, introduce an explicit runtime capability/policy or a real `SkillReloadRequestedEvent`.
- If a runtime instruction path also emits `SYSTEM_TASK_NOTIFICATION`, the service/stream path must avoid duplicate visible segments for the same self-evolution completion.
- Team-member active notification remains `next_run_only` in MVP unless a concrete active member notification path is designed.

The design spec and requirements were revised accordingly.
