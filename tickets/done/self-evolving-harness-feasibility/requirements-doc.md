...
10. Expose the manual trigger only as a labeled contextual CTA near the active run composer for the currently selected eligible source run; remove/revert the run-history row sparkle/star action.
...
- REQ-024: The manual self-evolution entrypoint must be a discoverable but minimal labeled contextual CTA near the active run composer for the currently selected eligible source run, not an action in the run-history row. The visible CTA copy should be the short label `Self improve` (localized equivalent such as `自我改进`). The CTA must use backend eligibility, must not appear for ineligible/old pre-snapshot runs or Skill Self-Evolver helper runs, and must avoid implying that helper completion guarantees skill improvement.

- REQ-025: The self-evolution UI must stay visually clean after a manual start. It must not show a persistent success card with evolution record IDs, an `Open Skill Self-Evolver run` action, or verbose technical eligibility messages such as `Run was created before self-evolution effective-config snapshots were recorded.` A transient short toast/status is acceptable.

- REQ-026: For active idle standalone target runs, self-evolution completion UI notification must be emitted by the server as a runtime-neutral local `AgentRunEventType.SYSTEM_TASK_NOTIFICATION` event on the target `AgentRun`. This local event is the canonical source for WebSocket `SYSTEM_TASK_NOTIFICATION` and frontend `SystemTaskNotificationSegment` rendering, independent of whether the target runtime is AutoByteus, Codex, Claude, or a future runtime. The frontend must route it consistently into the conversation's system notification segment for standalone runs as well as team/member streams.

- REQ-027: Runtime/model-consumed skill-refresh instruction must be designed as a separate optional step from the UI notification. The MVP must not rely on `activeRun.postUserMessage(new AgentInputUserMessage(..., SenderType.SYSTEM))` to produce the UI notification segment. If a future or runtime-specific implementation also sends a runtime-consumed instruction, it must be capability/policy gated, avoid duplicate visible notifications, and preserve next-run correctness as the baseline.

- REQ-028: Self-evolution completion notification copy should be concise and user-safe; raw skill paths, evolution record IDs, helper run IDs, and implementation details should not appear in the user-visible notification segment.
...
- AC-021: Design specifies the composer-adjacent `Self improve` CTA copy, visibility rules, hidden-by-default ineligible/old-run behavior, accessibility/localization requirements, removal/reversion of the row shortcut, and removal of verbose start/open/status cards.

- AC-022: Design identifies the current system notification segment routing gap and requires local runtime-neutral `SYSTEM_TASK_NOTIFICATION` emission plus handling in standalone frontend streaming in addition to the existing team handler path.

- AC-023: Design separates UI notification from runtime/model instruction, defines duplicate-notification guardrails, preserves team-member MVP `next_run_only` behavior, and requires tests proving local notification emission for mocked active idle standalone AutoByteus, Codex, and Claude runtime kinds.
...
| REQ-024 | UC-003, UC-004 |
| REQ-025 | UC-003, UC-004 |
| REQ-026 | UC-004, UC-005 |
| REQ-027 | UC-004, UC-005 |
| REQ-028 | UC-004, UC-005 |
...
| AC-021 | Verify manual self-evolution is discoverable, concise, and unambiguous in the primary run UI. |
| AC-022 | Verify active-target system notifications render as conversation notification segments on standalone and team/member streams from a runtime-neutral local event. |
| AC-023 | Verify UI notification and runtime/model instruction are separate concerns and runtime-specific instruction is not required for notification rendering. |

## Approval Status

Approved by user for formal design on 2026-06-04. Revised after architecture review round 1 on 2026-06-04 to clarify config snapshots and manual trigger strategy ownership. Revised again after user feedback to make self-evolution runtime/run-config owned rather than agent-definition owned, to target skill folders/packages with anonymized work-history evidence, and to remove dedicated change-recorder/audit and metrics services from the MVP. The formal design spec was further narrowed after fresh architecture review to remove start-mutation config overrides and raw trace path retention from the MVP contracts. Revised again after implementation-loop UX feedback to make the manual self-evolution entrypoint discoverable via a composer-adjacent CTA and to remove/revert the ambiguous row icon. Revised again on 2026-06-05 per user UI feedback to make the CTA minimal (`Self improve`), remove old-run technical reason text and persistent start/open cards from the composer area, and require standalone `SYSTEM_TASK_NOTIFICATION` rendering through the existing notification segment. Revised again on 2026-06-06 after code-review design-impact finding DI-001 to make runtime-neutral local notification event emission the canonical UI path and separate it from optional runtime/model instruction.
...
The user clarified that applied skill improvements should notify active target agents so they can reload or adopt the improved skill. Design implication: changing a skill-root file is insufficient for active runs because runtime skill registries and injected prompts can cache old skill content. Future design should include a skill-update event/notification path and an active-run reload policy. MVP should default to next-run application, with active-run user-visible notification delivered through a runtime-neutral local event; active model reload/instruction remains future/gated behavior.

## User Refinement — Evolver Launch Topology — 2026-06-04

The user asked whether enabling self-evolution at launch should create an agent team containing both the target agent and a self-evolver agent so the evolver can message the target. Analysis: ordinary team membership would expose the evolver in team context/rosters and may distract the target agent from its business role. The recommended architecture is a separate background evolution/reflection run, possibly implemented by an agent/team, connected to the target through an evolution service and a control-plane skill-update notification event. A visible coach/evolver teammate can be a separate explicit product mode, not the default.

## User Refinement — Simplified MVP Knobs — 2026-06-04

The user clarified that MVP should not expose separate approval/notification strategies. User-facing configuration should remain minimal: enabled/off, trigger strategy, and evolver selection. In manual mode, the user's explicit "improve" action should authorize running the visible evolver, which may directly edit target skill roots through existing shell tooling. Target UI notification should be default system behavior, not a separate strategy setting; active runtime/model reload remains future/gated behavior.


## User Refinement — Manual-First Strategy Architecture — 2026-06-04

The user initially discussed agent/team/run configuration, then clarified that self-evolution feels more like runtime/run configuration than an intrinsic agent definition attribute. MVP should therefore make self-evolution configurable at run-launch / team-run launch time, not as an agent definition property. Enabling self-evolution makes the target eligible for self-improvement and exposes a concise manual action such as `Self improve`; it should not imply automatic scheduling yet. The architecture should still use a trigger-strategy boundary so scheduled/cron or signal-based triggers can be added later without redesign. The evolver strategy should start as a single visible self-evolver agent, with multi-agent/team evolvers deferred.
...
The user clarified that the direct-edit MVP should trust the self-evolver's explicit instructions and should not add a redundant change recorder, post-run audit service, or formal metrics service. The simplified MVP boundary is: visible manual evolver run, exact editable skill roots in the task message, `run_bash` with `autoExecuteTools: true`, minimal provenance linking source run to evolver run, and default runtime-neutral target UI notification. Git remains the developer/testing rollback surface, but the product does not need to compute changed paths or harness-benefit metrics in the initial implementation.


## User Refinement — Discoverable Manual Self-Evolution CTA — 2026-06-05

During implementation review, the user observed that the run-history row sparkle/star action is hard to discover and ambiguous. The MVP should remove/revert the row icon action. The only user-facing trigger should be a labeled contextual CTA placed near the active run composer, slightly above the text area, for the currently selected eligible source run. The copy should be minimal: `Self improve` (localized equivalent such as `自我改进`). The UI should hide ineligible/old pre-snapshot runs rather than showing verbose technical reasons, and it should not render a persistent green success card, evolution record ID, or `Open Skill Self-Evolver run` action after start. A short transient toast/status such as `Self improve started` is sufficient. The CTA must not show for the Skill Self-Evolver helper run itself.

## User Refinement — System Notification Segment Visibility — 2026-06-05

The user expected the active target agent's self-evolution notification to appear in the frontend as the existing system notification segment. Initial investigation confirmed that AutoByteus `SenderType.SYSTEM` input can emit `SYSTEM_TASK_NOTIFICATION`, and the frontend already has a `SystemTaskNotificationSegment`; however, the standalone `AgentStreamingService` did not dispatch `SYSTEM_TASK_NOTIFICATION` to that handler. A later code-review design-impact finding clarified a deeper requirement: the UI notification must not depend on runtime command injection, because Codex/Claude/future runtimes do not guarantee that a posted system message emits a UI notification event. Design implication: the MVP should always emit a local runtime-neutral `AgentRunEventType.SYSTEM_TASK_NOTIFICATION` from the server-side target-notification path for active idle standalone target runs, then render that event via the shared frontend notification segment. Any runtime/model-consumed instruction is a separate optional step and must not be the source of the UI segment.
