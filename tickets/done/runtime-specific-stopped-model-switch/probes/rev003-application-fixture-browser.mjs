...
try {
  ...
  await page.getByText('REV003 Model Setup', { exact: true }).first().waitFor({ timeout: 30000 });
  ev.listUrl = page.url();
  ev.listText = (await page.locator('body').innerText()).slice(0, 5000);
  await page.screenshot({ path: dir + 'rev003-application-fixture-list.png', fullPage: true });
  ...
  await page.locator('[data-testid="application-launch-setup-slot-header"]').first().waitFor({ timeout: 60000 });
  ...
  await page.screenshot({ path: dir + 'rev003-application-fixture-setup.png', fullPage: true });
  ev.status = 'Pass';
} catch (e) {
  ev.status = 'Fail'; ev.error = e.message;
  ev.url = page.url(); ev.body = (await page.locator('body').innerText().catch(() => '')).slice(-8000);
  await page.screenshot({ path: dir + 'rev003-application-fixture-failure.png', fullPage: true }).catch(() => {});
} finally {
  await browser.close();
  await fs.writeFile(dir + 'rev003-application-fixture-browser-evidence.json', JSON.stringify(ev, null, 2));
}
