...
// Scenario 1: background bash, then idle -> expect CLI-initiated turn
send("S1", `Use Bash with run_in_background true to run exactly: sleep 12; echo DONE > ${marker} . Then reply only STARTED and end your turn. When later notified it completed, reply only NOTIFIED.`);
await nextResult();
console.log(ts(), "--- S1 result received; staying idle 25s for background completion");
await sleep(25000);
console.log(ts(), "marker exists:", fs.existsSync(marker));

// Scenario 2: mid-turn message while a foreground tool runs
send("S2a", "Use Bash (foreground, not background) to run: sleep 10; echo A_DONE . Then reply with the output.");
await sleep(5000);
send("S2b", "Additional instruction: at the end of your reply, also write the word BANANA.");
await nextResult();
await sleep(8000); // allow a possible second result
console.log(ts(), "--- S2 done, results so far", results);

// Scenario 3: interrupt() during a long foreground tool, then continue in same process
send("S3a", "Use Bash (foreground) to run: sleep 40; echo LONG_DONE . Then reply with the output.");
await sleep(6000);
