...
socket.on('message',raw=>{
 ...
 if(input.marker && JSON.stringify(msg).includes(input.marker)&&!JSON.stringify(msg).includes('SEND_MESSAGE')&&msg.type!=='COMMAND_RESULT') {
   // Keep bounded extra frames; assertions inspect complete captured trace, not this heuristic.
   if(!done){done=true;setTimeout(()=>socket.close(),3000);}
 }
});
