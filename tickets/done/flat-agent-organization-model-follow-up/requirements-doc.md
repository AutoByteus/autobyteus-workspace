...
- Current solution revision: SR-010 (DS-REV-003 preservation correction for F-002; approved behavior baseline remains SR-005).
...
- solution-revision-record.md: cumulative SR-001–010 approval/evidence/design history.
- design-spec.md: DS-REV-003 technical authority realizing this approved basis; no changed product intent. ARCH-REV-001/002 apply to DS-REV-001/002 only.
- solution-handoff.md: complete architecture-ready result and route record.
- bootstrap-handoff.md: historical bootstrap receipt.
No behavior-defining Product supplement applies. DS-REV-002 is a technical correction under unchanged SR-005 intended behavior.
...
- Design: design-spec.md DS-REV-003 Ready, Medium / High; revised review pending for CRR-005/F-002 manual task hydration/control preservation. F-001 resolved in actual API-REV-002; preserve IR-001/002. API overall Fail/confidence75.0%; incomplete B02–B04 and no delivery pass. Team scope may reconnect Active while member providers remain Offline (AINV-015).
...
## SR-009 — unchanged approval, clarified design/validation basis
CRR-003/F-001 exposes the approved kept-open restart scenario before the next Send: retained Org header must reconcile truthfully when the root is verified inactive. This is already required by the UI truth/continuity constraint, not a new requirement or availability guarantee. DS-REV-002 completes DS-002/005 without changing BEH/REQ/AC IDs or requiring new user approval. User reiterates checking standalone Teams and asks Designer to continue; retain the same parity scope. Source plus supplied API evidence shows Team's corresponding retained header becomes Offline; no speculative Team implementation expansion. Inactive inspection is read-only, unknown network/query failure is not Offline, and only legitimate work may restore/start providers. Naming/refactor, migration/reset and release exclusions remain.

## SR-010 — preserved manual task interaction, unchanged approval
CRR-005/F-002 exposes ordinary manual-mode task completion under preserved REQ/AC-003,005 and Team004: first task inspection must retain complete available history and an already-received pending tool decision. DS-REV-003 defines the technical authority boundary; no new always-auto-approval policy or requirement IDs. User's remembered auto-approval question was corrected, not approval to change settings. Recipient autoExecuteTools is preserved, and auto-start does not imply auto-approval. F-001 is resolved in real API-REV-002 and stays resolved; overall validation remains Fail/confidence75.0%. Missing pre-selection live-frame evidence is explicit, and actual frontend acceptance must verify delivery/state as well as the corrected projection. F-003 API-only duplicate-command candidate is rejected, not a required protocol change. Naming/schema/migration/release exclusions unchanged.
