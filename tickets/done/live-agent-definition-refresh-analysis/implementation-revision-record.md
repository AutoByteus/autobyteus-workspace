...
### IR-005 — Application ownership lease and Studio config guard

- Triggering role, report path, and round: architecture_reviewer; `/home/autobyteus/workspace/autobyteus-workspace-live-agent-definition-refresh-analysis/tickets/in-progress/live-agent-definition-refresh-analysis/design-review-report.md`; `ARCH-REV-004` / round 4 after code-review `CRR-006` routed `CR-F-003` upstream.
- Triggering finding IDs: `CR-F-003`, with `MP-CR-003`, `MP-CR-004`, and reclassified `MP-SR4-004`.
- Classification: Design Impact.
- Prior authoritative result: `CRR-006` Fail — Design Impact. Integrated General Process and Application Engine managers are intentionally separate, so Studio's General-only reads and stopped updates could falsely report an Application-owned live identity as stopped and persist configuration through the wrong runtime owner.
- Current authoritative result: Development commit `370f1f5fa` implements SR-005 / ARCH-REV-004. A startup-ready Application ownership reader verifies lookup, canonical provenance, referenced binding identity, exact root/member containment, and binding status; a Studio use-case service guards only the two resume reads and two stopped updates before unchanged General delegation. Live Application updates return `RUN_ACTIVE` with canonical state and no General write; ownership failures fail closed. The implementation is ready for source re-review.
...
- Related delivery revision IDs: `DR-001` is historical integrated-base context only.
- Why this implementation revision is recorded: Records the bounded ownership refactor required by CR-F-003 without merging managers, introducing a cross-owner mutex, leaking Application stores/managers, or broadening unrelated lifecycle operations.
- Approved behavior or requirement IDs affected: `BEH-003`, `BEH-004`, `BEH-005`, `BEH-006`, `BEH-008`; `REQ-002`, `REQ-003`, `REQ-006`, `REQ-009`, `REQ-012`–`REQ-014`; `AC-003`, `AC-004`, `AC-008`, `AC-010`, `AC-014`; `DS-001`, `DS-003`, `DS-006`, `DS-007`, `DS-009`.
- Implementation delta:
  - Added `ApplicationRunOwnershipService` behind `ApplicationPlatformRuntime.hostManagement.runOwnership`. It awaits startup readiness; cross-checks exact run lookup and optional canonical Agent/Team binding provenance; verifies the referenced binding and contained run/root/member identity; classifies `ATTACHED`, `TERMINATING`, and `FAILED` as live and `TERMINATED`/`ORPHANED` as released; and throws on missing, inconsistent, unsupported, or unreadable evidence.
  - Added `StudioRunModelConfigService` with explicit General canonical readers/update facades and the read-only Application ownership port. It locks owner-aware reads, returns canonical `RUN_ACTIVE` with zero General write for live Application updates, delegates unchanged after release, and maps ownership uncertainty to existing fail-closed query/mutation behavior.
  - Routed only `getAgentRunResumeConfig`, `getTeamRunResumeConfig`, `updateStoppedAgentRunModelConfig`, and `updateStoppedTeamRunModelConfigs` through the Studio service. Unrelated Stop, message, archive, delete, launch, restore, and input routes remain unchanged.
  - Composed General canonical readers against the exact General manager/catalog instances and exposed neither manager family. Added architecture constraints prohibiting GraphQL/Studio store or manager bypass.
  - Added focused ownership, Studio orchestration, resolver routing, startup recovery, reentry-provenance, terminal release ordering, terminal-input rejection, composition, and isolation regressions. Existing General Agent/Team lane and Claude configuration checks remain green.
- Changed files or areas: Application orchestration ownership service and runtime host-management composition; General Process canonical readers; Studio config orchestration and service registration; the four Agent/Team GraphQL config entries; focused unit/architecture tests. Exact paths are listed in `implementation-handoff.md`.
