...
try {
  ...
  const reasoningAfter = await reasoningSelect.inputValue();
  const reasoningValidationError = page.locator('#team-scope-root-reasoning_effort-error');
  ...
  recordAssertion('REAL-CODEX-ENUM-SELECTION-VALID',
    reasoningAfter === 'low'
      && await reasoningSelect.getAttribute('aria-invalid') !== 'true'
      && await reasoningValidationError.count() === 0,
    {
      reasoningBefore,
      reasoningAfter,
      ariaInvalid: await reasoningSelect.getAttribute('aria-invalid'),
      validationErrorCount: await reasoningValidationError.count(),
    },
  );
  recordAssertion('REAL-CODEX-ENUM-SAVE-ENABLED', !(await saveButton.isDisabled()), {
    reasoningBefore,
    reasoningAfter,
  });
  ...
  const saveOperation = [...evidence.graphql].reverse()
    .find((entry) => entry.operationName === 'UpdateStoppedTeamRunModelConfigs');
  const saveVariablesText = JSON.stringify(saveOperation?.variables ?? null);
  const canonicalExecutionTreeText = JSON.stringify(saveResult?.canonicalExecutionTree ?? null);
  recordAssertion('REAL-STOPPED-CONFIG-SAVED',
    saveResponse.status() === 200
      && saveResult?.success === true
      && saveResult?.outcome === 'UPDATED'
      && saveResult?.isActive === false,
    { reasoningBefore, reasoningAfter, status: saveResponse.status(), saveResult },
  );
  recordAssertion('REAL-STOPPED-CONFIG-MUTATION-CARRIED-LOW',
    saveVariablesText.includes('reasoning_effort') && saveVariablesText.includes('low'),
    { saveOperation },
  );
  recordAssertion('REAL-STOPPED-CONFIG-CANONICAL-LOW',
    canonicalExecutionTreeText.includes('reasoning_effort')
      && canonicalExecutionTreeText.includes('low'),
    { canonicalExecutionTree: saveResult?.canonicalExecutionTree ?? null },
  );
  await screenshot(page, '08-classroom-stopped-settings-saved');

  // Re-open Settings through the normal UI to force a second network-fresh canonical read. This
  // proves the saved value is not just optimistic client state before the later restore/message.
  await page.locator('[data-test="run-config-back-to-events"]').click();
  await page.locator('[data-test="workspace-header-edit-config"]').click();
  await page.locator('[data-test="team-run-config-form"][data-mode="existing"]').waitFor({ timeout: 60_000 });
  const persistedReasoningSelect = page.locator('#team-scope-root-reasoning_effort');
  await persistedReasoningSelect.waitFor({ timeout: 60_000 });
  const persistedReasoning = await persistedReasoningSelect.inputValue();
  recordAssertion('NETWORK-FRESH-PERSISTED-CODEX-LOW', persistedReasoning === 'low', {
    teamRunId,
    persistedReasoning,
  });
  await screenshot(page, '09-classroom-stopped-settings-reloaded');
  ...
  const providerProofToken = 'CLASSROOM_E2E_LOW_OK';
  await composer.fill(`Reply only with: ${providerProofToken}`);
  const sendButton = page.getByRole('button', { name: 'Send message', exact: true });
  await sendButton.waitFor();
  const sendStartedAt = new Date().toISOString();
  await sendButton.click();
  evidence.snapshots.realMessage = { sentAt: sendStartedAt, prompt: `Reply only with: ${providerProofToken}` };
  const assistantResponse = page.locator('div.flex.items-start.gap-3.pr-8')
    .filter({ has: page.locator('div.bg-emerald-50') })
    .filter({ hasText: providerProofToken })
    .last();
  await assistantResponse.waitFor({ timeout: 300_000 });
  const assistantResponseText = (await assistantResponse.innerText()).replace(/\s+/g, ' ').trim();
  evidence.snapshots.realMessage.assistantResponseText = assistantResponseText;
  evidence.snapshots.realMessage.completedAt = new Date().toISOString();
  recordAssertion('REAL-CLASSROOM-CODEX-TURN', assistantResponseText.includes(providerProofToken), {
    teamRunId,
    providerProofToken,
    assistantResponseText,
  });
  const realTeamSocket = evidence.webSockets.find((socket) =>
    socket.url.includes(`/ws/agent-team/${teamRunId}`));
  recordAssertion('REAL-CLASSROOM-CODEX-WEBSOCKET',
    Boolean(realTeamSocket && realTeamSocket.framesSent > 0 && realTeamSocket.framesReceived > 0),
    { teamRunId, realTeamSocket },
  );
  await screenshot(page, '10-classroom-real-codex-response');
