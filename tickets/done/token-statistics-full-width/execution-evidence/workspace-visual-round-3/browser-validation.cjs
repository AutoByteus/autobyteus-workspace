...
const geometry = async page => page.evaluate(() => {
  ...
  return {
    ...
    feedback: rect('[data-testid="settings-navigation-separator-feedback"]'),
...
function assertGeometry(g, width) {
  ...
  if (width === 0) {
    ...
    assert(near(g.feedback.x, 0) && near(g.feedback.width, 4), 'zero feedback geometry wrong', g);
    ...
    assert(near(g.feedback.x, Math.max(0, width - 2)) && near(g.feedback.width, 4), 'feedback geometry wrong', g);
  ...
  try {
    // Geometry, pointer, keyboard, accessibility, manager-state, session/remount.
    {
      ...
      try {
        ...
        const visualStyles = async () => page.evaluate(() => {
          const edge = document.querySelector('[data-testid="settings-navigation-separator-line"]');
          const feedback = document.querySelector('[data-testid="settings-navigation-separator-feedback"]');
          const target = document.querySelector('[data-testid="settings-navigation-resize-handle"]');
          const es=getComputedStyle(edge), fs=getComputedStyle(feedback), ts=getComputedStyle(target);
          return { edge:{ backgroundColor:es.backgroundColor, boxShadow:es.boxShadow, pointerEvents:es.pointerEvents }, feedback:{ backgroundColor:fs.backgroundColor, transitionProperty:fs.transitionProperty, transitionDuration:fs.transitionDuration, transitionTimingFunction:fs.transitionTimingFunction, pointerEvents:fs.pointerEvents }, target:{ backgroundColor:ts.backgroundColor, pointerEvents:ts.pointerEvents, outlineColor:ts.outlineColor, outlineStyle:ts.outlineStyle, outlineWidth:ts.outlineWidth, outlineOffset:ts.outlineOffset }, blueClassCount:[edge,feedback,target].filter(el=>/blue/i.test(el.className)).length };
        });
        await page.mouse.move(1000, 700); await page.locator('[data-testid="settings-page-content"]').click({ position:{x:500,y:500} }).catch(()=>{}); await page.waitForTimeout(250);
        const restStyle=await visualStyles();
        assert(['rgba(0, 0, 0, 0)','transparent'].includes(restStyle.feedback.backgroundColor), 'rest feedback is not transparent', restStyle);
        assert(restStyle.edge.backgroundColor==='rgb(229, 231, 235)' && /1px 0px 3px/.test(restStyle.edge.boxShadow) && /0\.1/.test(restStyle.edge.boxShadow), 'edge color/shadow wrong', restStyle);
        assert(restStyle.feedback.transitionProperty==='background-color' && restStyle.feedback.transitionDuration==='0.2s' && restStyle.feedback.transitionTimingFunction==='ease', 'feedback transition wrong', restStyle);
        assert(restStyle.edge.pointerEvents==='none' && restStyle.feedback.pointerEvents==='none' && restStyle.target.pointerEvents==='auto', 'layer pointer transparency wrong', restStyle);
        assert(restStyle.blueClassCount===0, 'separator retains blue class', restStyle);
        const handleVisual=page.locator('[data-testid="settings-navigation-resize-handle"]'); const vbox=await box(handleVisual);
        await page.mouse.move(vbox.x+4, 400); await page.waitForTimeout(250); const hoverStyle=await visualStyles();
        assert(hoverStyle.feedback.backgroundColor==='rgb(156, 163, 175)', 'hover feedback wrong', hoverStyle);
        await page.mouse.move(1000,700); await page.keyboard.press('Tab'); await handleVisual.focus(); await page.waitForTimeout(250); const focusStyle=await visualStyles();
        assert(focusStyle.feedback.backgroundColor==='rgb(156, 163, 175)', 'keyboard-focus feedback wrong', focusStyle);
        assert(focusStyle.target.outlineColor==='rgb(107, 114, 128)' && focusStyle.target.outlineStyle==='solid' && focusStyle.target.outlineWidth==='2px' && focusStyle.target.outlineOffset==='-2px', 'focus outline wrong', focusStyle);
        await page.mouse.move(vbox.x+4,400); await page.mouse.down(); await page.waitForTimeout(250); const activeStyle=await visualStyles();
        assert(activeStyle.feedback.backgroundColor==='rgb(107, 114, 128)', 'active feedback or precedence wrong', activeStyle); await page.mouse.up(); await page.mouse.move(1000,700); await handleVisual.blur(); await page.waitForTimeout(250);
        record('BROWSER-R3-000', 'Pass', { geometry:fresh, restStyle, hoverStyle, focusStyle, activeStyle });
        await page.screenshot({ path: path.join(OUT, 'desktop-fresh-1440x900.png'), fullPage: true });
        record('BROWSER-R3-001', 'Pass', fresh);
        ...
        record('BROWSER-R3-002', 'Pass', { partial, zero, hit, restored });
        ...
        record('BROWSER-R3-003', 'Pass', cleanup);
        ...
        record('BROWSER-R3-004', 'Pass', { focusedStyle, axZeroRelevant: axZero.filter(n => n.role === 'separator' || /Back to Workspace/i.test(n.name)), tabAtZero, axNonzeroBack: axNonzero.filter(n => /Back to Workspace/i.test(n.name)) });
        ...
        record('BROWSER-R3-007', 'Pass', { before, after, requests: state.requests });
        ...
        record('BROWSER-R3-006', 'Pass', { sessionWidth, remountWidth: (await geometry(page)).ariaNow });
    ...
    // Breakpoint/native narrow behavior and both exact focus directions.
    {
      ...
      try {
        ...
        record('BROWSER-R3-005', 'Pass', { narrowFromSeparator: { geometry: narrowFromSeparator.geometry, focus: narrowFromSeparator.focus }, desktopFromNav, unrelatedBefore, unrelatedAfter });
      } finally { await context.close(); }
    }
    ...
    for (const phase of ['loading', 'error', 'empty']) {
      ...
      try {
        ...
        record(`BROWSER-R3-008-${phase}`, 'Pass', { requestsBefore: count, requestsAfter: statRequestCount(state) });
    ...
    // Routes, modes and Back. Embedded fallback remains durable unit-covered because emulating bootstrap identity would bypass app ownership.
    {
      ...
      try {
        ...
        record('BROWSER-R3-009', 'Pass', { cases: evidence, backUrl: page.url(), embeddedFallback: 'covered by pages/__tests__/settings.spec.ts without bypassing bootstrap ownership' });
      } finally { await context.close(); }
    }
...
main().then(() => {
  ...
  const output = { result: failed.length ? 'Fail' : 'Pass', targetCommit: 'c448824203a9fd4ffc97e7884a992a7c03863b6f', generatedAt: new Date().toISOString(), results, consoleEvents };
  ...
  const output = { result:'Fail', targetCommit:'c448824203a9fd4ffc97e7884a992a7c03863b6f', generatedAt:new Date().toISOString(), error:error.message, detail:error.detail, stack:error.stack, results, consoleEvents };
