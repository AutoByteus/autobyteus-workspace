...
const stamp = `api_rev_005_${Date.now()}`;
...
const createAgent = async (name, instructions) => {
  ...
  }`, { input: { name: `${name}_${stamp}`, description: 'Disposable API-REV-005 real-runtime fixture.', instructions, category: 'api-e2e-disposable' } });
...
const createTeam = async (name, coordinatorMemberName, nodes, instructions) => {
  ...
  }`, { input: { name: `${name}_${stamp}`, description: 'Disposable API-REV-005 real-runtime fixture.', instructions, category: 'api-e2e-disposable', coordinatorMemberName, nodes } });
