...
const teamWsUrl = process.env.TEAM_WS_URL ?? 'ws://127.0.0.1:60310/ws/agent-team';
...
await new Promise((resolve) => {
  ...
  const poll = waitForTaskTerminal ? setInterval(async () => {
    ...
    try {
      ...
      if (tasks.length > 0 && tasks.every((task) => ['accepted', 'interrupted'].includes(task.status))) {
        clearInterval(poll);
    ...
    if (message.type === 'TEAM_EXECUTION_VIEW_SNAPSHOT' && !sent) {
      ...
      ws.send(JSON.stringify({
        ...
        payload: {
          ...
          message_id: `api-rev-005-${crypto.randomUUID()}`,
          dedupe_key: `api-rev-005-${crypto.randomUUID()}`,
        },
      }));
    }
