...
const stamp = `api_rev_006_${Date.now()}`;
...
const createAgent = async (name, instructions) => {
  ...
  }`, { input: { name: `${name}_${stamp}`, description: 'Disposable API-REV-006 real-runtime fixture.', instructions, category: 'api-e2e-disposable', toolNames: ['delegate_task', 'submit_task_result', 'review_task_result'] } });
...
const createTeam = async (name, coordinatorMemberName, nodes, instructions) => {
  ...
  }`, { input: { name: `${name}_${stamp}`, description: 'Disposable API-REV-006 real-runtime fixture.', instructions, category: 'api-e2e-disposable', coordinatorMemberName, nodes } });
