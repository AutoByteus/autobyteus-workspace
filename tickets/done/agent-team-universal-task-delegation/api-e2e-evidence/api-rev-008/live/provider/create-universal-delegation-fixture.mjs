...
const graphqlUrl = process.env.GRAPHQL_URL ?? 'http://127.0.0.1:60313/graphql';
...
const stamp = `api_rev_008_${Date.now()}`;
...
const createAgent = async (name, instructions) => {
  ...
  }`, { input: { name: `${name}_${stamp}`, description: 'Disposable API-REV-008 real-runtime fixture.', instructions, category: 'api-e2e-disposable', toolNames: ['delegate_task', 'submit_task_result', 'review_task_result'] } });
...
const createTeam = async (name, coordinatorMemberName, nodes, instructions) => {
  ...
  }`, { input: { name: `${name}_${stamp}`, description: 'Disposable API-REV-008 real-runtime fixture.', instructions, category: 'api-e2e-disposable', coordinatorMemberName, nodes } });
