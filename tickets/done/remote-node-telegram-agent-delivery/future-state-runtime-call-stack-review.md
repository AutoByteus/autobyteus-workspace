...
- Runtime Call Stack Document: `tickets/done/remote-node-telegram-agent-delivery/future-state-runtime-call-stack.md`
- Source Design Basis: `tickets/done/remote-node-telegram-agent-delivery/proposed-design.md`
...
- No-backward-compat review rule: the old public-URL callback behavior must not remain as fallback behavior.
...
- Round 1 was clean but cannot open implementation because the workflow requires two consecutive clean deep-review rounds.
- Round 2 remained clean with no artifact changes and no new use cases, so the Stage 5 gate is `Go Confirmed`.
