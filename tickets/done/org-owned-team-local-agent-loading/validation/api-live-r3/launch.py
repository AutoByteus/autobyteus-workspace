...
with (r/(mode+'-r3.log')).open('wb') as log:
 p=subprocess.Popen(cmd,cwd=cwd,env=env,stdout=log,stderr=subprocess.STDOUT);(r/(mode+'-r3-launch.json')).write_text(json.dumps({'pid':p.pid,'cmd':cmd,'environment_keys':list(env)},indent=2));print(mode,p.pid,flush=True);print('EXIT',p.wait(),flush=True)
