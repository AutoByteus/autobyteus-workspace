...
import {gql,config,stop,restore,save,info,evidence} from './api-r2-utils.mjs';
import {createFixture} from './api-r2-fixture.mjs';
...
export async function connect(id){
 ...
 const events=[];socket.on('message',raw=>{ events.push(JSON.parse(raw.toString())); void fs.appendFile(path.join(evidence,'api-r2-events.jsonl'),raw.toString()+'\n'); });
