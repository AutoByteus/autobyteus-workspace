...
const prior = process.argv.includes('--resume') ? JSON.parse(await fs.readFile(path.join(evidence, 'api-r2-stack.json'), 'utf8')) : null;
const runtimeRoot = prior?.runtimeRoot ?? path.join(testRuntimeRoot, `offline-org-r2-${process.pid}-${Date.now()}`);
const database = prior?.database ?? resolveTestDatabaseLocation(`file:./db/offline-org-r2-${process.pid}-${Date.now()}.db`);
...
await fs.writeFile(path.join(evidence, 'api-r2-stack.json'), JSON.stringify(info, null, 2));
console.log(JSON.stringify(info));
const logPath=path.join(evidence,'api-r2-backend-resumed.log');
...
const cleanup = async () => {
  await server.stop();
  await fs.writeFile(path.join(evidence, 'api-r2-backend.log'), server.output());
