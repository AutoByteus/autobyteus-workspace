...
## Document status
Ready for Approval — SR-004 revised baseline, 2026-09-17. Prior SR-001/SR-002 approval remains historical authority for unchanged Settings requirements. API-REV-001/CRR-003 records explicit user clarification expanding Plus to inherit existing configuration, and a separate user request for accurate empty-model feedback. This consolidated revision is presented for confirmation; do not infer that SR-002 already approved inheritance. IR-001 and its successful validation evidence are preserved; dependent correction is on hold.
...
SCN-001/003/004 retain approved SR-001/SR-002 scope. Revised SCN-002 and new SCN-005 are SR-004 proposed consolidated clarification, sourced to API-recorded user directions. Normal scenario basis is the user's screenshots/request and existing Team Settings/new-run flows. Alternate/failure basis is existing stopped-run validation and retained identity contract, not a new recovery product.
...
Readiness: evidence, scope, intended/preserved behavior, traceable ACs and data constraints complete. SR-004 Ready for Approval. Dependent design Needs Revision until confirmation; original Settings design/evidence retained. Consolidated scope changes are confined to REQ-005/AC-004 and REQ-007/AC-007.
