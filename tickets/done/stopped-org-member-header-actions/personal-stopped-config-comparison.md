...
## SR-007 correction: projection is not freshness
Pinned personal5645b49d6 Team saveTeam also writes the canonical history cache and Settings planner, while Plus reads retained immutable view configuration. Therefore the earlier comparison proves the configuration-only projection pattern, not successful Save→Plus fidelity. No old binary was tested. DS-REV-003 retains the pure factory/seed helper but sources Team new drafts from the existing canonical resume read. Both Team source-copy callers use that single authority; retained execution/presentation contexts are not reconstructed. Org already uses canonical inspection and its actual successes remain intact.
