...
describe('temporary Solution Designer Claude SDK tool-turn usage shape probe', () => {
  it('records safe numeric/model data from a bounded pwd tool turn', async () => {
    ...
    try {
      query = await client.startQueryTurn({
        ...
        model: 'opus[1m]', workingDirectory: workspace, allowedTools: ['Bash'],
        permissionMode: 'default', autoExecuteTools: false,
        canUseTool: async (name, input, options) => {
          if (name !== 'Bash' || input.command !== 'pwd') return { behavior: 'deny', message: 'Only pwd permitted for probe.' };
      ...
      console.log(JSON.stringify({ selected: 'opus[1m]', counts, assistants,
