...
describe('temporary cost-bounded live selected Claude SDK context', () => {
  it('selects Opus on active query and prices its real terminal usage without response text', async () => {
    ...
    try {
      ...
      const prompt = event?.params.latest_prompt_tokens;
      const capacity = event?.params.effective_context_window_tokens;
      const percent = event?.params.context_window_usage_percent;
      console.log(JSON.stringify({ live_prompt_tokens: prompt, live_capacity_tokens: capacity,
        live_context_percent: percent, stored_context_percent: summary?.context_window_usage_percent }));
      expect(typeof prompt).toBe('number');
      expect(capacity).toBe(1_000_000);
      expect(percent).toBeCloseTo(100 * (prompt! / capacity!), 10);
      expect(summary?.context_window_usage_percent).toBeCloseTo(percent!, 10);
    } finally {
      query?.close();
