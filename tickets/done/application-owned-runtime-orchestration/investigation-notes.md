...
- Current Status: Deep investigation completed; design reworked through architecture review round 13, further deepened with startup isolation for invalid applications plus persisted application resource-configuration requirements, and now extended to separate persisted app platform state from live bundle validity and to concretize package-registry / startup-inventory ownership after desktop startup failure evidence
...
- Follow-up user clarification on `2026-04-21`: persisted app state for removed/missing applications must not be treated as a fatal startup dependency; package registry, catalog discovery, and durable platform state should be separated clearly in the target design
...
| 2026-04-21 | Log/Code | User-provided Electron startup log in this ticket thread; `autobyteus-server-ts/src/server-runtime.ts`, `application-orchestration/services/application-orchestration-recovery-service.ts`, `application-storage/stores/application-platform-state-store.ts`, `application-storage/services/application-storage-lifecycle-service.ts`, `application-orchestration/stores/application-run-binding-store.ts`, `application-orchestration/stores/application-execution-event-journal-store.ts`, `application-bundles/providers/file-application-bundle-provider.ts` | Explain why the installed desktop build still crashes even though invalid applications are supposed to be quarantined | The server binds and starts listening successfully, then crashes during post-listen application-orchestration startup recovery. Recovery enumerates persisted known application ids from binding/journal stores, but binding/journal access still goes through bundle-dependent platform-state preparation (`ensurePlatformStatePrepared -> requireApplicationBundle`), so one missing app bundle throws `Application '...' was not found.` and `server-runtime.ts` exits the whole process. Separately, bundle-root scan failures can still disappear silently because `scanBundleRoot(...)` returns `[]` when the package root cannot be enumerated, so missing imported package roots need explicit diagnostics. The target design must separate package registry, best-effort catalog snapshot, and persisted app platform state, and it must support bundle-independent access to existing per-app platform DB state during recovery and journaling. | Yes |
| 2026-04-21 | Code | `autobyteus-server-ts/src/application-packages/services/application-package-service.ts`, `application-packages/stores/application-package-registry-store.ts`, `application-packages/stores/application-package-root-settings-store.ts` | Determine whether the current codebase already has a concrete home for package-registry ownership | The current `ApplicationPackageService` already owns package import/remove flows and composes `ApplicationPackageRegistryStore` plus `ApplicationPackageRootSettingsStore`; this is the natural subsystem to tighten into one authoritative package-registry boundary instead of leaving package-root persistence implicit inside bundle discovery. | Yes |
...
| `autobyteus-server-ts/src/application-packages/services/application-package-service.ts` and package registry/root-settings stores | Imported package persistence + remove flows | Current code already centralizes package import/remove and root persistence there, even though bundle discovery still bypasses it for direct store reads | Tighten this area into one authoritative package-registry owner instead of inventing a separate conceptual registry with no boundary |
| `autobyteus-server-ts/src/application-storage/stores/application-platform-state-store.ts` | Per-application platform DB access | `withDatabase(...)` currently forces bundle-dependent platform-state preparation before opening the DB; existing-state access without a live bundle is missing | The storage boundary must split prepared-state creation from existing-state recovery/journaling access |
| `autobyteus-server-ts/src/application-orchestration/stores/application-run-binding-store.ts` and `application-execution-event-journal-store.ts` | Durable binding/journal persistence | Recovery reads and some journal writes still route through bundle-dependent platform-state access, which turns stale app state into a startup killer | These stores need explicit existing-state access paths for recovery and quarantined-app journaling |
| `autobyteus-server-ts/src/application-bundles/providers/file-application-bundle-provider.ts` | Best-effort application discovery | Missing/unreadable package roots can currently disappear silently because `scanBundleRoot(...)` returns no results when the root cannot be enumerated | The discovery snapshot needs package-level diagnostics, not silent omission |
...
- Latest desktop startup evidence on `2026-04-21` added four more concrete design obligations:
  - package registry, best-effort catalog discovery, and persisted app platform state need distinct authoritative owners,
  - existing persisted platform-state access for recovery/journaling must not require a live bundle,
  - missing imported package roots must become explicit diagnostics instead of silent discovery gaps, and
  - one app-specific recovery failure must degrade to quarantine rather than crash the whole server when shared core stores are still healthy
...
- Current recovery/journaling access to per-app platform state still depends on bundle-dependent storage preparation, so the target design must split `prepared` versus `existing` platform-state access semantics explicitly
- Current `ApplicationPackageService` plus its two package stores already give the codebase a concrete subsystem for imported package persistence and remove flows; the target design should reuse/tighten that subsystem instead of leaving package-registry ownership conceptual
