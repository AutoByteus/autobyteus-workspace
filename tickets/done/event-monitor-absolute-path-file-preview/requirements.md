...
Refined — post-implementation verification clarifications pending bounded local fixes
...
| BEH-010 | User/System | The current absolute-path normalizer accepts a literal placeholder component such as `/Users/normy/autobyteus_org/.../file.md`, so a supported suffix can incorrectly receive an action. | A candidate must be a syntactically complete absolute path with no `.`/`..`/`...`/`…` placeholder or traversal component; invalid candidates remain in the original Markdown/prose/code rendering with no Event Monitor Files action. | Filesystem existence/readability checks remain deferred to the trusted content boundary for syntactically complete supported candidates. | REQ-002/003/004/017; AC-020 |
| BEH-011 | User | Supported Event Monitor paths currently render an adjacent bordered `Open ... in Files` button, which is visually heavy. | Render the supported path/label as an inline, underlined, keyboard-accessible action while preserving the same typed launcher, read-only preview, and source-copy behavior. | Unsupported/invalid paths, ordinary Markdown links, and non-Event-Monitor consumers retain their existing rendering. | REQ-004/005/013/015/018; AC-021 |
| BEH-012 | User/System | The Nodes item is present in the strip inventory, but its custom `autobyteus:nodes-network` icon is passed to Iconify without a registered definition and appears blank. | Render the same visible nodes-network SVG used by the expanded navigation panel in strip mode while preserving the existing Nodes label, route, and capability gating. | Other strip icons, navigation, and non-strip panels remain unchanged. | REQ-019; AC-022 |
| BEH-013 | User | Generated inline file actions still visibly include `Open` and `in Files`, which is redundant for an underlined clickable file label. | Show only the file display label for generated actions; retain non-visible accessibility metadata and the same action/preview behavior. | Authored Markdown labels, unsupported/invalid source rendering, and generic Markdown remain unchanged. | REQ-018/020; AC-021/023 |
...
| --- | --- | --- | --- | --- | --- |
| `/Users/normy/autobyteus_org/autobyteus-worktrees/event-monitor-absolute-path-file-preview/tickets/done/event-monitor-absolute-path-file-preview/task.md` | User-provided intake, scope, security requirements, and acceptance criteria | REQ-001–REQ-015 | AC-001–AC-018 | User-provided; treated as approved kickoff input | Authoritative scope and environment/security basis |
| `/Users/normy/autobyteus_org/autobyteus-worktrees/event-monitor-absolute-path-file-preview/tickets/done/event-monitor-absolute-path-file-preview/event-monitor-absolute-path-reference.png` | UX reference screenshot | REQ-001, REQ-002, REQ-007 | AC-001, AC-003–AC-005 | N/A; evidence/reference only | Confirms full path text remains visible/copyable in the conversation |
| `/Users/normy/autobyteus_org/autobyteus-worktrees/event-monitor-absolute-path-file-preview/tickets/done/event-monitor-absolute-path-file-preview/user-verification-unsupported-file-preview-report.md` | Post-build user verification and bounded local-fix evidence | REQ-016 | AC-019 | User-provided clarification; intended behavior approval applicable | Records that unsupported `.dmg`/`.zip` paths should not show an Event Monitor Files action |
| `/Users/normy/autobyteus_org/autobyteus-worktrees/event-monitor-absolute-path-file-preview/tickets/done/event-monitor-absolute-path-file-preview/user-verification-invalid-absolute-path-report.md` | User verification evidence and pure syntax-guard decision | REQ-017 | AC-020 | User-provided clarification; intended behavior approval applicable | Records that placeholder/truncated absolute paths remain original without a Files action |
| `/Users/normy/autobyteus_org/autobyteus-worktrees/event-monitor-absolute-path-file-preview/tickets/done/event-monitor-absolute-path-file-preview/user-verification-inline-file-link-report.md` | User verification evidence and approved inline-link UX decision | REQ-018 | AC-021 | User-provided clarification; intended behavior approval applicable | Records replacement of the bulky action button with inline link-style rendering |
| `/Users/normy/autobyteus_org/autobyteus-worktrees/event-monitor-absolute-path-file-preview/tickets/done/event-monitor-absolute-path-file-preview/user-verification-strip-nodes-icon-report.md` | User verification evidence and strip icon rendering decision | REQ-019 | AC-022 | User-provided clarification; intended behavior approval applicable | Records the missing Nodes icon in strip mode and the existing expanded-panel SVG owner |
| `/Users/normy/autobyteus_org/autobyteus-worktrees/event-monitor-absolute-path-file-preview/tickets/done/event-monitor-absolute-path-file-preview/user-verification-file-link-label-report.md` | User verification evidence and visible-label cleanup decision | REQ-020 | AC-023 | User-provided clarification; intended behavior approval applicable | Records removal of redundant visible `Open`/`in Files` wording while preserving accessibility metadata |
...
- REQ-002: Recognize POSIX paths beginning with a single `/` and Windows drive-absolute paths matching `^[A-Za-z]:[\\/]`, with at least one non-root path component. A syntactically complete candidate must not contain `.`/`..`/`...`/`…` as a complete path component, which prevents traversal and display-truncation placeholders from becoming actions. Bare-prose candidates are non-whitespace tokens; explicit link destinations and code tokens may contain encoded or literal spaces where their Markdown/code boundary is unambiguous. Trim sentence punctuation and balanced closing delimiters without changing the source text.
...
- REQ-017: Event Monitor action eligibility must reject syntactically incomplete or placeholder absolute paths. Reject `.`/`..`/`...`/`…` path components in the pure normalizer; preserve the original Markdown/prose/code rendering and do not perform filesystem/workspace reads for rejected candidates.
- REQ-018: Supported Event Monitor path actions must render as inline, underlined, keyboard-accessible link-style controls instead of separate bordered `Open ... in Files` buttons. Markdown-link labels remain authored; bare paths remain visible/copyable; inline/fenced code source remains copy-faithful. The action event, launcher, read-only intent, and generic Markdown behavior remain unchanged.
- REQ-019: When the Nodes item is available in the responsive left-navigation strip, it must render a visible nodes-network icon using the existing navigation icon design, retain its accessible label/title, and navigate to `/nodes`. The item remains hidden when its existing desktop-settings capability is unavailable.
- REQ-020: Generated inline Event Monitor file actions must display only the file label (normally the basename), without visible `Open` or `in Files` wording. Preserve an accessible descriptive label/title and the existing typed event, keyboard behavior, read-only preview, source copying, and generic Markdown behavior.
...
- AC-020: An absolute-looking candidate containing a placeholder/traversal component such as `/Users/normy/autobyteus_org/.../compaction-lifecycle-contract.md`, `/tmp/../report.md`, or `/tmp/…/report.md` remains in the original Markdown/prose/code rendering without an Event Monitor Files action, panel switch, local-file URL, Electron text read, or workspace request. A normal complete path such as `/Users/normy/autobyteus_org/project/compaction-lifecycle-contract.md` remains eligible subject to the supported-type gate.
- AC-021: A supported complete Event Monitor path renders as an inline underlined action using its authored label or visible path text, with no adjacent bordered Open-in-Files button. Pointer, Enter, and Space activation emit the same file action and preserve read-only Files behavior; the visible source/path remains copyable, and invalid/unsupported candidates remain non-action text.
- AC-022: In strip mode with desktop settings available, the Nodes navigation button contains a visible nodes-network SVG/icon, has the `Nodes` accessible label/title, and routes to `/nodes`; with desktop settings unavailable, neither the Nodes button nor its icon is rendered.
- AC-023: Generated supported-path actions visibly render only the file label (for example, `compaction-lifecycle-contract.md`), not `Open compaction-lifecycle-contract.md in Files`; pointer, Enter, and Space still emit the same action, and non-visible accessibility metadata remains present.
