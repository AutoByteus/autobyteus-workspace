...
| Date | Source Type | Exact Source / Command | Purpose | Observation | Design Impact? |
| --- | --- | --- | --- | --- | --- |
| 2026-05-13 | Validation report | `tickets/done/mixed-team-nested-agent-team/api-e2e-validation-report.md` | Review latest validation result. | Round 3 is `Fail`; backend nested runtime passed but frontend flattened the active team UI. | Yes: frontend nested display/context is now in scope. |
| 2026-05-13 | Failure note | `tickets/done/mixed-team-nested-agent-team/fullstack-nested-team-ui-validation-failure.md` | Read reproduction, backend evidence, frontend evidence, and setup. | Backend `getTeamRunResumeConfig` returned recursive `memberTree` with `BuildSquad` as `agent_team`; UI showed only `program_manager`, `review_lead`, `qa_specialist`. | Yes: display tree must use recursive topology. |
...
## Round 35 Initializing Status Git Integration Investigation (2026-05-17)

Delivery/user verification of the Electron candidate exposed a startup-status regression: an individual agent visibly transitions `offline -> initializing -> offline/done` after user send, and team/nested team members do not consistently show the expected `offline -> initializing -> running` path. I used git and current-code inspection to compare the merged `origin/personal` initializing work with this branch.

### Commands / Sources Consulted

| Date | Source Type | Exact Source / Command | Purpose | Observation | Design Impact? |
| --- | --- | --- | --- | --- | --- |
| 2026-05-17 | Git | `git log -1 --oneline` | Confirm current branch HEAD. | Current HEAD is `54cacc2a fix(status): remove provider lifecycle residue`. | Yes: latest status cleanup is after the nested-team merge work. |
| 2026-05-17 | Git | `git log -1 --oneline origin/personal` | Confirm current base. | `origin/personal` is `5f6e8dde chore(release): bump workspace release version to 1.3.16`. | Yes: branch contains latest personal base, so failure is not caused by missing base merge. |
| 2026-05-17 | Git | `git log --oneline --decorate origin/personal --grep='initializing' --all -n 10` | Locate initializing-status ticket work. | Found `5f1c4408 Merge branch 'codex/agent-initializing-status-ux' into personal` and related checkpoint/finalization commits. | Yes: initializing UX is present in personal. |
| 2026-05-17 | Git | `git log --oneline origin/personal..HEAD -- autobyteus-web/services/runHydration/runtimeStatusNormalization.ts autobyteus-web/services/runStatus/agentRuntimeStatusState.ts autobyteus-web/stores/agentRunStore.ts autobyteus-web/stores/agentTeamRunStore.ts autobyteus-server-ts/src/agent-execution/domain/agent-status-payload.ts autobyteus-server-ts/src/services/agent-streaming/agent-stream-handler.ts autobyteus-server-ts/src/services/agent-streaming/agent-team-stream-handler.ts autobyteus-server-ts/src/agent-team-execution/domain/team-status-aggregation.ts` | Identify current-branch status changes after personal. | Current branch changes include `7ce0ea49 fix(status): reject removed lifecycle tokens`, `54cacc2a fix(status): remove provider lifecycle residue`, and nested command/status integration commits. | Yes: later cleanup/refactor altered status behavior after personal was merged. |
| 2026-05-17 | Git diff | `git diff origin/personal -- autobyteus-web/services/runHydration/runtimeStatusNormalization.ts` | Compare frontend status normalization. | `origin/personal` mapped startup/lifecycle tokens such as `bootstrapping`, `starting`, `startup`, `uninitialized` to `initializing`; current branch accepts only canonical `initializing` plus small `active`/`terminated` mappings. | Yes: cleanup over-narrowed internal/frontend handling and conflicts with the initializing UX baseline. |
| 2026-05-17 | Git diff | `git diff origin/personal -- autobyteus-server-ts/src/agent-execution/domain/agent-status-payload.ts` | Compare backend status projection. | Current branch removed broad backend normalization sets that projected provider startup tokens to canonical public statuses. | Yes: provider-edge adaptation was removed along with public alias cleanup. |
| 2026-05-17 | Code | `autobyteus-web/services/runStatus/agentRuntimeStatusState.ts` | Inspect local accepted-startup status owner. | `applyAcceptedStartupStatus()` sets `currentStatus = initializing`, but `applyLiveAgentStatusEvent()` blindly overwrites with any live snapshot and clears `isSending` for non-running/non-initializing statuses. | Yes: passive initial snapshots can downgrade local accepted startup to `offline`/`idle`. |
| 2026-05-17 | Code | `autobyteus-server-ts/src/services/agent-streaming/agent-stream-handler.ts` | Inspect single-agent WebSocket connect sequence. | On connection the server immediately sends `AGENT_STATUS activeRun.getStatusSnapshot()` after `CONNECTED`. This can arrive before the user's `SEND_MESSAGE` work starts. | Yes: explains observed `initializing -> offline/idle/done` flicker after local optimistic startup. |
| 2026-05-17 | Code | `autobyteus-server-ts/src/agent-team-execution/backends/mixed/mixed-team-manager.ts`, `autobyteus-server-ts/src/services/agent-streaming/team-runtime-status-snapshot-service.ts`, `autobyteus-web/services/agentStreaming/TeamStreamingService.ts` | Inspect team member snapshot/routing path. | Mixed member snapshots currently include `agent_id`/`agent_name`; the nested frontend stream router prioritizes canonical route/path identity. Initial team snapshots should carry `member_route_key`/`source_path` so strict nested routing can apply consistently without display-name fallback. | Yes: team/nested startup status routing has an identity gap. |
| 2026-05-17 | Code | `autobyteus-web/stores/agentTeamRunStore.ts` | Inspect focused team send path. | Leaf focused sends can apply local member/team initializing; group/subteam focus does not have a leaf `focusedMemberContext`, so group startup state needs explicit handling and backend-resolved child coordinator events must drive leaf status. | Yes: team/subteam startup semantics need a dedicated spine, not only leaf-agent logic. |
| 2026-05-17 | Docs/code | `autobyteus-server-ts/docs/modules/agent_streaming.md`, `autobyteus-web/docs/agent_execution_architecture.md`, `autobyteus-web/docs/agent_integration_minimal_bridge.md` | Check status docs from personal branch behavior. | Docs still describe provider startup tokens projecting to canonical `initializing`. Current code removed that mapping. | Yes: design must distinguish public canonical wire status from internal provider-edge adaptation. |

### Investigation Conclusion

The current branch has merged the personal initializing-status UX, but later nested-team/status cleanup changed the status data-flow in a way that regresses it. The failure is not fixed by another base merge. The branch needs a targeted status-spine integration update:

1. Keep public/frontend status payloads canonical (`offline`, `initializing`, `idle`, `running`, `error`).
2. Restore provider-edge/raw lifecycle mapping before publication: provider tokens like `bootstrapping`, `starting`, `startup`, and `uninitialized` map to canonical `initializing`; active processing states map to `running`; provider failures map to `error`; shutdown/stopped states map to `offline`.
3. Add an accepted-startup invariant in the frontend status owner: after a user send/start is accepted and local state is `initializing`, passive initial WebSocket/history snapshots of `offline`/`idle` cannot downgrade the UI before runtime activity, explicit error, or explicit termination. Prefer explicit snapshot/source classification if available; otherwise the status owner must treat the connection's initial snapshot as passive while startup is pending.
4. Team/nested startup must use the same invariant. Leaf sends set leaf/team `initializing`; subteam group sends set group/aggregate `initializing`, and backend route/path events drive the resolved child coordinator leaf state.
5. Team status snapshots must include canonical route/path identity (`member_route_key` and/or `source_path`) so strict nested frontend routing does not depend on `agent_name`/`agent_id` fallback.
6. Tests should cover the exact regression sequence: individual agent `offline -> initializing` is not downgraded by the first passive snapshot, team leaf sends preserve initializing, subteam group sends show group initializing and then child coordinator activity, and provider-edge startup tokens project to canonical `initializing`.

## Round 36 Backend Status Source-of-Truth Correction (2026-05-17)

After the Round 35 git comparison, the user challenged the earlier frontend-startup-guard framing and clarified the intended architecture: backend should be the source of truth for runtime status. I re-checked the current code path and design principles. The `Authoritative Boundary Rule` applies directly: canonical runtime status is a backend/runtime lifecycle concern, so frontend must not own the same canonical field.

Additional evidence:

| Date | Source Type | Exact Source / Command | Purpose | Observation | Design Impact? |
| --- | --- | --- | --- | --- | --- |
| 2026-05-17 | Code search | `rg -n "applyAcceptedStartupStatus|applyAcceptedTeamMemberStartupStatus|beginLocalUserSubmission|AgentStatus\\.Initializing|initializing" autobyteus-web/services autobyteus-web/stores` | Verify whether frontend authors canonical initializing. | `agentRunStore.ts` and `agentTeamRunStore.ts` call `beginLocalUserSubmission(... applyInitializing ...)`; `agentRuntimeStatusState.ts` implements `applyAcceptedStartupStatus()` and `applyAcceptedTeamMemberStartupStatus()` that set `context.state.currentStatus = AgentStatus.Initializing`. | Yes: frontend currently writes canonical runtime status. |
| 2026-05-17 | Code | `autobyteus-server-ts/src/agent-execution/domain/agent-status-payload.ts` | Verify backend public status contract. | Backend status type is canonical: `"offline" | "initializing" | "idle" | "running" | "error"`. | Yes: backend already defines the authoritative public status vocabulary. |
| 2026-05-17 | Code | `autobyteus-server-ts/src/services/agent-streaming/agent-stream-handler.ts` | Inspect status snapshot source. | Server sends `AGENT_STATUS activeRun.getStatusSnapshot()` on stream connect. Snapshot must be treated as backend truth; if startup has been accepted, backend snapshot should itself be `initializing`. | Yes: fix should be backend lifecycle state, not frontend override. |
| 2026-05-17 | Code | `autobyteus-server-ts/src/agent-team-execution/backends/mixed/mixed-team-manager.ts`, `team-runtime-status-snapshot-service.ts` | Inspect team status snapshot source. | Team member status snapshots are produced by backend team managers and streamed as `AGENT_STATUS`/`TEAM_STATUS`. They need route/path identity and correct backend-owned initializing transitions. | Yes: nested team status must be backend-driven too. |

Corrected design conclusion:

- Round 35's central finding that current branch regressed initializing status remains valid, but the target ownership is corrected.
- Frontend may own local pending submit UI (`isSending`, disabled controls, optimistic message rows), but must not write canonical `currentStatus = initializing`.
- Backend runtime lifecycle owners must set and publish/snapshot canonical `initializing` when create/restore/send/start work is accepted and startup is in progress.
- `getStatusSnapshot()` for an accepted startup must return `initializing`; if it returns `offline`/`idle` after backend acceptance, that is a backend lifecycle/snapshot bug.
- Provider raw startup states still map internally to canonical backend `initializing` before publication.
- Team/nested team member status must follow the same backend-owned model, with canonical route/path identity in snapshots/events.
