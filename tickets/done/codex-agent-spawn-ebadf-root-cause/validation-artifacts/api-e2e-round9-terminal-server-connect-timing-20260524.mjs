...
const summaryPath = path.join(artifactDir, 'api-e2e-round9-terminal-server-connect-timing-20260524.json');
const eventLogPath = path.join(artifactDir, 'api-e2e-round9-terminal-server-connect-timing-20260524.log');
const serverLogPath = path.join(artifactDir, 'api-e2e-round9-terminal-server-connect-timing-20260524-server.log');
const finalLsofPath = path.join(artifactDir, 'api-e2e-round9-terminal-server-connect-timing-20260524-final-lsof.log');
...
async function main() {
  ...
  for (let i = 1; i <= 8; i += 1) {
    ...
    const outputMarker = `__AB_TERMINAL_TIMING_${i}__`;
    sendInput(socket, `printf "${outputMarker}:%s\\n" "$PWD"\n`);
    const { outputMs, outputTail } = await waitOutput(socket, outputMarker);
    const closeMs = await closeSocket(socket);
    await sleep(150);
    const item = { id, openMs, outputMs, closeMs, outputTail };
