...
Native prompt processing is additionally open-ended at two layers. `systemPromptProcessorNames` was authorable through file config, GraphQL, agent-management tools, and UI. Independently, core `AgentConfig` publicly accepts, stores, copies, and globally defaults an arbitrary `BaseSystemPromptProcessor[]`; `SystemPromptPipeline` executes that list; definition/registry/registration APIs remain package-exported. The first implementation removed only the server authoring layer. The carpenter contract makes Skills terminal, so both layers and the generic core pipeline must be removed. One platform-owned configured-Skills catalog appender is called directly by the native final-instruction step and is not injected through configuration or a registry.
...
- native AutoByteus: `AgentConfig.systemPrompt`, followed only by a direct platform-owned native `## Skills` catalog append inside `SystemPromptProcessingStep`;
...
| `BEH-008` | Contract | `R-010`, `AC-010` | Final prompt assembly | Findings 19, 35–39, 46–48; `CR-001`, `CR-002` | Stable ordered sections, correct fence-aware heading containment, and no configurable/public generic prompt-processor surface | Composer ordered assembly -> projection -> direct platform-owned native Skills append (`DS-001`, `DS-004`, `DS-005`) |
...
- Evidence: three runtimes independently constructed overlapping instruction subjects; native composed before resolving workspace; Claude placed persistent rules in every user turn; team prose was conditional on configurable provider tools even though the team context requires basic collaboration; team-definition lookup suppressed errors; and the first implementation removed optional processor selection only above core while `AgentConfig` plus the generic core pipeline still permitted arbitrary prompt mutation.
...
- Remove the optional system-prompt-processor authoring/runtime surface at both server and core boundaries. Replace the generic processor pipeline with one direct, platform-owned configured-Skills catalog append inside `SystemPromptProcessingStep`.
...
| `DS-005` | Bounded Local | `BEH-002`, `008`, `012` | Native carpenter portion + direct terminal Skills output | Final native provider instruction | `SystemPromptProcessingStep` | Singularly sequences the only platform-owned append, final invariant, state mutation, and LLM configuration |
...
- Native prompt: `AgentRunConfig -> AutobyteusAgentRunBackendFactory -> resolve workspace -> CarpenterPromptComposer -> AgentConfig.systemPrompt -> SystemPromptProcessingStep -> appendConfiguredSkillsCatalog -> final invariant -> LLM configureSystemPrompt -> provider request`
...
| `DS-005` | `SystemPromptProcessingStep` directly invokes the configured-Skills catalog appender, scans the complete result, and only then stores/configures it; a match fails bootstrap through the existing `AgentErrorEvent` path and the LLM is never configured with that payload. No caller-supplied processor list exists. | Final native system prompt | `SystemPromptProcessingStep` | Configured-Skills catalog appender |
...
- Native `SystemPromptProcessingStep` owns the direct terminal configured-Skills append, validation of the resulting complete provider instruction, state mutation, and `llmInstance.configureSystemPrompt`; failure uses its existing critical bootstrap failure/`AgentErrorEvent` behavior and never invokes the provider.
- `TeamRuntimeInstructionRenderer` owns only framework-generated current-member, communication, roster, and delegation prose.
- The shared runtime-tool exposure resolver owns normalization, deduplication, and automatic `send_message_to`/`delegate_task` union for `MemberTeamContext`; provider adapters consume that result without adding prompt logic.
- SkillService/materializers and the native configured-Skills catalog appender own configured skill exposure; they do not own the foundation.
...
| `appendConfiguredSkillsCatalog` | Native skill catalog | Purely append the validated configured catalog | Agent/team/environment content or runtime registration |
| `SystemPromptProcessingStep.execute` | Native final instruction boundary | Invoke the one platform-owned catalog append, validate the complete payload, then configure the LLM | Arbitrary processor execution, skill discovery, or carpenter composition |
...
| Core `AgentConfig.DEFAULT_SYSTEM_PROMPT_PROCESSORS`, `systemPromptProcessors` property/constructor/copy parameter | Still permits global mutation and caller injection after server authoring removal | Skill inputs stay on `AgentConfig`; `SystemPromptProcessingStep` directly invokes the terminal catalog appender | In This Change | Remove the constructor slot rather than leaving a null placeholder; update every positional caller and config test |
| `SystemPromptPipeline` and its barrel export | Generic execution boundary has no valid owner once Skills is direct | Direct sequence in `SystemPromptProcessingStep` | In This Change | Delete file/export/tests/usages |
| `BaseSystemPromptProcessor`, `SystemPromptProcessorDefinition`, `SystemPromptProcessorRegistry`, default registry, registration function, and package exports | Dormant/public optional-extension contract | Focused configured-Skills catalog appender API only | In This Change | Delete source and obsolete registry/definition/base/registration tests; do not retain aliases |
...
- `DS-005`: `shared carpenter prompt -> SystemPromptProcessingStep direct configured-Skills append -> complete-payload unresolved-placeholder assertion -> state/LLM configuration or bootstrap failure`.
...
| Native final-payload validation | `DS-005` | System prompt bootstrap | Validate the output of the one direct platform-owned Skills append | Only this point sees terminal Skills metadata | Invalid payload reaches LLM/provider |
...
Native prompt authority changes after the composer: `SystemPromptProcessingStep` calls `appendConfiguredSkillsCatalog(baseSystemPrompt, context)` exactly once, then runs the unresolved-placeholder matcher against that complete result before assigning `processedSystemPrompt` or calling `llmInstance.configureSystemPrompt`. A match throws/fails the existing bootstrap step, posts its `AgentErrorEvent`, and does not configure or invoke the LLM. `AgentConfig` carries skill names/access mode but no prompt processors, and no generic registry or pipeline can insert another mutation. The composer keeps an earlier defense for its portion, but must not call that earlier string the final provider payload.
...
| `SystemPromptProcessingStep.execute` | direct native Skills append + final-payload assertion + LLM configuration | native agent bootstrap | configurable processor lists or calling `configureSystemPrompt` with an unchecked post-Skills string | keep the closed sequence in this step, not `AgentConfig` or the earlier composer |
...
8. No optional system-prompt processor, public processor base/definition/registry/registration surface, generic system-prompt pipeline, compatibility wrapper, XML alternate format, or text tool manifest may remain.
9. Native final-payload validation must run after the one direct platform-owned configured-Skills append and before the processed prompt is stored or configured on the LLM. Skill metadata validation alone is not a substitute.
...
| `appendConfiguredSkillsCatalog` / provider materializers | Configured skill exposure | Validate and advertise/materialize | `Skill[]` + access mode + workspace | direct platform-owned function; no injectable processor abstraction |
...
| Native final-instruction step | Yes after removal | Yes | Low | Direct Skills append only; no configurable pipeline remains |
...
| Skill exposure | SkillService/current native catalog logic/provider materializers | Refactor/extend | One-skill model is correct, but native catalog logic must stop being an injectable processor | Extract/rename a focused direct catalog appender and retain provider materializers |
| Claude system channel | Existing SDK client | Extend | SDK supports custom `systemPrompt` | N/A |
| Optional prompt processor customization | Server authoring plus core config/pipeline/registry/public exports | Remove | Conflicts with closed contract and has no remaining valid owner | N/A |
| Native final Skills and payload invariant | Existing `SystemPromptProcessingStep` plus catalog logic | Refactor/extend | The step alone owns the closed append/validate/configure sequence | Focused `append-configured-skills-catalog.ts`; no processor base or registry |
...
| Native system-prompt bootstrap | direct terminal Skills append plus complete-payload invariant and LLM configuration | `DS-001`, `DS-005` | native backend | Refactor/extend | Actual final native instruction owner; no generic pipeline |
...
| Agent definition/API/UI/core `AgentConfig` | remove obsolete processor customization | `DS-001`, `DS-005` | authoring/runtime | Contract | Stored file supersets remain readable; runtime object no longer carries processors |
...
| `system-prompt-processing-step.ts` | native bootstrap | final payload owner | direct Skills append, complete-payload assertion, state/LLM configuration | Existing singular last instruction boundary | No |
| `append-configured-skills-catalog.ts` | native skills | focused pure appender | validate and append configured skill metadata/path catalog | Reuses current catalog policy without retaining an extension abstraction | No |
...
| Skill metadata validation | server loader + direct native catalog appender + provider materializers | skill subsystem | All providers consume same model | Yes | Yes | new skill taxonomy or injectable prompt processor |
...
| `autobyteus-ts/src/agent/bootstrap-steps/system-prompt-processing-step.ts` | native bootstrap | final payload boundary | directly append configured Skills, assert complete payload, then configure LLM | Only complete native instruction point | No |
| `autobyteus-ts/src/agent/system-prompt/append-configured-skills-catalog.ts` | native skills | catalog appender | validate and append `## Skills` metadata/path catalog | Focused policy extracted from the old processor without extension machinery | No |
...
| Existing skill loaders/materializers | skills | exposure | strict fields and lazy provider projection | Existing owner | Shared model |
| Core `AgentConfig`, pipeline/barrel, processor directory, and package root exports | native runtime contract | closed prompt mutation boundary | remove processor parameter/property/default/copy path; delete generic pipeline plus base/definition/registry/registration APIs; expose only the focused catalog behavior if a public export is actually needed | Complete core removal required by `CR-001` | N/A |
| Agent definition/API/UI files | authoring contract | definition surfaces | remove system-prompt processor field/options | Complete server removal | N/A |
...
| `.../markdown-heading-containment.ts` | File | authored-body normalizer | fenced ATX containment with distinct opening and closing recognition | Reusable internal concern | general Markdown rewrite |
...
| `autobyteus-ts/src/agent/bootstrap-steps/system-prompt-processing-step.ts` | File | native final provider-instruction boundary | call the configured-Skills appender exactly once, then reject any unresolved `{{...}}` payload before storing it or configuring the LLM | Only code point that sees and governs the complete native instruction before LLM configuration | generic processor iteration, skill discovery, carpenter composition |
| `autobyteus-ts/src/agent/system-prompt/append-configured-skills-catalog.ts` | File | native Skills catalog | focused direct function over base prompt and agent context; retain exact catalog validation/content | Skills is required platform behavior, not a configurable extension | processor registration, ordering, global defaults, arbitrary callbacks |
...
| `autobyteus-ts/tests/unit/agent/bootstrap-steps/system-prompt-processing-step.test.ts` | Test | native final-payload boundary coverage | register a real configured skill whose valid description contains placeholder-shaped text; execute the actual direct Skills append; prove rejection, `AgentErrorEvent`, and no state/LLM mutation | Exercises the actual invariant owner and input path without a custom processor | custom/injected placeholder processor |
...
| `autobyteus-ts/src/agent/context/agent-config.ts` | File | native runtime configuration | retain prompt/tools/skills/etc.; remove `DEFAULT_SYSTEM_PROMPT_PROCESSORS`, `systemPromptProcessors`, constructor argument, assignment, and copy propagation | Structurally closes caller injection and global mutation | prompt-mutator objects |
| `autobyteus-ts/src/agent/pipelines/system-prompt-pipeline.ts`, its barrel export, and `autobyteus-ts/src/agent/system-prompt-processor/{base-processor,processor-definition,processor-registry,register-system-prompt-processors,index}.ts` plus old processor file after catalog extraction | Files | N/A | Delete obsolete generic extension machinery and public exports | No production owner remains | compatibility aliases or dormant registry |
...
| `autobyteus-ts/src/agent/system-prompt` | Off-Spine Concern | Yes | Low | Focused direct native Skills catalog behavior only; former processor folder is deleted |
...
| Authored heading | `##` body heading shifts to `####` under responsibilities; inside `````md\n```not-a-close\n# code\n``` `` the `# code` line remains unchanged | raw `##` escapes beside carpenter sections, or a prefix-only fence close exposes code headings | Maintains structural ownership without mutating fenced content |
...
| Keep core processor classes/registry but call only Skills | Smaller source deletion | Rejected | Delete `AgentConfig` injection, generic pipeline, processor abstractions/registry/registration, and public exports; direct platform-owned Skills append only |
...
3. Tighten skill loader/materializer validation and retain the native `## Skills` catalog semantics. Extract the current native catalog logic into the focused direct `appendConfiguredSkillsCatalog` function; do not preserve a processor abstraction. This validation is a catalog defense, not the final native payload invariant.
...
5. Reorder native factory resolution so workspace precedes composition; switch to the shared context-only composer; delete native composer/fallback. Remove the `AgentConfig` prompt-processor default/property/constructor/copy surface, delete `SystemPromptPipeline`, and delete the processor base/definition/registry/default singleton/registration/barrel/root exports plus obsolete tests. Update every positional `AgentConfig` caller atomically so no null compatibility slot remains. In `SystemPromptProcessingStep`, directly call `appendConfiguredSkillsCatalog`, validate its complete result before state/LLM mutation, then configure the LLM. Add focused coverage in which a real registered configured skill has a valid placeholder-shaped description; assert bootstrap failure, `AgentErrorEvent`, and no LLM system-prompt configuration/provider call. Update skill integration tests to exercise the final step or focused appender rather than iterating `config.systemPromptProcessors`.
...
8. Remove `systemPromptProcessorNames` from server domain/config/service, agent-management tools, GraphQL types/converters/options, web form/detail/store/query/generated types, and built-in configs. Verify the clean cut across both layers with repository searches for `systemPromptProcessorNames`, `systemPromptProcessors`, `DEFAULT_SYSTEM_PROMPT_PROCESSORS`, `BaseSystemPromptProcessor`, `SystemPromptProcessorDefinition`, `SystemPromptProcessorRegistry`, `registerSystemPromptProcessors`, and `SystemPromptPipeline`; none may remain in current source, public barrels, or active tests/documentation except explicit historical ticket artifacts.
...
- Heading normalization must ignore fenced code blocks and preserve non-heading Markdown. Opening recognition may accept an info string, but while active, closing recognition requires the same marker, at least the opening length, and only trailing spaces/tabs. Cover same-marker non-closing content, backticks with an info string, tildes, longer legal closes, and overflow behavior.
...
- Keep `appendConfiguredSkillsCatalog` direct and platform-owned. Its heading is exactly `## Skills`; Codex/Claude do not receive a duplicate Markdown catalog. Do not introduce a replacement processor interface, registry, list, order, or global default.
- In native bootstrap, call that appender exactly once and assert the no-`{{...}}` invariant in `SystemPromptProcessingStep` before assigning `processedSystemPrompt` or calling `configureSystemPrompt`; preserve the existing critical-error event behavior.
...
- For `CR-002`, implement separate fence-opening and active-fence-closing checks. A line such as `````not-a-close`` inside a backtick fence is content, not a close; any following `#` line stays byte-for-byte unchanged until a legal close.
- Update at minimum the shared runtime exposure resolver and all imports, native resolver/factory/`AgentConfig`/`SystemPromptProcessingStep`, direct Skills appender, core barrels/tests/docs, Codex bootstrapper, Claude tooling/bootstrap/session/SDK client, team context/roster, skill loaders/materializers, agent-definition API/UI tests, plus shared composer/containment tests. The downstream API/E2E specialist remains responsible for final coverage validity decisions and broader execution.
