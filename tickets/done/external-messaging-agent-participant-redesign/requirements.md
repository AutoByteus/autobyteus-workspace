...
- Status: `Ready for Approval` (SR-017 Requirement Gap: REQ-121/AC-121 revision pending user approval; every other requirement stays approved under SR-014)
- Current solution revision ID: `SR-017` (proposed REQ-121/AC-121 revision; SR-014 approval still applies to all unchanged content)
...
| DEC-113 | (SR-017) What does "the gateway should build" cover? | (a) Standalone install + `pnpm build` + `pnpm test`. (b) Install + build only. (c) Also `build:runtime-package` and the gateway Docker image (Docker was already broken before this ticket; the runtime package only produced the download for the removed managed-gateway feature). | User | **Recommend (a)**: tests already pass standalone (268), so this costs nothing extra |
| DEC-114 | (SR-017) How does the gateway get its dependencies? | (a) As its own small project inside the repo: its own install config and lockfile, still outside the main workspace (AC-117 unchanged). (b) Put it back into the main pnpm workspace (reverses AC-117 / the ARCH-REV-001 AR-001 fix, and main-product installs again pull WhatsApp/Discord libraries). | User | **Recommend (a)**. It also makes a later move to its own repository trivial. |
...
| REQ-121 | (SR-017 proposed revision, pending approval) The `external-channel` message types move from `autobyteus-ts` into `autobyteus-message-gateway/` (as before). The gateway stays **out of the main pnpm workspace** (AC-117 unchanged) but must **build on its own**: from a fresh clone, one documented command sequence run inside `autobyteus-message-gateway/` installs its dependencies, and `pnpm build` succeeds (plus `pnpm test` passes, subject to DEC-113). It is **not** required to work at runtime. Its runtime-package script and Docker image are not required (DEC-113). It must not break the main product's install, build, tests, or releases. | — | Must | User 2026-09-24: "It's fine even though the message gateway doesn't work but it should build … I don't want that it doesn't even build." | User via `/api_e2e_engineer`, SR-017 |
...
| AC-121 | REQ-121 | — | (SR-017 proposed) Fresh clone; inside `autobyteus-message-gateway/` run the documented install, then `pnpm build` (and `pnpm test` if DEC-113 (a)) | Install creates gateway-local dependencies. Build succeeds (tests pass). `autobyteus-ts` has no `external-channel` folder, and gateway imports reference the local copy. The main product's root install and lockfile contain no gateway entries. | Runtime usability, `build:runtime-package`, and the gateway Docker image are not evaluated | Validation: clean-clone gateway build check + main-product install check |
...
- Remaining content blocker: user decision on DEC-113 and DEC-114 and approval of the SR-017 REQ-121/AC-121 revision

### Approved Basis Ready For Design

- User approval received: `Yes` (2026-09-24, SR-014) for all content except the SR-017 REQ-121/AC-121 revision, which is pending
