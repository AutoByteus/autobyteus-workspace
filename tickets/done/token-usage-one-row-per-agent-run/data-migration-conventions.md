...
- **Purpose:** Centralize the deterministic source-to-target rule, operating assumptions, reachability gate, current-application contract test, database-adapter transport rule, failure-isolation/later-upgrade contract, startup-scheduling-versus-public-recovery rule, worked classification examples, and proportionate recovery boundary that govern the token-usage migrations, so individual requirements and design sections can reference one authority instead of re-deriving speculative failure handling.
...
| Already-terminal migration summaries and historical logs | Explicitly out of scope for this token ticket. Do not project, compact, rewrite, relocate, or interpret them. The observed oversized status response is an accepted residual; a future ticket must establish its own approved contract. |
...
| Failed/pending startup-only consolidation is shown in Settings | Separate automatic startup scheduling from public recovery presentation. The runner publishes `RESTART_TO_RETRY` and derives `canRetry=false`; GraphQL carries the action; Settings renders localized restart guidance, disables Retry, and dispatches nothing. A later ordinary startup may execute required `NOT_RUN`, `FAILED`, or stale `RUNNING` through `runPending()`, while direct manual invocation remains restart-required. The UI does not infer policy or match migration IDs. |
...
`SR-008` and `SR-009` are historical solution rounds whose migration-audit expansion is withdrawn by user-directed `SR-010`. They create no current implementation requirement. The summary read projection, stored-summary/audit compactor, historical-log rewriting, audit-specific registry/scheduling behavior, audit-specific UI behavior, and their tests must be removed from this ticket. Existing migration-record summaries and logs remain unchanged as an informed residual for a separately bootstrapped future ticket.

`SR-011` corrects the cleanup inventory without reopening that scope. The retained token consolidation independently declares `STARTUP_ONLY`; therefore the generic runner must keep a policy-aware recovery classifier, its direct restart-required guard, and focused current consolidation coverage. Automatic `runPending()` scheduling is unchanged.

`SR-012` completes that current path after `AR-006`: the runner publishes the closed action `MANUAL_RETRY | RESTART_TO_RETRY | NONE`, derives `canRetry` only from `MANUAL_RETRY`, GraphQL transports it, and Settings renders localized restart guidance for `RESTART_TO_RETRY`. This is ordinary public-capability truthfulness for a supported current path, not a recovery state machine, summary projection, audit compaction, filesystem recovery, or historical-log lifecycle machinery.

This convention is a scope-control tool, not a completeness checklist that authorizes reviewers to invent additional migration machinery. Review only supported behavior named by the current requirements. The product-reachability and proportionality rules prohibit using hypothetical storage, shutdown, filesystem, summary-retention, or audit concerns to make this token ticket depend on unrelated framework redesign.
