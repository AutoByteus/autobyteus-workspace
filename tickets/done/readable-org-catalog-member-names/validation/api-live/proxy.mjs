...
const root=process.argv[2];if(!root.endsWith('/.local/api-catalog-names'))throw Error('owned only');
...
http.createServer(async(req,res)=>{
 ...
 const exact=/^GetAgentOrgReferenced(Agent|Team)$/.test(payload?.operationName||'');
 const u=http.request({host:'127.0.0.1',port:50981,path:req.url,method:req.method,headers:{...req.headers,host:'127.0.0.1:50981'}},async reply=>{
 ...
 const u=net.connect(50981,'127.0.0.1',()=>{u.write(`${req.method} ${req.url} HTTP/${req.httpVersion}\r\n`+Object.entries(req.headers).map(([k,v])=>`${k}: ${v}`).join('\r\n')+'\r\n\r\n');if(h.length)u.write(h);s.pipe(u).pipe(s)});u.on('error',()=>s.destroy());s.on('error',()=>u.destroy());
}).listen(50982,'127.0.0.1',()=>console.log('Owned observer50982'));
