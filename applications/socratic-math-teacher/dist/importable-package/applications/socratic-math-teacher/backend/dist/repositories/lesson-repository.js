...
export const createLessonRepository = (db) => ({
    getById(lessonId) {
        const row = db
            .prepare(`SELECT lesson_id, prompt, status, latest_binding_id, latest_run_id, latest_binding_status, last_error_message, created_at, updated_at, closed_at, artifact_catchup_completed_at
         FROM lessons
         WHERE lesson_id = ?`)
            .get(lessonId);
    ...
    upsertLesson(input) {
      ...
      ON CONFLICT(lesson_id) DO UPDATE SET
        ...
        artifact_catchup_completed_at = COALESCE(excluded.artifact_catchup_completed_at, lessons.artifact_catchup_completed_at)`).run(input.lessonId, input.prompt, input.status, input.latestBindingId ?? null, input.latestRunId ?? null, input.latestBindingStatus ?? null, input.lastErrorMessage ?? null, input.updatedAt, input.updatedAt, input.closedAt ?? null, input.artifactCatchupCompletedAt ?? null);
    },
    attachBinding(input) {
        ...
        db.prepare(`UPDATE lessons
          SET artifact_catchup_completed_at = ?,
              updated_at = CASE
                ...
                ELSE ?
              END
        WHERE lesson_id = ?`).run(completedAt, completedAt, completedAt, lessonId);
    },
    clearArtifactCatchupCompleted(lessonId) {
        db.prepare(`UPDATE lessons
          SET artifact_catchup_completed_at = NULL
        WHERE lesson_id = ?`).run(lessonId);
    },
});
