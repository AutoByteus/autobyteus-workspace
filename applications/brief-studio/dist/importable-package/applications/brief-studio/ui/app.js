...
var BRIEFS_QUERY = `query BriefsQuery {
  briefs {
    briefId
    title
    status
...
var BRIEF_QUERY = `query BriefQuery($briefId: ID!) {
...
var BRIEF_EXECUTIONS_QUERY = `query BriefExecutionsQuery($briefId: ID!) {
...
var CREATE_BRIEF_MUTATION = `mutation CreateBriefMutation($input: CreateBriefInput!) {
...
var LAUNCH_DRAFT_RUN_MUTATION = `mutation LaunchDraftRunMutation($input: LaunchDraftRunInput!) {
...
var APPROVE_BRIEF_MUTATION = `mutation ApproveBriefMutation($input: ApproveBriefInput!) {
  approveBrief(input: $input) {
    briefId
    status
  }
}`;
var REJECT_BRIEF_MUTATION = `mutation RejectBriefMutation($input: RejectBriefInput!) {
  rejectBrief(input: $input) {
    briefId
    status
  }
}`;
var ADD_REVIEW_NOTE_MUTATION = `mutation AddReviewNoteMutation($input: AddReviewNoteInput!) {
...
var createBriefStudioGraphqlClient = (applicationClient) => {
  ...
  return {
    ...
    briefExecutions: (briefId) => execute(BRIEF_EXECUTIONS_QUERY, "BriefExecutionsQuery", { briefId }, "briefExecutions"),
    createBrief: (input) => execute(CREATE_BRIEF_MUTATION, "CreateBriefMutation", { input }, "createBrief"),
    launchDraftRun: (input) => execute(LAUNCH_DRAFT_RUN_MUTATION, "LaunchDraftRunMutation", { input }, "launchDraftRun"),
    approveBrief: (input) => execute(APPROVE_BRIEF_MUTATION, "ApproveBriefMutation", { input }, "approveBrief"),
    rejectBrief: (input) => execute(REJECT_BRIEF_MUTATION, "RejectBriefMutation", { input }, "rejectBrief"),
    addReviewNote: (input) => execute(ADD_REVIEW_NOTE_MUTATION, "AddReviewNoteMutation", { input }, "addReviewNote")
  };
};

// frontend-src/brief-studio-renderer.js
...
var isFinalArtifact = (artifact) => artifact?.publicationKind === "final";
var formatArtifactLabel = (artifact) => {
...
var renderBriefStudioShell = (rootElement) => {
...
var renderBriefList = ({ state, elements, onSelectBrief, onError }) => {
  ...
  elements.briefList.innerHTML = state.briefs.map(
    (brief) => `
        <article class="brief-row${brief.briefId === state.selectedBriefId ? " active" : ""}">
          <button type="button" data-brief-id="${escapeHtml2(brief.briefId)}">
            <div class="brief-title-row">
              <strong>${escapeHtml2(brief.title)}</strong>
              <span class="badge">${escapeHtml2(brief.status)}</span>
            </div>
            <div class="brief-meta-row muted small" style="margin-top: 10px;">
              <span>Brief ${escapeHtml2(brief.briefId)}</span>
              <span>Updated ${escapeHtml2(formatTime(brief.updatedAt))}</span>
            </div>
          </button>
        </article>
      `
...
var renderExecutionHistory = (executions) => {
  ...
  return `
    ...
    (execution) => `
            <article class="note-row">
              <div class="brief-title-row">
                <strong>${escapeHtml2(execution.bindingId)}</strong>
                <span class="badge">${escapeHtml2(execution.status)}</span>
              </div>
              <div class="brief-meta-row muted small" style="margin-top: 10px;">
                <span>Run ${escapeHtml2(execution.runId)}</span>
                <span>${escapeHtml2(formatTime(execution.createdAt))}</span>
              </div>
              <div class="muted small" style="margin-top: 10px;">
                Definition ${escapeHtml2(execution.definitionId)} \xB7 Closed ${escapeHtml2(formatTime(execution.terminatedAt))}
              </div>
              <div class="muted small" style="margin-top: 6px;">${escapeHtml2(execution.lastErrorMessage || "No recorded runtime error")}</div>
            </article>
          `
  ).join("")}
    </div>
...
var renderRuntimeDiagnostics = (brief, executions) => `
  <details class="inline-details">
    ...
    <div class="meta-grid compact-meta-grid">
      <div>
        ...
        <div class="value small">${escapeHtml2(brief.latestBindingId || "\u2014")}</div>
        <div class="muted small">Status ${escapeHtml2(brief.latestBindingStatus || "\u2014")}</div>
      </div>
      <div>
        <span class="label">Latest run</span>
        <div class="value small">${escapeHtml2(brief.latestRunId || "\u2014")}</div>
        <div class="muted small">Last updated ${escapeHtml2(formatTime(brief.updatedAt))}</div>
      </div>
      <div>
        <span class="label">Runtime note</span>
        <div class="muted small">${escapeHtml2(brief.lastErrorMessage || "No recorded runtime error")}</div>
      </div>
    </div>
  ...
  elements.briefDetail.innerHTML = `
    <section class="detail-section">
      <div class="detail-header">
        <div>
          <h3>${escapeHtml2(brief.title)}</h3>
          <p class="muted small">briefId ${escapeHtml2(brief.briefId)}</p>
        </div>
        <span class="badge">${escapeHtml2(brief.status)}</span>
      </div>
      <div class="detail-actions muted small">
        <span>Created ${escapeHtml2(formatTime(brief.createdAt))}</span>
        <span>Updated ${escapeHtml2(formatTime(brief.updatedAt))}</span>
      </div>
      <div class="meta-grid compact-meta-grid">
        <div>
          <span class="label">Brief record</span>
          <div class="value small">${escapeHtml2(brief.briefId)}</div>
          <div class="muted small">Status ${escapeHtml2(brief.status)}</div>
        </div>
        <div>
          <span class="label">Draft outputs</span>
          <div class="value small">${escapeHtml2(String(artifacts.length))}</div>
          <div class="muted small">${escapeHtml2(String(finalArtifactCount))} final</div>
        </div>
        <div>
          <span class="label">Review</span>
          <div class="value small">${escapeHtml2(String(reviewNotes.length))} notes</div>
          <div class="muted small">Approved ${escapeHtml2(formatTime(brief.approvedAt))} \xB7 Rejected ${escapeHtml2(formatTime(brief.rejectedAt))}</div>
        </div>
      </div>
      <div class="action-row">
    ...
    <section class="detail-section">
      ...
      <div class="artifact-grid">
        ${artifacts.length === 0 ? `<div class="empty-state">No draft outputs yet.</div>` : artifacts.map(
    (artifact) => `
                  <article class="artifact-card">
                    <div class="brief-title-row">
                      <strong>${escapeHtml2(artifact.artifactKind)}</strong>
                      <span class="badge">${escapeHtml2(formatArtifactLabel(artifact))}</span>
                    </div>
                    <p class="muted small" style="margin-top: 8px;">${escapeHtml2(artifact.path)}</p>
                    <p class="muted small" style="margin-top: 6px;">${escapeHtml2(artifact.description || "No description provided")}</p>
                    <pre>${escapeHtml2(artifact.body || "")}</pre>
                  </article>
                `
  ).join("")}
      </div>
    ...
    <section class="detail-section">
      ...
      <div class="note-list">
        ${reviewNotes.length === 0 ? `<div class="empty-state">No review notes yet.</div>` : reviewNotes.map(
    (note) => `
                  <article class="note-row">
                    <div class="brief-title-row">
                      <strong>${escapeHtml2(note.noteId)}</strong>
                      <span class="muted small">${escapeHtml2(formatTime(note.createdAt))}</span>
                    </div>
                    <p style="margin-top: 10px; line-height: 1.6;">${escapeHtml2(note.body)}</p>
                  </article>
                `
  ).join("")}
      </div>
...
// frontend-src/brief-studio-runtime.js
var logBriefStudio = (message) => {
  console.info(`[BriefStudio] ${message}`);
};
var mountBriefStudio = ({
  applicationClient,
  runtimeBootstrap,
  browserWindow,
  createBriefStudioGraphqlClient: createBriefStudioGraphqlClient2,
  rootElement
}) => {
  renderBriefStudioShell(rootElement);
  const client = createBriefStudioGraphqlClient2(applicationClient);
  const state = {
    briefs: [],
    detail: null,
    executions: [],
