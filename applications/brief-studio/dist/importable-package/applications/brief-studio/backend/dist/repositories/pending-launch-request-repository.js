...
export const createPendingLaunchRequestRepository = (db) => ({
    getByLaunchRequestId(launchRequestId) {
        const row = db
            .prepare(`SELECT launch_request_id, brief_id, status, binding_id, created_at, updated_at, committed_at
    ...
    insertPendingLaunchRequest(input) {
        db.prepare(`INSERT INTO pending_launch_requests (
         ...
         updated_at,
         committed_at
       ) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(input.launchRequestId, input.briefId, input.status, input.bindingId, input.createdAt, input.updatedAt, input.committedAt);
    },
    markCommitted(input) {
