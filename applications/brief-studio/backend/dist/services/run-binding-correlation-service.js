...
const ensureBindingConsistency = (pendingLaunchRequest, existingBinding, input) => {
    if (pendingLaunchRequest && pendingLaunchRequest.briefId !== input.briefId) {
        throw new Error(`Pending launch request '${input.binding.launchRequestId}' belongs to brief '${pendingLaunchRequest.briefId}', not '${input.briefId}'.`);
    }
    if (pendingLaunchRequest?.bindingId && pendingLaunchRequest.bindingId !== input.binding.bindingId) {
        throw new Error(`Pending launch request '${input.binding.launchRequestId}' is already attached to binding '${pendingLaunchRequest.bindingId}'.`);
    }
...
export const createRunBindingCorrelationService = (context) => ({
    ...
    resolveBriefIdForBinding(binding) {
        ...
        return withAppDatabase(context.storage.appDatabasePath, (db) => withTransaction(db, () => {
            ...
            const pendingLaunchRequest = pendingLaunchRequestRepository.getByLaunchRequestId(launchRequestId);
            if (!pendingLaunchRequest) {
                throw new Error(`Brief Studio could not resolve binding '${binding.bindingId}' from launchRequestId '${launchRequestId}'.`);
            }
