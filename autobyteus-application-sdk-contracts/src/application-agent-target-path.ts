...
export const getApplicationAgentTargetPathSegments = (address: ApplicationAgentTargetAddress): string[] => {
  ...
  if (!memberRouteKey) throw new Error("Application agent memberRouteKey is required.");
  return [bindingId, "targets", "agent-team-member", memberRouteKey];
};

export const encodeApplicationAgentTargetPath = (address: ApplicationAgentTargetAddress): string =>
  `/${getApplicationAgentTargetPathSegments(address).map(encodeURIComponent).join("/")}`;

export const decodeApplicationAgentTargetPath = (path: string): ApplicationAgentTargetAddress | null => {
